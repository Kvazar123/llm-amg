...
What it copies (idempotent, overwrites the engine each run):
  ...
  <repo>/entrypoint/CLAUDE.md -> <testbed>/CLAUDE.md          (activation block)
  <repo>/entrypoint/settings.json -> <testbed>/.claude/settings.json  (lifecycle hooks)
  <repo>/entrypoint/commands/*.md -> <testbed>/.claude/commands/      (/amg command)

It also seeds an empty <testbed>/.claude/amg/digest.md if absent, so the activation
block's `@.claude/amg/digest.md` import resolves before the first consolidation. These
control-plane bits carry the Claude Code default paths; the Stage 10 installer renders
them per agent_dir/entrypoint (this manual sync is its Claude-Code-default predecessor).

What it never touches (the sandbox's own project data):
  <testbed>/.claude/amg/   nodes, config.yml, work, journal, archive
  <testbed>/.claude/settings.local.json (the sandbox's own permissions)
...
def sync(testbed: Path) -> None:
    ...
    # Control-plane extras the installer also lays down (Stage 8): the SessionStart/
    # SessionEnd hooks and the /amg slash command. settings.local.json (the sandbox's
    # own permissions) is a different file and is left alone.
    settings = REPO / "entrypoint" / "settings.json"
    if settings.is_file():
        shutil.copy2(settings, claude / "settings.json")
        print(f"  hooks   entrypoint/settings.json -> {claude / 'settings.json'}")
    cmd_src = REPO / "entrypoint" / "commands"
    if cmd_src.is_dir():
        cmd_dest = claude / "commands"
        cmd_dest.mkdir(parents=True, exist_ok=True)
        for f in sorted(cmd_src.glob("*.md")):
            shutil.copy2(f, cmd_dest / f.name)
            print(f"  command entrypoint/commands/{f.name} -> {cmd_dest / f.name}")
    # Ensure the graph root exists but never overwrite the sandbox's config/graph.
    (claude / "amg").mkdir(parents=True, exist_ok=True)
    digest = claude / "amg" / "digest.md"
    if not digest.exists():                 # so the block's @import resolves pre-build
        digest.write_text(
        ...
        print(f"  digest  seeded {digest} (placeholder until first consolidation)")
