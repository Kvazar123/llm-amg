...
### Added
- **Batched application:** `reconcile.py apply` accepts multiple paths and globs (expanded in-script); the new **`apply-derived`** verb consumes every `work/derived-*.json` — checkpoint parts included — in one transaction with one aggregated result, moves applied files to `work/applied/` and quarantines torn parts in `work/invalid/`; a re-run is the resume path. The orchestrator spends one call per round instead of one per file.
- **Prepared global-pass inputs:** `link_candidates.py --synth-input` distills the whole summary layer into one grouped, capped file (with deterministic gap material) — `amg-synth` reads it instead of scanning `nodes/`; `consolidate.make_plan` embeds candidate summaries and branch-member previews, so the judge works from the plan and opens bodies only for finalists.
- **Linker checkpoint parts + a judged-pairs memory:** the linker writes `derived-links-<n>-pNN.json` parts (~10 nodes each) ending with a `{"judged": […]}` record; `apply-derived` retires fully covered batches into `work/judged/`, and nomination skips judged pairs — "repeat until zero batches" now genuinely converges instead of re-rejecting an endless similarity tail.
- **The relative pack threshold:** activations rescale to max = 1 before assembly, so `activation_threshold` reads as a share of the top activation — scale-free; the pack no longer empties on a large graph (the critical field defect: a ~937-node graph returned a header-only pack).
- **Bare-qualname canonicalization:** a `code:<qualname>` target with no path heals against a unique qualname match (one sweep cured 107 of 169 field dangling edges), guarded never to touch `imports` targets or names that also read as files.
- **The compact pack profile `retrieve.py --compact`:** the caller's pointer profile for targeted lookups — built-in modest budgets (1200/2500/6000/40, deliberately ignoring the config's `token_budget`), pointer lines instead of unfolded bodies, `decision`/`adr` keep their rationale. Measured on the 11 labeled field cases: ×3.1 smaller (≈128 → ≈42 KB) at pack recall 0.92 (lexical) / 0.81 (transformer seeding) — the depth-for-size trade is explicit and documented.
- **The gated mid-session reminder:** `lifecycle.py prompt-hint` under a new `UserPromptSubmit` hook prints one line only when AMG is active with automation on, the prompt is task-shaped (≥ ~200 chars), this session's pack log is absent or older than ~15 minutes, and ~10 minutes passed since the last hint; every other prompt gets nothing. Claude Code only; the Codex/generic blocks carry an honest no-mechanism caveat.
- **Write for the workers:** builder/linker/synth/consolidator write their artifacts with the Write tool (bash heredocs tore on quotes in real summaries) and validate parts without re-reading them.
- **The start-of-session diff question and the wrap-up trigger:** the loop runs a free deterministic `reconcile.py plan` and asks before starting the paid half; the user's wrap-up signal triggers the judged consolidation, and an overdue nudge (≥ 3 weight folds since the last judged pass, or leftover plan/actions) surfaces in session-start and `status`.
- **Two-level retrieval:** the direct CLI call is the default (the printed pack is the working context); the `amg-retriever` subagent is the deliberate isolation exception — a pointer query no longer pays ~50k tokens of subagent overhead.
- **Offline-first embeddings:** sentence-transformers loads with `local_files_only=True` first (falling back to a normal load), model2vec with `force_download=False` — a cached model never makes a HF Hub round-trip again.
- **The honest token estimate:** `_toklen` prices ASCII / other alphabets / CJK at ~4 / ~2.2 / ~1.5 chars per token in retrieval and consolidation — a Cyrillic pack shrank ~47 % at the same budgets instead of silently overflowing them.
- **The closing verification sweep:** `verify_claims.py --all --write` stamps freshly built nodes `verified` as the sync's last step, so an `unverified` mark again means "changed since the last sweep" rather than burning on every node.

### Changed
- **Batch caps** raised 40→60 builder units / 40→80 linker nodes (quality-invariant; halves the per-agent fixed overhead), worker waves of 5–10 agents per message, explicit move budgets, and a ban on ad-hoc graph-editing scripts in the SKILL.
- **The imperative usage loop in all three activation blocks:** decompose a multi-topic prompt, re-retrieve on a focus shift, graph before filesystem search (the agent directory excluded from sweeps), a visibility line, the profile choice (a targeted lookup → `--compact`), the exact-entry-points exception, and hint-line attribution.
- **The installer flow:** questions in fixed series of ≤ 4 (the survey tool's per-call limit), the absorb question mandatory, an explicit session-restart requirement after install; the `eval_gate.cases` default moved under the store (a reinstall replaces the skills tree and would wipe labels kept there).
- **Measured-and-kept defaults:** linker caps stay 5/0.35 (tightening to 3/0.5 would drop confirmed-pair coverage 67 % → 47 %); `apply_hebbian` stays `false` with field numbers — on a dense real graph (~1130 nodes, an 11-record usage log) the flip moved recall/hop-recall/pack-recall by exactly 0.0000 under both seeding modes; adaptive mass-stop pack assembly measured and rejected (×1.01 shrink — PPR mass is spread by construction). The READMEs now explain the Hebbian default newcomer-plainly: enabling is safe and spends no model tokens (the fold is script work).
- **Shipped artifacts carry no roadmap references and an English-only control layer:** legacy `(1.xx)`/`(task N)`/stage-named test cases cleaned from every shipped script and the config template; multilingual test fixtures keep their Cyrillic by design.

### Fixed
- Session-dump doc nodes no longer count against `doc_without_documents`; the queue helpers warn about leftover `derived-*.json` before re-partitioning; the retriever states absence explicitly instead of presenting neighbors as the answer, and carries the full rendered `verify_claims` path; the generic/codex activation blocks carry the full loop surface (`--intent`, the verify-before-answering rule, linking convergence, the closing sweep); the Russian corpus swept for calques per the glossary and roadmap §3 (derivation-family prose → «обогащение», «чекпоинт» → «контрольные части», natural ingest verbs), with the stale "awaits a reworked rule" Hebbian line removed.

Roadmap: the Stage 22 body compacted to its "Выполнено" line; audit §1.55–§1.76 collapsed to one-liners; the §1.75 research reserves recorded in §8.5; the cost benchmark rides along Stage 23 task 1. DoD green: 19 engine selftests + `selftest_install` + mypy (18 modules) + the eval demo (pack recall 1.00) + the link validator.
...
### Added
- **Unit text in the work queue** (audit 1.47): every chunker attaches the unit's exact content slice as `text` (a function's source, a markdown/RST section, a serialized JSON/NDJSON record, a session turn — on top of the binary/CSV/log/chat formats that already carried it), and `_queue_item` inlines it into `work/queue.json` under `queue_text_max_chars` (default 20000; 0 = pointer-only queue) together with `line_end`. Builders summarize straight from the queue and never re-open sources — inlining is strictly cheaper than a Read call, which costs the same content PLUS a re-send of the subagent's whole accumulated context (the dominant token sink of a build).
- **Deterministic auto-summary for trivial units** (audit 1.47): a code function spanning ≤ `trivial_unit_max_lines` lines (template 3; engine default 0 — the code-vs-template convention, so hand-made minimal configs keep the old behavior) is derived by code: its own source collapsed to one line becomes the summary, `derived_from_hash = source_hash`, status `active`, never queued — a mere skip would loop in the re-queue rule forever. Protocol dunders (`__call__`, `__enter__`, `__getitem__`, …) always reach the model (their semantics is the fact of their existence), and a unit with a cached derivation queues instead so the earned judged summary restores verbatim (the cache wins over the template). `plan()` reports `auto_summarized`.
- **Bounded batches + checkpoint parts + resume** (audit 1.48): `partition_queue` caps every subtree batch by unit count and estimated input volume (`builder.batch_units` 40 / `builder.batch_max_chars` 120000; `-NN` part files; a re-run replaces the batch set), so a dense directory never becomes one giant batch; builders (and synth) write output in numbered checkpoint parts (`derived-<batch>-pNN.json`, each a complete JSON array) — an interruption loses at most the units since the last part, and resume applies leftover parts first (no engine change: apply is per-file, `content_sha` keeps replays safe).
- **Honest completion signal + progress** (audit 1.48): agents must end with `BATCH COMPLETE|PARTIAL: N/M` (`SYNTH COMPLETE|PARTIAL`) — never a bare "Done" after an interruption — and the orchestrator verifies `applied + skipped_*` against the batch size deterministically instead of trusting the report; `inspect_queue` gained a `progress` block (derived vs stale nodes, percent) reported after every apply round; the bootstrap skill gained a "Thin orchestration" section (paths to subagents, aggregates only — the orchestrator's own context is re-sent every turn).
- **Real paragraph line numbers** (audit 1.51): `_text_units` emits true `lineno`/`line_end` per blank-line block with byte-identical block content and hashes — existing graphs converge by cheap pointer drift, never by re-derivation.
- New theory: THEORY §4.1 extended (the deterministic-before-the-model boundary now reaches summaries: a trivial unit's code IS its best summary) and a new §4.4 «Экономика и устойчивость сборки» (re-send economics — data travels to the worker once, the orchestrator handles aggregates; durable increments; completion as a verifiable claim, extending the crash-safety discipline to the judgment layer).
- Regression: `selftest_build` (queue text/line_end + cap, trivial auto-summary + dunder guard + cache-wins), `selftest_queue` (`test_caps`, progress), `selftest_chunkers` (`test_paragraph_linenos` with a hash-parity guard). 22 engine selftests + `selftest_install` + mypy (18 modules) + the eval demo (pack-recall 1.00) green.

### Changed
- `amg-builder` prompt: the queue `text` is authoritative for every format (sources opened only for an over-cap unit, exactly the `lineno`–`line_end` slice), output written in checkpoint parts, the COMPLETE/PARTIAL final-line protocol; `amg-synth` / `amg-linker` carry the same completion discipline; the `amg-bootstrap` SKILL orchestrates thin, verifies apply counts, applies leftover checkpoint parts first on resume, and reports `derived_percent`.
- Per the shipped-artifact rule, the new script comments state their rationale by meaning — no roadmap stage/audit references (the uniform sweep of legacy attributions stays a stage-21 task).

Commits `d116aef` / `b88642e` / `8218fca` / `0f13ace` / `463feb2` / `c30e475` plus the docs sync and the close.
...
### Found (deferred to Stage 21)
- §1.34 — hardcoded non-English (Russian) verbal-intent examples in `entrypoint/CLAUDE.md`. The engine is English-base and matches intent in any language semantically, so hardcoded words violate that invariant; the Stage 15 additions were fixed, the pre-existing rest is scheduled for the Stage 21 prompt pass.
- §1.35 — verbal memory commands are not anchored to an "AMG"/memory keyword, so a generic phrase ("show the graph", "wrap up") could false-fire an `/amg` operation; anchoring the triggers (across the activation blocks and the docs) is scheduled for Stage 21.
