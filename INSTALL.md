...
2. **Среда, каталог агента и точка входа?** *(по умолчанию Claude Code, `.claude` / `CLAUDE.md`.)* Для иной среды (Codex, Qwen Coder и прочие, читающие `AGENTS.md`) — `--env generic` плюс её имена, готовый пресет `.agents` / `AGENTS.md`: установщик вместо скиллов/хуков вольёт переносимый блок без скиллов (см. «Другие среды» ниже).
...
- **`--env generic`** — для любой среды, читающей `AGENTS.md` (OpenAI Codex, Qwen Coder и прочие): установщик вливает **отдельный блок без скиллов** из шаблона `entrypoint/AGENTS.md` и **не пишет** хуки и команду (это механизмы именно Claude Code, в иной среде их может не быть). Та же петля памяти ведётся **прямыми вызовами скриптов**, а промпты `agents/*.md` модель читает как инструкцию (не запускает изолированно). Типовая команда:
  ```bash
...
> ⚠️ **ПРОВЕРЕНО ТОЛЬКО В CLAUDE CODE.** Режим без скиллов (`--env generic`, `AGENTS.md`) спроектирован под любую AGENTS.md-среду, но его работоспособность и стабильность в Codex / Qwen Coder и других **пока не подтверждены** — все тесты проводились в Claude Code. Проверка этих сред — **Этап 19** дорожной карты.
