...
- **Журнал состава пакета** `work/pack-log.jsonl` — дописыванием, под тем же гейтом, что ко-активации: на запрос пишется строка `{ts, q, pack:[{id, source_path}]}` (узлы в ярусах пакета). Это вход для **провенанса использования** (Этап 13): конец сессии пересекает состав пакетов с реально правленными файлами и пишет `work/usage.log` — какие узлы были не просто извлечены, а использованы (см. [Субагенты и скиллы](./08-agents-skills.md), слой жизненного цикла). Журнал ведётся **отдельно** от слепого `coactivation.log` намеренно: членство в пакете циркулярно (§8.1), а использование приходит извне петли (теория — [§15.5](../THEORY.md)); консолидация его пока не трогает.
