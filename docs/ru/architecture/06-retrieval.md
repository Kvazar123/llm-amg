...
Перед именем/сводкой узла пакет может нести приписку-предупреждение `⟨…⟩` (функция `_trust_marks`) — это слой доверия Этапа 13 (теория — [§15](../THEORY.md)). Пометка **не понижает активацию** (понижение исказило бы многошаговость; свежеправленный узел часто и есть самый нужный) — она лишь велит модели перепроверить факт перед тем, как на него опереться. Что помечается:

- `stale` — статус узла `stale`: сводка отстала от изменившегося источника (приписка безусловна, существовала и до Этапа 13);
- `disputed` — узел в неразрешённом противоречии (арбитраж Этапа 14): есть конфликтующее утверждение, показать обе стороны;
...
Несколько причин складываются в одну приписку. Источник истины при конфликте — живой код, поэтому пометка — приглашение запустить `verify_claims` (ниже), а не приговор узлу. При `verification.enabled: false` остаётся только флаг `stale` (поведение до Этапа 13).
...
`verify_claims.py` (рядом с `retrieve.py`) — лёгкая проверка факта по живому источнику, программная опора правила «перепроверь утверждение о коде до ответа» (слой доверия Этапа 13; теория — [§15](../THEORY.md)). По умолчанию **только для чтения**, поэтому её может запустить даже read-only извлекатель. Для узла, спроецированного из файла, скрипт пере-нарезает текущий источник тем же нарезателем, что и извлечение структуры (хеш считается идентично), и сравнивает:
...
- **Журнал ко-активаций** `work/coactivation.log` — дописыванием (append-only): пары узлов, оказавшихся вместе в пакете и связанных ребром, записываются строкой JSON `{ts, q, coactivated}`. Это сигнал **экспозиции** для консолидации: с Этапа 14 ко-активация больше **не усиливает** веса (это было слепое циркулярное правило, измеренно вредившее полноте, §8.1) — она служит сигналом *затухания* (ребро, показанное в пакете, но не приведшее к использованию, гаснет) и по-прежнему копит счётчик `coact` для значимости; усиление даёт отдельный журнал использования (ниже). Свёртку выполняет консолидация (см. [Консолидация](./07-consolidation.md)); извлечение веса не меняет — только копит сигнал. **Связка с `--no-pack`:** в CLI журналирование привязано к записи пакета (`log_coactivation = write_pack`), поэтому флаг `--no-pack` выключает *и* запись `cache/pack.md`, *и* этот журнал ко-активаций. Чтобы развести их (собрать пакет без сигнала или наоборот), вызывают `retrieve()` напрямую с раздельными `write_pack`/`log_coactivation`.
- **Журнал состава пакета** `work/pack-log.jsonl` — дописыванием, под тем же гейтом, что ко-активации: на запрос пишется строка `{ts, q, pack:[{id, source_path}]}` (узлы в ярусах пакета). Это вход для **провенанса использования** (Этап 13): конец сессии пересекает состав пакетов с реально правленными файлами и пишет `work/usage.log` — какие узлы были не просто извлечены, а использованы (см. [Субагенты и скиллы](./08-agents-skills.md), слой жизненного цикла). Журнал ведётся **отдельно** от слепого `coactivation.log` намеренно: членство в пакете циркулярно (§8.1), а использование приходит извне петли (теория — [§15.5](../THEORY.md)). Улучшенное правило весов читает `usage.log` как сигнал исхода при `apply_hebbian` (Этап 14; см. [Консолидация](./07-consolidation.md)).
