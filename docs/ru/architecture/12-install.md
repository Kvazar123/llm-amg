...
`install.py` — детерминированный, управляемый параметрами скрипт: модель ведёт диалог по [INSTALL_RU.md](../../../INSTALL_RU.md) и вызывает его с ответами, а файловую работу делает скрипт. По шагам он размещает движок, рендерит шаблоны точки входа и **сами промпты движка** под выбранный каталог агента, вливает блок активации между маркерами, сливает `settings.json`, при глобальной установке пишет **глобальный конфиг личных умолчаний**, пишет локальный `config.yml`, рендерит блок `models` в определения субагентов, засевает пустой `digest.md`, по желанию ставит зависимости и проверяет хранилище. Граф он **не строит** — активация остаётся выбором пользователя, а структурную сборку ведёт петля или `/amg sync` (флаг `--build` — явное исключение).
...
| `codex` | OpenAI Codex (есть скиллы и субагенты) | скиллы в `.agents/skills`, субагенты — **TOML в `.codex/agents`** (`model`/`model_reasoning_effort`), skill-aware блок `entrypoint/AGENTS.codex.md`; хуки не пишутся (хуковая поверхность Codex не подтверждена — проверяется живьём), `/amg` нет |
| `opencode` | OpenCode (есть скиллы; `.agents/skills` находит сам) | скиллы в `.agents/skills`, субагенты рендерятся в **`.opencode/agent/*.md`** (`mode: subagent`; настоящий id модели проходит, Claude-алиас опускается), переносимый skill-aware блок `entrypoint/AGENTS.skills.md`; хуков (аналог у OpenCode — плагинный JS-API) и команды нет |
| `qwen` | Qwen Code (есть скиллы, markdown-субагенты **и хуки**) | скиллы в `.qwen/skills`, промпты `agents/amg-*.md` ложатся в `.qwen/agents` (родные субагенты Qwen) с очисткой Claude-специфичного фронтматтера (`tools`/`effort` убраны; алиас модели убран, настоящий id проходит), тот же блок `AGENTS.skills.md` — в `QWEN.md`, сессионные хуки сливаются в `.qwen/settings.json` (Qwen читает hooks-блок Claude-формы) |
...
Каталог агента и точку входа установщик подставляет по среде, если они не заданы явно: Claude Code → `.claude`/`CLAUDE.md`, Qwen Code → `.qwen`/`QWEN.md`, иначе → `.agents`/`AGENTS.md`. **Режимы кроме Claude Code пока не протестированы живьём** (проверка сред — идущий сейчас этап дорожной карты).
...
Для `--env claude-code` **и `--env qwen`** установщик сливает хуки `SessionStart`/`SessionEnd`/`UserPromptSubmit` из шаблона `entrypoint/settings.json` с существующим `settings.json` (`merge_settings`): чужие хуки и ключи сохраняются, прежние AMG-хуки заменяются (по сигнатуре `lifecycle.py` в команде, независимо от события) — Qwen Code читает hooks-блок той же формы в `.qwen/settings.json`, так что один шаблон обслуживает обе среды. Слэш-команда `entrypoint/commands/amg.md` рендерится в `<agent_dir>/commands/` только для Claude Code. Для `codex`/`opencode`/`generic` ни хуки, ни команда не пишутся — их роль берут стартовая проверка петли и сигнал завершения.
