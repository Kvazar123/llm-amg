...
| дрейф указателя | хеш совпал, но `lineno`/`qualname`/`policy` разошлись с единицей | тихо обновить только эти поля, без передеривации (счётчик `pointer_refreshed`) |
