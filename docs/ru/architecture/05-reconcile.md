...
Итог `plan()` — словарь `{added, changed, moved, deleted, unchanged, requeued_stale, pointer_refreshed, frozen, queued_for_semantic}`; `queued_for_semantic` — размер очереди на обогащение (added + changed + requeue + недовыведенные перенесённые), `frozen` — число заморожённых узлов `absorb_once`, чьё изменение проигнорировано. Дополнительно, когда они есть, добавляются диагностические поля: `policy_conflicts` — файлы, попавшие под две политики сразу (пересечение `mirror`/`absorb`/`absorb_once`, аудит 1.29), и `missing_sources` — перечисленные в конфиге, но не существующие на диске пути источников (аудит 1.30), чтобы опечатка в `mirror_path` не пряталась за «added: 0».
