...
Поля `part_of` (членство) и `edges` (рёбра) здесь **не вычисляются** — их формирует слой сверки и смысловой слой (см. [Модель данных](./02-data-model.md) и [Сверка и семантическое обогащение](./05-reconcile.md)). Единственное исключение — поле `follows` у чата: это не готовое ребро, а подсказка, из которой сверка детерминированно строит структурное ребро `follows`, как из `imports`/`calls`.
...
- **Сегментатор по смысловому дрейфу (заглушка).** Для длинной неструктурированной прозы (расшифровки созвонов, длинные тексты без заголовков, книги) — статистическая нарезка на эпизоды по смене темы. Отложено сознательно: структурные нарезатели выше закрывают подавляющую часть входов, а явная структура (роли, заголовки, метки времени) задаёт более надёжные границы эпизодов, чем статистический порог, — точность важнее экономии токенов. Сегментатор вводится только при измеренной боли и только под автоматической проверкой полноты — см. [Дорожная карта](./11-roadmap.md), п. 4.6 (полная спецификация и условия включения) и Этап 17, задача 6 (дом переоценки боли и реализации).
