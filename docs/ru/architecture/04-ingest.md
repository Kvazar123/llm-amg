...
Реестр `CHUNKERS` сопоставляет ключ нарезателя его реализации: `python`, `treesitter`, `headings`, `paragraphs`, `json`, `pdf`, `docx`, `xlsx`, `session`. Для обычных источников файл диспетчеризуется по ключу из классификатора; нарезатель сессий (`session`) вызывается отдельной веткой драйвера для каталога сохранённых сессий (см. подраздел «Сессии» ниже).
...
**Формат — общий контракт.** Маркеры ролей и маркер вложений `== Attachments N ==` определены в `extract_structure` (`session_role_marker` / `session_attachments_marker`) и импортируются выгрузчиком, чтобы писатель дампа и нарезатель не разошлись.
