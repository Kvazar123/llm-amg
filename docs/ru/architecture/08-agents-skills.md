...
Честная оговорка: **жёсткое завершение процесса может не вызвать `SessionEnd`.** Страховка — фиксация выводов по ходу через `notes.py add` (транзакционна, переживает сбой), а журнал упреждающей записи ловит недописанные транзакции при следующем `recover`.
