...
2. **Зафиксировать выводы сессии** — записать решения, выводы, открытые вопросы и планы через безопасный API заметок `notes.py add` (а **не** правкой `nodes/` руками): `notes.py add --type decision --summary "…" [--body "…"] [--tags "…"]`; типы `note`/`decision`/`adr`/`open_question`/`plan`, по умолчанию статус `captured`, запись транзакционная и крэш-безопасная. Это эпизодический вход для шага значимости; при сомнении — фиксировать (отбор будет дальше).
