...
Поэтому установщик (`install.py`, флаг `--env`) **разворачивает механизм под среду**, а не только подставляет пути: для Claude Code (`--env claude-code`, умолчание) он раскладывает блок со скиллами + хуки + команду `/amg`; для любой иной среды (`--env generic`) вливает **отдельный блок без скиллов** из шаблона `entrypoint/AGENTS.md` и хуки с командой **не пишет**. Базовая работоспособность памяти от среды не зависит, набор удобств — зависит.

**Как ведётся петля без скиллов.** В блоке без скиллов те же три операции выполняются модель-ведомо, прямыми вызовами скриптов, а роли субагентов исполняет сам основной экземпляр модели, читая их промпты как инструкцию:
