...
- **Хуки `SessionStart` / `SessionEnd`** — автоматизируют восстановление на старте и консолидацию плюс сохранение сессии в конце. Честная оговорка: жёсткое завершение процесса может не вызвать `SessionEnd`, поэтому фиксация по ходу (`notes`) остаётся страховкой, а журнал ловит недописанные транзакции.
