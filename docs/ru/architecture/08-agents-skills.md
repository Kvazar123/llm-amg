...
5. **Синтез и отчёт о пробелах.** Один субагент `amg-synth`: верхнеуровневые узлы-обзоры и хабы, межслойные рёбра, взвешенное мультичленство, **узлы-паттерны** (при настоящих повторяющихся паттернах) и **отчёт о пробелах** (`gap-report.md`). Его обогащение применить так же (шаг 4), отчёт показать пользователю.
