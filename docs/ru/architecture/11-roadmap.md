...
1. Закрыты (Этапы 2, 5 и 7): API заметок `notes.py add`, выбор моделей эмбеддингов; на Этапе 7 — ссылка на `README_RU.md`, согласование `absorb`, язык §3, **отчёт о пробелах `gap-report.md`** (2.12.6), привязка опережающих разделов пометками «Планируется». На **Этапе 8** закрыты разделы «Активация и режимы» и «Что происходит автоматически» (снята пометка «Планируется (Этап 8)»): единая команда `/amg` с синонимами и таблицей операций, `automation: true`, хуки `SessionStart`/`SessionEnd`, always-on дайджест, ручные аналоги, оговорка про среды кроме Claude Code. Осталось опережающим: sessions (Этап 9), 3D (Этап 15), structured `models` (1.14).
...
Текущее состояние: блок реализует петлю захвата через `notes.py add` (Этап 5), команды `/amg`, `automation` и always-on дайджест (Этап 8); описание журнала/архива/`log.md` соответствует коду. Опережающими в управляющем слое остаются сохранение сессий (Этап 9) и рендеринг шаблонов точки входа / `settings.json` / `commands` установщиком по `agent_dir`/`entrypoint` (Этап 10).
...
9. **Поверхностно сообщать о неаккуратном завершении прошлой сессии.** Жёсткое завершение (закрыт терминал, убит процесс) пропускает `SessionEnd`, поэтому свёртка/дайджест/выгрузка в конце не выполняются. Хранилище всё равно самовосстанавливается на следующем `SessionStart` (`recover` + `verify --repair`), но **молча**. Сделать так, чтобы `lifecycle.py session-start` и `repair` **обнаруживали** залеченное неаккуратное завершение (восстановленные транзакции или снятая устаревшая блокировка) и выдавали дружелюбную заметку — «предыдущая сессия закрыта некорректно: проиграно N незавершённых записей / снята устаревшая блокировка; продолжаю». `session-start` должен печатать контекст **только когда что-то залечил** (чистый старт — молча, без пер-сессионного шума). Заметки, зафиксированные через `notes.py` в убитой сессии, **не теряются** (каждая — отдельная закоммиченная транзакция) — упомянуть это в сообщении. Связано с уже описанной в `08` оговоркой про жёсткое завершение (страховка — фиксация по ходу через `notes`).
