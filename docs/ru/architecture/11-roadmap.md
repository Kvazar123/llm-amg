...
Вопрос пользователя о цене sync. Память отклонений (§1.66) уже дала сходимость, но каждая подтверждаемая пара по-прежнему стоит суждения. Кандидат-решение: замеренная авто-полоса — `linker.auto_confirm_sim` (по умолчанию off): пары с косинусом выше порога получают детерминированное слабое ребро (`relates_to`, w≈0.4) без модели и уходят в judged; средняя полоса — модели, ниже `min_sim` — отбрасывается. Ключевой риск: высокая косинусная близость ≠ смысловая связь (линкеры отклоняют 60–70 % номинаций!), поэтому порог включается ТОЛЬКО по замеру precision@sim на ретроспективе подтверждённых пар стенда (полоса принимается при precision ≥ ~0.95). Типизацию сильных связей (documents/exemplifies) авто-полосе не отдавать — тип различает только суждение. **ЗАКРЫТ на Этапе 23 (сессия 3, задача 14): замер дал ОТКАЗ — риск подтвердился числами.** Верхняя полоса пуста (ни одной рассуженной пары с sim ≥ 0.90; максимум номинаций — 0.862), [0.85–0.90) — precision 0.800 на 5 парах, ≥0.80 кумулятивно — 0.277: высокий косинус на живом графе — это дубли-ходы стенограмм и однотипные методы разных классов, не связи. Ключ `auto_confirm_sim` не вводится; полные числа и методика — задача 14 Этапа 23.

---

### 1.89. `selftest_export`: сравнение «записанный JSON == свежая сборка» флакует на границе секунды

Замечено при прогоне батареи Этапа 23 (1 падение, 3 повторных прогона зелёные): `test_round_trip_and_cli` сравнивает записанный `graph.json` со свежей `build_graph_data`, а метаданные несут метку времени — на границе секунды сравнение рвётся. Исправлено на Этапе 23 (сессия 3): единственный волатильный ключ meta — `generated` (остальное детерминировано из хранилища) — исключается из равенства с обеих сторон; тест зелёный. **Дом — попутно (исправлено).**
...
### 1.91. `work/queue.json` не освежается применением — status и sync-defer лгут о семантической очереди

Подтверждено вторым OpenCode-прогоном (status: «semantic queue: 820» при реальных 2 stale; во второй стенограмме status показывал 2 при пустой очереди inspect_queue). Очередь пишет только `plan()`; `apply-derived`/`apply` её не трогали, а `lifecycle._queue_size` читал сырой файл — между применением и следующим планом длина файла лгала status, sync-defer записывал устаревшую цифру, а `partition_queue` нарезал бы уже сделанную работу. Исправлено на Этапе 23 (сессия 3, задача 29): `_refresh_queue` после каждого применения (единица остаётся, только пока её узел `stale`; `apply_cached` переиспользует тот же механизм), итог apply несёт `queue_remaining`; status считает пересечение «очередь × живые stale-узлы» (`_semantic_backlog`), sync-defer пишет ту же честную цифру. Regression — `selftest_build` (queue refreshed after apply); детали → `05-reconcile.md` («Очередь обогащения»).

---

### 1.92. `skipped_missing` — голый счётчик: не видно, какие несуществующие цели выдумал исполнитель

Полевая жалоба (skipped_missing: 46 на применении синтеза — рёбра к несуществующим узлам, диагностировать нечем). Исправлено на Этапе 23 (сессия 3, задача 29): `_apply_items` собирает капованную выборку `missing_sample` (первые 10 id) по образцу поля `invalid`. Regression — `selftest_build` (ghost target sampled); детали → `05-reconcile.md`.

---

### 1.93. Гейт связности не различает висячие цели по ответственности; doc-метрика шумит структурными подсекциями

Полевая жалоба: вердикт `attention` навсегда — 154 висячих цели модельных рёбер (ADR-секции со слагами, которых нет) при `max_dangling_internal: 0`, плюс 149 `doc_without_documents` — подсекции «контекст/последствия», которые сами по себе ничего не документируют. Разбор: детерминированный слой висячих внутренних целей не создаёт по построению (резолвер не гадает), значит весь dangling_internal свежей сборки — модельный; он инертен (retrieval отбрасывает) и автоматического лекарства не имеет, а гейт, который не может вернуться в `ok`, перестаёт нести сигнал (та же логика, что у gated-напоминания §1.76). Исправлено на Этапе 23 (сессия 3, задача 29): `graph_metrics` разводит `dangling_structural` (гейтуется `max_dangling_internal`, по-прежнему 0 — это корректность движка) и `dangling_semantic` (число + образцы, вердикт не переключает); doc-метрика агрегируется по файлам — `doc_files_without_documents` (файл непривязан, только если ни одна его секция не несёт `documents`). Совет тестировщика «поднять порог до ~50» отклонён — лечится классификация, не терпимость; капы линковщика top_k/min_sim не тронуты (замер Этапа 22: ужатие роняло покрытие 67 %→47 %). Regression — `selftest_build`; детали → `05-reconcile.md` («Метрики связности»), `09-config.md`, GUIDE.

---

### 1.94. Поток установки: выбранный embeddings-бэкенд не закрепляется; режим зависит от глагола пользователя; ключевые бюджеты, модели и компакция не спрашиваются

Полевые наблюдения OpenCode-установки: (а) пользователь ответил «sentence-transformers», а в конфиге остался `backend: auto` — поток ставил только `enabled=auto`, бэкенд не писал, и auto-порядок предпочитает model2vec; (б) «установи AMG» гонит полный опросник заново поверх живого конфига, «переустанови» ведёт себя по-разному от модели к модели — режим должен определяться состоянием (`<agent dir>/amg/config.yml`), не формулировкой; (в) вопрос о бюджетах не объясняет, что это потолок токенов на один запрос по трём ярусам; (г) модели трёх ролей, `compaction.enabled` (+бюджеты ветки) и `linker.synth_input_max_chars` не спрашиваются вовсе, а комментарий шаблона у compaction лжёт («Off by default» при `enabled: true`). **Дом — Этап 23, группа F″ (задача 30; исправлено в сессии 3).** Дополнение вторым прогоном: списки вариантов в вопросах плавали от прогона к прогону (среды то с пропуском, зависимости то полным списком, то тройкой) — в INSTALL×2 внесено правило «списки вариантов — часть контракта, подавать как написано; адаптируется только уже решённое ранним ответом» (задача 34).

---

### 1.95. Слабые исполнители переписывают id вместо копирования — тихие `skipped_missing` и висячие модельные рёбра

Подтверждено вторым живым прогоном (сборка слабой моделью) и форензикой применённых частей: «переименованных движком id» не существует — билдер/линкер/синтез **пере-набирают** id вместо дословного копирования из пачки: метод пишется без префикса класса (`::метод` при реальном `::Класс.метод`), слаг секции перефразируется, синтез сочиняет «правдоподобный» id хаба вместо взятого из `existing_hubs`. Движок защищался корректно (skip + `missing_sample`), но пары уходили в изоляты, а вокруг них не сходилась номинация. Сопутствующие ложные диагнозы тестировщика опровергнуты кодом: partition_queue пишет плоские имена (`/`→`_` в имени файла) — «подпапок queue-…» не существует; markdown- и RST-нарезатели дедуплицируют слаги суффиксом `-N` — «extractor породил неуникальные id» неверно. Замер безопасного внутрифайлового ремонта квалификатора (цель `путь::метод` при единственном `::*.метод` в том же файле): вылечил бы лишь 17 из 201 висячих модельных рёбер стенда (0 неоднозначных; 66 — выдуманные пути, неремонтопригодны) — масштаб отклонённого на Этапе 22 «последнего сегмента», ремонт НЕ вводится (те же числа-аргументы). Исправлено на Этапе 23 (сессия 3, задача 34) промптами: правило «id копируется посимвольно» у билдера/линкера/синтеза (у синтеза — «id хаба только из existing_hubs дословно»); плюс инертные висячие пары само-лечатся переноминацией — их цель существует под настоящим id, и дисциплинированный линкер связывает их следующим sync/relink.

---

### 1.96. Часть линковщика без judged-записи невидима — волна «не сходится», и никто не говорит почему

Полевой сбой: слабые линкеры забывали служебный элемент `{"judged": […]}`, пачки не ретировались, номинация повторяла те же 12 пачек — оркестратор диагностировал вслепую («движок не вычитает осуждённые пары»). Исправлено на Этапе 23 (сессия 3, задача 34): `apply-derived` называет такие части полем `links_parts_without_judged` — упущение видно сразу, а не выводится из «застрявшей» волны. Regression — `selftest_build` (5b); детали → `05-reconcile.md` («Память рассуженных пар»). **Подтверждено полем в тот же день (третий прогон):** оркестратор слабой модели увидел поле, сам понял причину («у волны 1 не хватило итогового judged-блока») и осознанно перезапустил ровно недоретированные пачки — все 12 ушли в judged/.

---

### 1.97. Ре-спавн исполнителя с «рассказом о сбое» уводит его в инвентаризацию общего состояния

Третий прогон: перезапущенный после сбоя линкер получил от оркестратора историю неудачи («прежний прогон не дописал judged») — и вместо суждения по СВОЕЙ пачке потратил десятки ходов на криминалистику `work/`/`applied/`/`judged/`/actions.log (листинги, перепроверки листингов, разбор кодировок консоли), прежде чем сделать работу. Контракт исполнителя — пачка на входе, части на выходе; восстановление принадлежит оркестратору (apply идемпотентен). Исправлено на Этапе 23 (сессия 3, задача 34): правило рамок в промптах линковщика и билдера («рабочий набор — файл пачки и собственные части; work//applied/ не инвентаризировать; при ре-спавне просто судить свою пачку заново с `-p01`, перезапись своих частей безопасна») + правило ре-спавна в SKILL шага 6 («то же задание, что и в первый раз, — путь пачки, путь вывода, язык, и ничего больше: никакой истории сбоя»).
...
14. **Замер авто-полосы линковки (§1.88) — сначала числа, потом код.** На копии стенда: precision@sim подтверждённых пар против отклонённых по полосам косинуса (кэш эмбеддингов + рёбра графа + `work/judged/`); полоса `auto_confirm_sim` вводится ТОЛЬКО при precision ≥ ~0.95 на верхней полосе (деталь дизайна — в §1.88: слабое `relates_to` w≈0.4, judged-учёт, типизацию сильных связей модели); при провале замера — зафиксировать отказ с числами (как §8.5). Попутно ответить на вопрос пользователя «почему такие priors» в 09-config: таблица `relation_priors` — порядковая шкала смысловой силы, настроечные константы (THEORY §14.1), меняются только с eval. — **выполнено (сессия 3): замер сделан, вердикт — ОТКАЗ с числами.** Копия стенда (1509 узлов, 4556 связанных пар, кэш ST-векторов полный), эталон — 1100 рассуженных пар из `work/judged/` с записанным номинационным sim. Верхняя полоса пуста: ни одной пары с sim ≥ 0.90 (номинации на живом многоязычном графе просто не достигают таких косинусов — макс. 0.862); [0.85–0.90) — 5 пар, precision 0.800 (ниже порога 0.95 и статистически ничтожно); [0.80–0.85) — 42 пары, precision 0.214; кумулятивно ≥0.80 — 0.277, ≥0.70 — 0.087. Отклонённые пары с высоким sim — ровно предсказанный §1.88 класс: ходы стенограмм разных сессий об одном и том же (0.82–0.86) и однотипные методы разных классов (`__init__`↔`__init__`, `__iter__`↔`__repr__`). Объёма тоже нет: в подтверждённой популяции (3471 кросс-файловая семантическая пара) выше cos 0.90 лежит 0.5 %, выше 0.85 — 2.1 %, а масса подтверждённых связей живёт на 0.6–0.8 — там, где точность 7–9 %. Вывод: `linker.auto_confirm_sim` НЕ вводится ни в каком виде (даже «идеальный» порог 0.85 дал бы 1 ложное ребро на 4 автоподтверждения при выигрыше ~2 % объёма); экономику линковки несут память отклонений (§1.66) и укрупнённые части/ходы (задача 27). Оговорка честности: judged-выборка — одна волна из 3 пачек (стенд собран до появления judged-памяти), а «подтверждено» = «ребро есть сейчас» слегка завышает precision (ребро могло добавиться позже другим проходом) — то есть настоящая точность полосы ещё ниже; отказ устойчив. Замер: `D:/tmp-amg/task14/measure_autoband.py` (вне репо).
...
28. **Форк-ретривер: контекстно-осведомлённая консультация с памятью (идея пользователя; факт механизма подтверждён — Claude Code `context: fork`, субагент наследует весь родительский разговор, его вызовы не попадают в родительское окно, первый запрос переиспользует кэш родителя).** Третий вариант извлечения между «прямой вызов» и «чистый изолят»: форк читает пак в СВОЁМ окне, взвешивает против всего известного сессии и возвращает осведомлённую выжимку 5–15 строк (добавляет/подтверждает/противоречит; отсутствие — явно) — главное окно не раздувается, цена — переотправка унаследованного контекста (кэш-чтения) и латентность. Правило выбора из трёх — в SKILL amg-retrieve и блоке CLAUDE.md («рабочий контекст → прямой вызов; дешёвая выжимка → retriever; консультация, взвешенная против сессии → форк»). Только Claude Code: установщик не ставит агента в прочие среды (place_engine/codex/opencode-рендеры пропускают), тиринг не применяется (форк наследует модель родителя — комментарий в ROLE_AGENTS). — **выполнено (сессия 2):** `agents/amg-retriever-fork.md` (`context: fork`), скипы в install.py + контракты в `selftest_install` (есть в claude-code, отсутствует в generic/qwen/opencode/codex), SKILL/блок/08×2/01×2; живая проверка форка — вместе с группой T. **Уточнено вторым ревью пользователя:** длина ответа — ориентир промпта, не конфиг-ключ («обычно 5–15 строк; противоречие излагается полностью — обе стороны, доказательства, указатели»); контракт запуска — промпт спавна несёт ЗАПРОС (вопрос + форма суждения: брифинг/дельта/проверка противоречий/ревизия), подсказки засева необязательны (форк формулирует запрос сам из унаследованной сессии); применимость расширена со «поздней стадии» на все три (рано — плотный брифинг вместо импорта полного пакета, форк тогда дешевле всего; середина — дельта при смене фокуса без повторов уже известного окну + проверка согласия перед решением; поздно — ревизия перед завершением), ключевое отличие от чистого ретривера названо явно — дедупликация против окна сессии; «только Claude Code» — сверено по документации сред: OpenCode изолирует потомка явно, Codex наследует настройки, но не разговор (`/side` — команда пользователя), а **у Qwen Code родной форк ЕСТЬ** (наследует весь разговор, общий кэш-префикс) — подключение consult-агента к нему добавлено кандидатом в задачу 23 (живой Qwen-прогон). **Третье ревью пользователя:** consult поднят в видимый слой — глагол `/amg consult <запрос>` (amg.md, help, таблица блока CLAUDE.md, таблицы GUIDE×2), READMEs — пункт «извлечение суждением, а не только сырым контекстом» вторым в «Чем выделяется», абзац «Консультация с памятью» перед дайджестом, строка в таблице команд; «заметна» → «полезна» в абзаце о слабых моделях; абзац о `model` в установке переписан двумя абзацами разговорным регистром (§3.4) без потери содержания.

**Группа F″ — полевые доработки по стенограммам второго OpenCode-прогона (сессия 3; §1.91–§1.94; происхождение — установка и сборка на слабой модели, вопросы пользователя о честности отчётов и опросника).**
29. **Полевые фиксы движка (§1.91–§1.93).** Очередь освежается применением + честный status/sync-defer; `missing_sample` в итоге apply; сплит висячих по ответственности (структурные гейтуются, модельные репортятся) + doc-метрика по файлам; quick-reference relink во всех блоках ×4 и `/amg`-команде развёрнут в полный цикл «номинация → суд → apply-derived → до нуля пачек» с явным «номинация сама ничего не применяет» (слабая модель останавливалась после первой команды). Отклонено при разборе стенограмм: поднятие `top_k`/понижение `min_sim` (замер Этапа 22 в силе), «линкер не видит обе стороны пары» (номинация глобальна, обе сводки в записи кандидата), «перезапускать линковку после синтеза» (порядок SKILL уже таков). — **выполнено (сессия 3):** код reconcile/lifecycle, regression-случаи selftest_build, доки 05/09/10/GUIDE зеркально.
30. **Установщик честен и полон (§1.94).** INSTALL×2: шаг 0 «режим по состоянию конфига» (есть конфиг → reinstall при любой формулировке: `in force:` + один вопрос «что изменить?», без полного опросника; нет — свежая установка); вопрос 7 закрепляет бэкенд (`--set retrieval.embeddings.backend=…` + его группа зависимостей тут же, трейд-офф model2vec/sentence-transformers и дефолтные модели по языку в справке вопроса); расшифровка бюджетов (потолок на запрос по трём ярусам); новая серия V — модели ×3 ролей (с форматом строки per среда), `compaction.enabled` + бюджеты ветки, `linker.synth_input_max_chars` (слабой модели — 300000); deps-вопрос сужен до text/treesitter; комментарий compaction в шаблоне config.yml исправлен («включена, но простаивает, пока ветка в бюджете»). — **выполнено (сессия 3):** INSTALL×2 (шаг 0, серия V, вопросы 7/10/11, таблица зависимостей с `embeddings-st`), READMEs/GUIDE/12-install зеркально; regression в `selftest_install` (закрепление backend, вложенные ключи компакции/линкера, flow-словарь роли).
31. **Turn-бюджет билдера.** В промпте amg-builder — явная цель по ходам (~шесть на пачку 60: чтение + 2–3 write-хода по несколько частей + одна валидация + финал), как у линковщика; слабые модели без числа игнорируют правило «несколько Write за ход». — **выполнено (сессия 3).**
32. **Диагностическая команда `reconcile.py audit`** — постоянный поставляемый read-only свип инвариантов хранилища вместо предлагавшихся временных «маячков» (их пришлось бы вычищать, а транзакционный слой и так журналирует записи): соответствие id↔путь файла (`node_relpath`), дубликаты id (load_nodes молча берёт последний — класс «узлы незаметно переписывают друг друга»), непарсящиеся/без-id файлы, форма рёбер и part_of, согласованность `derived_from_hash`↔`status`, очередь×stale, сироты work/. Regression в `selftest_build`; доки 05/01/GUIDE зеркально. — **выполнено (сессия 3):** `audit()` + CLI, вердикт `clean|attention` с образцами по каждому классу; в selftest_build — чистое хранилище + подсаженные аномалии (дубликат id, чужой путь, рваный файл, active-при-отставшем-хеше, лгущая очередь). **Дополнено по вопросу пользователя:** аудит встроен в `/amg repair` отчётной половиной (после восстановления; одна строка-заметка при `attention`, полный отчёт — CLI; словесный триггер repair и так «проверь граф памяти», и он уже прецедентно репортит нелечимое — git-конфликты); в session-start НЕ встроен намеренно (O(узлов)-свип каждую сессию — налог); codex-блок переведён с прямых `graph_store recover/verify` на `lifecycle.py repair` — единообразие сред; regression в selftest_lifecycle (repair аудирует и называет свип в заметке).
33. **Ретенция `work/applied/` (вопрос пользователя о «файлах для аудита»).** Разбор: аудит сам не создаёт НИ одного файла и ключа включения не требует — он read-only поверх того, что операции и так ведут для себя (журнал пуст в покое, `actions.log` ротируется на 500 строках, `coactivation`/`usage` потребляются свёрткой в `archive/`, `pack-log` — концом сессии). Единственный неограниченно растущий каталог — `work/applied/`: применённые части остаются как страховка ручного повтора и форензика, хотя их содержимое продублировано кэшем обогащения (руками удалять уже можно — таблица GUIDE). Решение: конфиг-ключ аудита НЕ вводить (выключать нечего); ретенционную зачистку applied/ (например, `apply-derived` удаляет части старше ~30 дней, константа с обоснованием на месте, без спекулятивного ключа) сделать ПОСЛЕ группы T — аудит-сессии по свежему графу пользователя нужна полная история сборки. **Дом — после группы T.**
34. **Разбор первого прогона нового опросника и сборки слабой моделью (§1.95–§1.96; аудит стенда `clean`, gate `ok`).** Аудит-сессия по свежему полевому графу (915 узлов): инварианты ядра выдержаны (0 аномалий — дубликатов id, расхождений путей, лгущей очереди нет), новый гейт работает по замыслу (0 структурных висячих при 201 модельных, репортных). Сделано: id-дисциплина в промптах билдера/линкера/синтеза (§1.95), `links_parts_without_judged` в apply-derived (§1.96), verbatim-правило списков опросника (дополнение §1.94), строка «старт-проверка — первое действие сессии» в activation gate трёх бесхуковых блоков (полевой сбой: слабая модель забывала петлю в начале каждой сессии), оговорка last-writer-wins для `cache/pack.md` в промпте ретривера (вопрос пользователя о гонке параллельных извлечений: печатный вывод — первичный канал, файл — зеркало последнего пакета). Ложные диагнозы тестировщика зафиксированы опровергнутыми в §1.95. — **выполнено (сессия 3, 2026-07-17).**
...
22. Codex: полный цикл `--env codex` (активация → bootstrap со скиллами и TOML-субагентами → apply → retrieve → notes → consolidate); сверить факты среды (класс бага #44385, кламп `model_reasoning_effort`); попутно стоимость полной сборки против ориентира задачи 10 Этапа 22 (некэшевые ≤ ~3 М, cache-read ≤ ~15 М на ~1 МБ — не блокер). **Довеска по хукам (находка пользователя, сессия 3; два вторичных источника — `deepwiki.com/openai/codex/3.11-hooks-system` (генерация по коду репозитория) и `doc.tonyhub.xyz/openai/codex/hooks.html` (зеркало доков)):** hooks-движок — **механизм ЯДРА** (crate `codex-rs/hooks` + `hook_runtime.rs` в codex-core), Claude-style, включён по умолчанию (`[features].hooks = true`); плагины — лишь дополнительный источник определений. Конфиг: `hooks.json` ИЛИ inline `[hooks]` в `config.toml`, слои `~/.codex/` и `<repo>/.codex/` (все совпавшие срабатывают; проектный слой — только при доверенном `.codex/`). События: SessionStart (matcher startup|resume|clear|compact), UserPromptSubmit, PreToolUse/PostToolUse/PermissionRequest, PreCompact/PostCompact, SubagentStart/Stop, **Stop — TURN-scope (конец хода, НЕ сессии; SessionEnd-аналога НЕТ)**. stdin — JSON c `session_id`/`transcript_path`/`cwd`/`hook_event_name`; timeout в секундах (деф. 600); `command_windows` для Windows; только `type: "command"`. **Критично для установки: trust-flow** — немanaged-хуки не запускаются, пока пользователь не одобрит их через `/hooks` (доверие по хешу определения; смена хука = повторный review) → INSTALL обязан завершаться шагом «откройте /hooks и доверьте AMG-хуки». Дизайн-следствия для живой проверки: SessionStart→session-start и UserPromptSubmit→prompt-hint ложатся напрямую (merge-вариант в `.codex/hooks.json`); SessionEnd отсутствует — свёртку/дамп в конец сессии напрямую не повесить (кандидаты: инкрементальный дамп на turn-scope Stop — переписывать файл сессии каждый ход, crash-proof, но цена за ход; или оставить wrap-up дисциплиной блока) — решить по живому прогону; формат стенограммы нестабилен (как у Qwen — мягкий skip при несовпадении). — **Dev-половина выполнена (сессия 4, 2026-07-17; сверено по коду `openai/codex`, крейт `codex-rs/hooks`):** факты подтверждены первоисточником — hooks-движок в ядре, конфиг `hooks.json`/`[hooks]` по слоям (`~/.codex`, `<repo>/.codex`), обработчики `type: command` с `timeout_sec` (дефолт 600) и `command_windows`, trust-flow обязателен (`discovery.rs`: незаманаженный хук исполняется только со статусом Trusted; `/hooks` в слэш-энуме TUI есть). **Расхождение с ожиданием roadmap:** контекст внедряется ТОЛЬКО через wire-JSON `hookSpecificOutput.additionalContext` (`output_parser.rs`: `parse_json(stdout)`, плоский текст игнорируется) — у lifecycle появился флаг `--hook-json`. Сделано: шаблон `entrypoint/hooks.codex.json` (SessionStart → `start-check --hook-json`, UserPromptSubmit → `prompt-hint --hook-json`), merge тем же `merge_settings`, uninstall-чистка, шаг «доверьте AMG-хуки через /hooks» в INSTALL×2 и финальном print, блок AGENTS.codex.md переписан под хуки+trust; contract-тесты в selftest_install (merge, чужой хук цел, реинсталл без дублей, uninstall). Дамп на turn-scope Stop НЕ подключён — решение по живому прогону (формат rollout-стенограммы не смотрели живьём; дисциплина блока покрывает). Остаётся: живой прогон пользователя (полный цикл + 22a; отдельно проверить, доходит ли НАБРАННЫЙ `/amg …` до модели — слэш-меню Codex фиксированное, неизвестная команда может не отправиться, тогда рабочая форма только словесная). **НАХОДКА сессии 4 при повторном чтении ядра: у Codex ЕСТЬ форк-аналог** — collab-слой multi-agent v2: инструмент `spawn` с `fork_turns: "all"` отдаёт ребёнку полную историю родителя (`load_agent_model_context` родительского треда; `LastNTurns(n)` усечёт) и закрепляет родительскую модель/усилие («Full-history forks … inherit the parent model and reasoning effort and do not accept overrides» — подсказка конфига), а канал результата есть: `wait_agent` будится почтой (`MailboxActivity`), `send_message`/`followup_task`/`list_agents` — межагентская связь. Прежний вердикт «наследуются настройки, не разговор» относился к классическим TOML-субагентам и `/side` и остаётся верным для них. Живому прогону проверить: включён ли multi-agent v2 по умолчанию (или за флагом конфига `multi_agent_v2`), доходит ли финальный ответ форка до родителя штатно — и при успехе подключить `/amg consult` по образцу Claude Code (спавн-промпт несёт ЗАПРОС; агент-файл не нужен — форк работает на контексте родителя). **Протокол живой проверки форка (3 шага, выполняется в обычной Codex-сессии):** (1) поговорить пару ходов о чём-то конкретном, затем попросить: «спавни субагента-форк с полной историей нашего разговора (fork_turns: all) и попроси его перечислить, что мы уже обсудили» — если модель отвечает, что такого инструмента нет, посмотреть `/subagents` и конфиг-флаг multi_agent_v2; (2) если спавн прошёл — смотрим, ВИДИТ ли ребёнок историю (перечислил обсуждённое = полная история подтверждена; пустой пересказ = форк не сработал); (3) решающий: «дождись его ответа и передай мне дословно» — если родитель приводит текст ребёнка (а не только «агент завершился»), канал результата есть, и consult подключаем; если родитель видит лишь статус — фиксируем «форк есть, обратной связи нет» (класс Qwen) с точной формулировкой.
22a. Средовая механика Codex: метка завершения поверх упавшего агента, видимость промежуточных строк, подхват свежесозданных скиллов/TOML на лету или при старте; различия → `08-agents-skills.md`/GUIDE.
23. OpenCode повторно — уже штатным `--env opencode` (skill-aware блок): пройти цикл, проверить, что путаница «блок против видимых скиллов» снята; Qwen Code — `--env qwen` (скиллы + субагенты `.qwen/agents` + хуки в `.qwen/settings.json`; проверить, доходит ли дамп стенограммы — формат может отличаться от Claude, тогда мягкий skip; **родной форк Qwen — ВЕРДИКТ УТОЧНЁН полным текстом доков (сессия 3; первоисточник: `qwenlm.github.io/qwen-code-docs/en/users/features/sub-agents/`):** механизм есть (`subagent_type: "fork"`, наследует весь разговор, общий кэш-префикс DashScope, спавнится моделью автоматически), НО consult к нему в текущем виде НЕ подключается: (а) «No result feedback» — форк детачится в фон, родитель видит плейсхолдер и НЕ МОЖЕТ использовать его вывод, а consult весь построен на возвращённом вердикте; (б) форк работает на системном промпте РОДИТЕЛЯ (ради кэша) — наш агент-файл к нему не применяется, ask ехал бы текстом задачи; (в) файловый обход (форк пишет вердикт в work/, родитель опрашивает) отклонён — детач без сигнала завершения означает опрос и гонки, слабой модели противопоказан. Итог: «consult — только Claude Code» остаётся верным; при живом Qwen-прогоне лишь перепроверить, не снято ли ограничение (помечено как Current Limitations), и зафиксировать оговорку в GUIDE/08 при закрытии). Модель среды желательно sonnet-класса и выше; слабее — фиксировать отдельной колонкой наблюдений. Дополнительно (запрос пользователя, сессия 3): **исследовать по свежим первоисточникам форк-аналог в OpenCode и Codex** — умеет ли субагент наследовать ЖИВОЙ контекст родителя (на 2026-07 сверено: OpenCode изолирует потомка, Codex наследует настройки, не разговор — но поверхности сред меняются быстро); при обнаружении механизма подключить `/amg consult` по образцу Claude Code/Qwen (рендер агента установщиком + строка в блоке среды). Туда же — **микроплагин OpenCode (JS) как событийная замена хуков**: полевой факт третьего прогона — слабая модель пропускает стартовую проверку даже при императиве первой строкой блока И строке в activation gate (не проверила и сам гейт; начала пользоваться памятью только после любого упоминания о ней) — блоковые указания на этом классе моделей потолок исчерпали, системный фикс только событийный. Карта событий подтверждена гайдом плагинов (находка пользователя: `dev.opencode.ai/docs/ru/plugins/`): **`session.created`** → `lifecycle.py session-start` (стартовая дисциплина закрывается штатно); **`message.updated` (ход пользователя)** → аналог prompt-hint (гейты те же: длина/возраст pack-log/кулдаун; исследовать поверхность внедрения строки — вставка контекста через SDK против `tui.toast.show` пользователю); **`session.idle`** → кандидат на детерминированную половину конца сессии (свёртка+digest, с дебаунсом — idle срабатывает после каждого ответа); **`tool.execute.after` (Edit/Write)** → плагин сам ведёт список правленных файлов → usage-атрибуция становится возможна и вне Claude Code; message-события позволяют плагину писать **инкрементальный дамп стенограммы самостоятельно** (богаче Claude Code, где transcript доступен только в конце). Принцип: JS — тонкий событийный переходник, ВСЯ логика остаётся в `lifecycle.py` (плагин лишь вызывает его шеллом); раскладка — проектный `.opencode/plugin/` при local-установке, глобальный каталог плагинов при global (симметрично hooks/settings.json других сред); ставит установщик рендером под agent_dir. Реализация — после живой проверки payload'ов и SDK-поверхности (следующая сессия). Первый прогон OpenCode-цикла НА НОВОМ опроснике и промптах выполнен пользователем и разобран (сессия 3, задача 34; аудит стенда clean, gate ok) — повторная приёмка после переустановки движка с фиксами §1.95–§1.96. **Сессия 4 (2026-07-17) — исследование по первоисточникам (`sst/opencode`, код) и реализация плагина.** Факты кода: каталоги `{plugin,plugins}/*.{ts,js}`, `{command,commands}/**/*.md`, `{agent,agents}/**/*.md` — сканируются ОБА имени (config/plugin.ts, command.ts, agent.ts) из каждой конфиг-директории цепочки (`~/.config/opencode`, все `.opencode` вверх до корня worktree, `~/.opencode`, `$OPENCODE_CONFIG_DIR`) — для глобальной установки выбран `~/.opencode/`; хук `chat.message` получает МУТИРУЕМЫЙ `parts` (prompt.ts: тот же массив уходит модели и в сохранение) — поверхность внедрения prompt-hint и стартовой заметки, часть-инъекция synthetic:true с самодельным `prt_`-id (встроенный synthetic-паттерн); события `session.created {info}` (у детей `parentID` — фильтр субагентских сессий), `session.idle {sessionID}`, `file.edited {file}` (его публикуют edit/write/apply_patch — но выбран `tool.execute.after` ради sessionID; apply_patch-правки атрибуции не получают, отмечено в коде); стенограмма — SDK `client.session.messages` `[{info, parts}]`. **Форк-аналог в OpenCode: НЕТ** (tool/task.ts: дочерняя сессия получает только текст задания, родительские сообщения не копируются) — consult остаётся Claude-Code-only. Реализовано: `entrypoint/plugin/amg.js` (тонкий переходник: session.created→start-check, chat.message→flush заметки+prompt-hint, session.idle→дроссельный session-end каждые ~2 тихих минуты + dispose, tool.execute.after→учёт правок; вся логика в lifecycle.py, вызовы шеллом с payload на stdin); lifecycle: глагол `start-check` (heal+digest+plan+вопрос о sync по каденции ×1.5/+50 — каденция из прозы блоков переехала в код), OpenCode-ветка `session-end` (`format: opencode`: рендерер сообщений SDK — reasoning/synthetic/ignored отрезаются, tool/file → маркеры вложений; СТАБИЛЬНОЕ имя файла `ГГГГ-ММ-ДД-ЧЧММ-<хвост sid>.md` → инкрементальный дамп перезаписью; usage из `edited_files` payload; штамп `work/pack-log-stamp` при потреблении pack-log серединой сессии — гейт prompt-hint остаётся честным), `--hook-json`; hook-payload `cwd` теперь используется как корень проекта. Регрессии: selftest_lifecycle (start-check каденция+wire, oc-end двойной дамп в один файл/фильтры/usage/штамп), selftest_install (плагин+команда+uninstall). **Qwen (поправка задачи 35 по свежим докам): команды — markdown с `{{args}}` РЕКОМЕНДОВАНЫ, TOML устаревает** → рендерим `.qwen/commands/amg.md`. **Форк Qwen перепроверен (2026-07-17): ограничение «No result feedback» в силе дословно** — consult не подключается, вердикт задачи 23 стоит. Остаётся живьём: прогон плагина/команды в OpenCode, цикл Qwen с дампом стенограммы.
24. Зафиксировать расхождения блоков/установщика по итогам прогонов; внести правки. — **частично (сессия 4):** расхождения исследования внесены правками: (а) `.codex/prompts/*.md` из вторичных источников в актуальном ядре Codex ОТСУТСТВУЕТ — по запросу пользователя перепроверено сплошным грепом ВСЕХ 107 крейтов codex-rs: протокольной операции `ListCustomPrompts` и загрузки каталога `prompts` нет нигде, `custom_prompt`-хиты — лишь кастомный промпт компакции и модальное окно ввода `CustomPromptView`; слэш-попап заполняется только фиксированным енумом `SlashCommand` (даже плагины Codex своих команд не регистрируют — флаги лишь прячут встроенные пункты). Механизм 2025 года `~/.codex/prompts` из текущего ядра исчез полностью; поверхность обнаружения — попап скиллов/`$`-дополнение — задача 35 для Codex закрыта отказом; (б) stdout Codex-хуков строго JSON (см. задачу 22); (в) TOML-команды Qwen устарели → markdown `{{args}}`; (г) по требованию пользователя установщик переведён на декларативный **реестр сред** `EnvProfile`/`ENVS` (профиль = данные: пресеты, шаблон блока, носитель хуков, команда+плейсхолдер, плагин, формат субагентов, ships_fork; install/uninstall/пресеты — универсальные исполнители; uninstall подметает назначения ВСЕХ профилей) — новая среда впредь = запись в реестре (+форматтер, только если формат субагентов новый). (д) вопрос пользователя о глобальных установках вскрыл РАСХОЖДЕНИЕ: код клал глобальный блок в `~/<точка входа>`, тогда как INSTALL×2 уже обещал `~/.claude/CLAUDE.md`, а домашний корень другие среды не читают вовсе (Codex: `agents_md.rs` выше корня проекта не поднимается; OpenCode: глобальные правила — `~/.config/opencode/AGENTS.md`, доки; Qwen: глобальный контекст — `~/.qwen/QWEN.md`, доки) — введено поле профиля `global_entry` (claude `~/.claude/CLAUDE.md`, codex `~/.codex/AGENTS.md` — подтверждение живым прогоном, opencode `~/.config/opencode/AGENTS.md`, qwen `~/.qwen/QWEN.md`, generic по-прежнему `~/AGENTS.md`), uninstall-свип разворачивает `{agent_dir}` и пресетом каждого профиля, и переданным каталогом (иначе `--env opencode`-удаление не чистило бы `~/.qwen/*`); contract-тест `test_global_envs`. Осталось: расхождения ЖИВЫХ прогонов Codex/Qwen.
35. **`/amg`-команда с подсказками во ВСЕХ средах (пробел найден пользователем: в OpenCode ввод `/amg` не подсказывает ничего — команду мы ставим только в Claude Code).** Командные поверхности есть у всех трёх: OpenCode — markdown-команды `.opencode/command/<имя>.md` (фронтматтер + `$ARGUMENTS`), Qwen Code — TOML-команды `.qwen/commands/<имя>.toml` (поле prompt + подстановка аргументов), Codex — custom prompts `.codex/prompts/<имя>.md`. Сделать: проверить каждую поверхность живьём (формат, проектный vs глобальный слой, подстановку аргументов), отрендерить наш `entrypoint/commands/amg.md` в родной формат каждой среды установщиком (рендер путей как у блоков; uninstall-чистка; контракты в selftest_install), обновить 12-install/08/GUIDE/READMEs («в средах без команд — словами» станет уже, оставаясь верным для generic). Дом — группа T (вместе с живыми прогонами сред). — **выполнено (сессия 4) для OpenCode и Qwen:** единый рендерер `render_command` от профиля — Claude Code дословно; OpenCode `.opencode/command/amg.md` (описание+тело, `$ARGUMENTS` — его же плейсхолдер; Claude-специфичный хвост срезается); Qwen `.qwen/commands/amg.md` (`$ARGUMENTS`→`{{args}}`); глобальная установка кладёт в `~/.opencode/command` / `~/.qwen/commands`; uninstall чистит; контракты в selftest_install. **Codex — отказ:** командной поверхности в актуальном ядре нет (см. задачу 24), печатный `/amg <глагол>` исполняет петля по блоку. Живьём проверить автодополнение — вместе с прогонами.
25. По итогам снять или смягчить оговорку «не протестировано» в README*/INSTALL/08/GUIDE для подтверждённых сред. — **частично (сессия 4):** оговорка смягчена для OpenCode во всех носителях (README×2, INSTALL×2, GUIDE×2, 12×2): «полный цикл подтверждён живыми прогонами; свежедобавленным плагину и команде проверка предстоит»; Codex/Qwen/generic — честно «собраны по проверенным поверхностям, живьём не подтверждены». Полное снятие — после их прогонов.

**Закрытие (THEORY) — развёрнутые тезисы для закрывающей сессии** (зафиксированы в сессии 3, чтобы суждение не терялось между сессиями; писать native-прозой по CLAUDE.md §7 и §3.4, stage-free, зеркально ru+en):

- **Гейт качества охраняет продуктовую поверхность, а не ранжирование (THEORY §10.3).** Мерить нужно то, что получает модель, — собранный пакет (pack-recall + pack-hop-recall); ранговый hop-recall наказывает легитимную индирекцию (суб-хаб углубляет путь: ранги сползают при неизменном пакете — полевое −2.27 % при pack-recall 1.00 до и после). Эталон многошагового подмножества замораживается от базлайна: лексическое ранжирование граф-зависимо, и пересчитанный на изменённом графе hop-эталон превращает дрейф множества в фантомную дельту метрики.
- **Неразрушающие действия — не заложники гейтуемых (THEORY §13).** Арбитраж и promote обратимы и применяются всегда; гейт меряет только компакционное подмножество на клоне пост-безопасного состояния. Общая форма принципа: предохранитель охраняет ровно тот класс действий, чей риск он меряет, — иначе он запрещает безопасное заодно с опасным, и его начинают обходить.
- **«Похожесть номинирует — суждение решает» — измеренная необходимость, не предпочтение (дополнение THEORY §4.2).** precision@sim по реальной истории суждений живого графа: пар с косинусом ≥0.9 не существует (максимум номинаций 0.862), кумулятивно ≥0.8 точность 0.277, масса подтверждённых связей живёт на 0.6–0.8 при точности 7–9 %; высокая близость на живом графе — почти-дубликаты (ходы стенограмм разных сессий об одном и том же, одноимённые методы разных классов), не связи. Автоматическое подтверждение по порогу близости несостоятельно при любом пороге — фильтр суждением конструктивно необходим. (Числа — задача 14; часть тезиса с числами уже внесена сессией 4 в §14.2 обоих THEORY — «похожесть — ещё не связь», по формулировкам пользователя: при закрытии дополнять §4.2 БЕЗ дублирования §14.2, перекрёстной ссылкой.)
- **Сигнал, который не может погаснуть, перестаёт быть сигналом (THEORY §13, семейство гейтованных сигналов).** Родственные механизмы: gated-напоминание петли (§1.76), сплит висячих целей (структурные гейтуются — лечимы и нулевые по построению; модельные репортятся — автоматического лекарства нет, и вечный `attention` обнулил бы вердикт), напоминание об отложенной судейской консолидации. Общий принцип: тревога обязана уметь очищаться, иначе её учатся игнорировать.
- **Дисциплина, которую промпт не удерживает, переезжает в событие (THEORY §13; тезис добавлен сессией 4).** Полевой потолок: императив стартовой проверки, повторённый трижды (первая строка блока, activation gate, шаг петли), слабой моделью всё равно пропускался — указание конкурирует со всем остальным контекстом, а событие исполняется всегда. Отсюда правило заселения сред: детерминированные половины петли (лечение, дифф, свёртка, дамп, гейтованное напоминание) вешаются на любую событийную поверхность среды — хуки Claude/Qwen/Codex, событийный плагин OpenCode, — а модельной дисциплине остаётся только то, что событие дать не может: решение (синхронизировать или отложить) и суждение. Инкрементальный дамп OpenCode — следствие того же принципа при ОТСУТСТВУЮЩЕМ событии конца: невидимый конец сессии заменяется многократной перезаписью по тихим моментам, и потеря ограничивается хвостом после последнего события.
- **«Извлечение суждением» — третий профиль обращения к памяти (THEORY §7 или §13).** Прямой пакет (рабочий контекст) / изолированная выжимка (дёшево, вне окна) / форк-консультация (вердикт, взвешенный против всего известного сессии). Ключевое отличие форка — дедупликация против окна сессии, которой изолированный ретривер не может дать по построению; цена — переотправка унаследованного контекста (кэш-чтения) за плотное, нераздутое окно.
