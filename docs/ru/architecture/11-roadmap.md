...
Полевой сбой: слабые линкеры забывали служебный элемент `{"judged": […]}`, пачки не ретировались, номинация повторяла те же 12 пачек — оркестратор диагностировал вслепую («движок не вычитает осуждённые пары»). Исправлено на Этапе 23 (сессия 3, задача 34): `apply-derived` называет такие части полем `links_parts_without_judged` — упущение видно сразу, а не выводится из «застрявшей» волны. Regression — `selftest_build` (5b); детали → `05-reconcile.md` («Память рассуженных пар»).
