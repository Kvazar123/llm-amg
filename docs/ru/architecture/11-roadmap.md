...
**Состояние (живое).** Группы 1–2 закрыты (фундамент + bench + барьер; производный индекс под `load_nodes`, 15× на 7600 узлах); Группы 3–4 не начаты. Порядок — по группам, коммит после группы. Развилки ниже зафиксированы с пользователем (после изучения всех доков/кода) — это расширенное, детальное понимание этапа, более широкое, чем исходные тезисы.
...
### Группа 3 — план / возобновляемость / хелперы (НЕ начато)
- **Задача 11 — ограничить near-dup в `consolidate.make_plan` (§1.27).** Сейчас Жаккар по ВСЕМ парам узлов (O(n²)); на 10⁴ узлов ~5·10⁷ сравнений. Сузить кандидатов до типов `episodic_types` (именно их сливает `merge_near_duplicates`) → O(k²) по эпизодическим. MinHash/LSH-предфильтр — только если бенч покажет большой k. Правка ~2 строки (`ids`/`toks` в цикле → эпизодические узлы).
- **Задача 13 — возобновляемая деривация без перерасхода токенов.** Корректность уже надёжна (очередь пересобирается из состояния графа; дублей/потерь нет). Пробел — СТОИМОСТЬ: билдер дописал `derived-*.json`, но `apply` не успел → узлы деривируются заново (зря токены). Делать: (а) **билдер ЭХОит `content_sha`** в каждый derived-элемент — правка `agents/amg-builder.md` (сейчас выдаёт `{id, summary, lang, edges}`; в очереди `content_sha` уже есть, просто прокинуть в выход); (б) `reconcile.apply_derivation` сверяет `content_sha` элемента с ТЕКУЩИМ `source_hash` узла — расходится → **skip** (`skipped_stale`), узел остаётся stale и пересверка вернёт его в очередь (никакой слепой деривации против изменившегося источника — это подводный камень); (в) SKILL `amg-bootstrap` (шаг 3/4): применить уцелевшие `work/derived-*.json` ПЕРВЫМИ (теперь freshness-safe), затем bootstrap пересоберёт очередь, билдеры — только на остаток.
- **Задачи 5–6 — хелперы очереди (вместо ad-hoc `_partition.py`).** `partition_queue.py` (читает `work/queue.json`, делит на партии по поддеревьям/пакетам → `work/queue-<part>.json`) и `inspect_queue.py` (сводка очереди: счётчики по category/поддереву/kind). В `amg-bootstrap/scripts`. Заменяют импровизацию из `SKILL.md` шаг 3 («split into batches by subtree») — экономия токенов, без однострочников на Windows.
- **Задача 7 — lazy/on-demand: строительные блоки сейчас, полная лень отложена.** Скрытый переключатель `derivation` НЕ вводить (нельзя обоснованно включить без индекса + eval; §6.3). Блоки уже есть/делаются: структурный-граф-первым by construction; scoping — `partition_queue` (5/6); безопасный токен-выигрыш — возобновляемость (13). Полная on-demand/фоновая лень с eval-проверенным переключением дефолта — **§4.10** (честный разбор «лень стоит качества до первого касания», предпосылки, фазы A–D, предохранители, DoD), дом реализации — **Этап 17, задача 7**.

### Группа 4 — тиринг моделей (НЕ начато) — задача 8
Механизм 1.14 готов (per-role `model`+`reasoning_effort` рендерится установщиком в субагентов: discovery→{classifier,retriever}, module_summary→{builder}, synthesis→{synth,consolidator}). Здесь: в шаблон `config.yml` блок `models` — добавить структурную форму с `reasoning_effort` (низкий на discovery, высокий на synthesis); задокументировать протокол замера (eval off/on по тирам; дешёвая модель там, где eval не проседает; reasoning не урезать, где качество падает). **Живой (платный) замер отложить** (как Хебб / Этап 19) — здесь дефолты + метод.
