...
7. **Объяснимость (§14.2).** «Путь активации можно инспектировать» — переобещание: инструменты показывают только итоговые активации, путей нет. Смягчить формулировку либо оставить её после реализации `--explain` (Этап 2). — (Этап 2, задача 9) `--explain` реализован: показывает рёбра с наибольшим вкладом притока массы для верхних узлов. Формулировку §14.2 на закрытии **оставить** (стала истинной), при желании дополнить ссылкой на `retrieve.py --explain`.
...
1. Согласовать defaults для embeddings:
   ...
   - `INSTALL.md`;
   - `GUIDE.md`.
   (Этап 2, задача 5) дефолт модели теперь **зависит от `working_language`**: `en` → `_DEFAULT_MODEL` (model2vec `potion-retrieval-32M` — лучшая статическая retrieval-модель; st `all-MiniLM-L6-v2`), иначе → `_DEFAULT_MODEL_MULTILINGUAL` (model2vec `potion-multilingual-128M`; st `paraphrase-multilingual-MiniLM-L12-v2`). **Принцип выбора** (зафиксировать в 09-config, 06-retrieval, GUIDE, далее README): эмбеддинг — опциональное лёгкое обогащение поверх BM25, поэтому дефолты лёгкие и retrieval/мультиязычно-настроенные, а НЕ ~8B-лидеры MTEB (Qwen3-Embedding, Llama-Embed-Nemotron, Gemini); качество меряется на своём графе через `--compare-embeddings`; любую HF-модель можно задать `retrieval.embeddings.{backend,model}` (апгрейд для мультиязычного — `Alibaba-NLP/gte-multilingual-base`). `config.yml` уже снабжён комментарием. Модели подобраны по веб-поиску июня 2026 (живой замер RU в этой среде флаки — желателен ручной прогон пользователем).
...
8. (Этап 2) Описать **выбор моделей эмбеддингов**, чтобы пользователь сразу видел зависимости и возможности: семантический засев опционален; зависимости (`pip install model2vec` — лёгкий / `sentence-transformers` — тяжёлый, нужен torch); per-language дефолты с именами (en → `potion-retrieval-32M` / `all-MiniLM-L6-v2`; non-en → `potion-multilingual-128M` / `paraphrase-multilingual-MiniLM-L12-v2`); **мультиязычная модель обязательна для не-английских (в т. ч. русских) проектов** — иначе кросс-язычная близость не работает; принцип «лёгкое обогащение поверх BM25, не ~8B-лидеры MTEB»; переопределение `retrieval.embeddings.{backend,model}` (апгрейд `Alibaba-NLP/gte-multilingual-base`); измерение `--compare-embeddings`. Закрывается на Этапе 2 (сверка с реализацией).
...
7. Расширить eval cases: — **выполнено**.
   ...
   - multi-hop task.
   **Демо разделено на два графа** (иначе связный кейс перетягивает массу PPR у изолированного через dangling-редистрибуцию — выяснено живым тестом): `build_demo_store` (`--make-demo`) — только multi-hop (AMG 1.00 vs лексика 0.33, hop 1.00 vs 0.00); `build_embeddings_demo` (`--compare-embeddings`) — изолированная кросс-язычная пара (RU-gold «ротация ключей шифрования» против EN «ложного друга» про ротацию шин), **AMG recall 0.00 OFF → 1.00 ON** на живой model2vec. Superseded/stale: метрика recall@K=1 слишком хрупка для свойства переранжирования, поэтому проверяется юнит-тестом `selftest_retrieve` (`test_superseded_ranks_below_active`, `_STALE_MARK`), а не демо-кейсом. Живой прогон пользователя: model2vec sim(routing,роутинг)=0.41, кросс-язык работает в изоляции (sim RU-ключи/EN-запрос ≈0.69); **sentence-transformers не грузится — сломан torch C-extension на Python 3.14 (проблема среды, не AMG; мягкий откат в BM25 сработал штатно)**.
