...
Пункты 1.39–1.49 добавлены по итогам **dogfood-сборки на реальном проекте** (~1 МБ: исходный код, документация с ADR, тесты; сам предмет проекта для AMG не важен — важны форма «код+документация+тесты» и полученные цифры). Сборка выявила системные дефекты семантического конвейера и стоимости: граф собрался **фрагментированным** (322 компоненты связности, 267 сирот, 1838 висячих рёбер, гайды 56/234 в основном компоненте), потратив **~70 Мтокенов и ~30 часов** (с прерываниями на лимиты) на граф в 907 узлов, и потребовал массу ручной доводки. Каждый пункт подтверждён чтением исходников; дома — Этапы 18–20 (см. §5). Механический слой (`extract_structure`→`reconcile`→`graph_store`) при этом отработал детерминированно и идемпотентно — все дефекты в **семантическом слое (субагенты), в полу-реализованном `§4.2`, и в отсутствии контроля качества/воспроизводимости/стоимости**.
...
Обнаружено на dogfood-сборке. `graph_store.verify --repair` вернул `clean` и `dangling_references: []` при 1838 висячих рёбрах и 322 компонентах: терпимость к рёбрам на несуществующие узлы заложена намеренно (внешние/ещё-не-созданные цели), но тем же механизмом маскируются **внутренние** промахи префикса. Нигде нет метрики «доля разрешённых рёбер», «число компонент связности», «крупнейший компонент», «doc-узлы без единого `documents`», «сироты». Поэтому фрагментацию видно только глазами в 3D-viewer. Нужен **приёмочный гейт связности** в bootstrap (и метрики в `verify`/`status`), чтобы плохая сборка была видна сразу и подсказывала «запусти глобальный линковщик». **Дом — Этап 19.**

---

### 1.45. Слабый глобальный смысловой линковщик: острова; `documents` необязателен; хабы рождаются поздно

Обнаружено на dogfood-сборке. Кросс-доменные смысловые рёбра (example↔doc, doc↔doc, ADR→код) в графе почти отсутствуют — узлы плавают островами. Три причины: (1) **билдер изолирован своим батчем**, кросс-доменную полноту он обеспечить не может by design (архитектурный размен ради контекста); (2) `documents` для doc-узлов **необязателен и не проверяется** — билдеру проще накидать `relates_to` на соседние секции (на dogfood 98 гайд-секций без рёбер, 135 только doc↔doc, ADR 0/60); (3) **хабы создаёт `amg-synth` ПОСЛЕ по-юнитных билдеров** — когда работали билдеры, узлов-хабов ещё не было, связать секцию с подсистемой было физически нечем, а сам synth недотянул хабы→гайды/ADR (на dogfood 11 хабов адоптировали по 9–33 узла **кода** каждый, но лишь 0–2 **гайда**; у тестового приложения `tests/app` — 144 внутрибатчевых ребра против **12** исходящих кросс-доменных: билдер хорошо связывает внутри своего батча и почти не связывает между доменами). Глобальную смысловую полноту обязан обеспечивать **сильный глобальный пасс поверх слоя summary** (summary — это сжатие: сотни КБ на весь граф, помещаются в контекст), с **кандидатами рёбер по эмбеддингам** (ANN top-K уже считаемых для засева векторов → LLM подтверждает — near-детерминированный бустер полноты, шардируемый). **Дом — Этап 19.**
...
**Происхождение.** Эти три этапа добавлены после первой сборки графа на **реальном проекте** (~1 МБ: исходный код, документация с ADR, тесты — предмет проекта для AMG не важен). Сборка выявила, что в текущей реализации памятью **пользоваться нельзя**: граф в 907 узлов собирался **~30 часов** (с прерываниями на лимиты) и **~70 Мтокенов**, вышел **фрагментированным** (322 компоненты связности, 267 сирот, 1838 висячих рёбер; практический слой — гайды — 56/234 в основном компоненте) и потребовал массы ручной доводки оператором. Полный разбор дефектов с подтверждением по коду — аудит §1.39–§1.49; здесь — план их устранения.
...
1. **Разрешение store — отличать реальный store от чекаута/шаблона (§1.39).** `graph_store.resolve_amg_root` (шаг 3 «первый `amg/config.yml` вверх») чинится так, чтобы кандидат-`amg/` признавался store, **только** если это инициализированный store (есть `nodes/` и `journal/`), а не исходный чекаут движка/шаблон (`config.yml` рядом с `skills/`/`install.py`). Приоритет пресетов `<agent_dir>/{.claude,.agents}/amg` над «голым» `amg/` предка; при неоднозначности — явная диагностика, а не молчаливый выбор пустого шаблона. Зеркально — `retrieve._default_store` (та же цепочка). Regression: селфтест сценария «движок-сосед с `config.yml` не перехватывает установленный store».
...
3. **Вопросы установочного потока (§1.49 п.2–3).** Поток (модель по INSTALL.md и/или CLI `install.py`) спрашивает про **absorb-источники и исключения**, а не только mirror; **показывает существующие значения** `config.yml` и просит подтвердить/изменить; предлагает `auto`-эмбеддинги при согласии пользователя (шаблон остаётся `off`).
...
- свежая установка в реальный проект **всегда** резолвит `<agent_dir>/amg` (никогда — чужой `amg/` чекаута/шаблона), не захламляет проект;
...
- селфтесты (вкл. новый сценарий разрешения store) зелёные; закрыты §1.37/§1.38/§1.39/§1.49.

Закрытие (доки): `12-install.md` (архитектура установщика, мерж, вопросы), `INSTALL.md` (место установки, команда, absorb/исключения), `09-config.md` (слои конфига и ключи, project-vs-personal), `03-storage.md`/`01-overview.md` (разрешение store — новая логика), `GUIDE.md`/READMEs (место установки, локально vs глобально, embeddings-дефолт). Новой теории этап, вероятно, не вводит (инфраструктура); если всплывёт принцип разрешения store — короткая заметка в THEORY.
...
5. **Кандидаты рёбер по эмбеддингам + масштабируемое подтверждение.** Переиспользовать уже считаемые для засева эмбеддинги (`embed.py`, кэш `cache/embeddings.json`): для каждого узла ANN top-K ближайших из **других** доменов → правдоподобные кросс-связи (ловит ровно случай «в обоих summary про одно и то же»: пример-контроллер ↔ гайд по той же теме — высокая близость). Линковщик **подтверждает кандидатов пачками** вида «узел + summary его K кандидатов»: контекст пачки ограничен, пачки **параллелятся**, но эффект **глобальный** (кандидаты берутся со всего графа через ANN — не мега-вызов); он же **ставит веса** подтверждённым рёбрам и, где уместно, **рождает хабы**. Так глобальная полнота достигается батчевым исполнением с глобальной информированностью — дёшево и масштабируемо на большие графы (не «мега-билдер №2»).
...
8. **Per-item валидация/нормализация на apply**: битый элемент (перепутанные `confidence`/`edges`, двойной префикс id) **чинится/пропускается**, не роняя весь батч; идемпотентность становится фактической.
9. **Приёмочный гейт связности**: bootstrap считает и показывает метрики качества графа (число компонент, доля в крупнейшем компоненте, доля разрешённых рёбер, doc-узлы без `documents`, сироты) как часть приёмки; те же метрики — в `verify`/`status`. Фрагментация видна сразу, без 3D-viewer, и подсказывает «запусти глобальный линковщик».

**Группа D — воспроизводимость (§1.46).**
10. **Persistent derivation-cache** по ключу `content_sha` (+ версия контракта деривации) вне `nodes/` (напр. `cache/derivations/`): тот же контент → та же сохранённая деривация → детерминизм на практике (сильнее `temperature=0`) и почти нулевая стоимость повторной/частичной сборки. Инвалидируется по смене контракта.
...
1. **Текст юнита в `queue.json` (§1.47, крупнейший выигрыш).** `reconcile._queue_item` кладёт уже нарезанный `extract_structure` текст юнита (`slice_src` и аналоги) для кода/markdown, а не только для бинарных форматов. Билдер делает ~1 чтение (очередь) вместо десятков `Read`/`Grep` → обвал cache-read/ресенда. Контент тот же — **качество не меняется**; попутно снимается половина причины висячих id (билдеру меньше поводов угадывать).
2. **Агрегация/пропуск тривиальных юнитов (§1.47).** Дандеры и однострочные геттеры не суммировать каждый отдельной LLM-задачей — агрегировать (напр. сводкой класса) или пропускать под guard'ом (значимые дандеры — `__call__`/`__enter__` — сохранять). Конфиг-ключ порога.
...
- селфтесты зелёные; закрыты §1.47/§1.48.
