...
10. **Живой прогон на dogfood-стенде** исправленным движком ПОСЛЕ задач 1–8 (сессии пользователя, вне dev-сессий): связность (один крупный компонент, ~0 внутренних висячих, `gate: ok`) **и** стоимость — число субагентов/ходов по фазам и токены из панели пользователя, сравнение с базлайном первой сборки (~30 ч / ~70 Мтокенов) и второй (~2 ч / ~80 Мтокенов cache-read). Ограничение диагностики (поле): среда не персистит транскрипты субагентов (файлы 0 байт) — ретроспективный разбор cache-read невозможен, надёжный per-agent сигнал только `<usage>subagent_tokens` из уведомления о завершении (вести по ходу). Цель ×20–30 к первичному базлайну подтверждается, либо остаток раскладывается по статьям с решением по каждой. Здесь же решается судьба задачи 6 (напоминание), проверяется приём внутриагентного прогресса (задача 8) и снимаются два отложенных dogfood-пункта Этапа 17 (эмиссия паттернов живым synth; `usage.log` начинает копиться для будущего флипа Хебба).
11. **Измерительные резервы стоимости — только под eval, дефолты без замера не менять.** После задач 1–3 структурных резервов не остаётся (стоимость = текст юнитов один раз + вывод сводок + O(раундов) ходов); дальнейшее сжатие возможно лишь ценой, которую обязана одобрить метрика: (а) `models.module_summary: haiku` — тиринг уже позволяет, нужен только замер recall/hop-recall off/on на стенде (сводки питают BM25/эмбеддинги — понижение модели без замера запрещено конвенцией 09-config); (б) капы партий `builder.batch_units` 40→60–80 (меньше пачек = меньше ходов и результатов у оркестратора; следить за потолком вывода агента); (в) `linker.top_k`/`min_sim` (меньше кандидатов = дешевле подтверждение, риск недосвязности — смотреть на гейт; **полевые данные третьей установки:** линкеры отбрасывают 60–70 % номинаций как случайные, пачка ≈130–150 тыс. токенов — самый дорогой субагент, 15 % всего расхода; резать `top_k 5→3` / `min_sim 0.35→0.5` только с eval-контролем — реальное doc↔code ребро бывает с умеренным косинусом). Итоги замеров — в протокол 09-config «Модели субагентов»; отвергнутое фиксировать с числами.
...
- живой прогон: `gate: ok` и измеренное кратное снижение стоимости против второй установки (порядок — единицы Мтокенов чтения кэша, не десятки); вердикт по ×20–30 зафиксирован в этом этапе;
