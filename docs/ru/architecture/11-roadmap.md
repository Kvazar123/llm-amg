...
Остаётся **систематический сквозной проход** (назначен в Этап 21, задача ниже): пройти весь код движка на любые зашитые языковые допущения (списки слов, стоп-слова, англо-эвристики классификации/маршрутизации) и подтвердить, что всё язык-зависимое идёт через `working_language` или модель. Попутная мелочь: пример `--grep роутинг` в докстринге `inspect_graph.py` нейтрализовать на язык-нейтральный.
...
**Дом:** отдельное косметическое исправление между этапами (bump `z`) либо проход Этапа 21 (он и так правит эти доки). **Вне периметра Этапа 16** (записано по CLAUDE.md §4.3). Связано: при «графе в git» лог-файл попадает в рекомендованный `.gitignore` хранилища (Этап 16).
...
Альтернатива (B) — убрать ложное обещание из `INSTALL.md` — **отклонена** (контракт удобен и уже задокументирован). **Вне периметра Этапа 17** (config/install-территория); **дом — Этап 18 (развёртывание и разрешение store)**, куда сведены все config/install-дефекты. Записано по CLAUDE.md §4.3.
...
Обнаружено на dogfood-сборке. `graph_store.resolve_amg_root` (шаг 3) берёт **первый** каталог вверх от cwd, содержащий `amg/config.yml`, и считает его store. Если движок AMG распакован в проект/рядом папкой с именем `amg` (или его собственные dev-данные `<repo>/amg/`), у неё есть корневой `config.yml` — и резолвер молча выбирает **её**, а не установленный `<agent_dir>/amg`. **Почему на dogfood система «искала каталоги `src` и `doc`»** (вопрос пользователя): у шаблонного `config.yml` движка ключ `mirror_path: [src, doc]` задан как **пример**; когда резолвер выбрал этот шаблон за store, сверка честно пошла искать источники `src`/`doc` — которых в реальном проекте нет → `missing_sources:["doc"]`, 0 узлов, пока пользователь не задал `AMG_AGENT_DIR=.claude` вручную. То есть искомые папки берутся не «ниоткуда», а из примера в перехваченном шаблоне. Корень: резолвер **не отличает инициализированный store (есть `nodes/`, `journal/`) от исходного чекаута/шаблона** (`config.yml` рядом с `skills/`, `install.py`, без `nodes/`). Это дефект движка, а не операторская ошибка (она им спровоцирована). **Дом — Этап 18** (вместе с §1.49 — две стороны «движок внутри проекта»).

---

### 1.40. `calls`-рёбра не резолвятся: висячие на builtins и межфайловые вызовы

Обнаружено на dogfood-сборке. `reconcile._structural_edges` эмитит для каждого имени из `calls` ребро `to = f"code:{rel}::{callee}"`, приклеивая **голое имя к пути того же файла**. Последствия: (1) вызовы встроенных (`isinstance`, `type`, `.get`, `startswith`…) → узла нет → висячее (на dogfood — подавляющее большинство всех висячих рёбер, чистый шум); (2) **межфайловый** вызователь приписывается своему файлу → неверный id → висячее, то есть самое ценное cross-file `calls`-ребро **не создаётся вовсе**. Комментарий в коде прямо признаёт «cross-file calls ignored **until resolved**», но пасса резолва **нет**. Это невыполненная половина `§4.2` (детерминированные рёбра до LLM) и продолжение резолвера импортов §1.19 (импорты резолвятся, вызовы — нет). **Полный разбор висячих рёбер на dogfood** (диагностика оператора, три класса по убыванию доли): **шумовые** `calls` на builtins/методы (`isinstance`/`type`/`.get`/`startswith`… — узлов нет и не будет, инертны, чистить не нужно); **законно-внешние** `imports` (stdlib и сторонние библиотеки — корректно мёртвые, оставить); **чинимые** рёбра — меньшая, но самая ценная часть (реальный внутренний узел существует, но цель записана без корневого префикса пути — это разрешаемые `documents`/`calls`/`imports`), их и привязывает резолвер (§1.42). Вывод: подавляющая масса висячих — либо инертный шум, либо законно-внешние; ценные внутренние промахи — чинимый класс плюс невыполненные cross-file `calls`. **Дом — Этап 19.**

---

### 1.41. Нет структурного «хребта»: ни `defines` (module→symbol, class→method), ни наследования

Обнаружено на dogfood-сборке. `_structural_edges` эмитит только `imports`/`calls`/`follows`; **ни одного ребра принадлежности** module→function/class или class→method. `_part_of_for` даёт `topic` = **строка-каталог** (напр. `src/pkg/core`), а не id узла, поэтому листовые узлы не связаны со своим модулем (на dogfood ни один функциональный узел не был связан со своим модулем; листовые методы-дандеры — сироты). Наследование (`class X(Base)`) `extract_structure._python_units` не извлекает вовсе (`node.bases` не читается) — ловится лишь если LLM заметит как `depends_on`, ненадёжно. **Двойной удар:** backbone нужен и извлечению (связность), и **консолидации** — аудит §1.20 научил `consolidate._branch_members` ходить ВНИЗ по `HUB_DOWN_RELS` (`documents`/`defines`/`specifies`/`implements`/`contains`), но раз структурный `defines` никто не эмитит, обход держится лишь на **ненадёжных LLM-рёбрах** `documents`/`implements` — на реальном графе бюджеты веток/компрессия тоже вырождены. **Дом — Этап 19.**

---

### 1.42. Билдер слеп к каноническим id → систематический промах префикса

Обнаружено на dogfood-сборке. `reconcile._queue_item` кладёт в очередь только `{id, kind, source_path, category, content_sha, qualname, lineno, lang}` (+ `text` лишь для юнитов, уже несущих его из нарезателя — бинарные форматы, CSV, log, чат), **без манифеста существующих id и без резолвинга на apply**. Промпт `amg-builder` просит «форму `code:<path>::<qual>`» и «не выдумывать цели», но проверить существование цели билдеру нечем, и он видит только свой батч. Читая `src/pkg/core/foo.py`, он логично пишет `code:core/foo.py::Bar` или `code:pkg/...` — правдоподобно, но не совпадает с каноном `code:src/pkg/core/foo.py::Bar`. Это весь класс «висячих `documents`». Эмпирика: **дав билдерам манифест id, оператор получил 100% резолва** — прямое доказательство, что дефект в отсутствии манифеста/резолвинга, а не в модели. **Правильное решение — детерминированный резолвер (§1.40/§1.41) + нормализация id на apply (§1.43) + сильный глобальный линковщик (§1.45)**, а не манифест-костыль билдеру. **Дом — Этап 19.**

---

### 1.43. `apply_derivation` не валидирует item → один битый элемент рушит весь батч

Обнаружено на dogfood-сборке. `reconcile.apply_derivation` применяет элементы без per-item валидации/нормализации и без try/except вокруг каждого: билдер перепутал поля (`confidence` держал массив рёбер, `edges` — float `0.9`) → `_merge_edges` итерирует float → `TypeError`, **падает весь `apply`**; отдельно встречался двойной префикс id (`hub:overview:<topic>` вместо `overview:<topic>`). То есть идемпотентность «на бумаге» есть, «на практике» ломается: сбой на одном элементе блокирует всю партию и требует ручной починки. Нужна per-item валидация схемы с авто-нормализацией/пропуском битого (не роняя батч) и нормализация неканонических id, когда канон существует. **Дом — Этап 19.**

---

### 1.44. Нет метрики/гейта связности: фрагментация невидима

Обнаружено на dogfood-сборке. `graph_store.verify --repair` вернул `clean` и `dangling_references: []` при тысячах висячих рёбер и сотнях компонент связности: терпимость к рёбрам на несуществующие узлы заложена намеренно (внешние/ещё-не-созданные цели), но тем же механизмом маскируются **внутренние** промахи префикса. Нигде нет метрики «доля разрешённых рёбер», «число компонент связности», «крупнейший компонент», «doc-узлы без единого `documents`», «сироты». Поэтому фрагментацию видно только глазами в 3D-viewer. Нужен **приёмочный гейт связности** в bootstrap (и метрики в `verify`/`status`), чтобы плохая сборка была видна сразу и подсказывала «запусти глобальный линковщик». **Дом — Этап 19.**

---

### 1.45. Слабый глобальный смысловой линковщик: острова; `documents` необязателен; хабы рождаются поздно

Обнаружено на dogfood-сборке. Кросс-доменные смысловые рёбра (пример↔дока, doc↔doc, ADR→код) в графе почти отсутствуют — узлы плавают островами. Три причины: (1) **билдер изолирован своим батчем**, кросс-доменную полноту он обеспечить не может по построению (архитектурный размен ради контекста); (2) `documents` для doc-узлов **необязателен и не проверяется** — билдеру проще накидать `relates_to` на соседние секции (на dogfood почти все разделы практического слоя остались либо вовсе без рёбер, либо связанными только между собой; слой ADR — полностью изолированный остров); (3) **хабы создаёт `amg-synth` ПОСЛЕ по-юнитных билдеров** — когда работали билдеры, узлов-хабов ещё не было, связать секцию с подсистемой было физически нечем, а сам synth недотянул хабы→практический слой (на dogfood хабы адоптировали в основном узлы **кода** и почти ни одного раздела руководств; изолированный батчем домен примеров получил на порядок больше внутрибатчевых рёбер, чем исходящих кросс-доменных: билдер хорошо связывает внутри своего батча и почти не связывает между доменами). Глобальную смысловую полноту обязан обеспечивать **сильный глобальный пасс поверх слоя summary** (summary — это сжатие: сотни КБ на весь граф, помещаются в контекст), с **кандидатами рёбер по эмбеддингам** (ANN top-K уже считаемых для засева векторов → LLM подтверждает — near-детерминированный бустер полноты, шардируемый). **Дом — Этап 19.**

Две смежные проверки (чтобы будущая сессия не пере-расследовала): (а) **порядок «сначала все узлы, потом связи» уже соблюдён** — `reconcile bootstrap` пишет полный структурный скелет **всех** узлов до любой деривации; настоящая ордеринг-проблема только в тайминге **хабов** (п. 3), не в узлах-источниках; (б) **направленность PPR — не дефект:** `retrieve.build_adjacency` **симметризует** рёбра (`acc[(u,v)]+=c; acc[(v,u)]+=c`), поэтому направленное `documents` (doc→code) проводит в обе стороны — засев от кода дотягивается до доки и наоборот; отдельного действия не требует. (в) **absorb/mirror — НЕ причина недосвязи** (модель это явно исправила про себя): впитанные (`absorb`) источники обязаны линковаться так же полно, как зеркала; острова порождает изоляция билдера батчем, а не политика источника — важно, чтобы будущая сессия не свалила фрагментацию на `absorb`.

---

### 1.46. Недетерминированная сборка: нет derivation-cache; нестабильная таксономия хабов

Обнаружено на dogfood-сборке. Деривации (summary+рёбра) хранятся **только в самих узлах** и теряются при сборке с нуля, поэтому каждый `wipe+rebuild` передеривирует всё заново и **по-другому** (LLM недетерминирован) — «граф каждый раз другой». Идемпотентность инкрементальной сверки есть, а **воспроизводимости с нуля нет**. Лечится **persistent derivation-cache** по ключу `content_sha` (+ версия контракта деривации) вне `nodes/` (напр. `cache/derivations/`): тот же контент → та же сохранённая деривация → детерминизм на практике (сильнее, чем `temperature=0` и закреплённая версия модели — те лишь снижают разброс вывода, а кэш даёт полную воспроизводимость и снимает стоимость повтора) **и почти нулевая стоимость повторов**. Смежно: `amg-synth` выдумывает id хабов каждый прогон → нестабильный стратегический слой и нестабильные цели линковки между пересборками — нужна более детерминированная таксономия хабов (напр. кандидаты из структуры каталогов/кластеризации, synth лишь именует/уточняет). **Дом — Этап 19.**

---

### 1.47. Дорогая сборка: текст юнита не в очереди → билдеры перечитывают исходники

Обнаружено на dogfood-сборке (крупнейший сток токенов). `reconcile._queue_item` кладёт `text` только для юнитов, уже несущих его из нарезателя (бинарные PDF/DOCX/XLSX/PPTX, CSV, log-эпизоды, чат); для **кода и markdown** — подавляющей массы реального проекта — билдер получает лишь путь+`lineno` и сам открывает файлы через `Read`/`Grep` (`inspect_queue` показал `with_text: 0`). Отсюда десятки tool-uses на батч, а каждый ход агента **ресендит весь накопленный контекст** → десятки Мтокенов cache-read (соотношение read:write ≈ 2.5:1 — прямой признак переотправки одного префикса). При этом `extract_structure` **уже нарезал текст юнита** (`slice_src` в `_python_units` и т. п.) — и выбросил. Положить срез в `queue.json` = ~1 чтение вместо десятков, обвал cache-read, **×5–10 по токенам без потери качества** (контент тот же), и попутно снимается половина причины висячих id (§1.42). Смежно: тривиальные юниты (дандеры, однострочные геттеры) суммируются каждый отдельной LLM-задачей — агрегация/пропуск под guard'ом (часть дандеров значима) режет число вызовов; толстая оркестрация (сырые листинги в главный Opus-контекст) ресендится каждый ход — держать оркестрацию тонкой (агрегаты, делегировать чтение субагентам). **Дом — Этап 20.**

---

### 1.48. Билдеры не чекпоинтят/не резюмируют внутри батча; ложный «Done» при обрыве; нет прогресса

Обнаружено на dogfood-сборке. Билдер пишет `derived-*.json` **в конце** батча, поэтому обрыв связи/лимит/превышение output-лимита (32K вывода на крупном плотном батче) **теряет часы работы и миллионы токенов** — приходится начинать батч заново (возобновляемость Этапа 12 спасает между `apply`-ами, но не внутри незавершённого батча). Нужны: **инкрементальные чекпоинты** (дописывать деривацию по мере готовности, чтобы почти-законченный батч не пропадал), **авто-чанкинг** по лимиту вывода (дробить крупный батч заранее), **авто-resume** павших батчей. Отдельно — гигиена сигнала завершения: упавший в середине агент рапортует «Done»/«Finish», не отличить успех от обрыва (`subagent_tokens: 0`); нужен **явный маркер успех/обрыв** и **индикатор прогресса в процентах** (сколько юнитов очереди уже деривировано/применено), чтобы оркестратор и человек видели, какая часть пути пройдена. **Дом — Этап 20.**

---

### 1.49. Установка: захламление проекта, не спрашивается absorb/исключения/существующий конфиг

Обнаружено на dogfood-сборке. **(1) Место движка и захламление — суть обратна тому, как кажется на первый взгляд.** Чтобы сработала модель-ведомая установка «установи AMG по INSTALL.md», движок сейчас должен лежать **внутри** проекта (иначе `INSTALL.md` не найти). Обе текущие раскладки плохи, каждая по-своему: распаковать **все файлы движка прямо в корень проекта** (как исходно и рассчитывал установщик — `INSTALL.md` в корне) — **захламляет** проект россыпью чужих файлов; распаковать в **подпапку** (напр. `amg/`) — **НЕ** захламляет и легко удаляется, но её шаблонный `config.yml` (с примером `mirror_path: [src, doc]`) перехватывается `resolve_amg_root` → срабатывает §1.39 (система индексирует пустой шаблон и ищет `src`/`doc`). Именно так и вышло на dogfood: распаковка в подпапку → поиск несуществующих `src`/`doc`. Правильно: установка **не должна требовать движок внутри проекта** — ставить из каталога **вне** проекта (или глобально) командой `install.py --target <проект>`; исходный чекаут после установки не нужен (движок скопирован в `<agent_dir>/`). **Переписать README/INSTALL/GUIDE**, где установка вызывается «из корня проекта», на средо-нейтральную (движок снаружи/глобально, `--target`). (2) Установочный поток (модель по INSTALL.md и/или CLI) спрашивает только про mirror-каталоги, **не спрашивая про absorb и исключения**, и **не показывает существующие значения конфига** для подтверждения. (3) `embeddings.enabled` в шаблоне — `off`; шаблон оставить консервативным, но install-поток предлагает `auto`, когда пользователь согласился на эмбеддинги. **Дом — Этап 18.** Тесно связан с §1.39 (перехват store) — это две стороны одной проблемы «движок внутри проекта».
...
Обнаружено аудитом. `extract_structure.load_config` читает `<amg_root>/config.yml` без обработки отсутствия файла: в свежем проекте без установки (резолвер упал на умолчание `<project>/.claude/amg`, которого нет) `reconcile.py bootstrap .` и `extract_structure.py .` падают необработанным `FileNotFoundError` вместо понятного «AMG в этом проекте не установлен: не найден `<agent_dir>/amg/config.yml`». Остальные загрузчики (`retrieve.load_config`, `consolidate.load_config`, `lifecycle._read_config`) отсутствие переживают. Тот же сценарий — первый симптом промаха разрешения store (§1.39): дружелюбная диагностика должна называть, **какой** корень был разрешён и почему он не выглядит установленным store. **Дом — Этап 18** (территория развёртывания/разрешения store).

---

### 1.51. Абзацный нарезатель не проставляет реальные номера строк

Обнаружено аудитом. `extract_structure._text_units` даёт каждому блоку `lineno: 1` (и не считает `line_end`): у обычного текста и неоднозначных файлов без расширения все указатели `путь:строка` ведут на первую строку файла. Ломается точность указателя в пакете и фокус билдера на срезе (у markdown/RST/log/сессий номера настоящие — дефект только у `paragraphs`). Починка тривиальна — резать с отслеживанием позиций строк, как соседние нарезатели. **Дом — Этап 20** (тот же круг файлов: текст юнита и срезы в очереди).

---

### 1.52. Точечные рассогласования документации с кодом (накопились после Этапов 12–17)

Обнаружено аудитом; три конкретных дрейфа, каждый ждёт свой этап:

- **`07-consolidation.md`, раздел «Применение»** до сих пор описывает `log.md` как «best-effort append вне транзакции… пишет его только консолидация — `reconcile.py` в `log.md` не пишет вовсе» — противоречит реализованному на Этапе 12 транзакционному логу (дедуп по `txid`, ротация, пишут оба слоя) и его же описанию в `03-storage`/`02-data-model`/`consistency-model §11`. Абзац пропущен синхронизацией Этапа 12. **Дом — Этап 21**, рядом с §1.36 (тот же набор файлов при переводе лога в `.log`).
- **`09-config.md`** документирует `schema_version: 2`, шаблон `config.yml` несёт `3` (поднято Этапом 13). **Дом — Этап 18** (09-config правится там по слоям конфига).
- **`INSTALL.md`** не описывает режим `--env codex` (в перечне флагов и разделе «Другие среды» только `claude-code|generic`), хотя код (`install.py _env_kind`), `12-install.md` и GUIDE его несут с подэтапа 1.1.0. **Дом — Этап 18** (INSTALL.md там переписывается под установку извне проекта).
...
> **Статус (dogfood, Этап 17):** этот принцип реализован **лишь частично** — импорты резолвятся (§1.19), а `calls`, backbone `defines` (module→symbol, class→method) и наследование **не извлекаются или висят** (§1.40/§1.41), из-за чего реальный граф вышел фрагментированным. До-выполнение — **Этап 19, Группа A**. Пункты ниже задают целевой (полный) набор.
...
Цель: установка AMG в реальный проект надёжна — операции **всегда** бьют в нужный store, установка не захламляет проект, поток задаёт правильные вопросы, глобальный→локальный конфиг наследуется. Сюда сведены все config/install-дефекты (§1.37/§1.38/§1.39/§1.49).

Задачи:

1. **Разрешение store — отличать реальный store от чекаута/шаблона (§1.39).** `graph_store.resolve_amg_root` (шаг 3 «первый `amg/config.yml` вверх») чинится так, чтобы кандидат-`amg/` признавался store, **только** если это инициализированный store (есть `nodes/` и `journal/`), а не исходный чекаут движка/шаблон (`config.yml` рядом с `skills/`/`install.py`). **Критично: проверка «инициализированности» сама по себе недостаточна** — один-единственный промахнувшийся прогон (`plan()` безусловно зовёт `GraphStore.init()`) создаёт `nodes/`+`journal/` прямо в перехваченном чекауте, после чего наивная проверка проходит; поэтому обязателен и **вето по сигнатуре движка**: кандидат-каталог, содержащий `skills/`, `agents/` или `install.py`, — чекаут, а не store, независимо от наличия `nodes/`/`journal/` (дев-раскладку самого репозитория покрывает шаг 4 цепочки — расположение движка). Приоритет пресетов `<agent_dir>/{.claude,.agents}/amg` над «голым» `amg/` предка; при неоднозначности — явная диагностика, а не молчаливый выбор пустого шаблона. Зеркально — `retrieve._default_store` (та же цепочка). Regression: селфтесты сценариев «движок-сосед с `config.yml` не перехватывает установленный store» и «чекаут с случайно созданными `nodes/`+`journal/` тоже не перехватывает». — **выполнено** (вето `_looks_like_engine_checkout` + `_initialized_store` для «голого» `amg/`; пресеты проверяются первыми и инициализированности не требуют — иначе ломается ручная установка «конфиг руками → первый bootstrap сам init»; `selftest_reconcile` случаи 15–16). **Два решения сверх буквы задачи, принятых при проектировании** (задача 4 воспроизводила §1.39 на уровне дома): (а) восходящий поиск **пропускает уровень домашнего каталога** — `~/<agent_dir>/amg/config.yml` это слой глобальных умолчаний, не store; (б) шаг 4 (расположение движка) требует **инициализированный** store, не просто существующий каталог — иначе глобальный движок с появившимся `~/.claude/amg/` перехватывал бы умолчание свежего проекта. Детали → 03-storage «Интерфейс командной строки».
2. **Место установки и команда (§1.49 п.1).** Установка **не должна требовать движок внутри проекта**: ставить из каталога **вне** проекта (или глобально) командой `install.py --target <проект>`; исходный чекаут после установки **не нужен** (движок скопирован в `<agent_dir>/`). Иначе — либо россыпь файлов движка в корне проекта (захламление), либо подпапка-`amg/`, чей шаблонный `config.yml` перехватывается резолвером (§1.39). **Переписать README/INSTALL/GUIDE**, где установка вызывается «из корня проекта», на средо-нейтральную (движок снаружи/глобально, `--target`). Разъяснить локальную vs глобальную установку и почему глобальная чище (движок вне репозитория, нет §1.39 и §1.38). — **выполнено** (README/README_RU/INSTALL/GUIDE переписаны: исходный каталог вне проекта, модель-ведомая установка по пути `<путь-к-AMG>/INSTALL.md`, после установки каталог удаляем; кода не потребовалось — `install.py` уже работал от `Path(__file__)`).
3. **Вопросы установочного потока (§1.49 п.2–3).** Поток (модель по INSTALL.md и/или CLI `install.py`) спрашивает про **absorb-источники и исключения** (включая `absorb_once` как вариант ответа), а не только mirror; **показывает существующие значения** `config.yml` и просит подтвердить/изменить; предлагает `auto`-эмбеддинги при согласии пользователя (шаблон остаётся `off`). Заметка по реализации (из аудита): `install._set_scalar`/`--set` умеют только ключи **верхнего уровня** (якорь `^(# ?)?ключ:`), а `retrieval.embeddings.enabled` — вложенный ключ; для него нужен вложенный сеттер (или отдельный флаг), иначе `--set enabled=auto` допишет неверный top-level ключ. CLI дополняется флагом для `absorb_once_path` (сейчас только `--mirror`/`--absorb`/`--exclude`). — **выполнено** (`_set_nested` для точечных путей в `--set`, флаги `--absorb-once` и `--set-global`; существующий конфиг печатается строкой `in force:`, INSTALL.md велит модели показать и подтвердить; `selftest_install.test_nested_set_and_absorb_once`).
4. **Мерж глобального→локального конфига (§1.37, вариант A).** Установщик пишет глобальный конфиг общих умолчаний по `~/<agent_dir>/amg/config.yml` (путь — через ту же цепочку `resolve_amg_root`, разные каталоги сред); загрузчики (`retrieve`/`consolidate`/`extract_structure`.`load_config`) читают глобальный→локальный с переопределением по ключам (переиспользовать `_deep_merge`). Разделение project-vs-personal: проектное (`mirror_path`/`working_language`) — локально (git-канон), личное одной машины (`models`, бэкенд эмбеддингов) — глобально (в репозиторий не попадает). — **выполнено**, с тремя решениями «продумать ключи»: (а) глобальный слой = **блоки `models` и `retrieval.embeddings` целиком** (из шаблона + `--set-global`), всё остальное проектное и остаётся локальным; (б) при глобальной установке локальный конфиг пишется **без** этих блоков (`_strip_top_block`/`_strip_sub_block`) — полный шаблон затенял бы глобальный слой каждым ключом; локальная установка самодостаточна (полный шаблон, глобальный файл не создаётся); (в) **наследование включает ключ `agent_dir` локального конфига** (он и называет домашний слой среды, и метит конфиг как установленный) — минимальные рукописные конфиги/тест-фикстуры остаются герметичными, чужой глобальный конфиг машины не подмешивается в селфтесты. `_read_models` установщика рендерит субагентов из того же объединённого взгляда; `--uninstall` глобальный конфиг сохраняет (данные предпочтений). Regression: `selftest_install.test_global`/`test_project_only_global`; e2e — глобальный движок ведёт bootstrap в локальный store, домашний слой остаётся config-only.
5. **gitignore движка при локальной установке (§1.38).** GUIDE «Граф в git»: для команды предпочтительна глобальная установка (движок вне репозитория); при локальной — либо игнорировать движок в `.gitignore`, либо коммитить единообразно. — **выполнено** (GUIDE «Граф в git», абзац «Движок при командной работе лучше держать вне репозитория»; заодно связан с новым слоем — личные настройки при глобальной установке не попадают в канон).
6. **Дружелюбная диагностика «AMG не установлен» (§1.50).** `extract_structure.load_config` (и путь `reconcile bootstrap` через него) при отсутствии `config.yml` по разрешённому корню сообщает понятным текстом, какой корень был разрешён и что в нём нет установленного store, вместо необработанного `FileNotFoundError`; выровнять с мягким поведением остальных загрузчиков. — **выполнено** (SystemExit с именем разрешённого корня и подсказкой `--root`/`AMG_AGENT_DIR`/`install.py`; намеренно **не** молчаливый `{}` — извлечение без конфига дало бы замаскированный пустой прогон «added: 0», ровно тот класс отказов, что этап устраняет; `selftest_reconcile` случай 16).

Definition of done:

- свежая установка в реальный проект **всегда** резолвит `<agent_dir>/amg` (никогда — чужой `amg/` чекаута/шаблона, даже «случайно инициализированный»), не захламляет проект;
- поток спрашивает absorb/исключения и подтверждает существующий конфиг;
- глобальные умолчания наследуются локальным по ключам; проектное и личное разведены по слоям;
- запуск в неустановленном проекте даёт диагностику, а не трассировку;
- селфтесты (вкл. новые сценарии разрешения store) зелёные; закрыты §1.37/§1.38/§1.39/§1.49/§1.50 и части §1.52 (INSTALL `--env codex`, `schema_version` в 09-config).

Закрытие (доки): `12-install.md` (архитектура установщика, мерж, вопросы), `INSTALL.md` (место установки, команда, absorb/исключения; попутно — режим `--env codex` в флагах и «Других средах», §1.52), `09-config.md` (слои конфига и ключи, project-vs-personal; попутно `schema_version` 2→3, §1.52), `03-storage.md`/`01-overview.md` (разрешение store — новая логика), `GUIDE.md`/READMEs (место установки, локально vs глобально, embeddings-дефолт). В правимых этим этапом файлах попутно обновить устаревшие ссылки «Этап 19» (проверка сред) → «Этап 22» — частичное досрочное закрытие сквозной сверки Этапа 21. Новой теории этап, вероятно, не вводит (инфраструктура); если всплывёт принцип разрешения store — короткая заметка в THEORY.

*Пометки рабочих сессий (2026-07-03, для закрывающей сессии):* весь список доков выше **уже синхронизирован** в рабочих сессиях (12-install/INSTALL/09-config/03-storage/01-overview/GUIDE/READMEs, включая обе части §1.52); устаревшие «Этап 19 (среды)» заменены **неявной формулировкой** («отдельный этап дорожной карты впереди») — предпочтительный по слоям вариант из списка Этапа 21; досрочно поправлены и не входившие в периметр `08-agents-skills.md` (та же ссылка) и `entrypoint/AGENTS.codex.md` (будущее «Stage 19» в комментарии блока — см. новый пункт Этапа 21 о ссылках на roadmap из управляющего слоя). Решение по THEORY: новая теория **не вводится** — разрешение store и слои конфига суть механика развёртывания, а не «почему» памяти (подтвердить при закрытии). Остаток закрывающей сессии: DoD + селфтесты, сворачивание (§1.37/§1.38/§1.39/§1.49/§1.50, части §1.52, тело этапа), проверка строки Этапа 19 в таблице загрузки BASE-PROMPT, выпуск v1.9.0.

---

## Этап 19 — корректная и связная сборка графа

Цель: `bootstrap` **всегда** даёт связный, воспроизводимый граф — без островов, без висячих внутренних рёбер, — до-выполняя `§4.2` (детерминированные рёбра до LLM) и добавляя сильный глобальный смысловой линковщик, приёмочный гейт связности и воспроизводимость. Устраняет §1.40–§1.46.

**Группа A — детерминированная структурная полнота + глобальный резолвинг ссылок (§1.40/§1.41/§1.42; до-выполнение §4.2).** **Самый важный рычаг этапа:** один детерминированный проход одним махом убирает и «висячие префиксы», и «cross-file, отложенные до резолва» — без LLM и без манифеста в промпте билдера.
1. **Детерминированный глобальный пасс резолвинга ссылок (ключевой).** После создания **всех** узлов (скелет `bootstrap` пишет их до деривации) — механически привязать **каждую** цель ребра к каноническому id, где он существует: (а) **починка префикса** — `code:core/foo.py::Bar` → `code:src/pkg/core/foo.py::Bar`, когда такой узел есть; (б) **разрешение импортированного имени** в id узла по таблице импортов модуля-владельца (тот же приём, что уже резолвит импорты §1.19 через `_module_map` — распространить на `calls`); (в) **привязка cross-file целей** (вызов функции из другого модуля → её реальный узел). Внешние/неразрешимые (`isinstance`/`.get`/stdlib/сторонние) — **не эмитятся** и не «висят». Пасс **детерминирован и идемпотентен** (повторный прогон — тот же результат), **без LLM и без манифеста билдеру**, и снимает весь класс §1.40 (висячие `calls`) и §1.42 (промах префикса) **разом**. Работает и над структурными рёбрами при извлечении, и над LLM-рёбрами на `apply` (нормализация цели, когда канон существует). Комментарий `_structural_edges` («cross-file calls ignored **until resolved**») намекает, что такой пасс задумывался, но не реализован. Дом — `reconcile._structural_edges`/`_module_map` + новый резолвер-пасс + `reconcile.apply_derivation`.
2. **Backbone `defines`**: детерминированные рёбра принадлежности module→function/class и class→method (сейчас их нет; part_of кодового узла — строка-каталог, не id). Чинит и связность извлечения, и downward-обход `consolidate._branch_members` (§1.20 без структурного `defines` держится лишь на ненадёжных LLM-рёбрах `documents`/`implements`).
3. **Рёбра наследования**: `extract_structure._python_units` извлекает `node.bases` → ребро (напр. `depends_on`/`inherits`) для `class X(Base)`; для прочих языков — по возможности через tree-sitter. Их цели привязывает тот же пасс (1).

**Группа B — сильный глобальный смысловой линковщик (§1.45).**
4. **Сильный глобальный смысловой пасс линковки.** Усилить `amg-synth` (или отдельную роль) так, чтобы после готовности **всех** summary он прошёл по всему набору узлов и достроил кросс-доменные рёбра **по смыслу, автоматически** — без ручного манифеста и без инструкций пользователя: пример-код→фича, пример↔дока, doc↔doc между файлами, ADR→код. Ключевая мысль, снимающая мнимый спор «батчи vs мега-билдер»: линковке **не нужны исходники — нужен слой summary**, а summary — это уже сжатие (сотни КБ на весь граф, помещаются в контекст сильной модели или в несколько проходов). Причина островов — изоляция билдера своим батчем (кросс-домен он обеспечить не может by design); глобальную полноту обязан давать этот пасс, **который сейчас synth недотягивает** (сфокусирован на хабах и framework doc↔code, пропускает пример↔дока и doc↔doc). Дорогую работу (чтение каждого юнита) делают дешёвые параллельные батчи один раз; глобальную полноту — отдельный пасс поверх сжатого слоя.
5. **Кандидаты рёбер по эмбеддингам + масштабируемое подтверждение.** Переиспользовать уже считаемые для засева эмбеддинги (`embed.py`, кэш `cache/embeddings.json`): для каждого узла ANN top-K ближайших из **других** доменов → правдоподобные кросс-связи (ловит ровно случай «в обоих summary про одно и то же»: узел-пример ↔ раздел руководства той же темы — высокая близость). Линковщик **подтверждает кандидатов пачками** вида «узел + summary его K кандидатов»: контекст пачки ограничен, пачки **параллелятся**, но эффект **глобальный** (кандидаты берутся со всего графа через ANN — не мега-вызов); он же **ставит веса** подтверждённым рёбрам и, где уместно, **рождает хабы**. Так глобальная полнота достигается батчевым исполнением с глобальной информированностью — дёшево и масштабируемо на большие графы (не «мега-билдер №2»). Две готовые опоры из аудита: (а) **summary-слой уже материализован** производным read-индексом `cache/index.sqlite` (проекция id/type/status/summary) — линковщику и подготовителю его пачек не нужен повторный скан `nodes/*.md`; (б) **без установленного бэкенда эмбеддингов** ANN-кандидатов нет — линковщик мягко деградирует до работы по summary-слою без предфильтра (медленнее/дороже, но функционально; та же мягкая деградация, что у засева).
6. **`documents` обязателен и проверяется** для doc-узлов: промпт билдера/линковщика требует `documents`→код, а гейт (Группа C) сигналит doc-узлы без него.
7. **Порядок и таксономия хабов (§1.46)**: хабы существуют **до** финального пасса линковки (чтобы секции связывались с подсистемами); таксономия хабов более детерминирована (кандидаты из структуры каталогов/кластеризации, synth именует/уточняет) — стабильный стратегический слой между пересборками.

**Группа C — устойчивость apply и приёмочный гейт (§1.43/§1.44).**
8. **Per-item валидация/нормализация на apply**: битый элемент (перепутанные `confidence`/`edges`, двойной префикс id, отсутствующий `id`, не-списочные `edges`/`part_of`) **чинится/пропускается**, не роняя весь батч; идемпотентность становится фактической. Итог `apply` дополняется счётчиком `skipped_invalid` и краткими причинами — оркестратор видит брак сразу, без ручного разбора JSON.
9. **Приёмочный гейт связности**: bootstrap считает и показывает метрики качества графа (число компонент, доля в крупнейшем компоненте, доля разрешённых рёбер, doc-узлы без `documents`, сироты) как часть приёмки; те же метрики — в `status` и рядом с `verify`. Фрагментация видна сразу, без 3D-viewer, и подсказывает «запусти глобальный линковщик». Архитектурный ориентир (из аудита): `graph_store` **доменно-слеп** (намеренно не знает об узлах и рёбрах — 03-storage) — метрики связности живут в слое, читающем модель данных (`reconcile`/инспекция при bootstrap), и выводятся через `lifecycle.status`; в сам `graph_store.verify` их не вносить. **Regression без модели (заглушка-билдер):** headless-фикстура «мини-проект» (код+доки+данные во временном каталоге), на которой селфтест прогоняет конвейер сборки целиком — скелет → **детерминированная заглушка вместо LLM-билдера** (шаблонные сводки и рёбра из полей юнита, включая нарочно битые элементы и неканонические id для проверки валидации/нормализации) → `apply` → резолвер ссылок → гейт — и сверяет метрики связности с эталоном фикстуры. Так регрессии конвейера (Группы A–C) ловятся селфтестом без единого вызова модели; семантическое качество живого линковщика заглушка не проверяет — его меряет приёмочный стенд.

**Группа D — воспроизводимость (§1.46).**
10. **Persistent derivation-cache** по ключу `content_sha` (+ версия контракта деривации + `working_language` — смена языка сводок обязана инвалидировать кэш, иначе пересборка на другом языке молча вернёт старые сводки) вне `nodes/` (напр. `cache/derivations/`): тот же контент → та же сохранённая деривация → детерминизм на практике (сильнее `temperature=0`) и почти нулевая стоимость повторной/частичной сборки. Инвалидируется по смене контракта.

**Совместимость с ленивой деривацией Этапа 17 (не сломать; проверить при реализации).** Этот этап трогает деривацию, поэтому обязан сохранять `derivation: lazy` — и в основном её **усиливает**: детерминированный скелет + backbone (Группа A) строится для **всех** узлов, включая отложенные (лень откладывает лишь семантический слой), так что отложенный узел структурно связан ещё до деривации; derivation-cache (задача 10) и текст-юнита-в-очереди (Этап 20) делают первое касание (`retrieve.stale_in_pack`, §4.10 фаза B) и фоновый добор дешёвыми. Но **два механизма обязаны быть lazy-aware**: (1) **приёмочный гейт связности** (задача 9) не считает отложенные `stale`-узлы (пустая summary, нет смысловых рёбер) фрагментацией — под лень это ожидаемое состояние, а не дефект (иначе гейт ложно забракует здоровый ленивый граф); (2) **глобальный смысловой линковщик** (задачи 4–5) работает над слоем summary, поэтому отложенные узлы входят в линковку **по мере деривации** (первое касание / фоновый добор), а не разом — линковщик должен быть повторно-запускаемым инкрементально. С этими двумя оговорками Этапы 18–20 лень не ломают, а удешевляют (замер off/on §4.10 подтвердил: первое касание держит многошаговость). Дефолт остаётся `eager`; lazy — явный opt-in.

Definition of done:

- сборка с нуля на реальном проекте даёт **один крупный компонент связности** (нет островов у по-настоящему связанных узлов), **~0 висячих ВНУТРЕННИХ рёбер** (внешние stdlib/сторонние корректно исключены), у каждого doc-узла с реальным предметом есть `documents`; приёмочный гейт связности проходит;
- пересборка из кэша **детерминирована** (тот же вход → тот же граф) и дёшева;
- битый элемент деривации не роняет батч; `calls`/наследование/backbone эмитятся детерминированно;
- селфтесты + regression связности зелёные; закрыты §1.40–§1.46.

Закрытие (доки): `04-ingest.md` (резолв `calls`, backbone `defines`, наследование), `05-reconcile.md` (нормализация id, валидация apply, derivation-cache), `08-agents-skills.md` (глобальный линковщик, эмбеддинг-кандидаты, порядок/таксономия хабов, обязательный `documents`), `06-retrieval.md` (переиспользование эмбеддингов под кандидаты), `10-eval-tools.md` (метрики связности/приёмки), `02-data-model.md` (рёбра `defines`/наследования), `09-config.md` (ключи кэша/линковщика/гейта), **THEORY §4.2/§6** (принцип «детерминированные рёбра до LLM» + глобальная смысловая линковка поверх summary — новая теория «почему так»), `GUIDE.md`/READMEs. Новая теория этапа — глобальный смысловой пасс и кандидаты по эмбеддингам как механизм полноты — завести в THEORY.

---

## Этап 20 — экономная сборка (токены и время)

Цель: снизить стоимость сборки на реальном проекте **в 20–30 раз по токенам и времени БЕЗ потери качества** и сделать сборку устойчивой к прерываниям. Экономия — из устранения перечитывания/ресенда/переделок, не из урезания смысла. Устраняет §1.47/§1.48.

Задачи:

1. **Текст юнита в `queue.json` (§1.47, крупнейший выигрыш).** `reconcile._queue_item` кладёт уже нарезанный `extract_structure` текст юнита (`slice_src` и аналоги) для кода/markdown, а не только для заранее извлечённых форматов (сейчас `text` несут бинарные PDF/DOCX/XLSX/PPTX плюс CSV/log/chat — сверить формулировку в `05-reconcile`, она упоминает только бинарные). Билдер делает ~1 чтение (очередь) вместо десятков `Read`/`Grep` → обвал cache-read/ресенда. Контент тот же — **качество не меняется**; попутно снимается половина причины висячих id (билдеру меньше поводов угадывать). Заметки по реализации: добавить в элемент очереди и `line_end` (билдер видит границы среза без повторного парса); для аномально крупных юнитов предусмотреть порог (текст свыше порога — указатель как раньше, чтобы `queue.json` не разбухал на файлах-гигантах).
2. **Агрегация/пропуск тривиальных юнитов (§1.47).** Дандеры и однострочные геттеры не суммировать каждый отдельной LLM-задачей — агрегировать (напр. сводкой класса) или пропускать под guard'ом (значимые дандеры — `__call__`/`__enter__` — сохранять). Конфиг-ключ порога. **Ловушка (из аудита): пропущенный юнит нельзя просто не деривировать** — правило requeue в `plan()` (`derived_from_hash != source_hash` или `status: stale`) вернёт его в очередь навечно; тривиальный юнит должен получать **детерминированную авто-сводку** (шаблон из сигнатуры, без LLM) с выставлением `derived_from_hash = source_hash` — тогда он «выведен» и очередь чиста, а указатель и структурные рёбра сохраняются.
3. **Тонкая оркестрация (§1.47).** Инструкция SKILL: оркестратор держит контекст тонким (агрегаты, не сырые листинги узлов/дерева файлов, которые ресендятся каждый ход), делегирует чтение субагентам.
4. **Чекпоинты, авто-чанкинг, авто-resume билдеров (§1.48).** Инкрементальные чекпоинты — почти-законченный батч не теряется при обрыве/лимите; авто-чанкинг батча по лимиту вывода (не упираться в 32K): `partition_queue` получает кап по числу юнитов/оценке объёма вывода, потому что разбиение только по поддереву даёт и сверхкрупные батчи (плотный каталог документации целиком); авто-resume павших батчей. **Чанкинг и честный сигнал распространяются и на synth/глобальный линковщик** — их вывод на большом графе упирается в тот же потолок. Опирается на возобновляемость Этапа 12 (`content_sha`) и derivation-cache Этапа 19 (готовые юниты не передеривируются).
5. **Честный сигнал завершения и прогресс (§1.48).** Упавший в середине агент **не пишет «Done»/«Finish»** — явный маркер «успех/обрыв» (иначе не отличить); индикатор **прогресса в процентах** (сколько юнитов очереди деривировано/применено), чтобы оркестратор и человек видели пройденную часть пути.
6. **Реальные номера строк у абзацного нарезателя (§1.51).** `extract_structure._text_units` проставляет фактические `lineno`/`line_end` блоков (сейчас у всех блоков `lineno: 1`) — указатели `путь:строка` и срезы для очереди становятся точными и у обычного текста.

Definition of done:

- пересборка реального проекта измерена как **≈×20–30 дешевле по токенам и заметно быстрее** относительно dogfood-базлайна, при **удержании качества** (связность/recall Этапа 19 не падают);
- прерванная сборка **резюмируется**, не переделывая готовое, и честно рапортует статус (успех/обрыв) и прогресс в %;
- тривиальные юниты не зависают в очереди (авто-сводка выводит их из requeue-цикла);
- селфтесты зелёные; закрыты §1.47/§1.48/§1.51.

Закрытие (доки): `08-agents-skills.md` (текст-в-очереди, чекпоинты/чанкинг/resume, тонкая оркестрация, сигнал/прогресс), `04-ingest.md`/`05-reconcile.md` (текст юнита в очереди), `09-config.md` (ключи тривиальных юнитов/чанкинга), `GUIDE.md` (ожидания по стоимости и устойчивости сборки), READMEs. Новой теории этап обычно не вводит (инженерная экономия); принцип «summary — это сжатие, линковке не нужны исходники» уже ляжет в THEORY на Этапе 19.
...
**Терминологический глоссарий ru→en — первый артефакт этапа (по запросу пользователя).** Перевод растянется на много сессий и файлов, и без зафиксированных соответствий терминология неизбежно разъедется («сверка» станет то reconciliation, то sync; «впитывание» — то absorb, то ingest). Поэтому первая сессия этапа составляет рабочий глоссарий ключевых терминов проекта (например, `docs/en/glossary.md`): сверка → reconciliation, впитывание → absorb, семантическое обогащение → semantic derivation, засев → seeding, ярус → tier, пакет → context pack, ветка (графа) → branch, значимость → salience, узлы-паттерны → pattern nodes, остов → spanning tree, указатель → pointer и далее по корпусу, — и каждый следующий файл переводится строго по нему; термин, впервые встреченный при переводе, дописывается в глоссарий в тот же момент. Требование к качеству — **двунаправленная нативность**: английский термин обязан быть тем, что употребил бы носитель в технической документации (не пословная калька с русского), и зеркально — русский термин остаётся естественным для русского технического текста (правила раздела 3 действуют в обе стороны). При закрытии этапа глоссарий предпочтительно **оставить постоянным справочником** согласованности двух языковых зеркал — оба будут правиться и после Этапа 21, на каждом следующем этапе; решение зафиксировать при закрытии.
...
**Дополнительно к проходу качества (по запросу пользователя), все — до перевода:**
...
- **Сверка ссылок на номера этапов (перенумерация при вставке Этапов 18–20).** При добавлении Этапов 18–20 перевод стал Этапом 21, а проверка сред — Этапом 22. В roadmap ссылки обновлены сразу; в **прочих шипнутых доках** остались устаревшие «Stage 18» (перевод → 21) и «Stage 19» (среды → 22): `CLAUDE.md`, `CHANGELOG.md`, `README.md`/`README_RU.md`, `GUIDE.md`, `INSTALL.md`, `12-install.md`, `08-agents-skills.md`, `09-config.md`, `entrypoint/AGENTS.codex.md`, `STATUS.md`. Этот проход (нейтрализация/сверка ссылок на этапы) их и вычищает — сверить каждую и обновить 18→21, 19→22, либо (предпочтительно, по слоям доков) сделать ссылку неявной. Часть ссылок Этап 18 обновит досрочно в файлах, которые и так правит (см. закрытие Этапа 18), — здесь проверить остаток.
- **Закрытие §1.36 и первой части §1.52 — тем же проходом.** Перевод аудит-журнала `log.md` в плоский `.log` (§1.36) правит `graph_store.append_log`, `lifecycle._last_consolidation`, селфтест и упоминания в `02-data-model`/`03-storage`/`consistency-model §11`/`01-overview`; в тот же набор файлов входит устаревший абзац о `log.md` в `07-consolidation.md` «Применение» (§1.52, п.1 — до сих пор описывает лог как best-effort вне транзакции с единственным писателем) — закрыть оба одним изменением.
- **Ссылки на дорожную карту в управляющем слое (зафиксировано на Этапе 18, по вопросу пользователя).** Установленному проекту roadmap не поставляется (копируются только skills/agents/блок), а номера **будущих** этапов уже один раз сдвигались (вставка Этапов 18–20: перевод 18→21, среды 19→22), поэтому правило двухуровневое: (а) в **исполняемых промптах** (блоки точки входа, `SKILL.md`, `agents/*.md`) ссылок на этапы не должно быть вовсе — формулировка обязана быть самодостаточной; энтрипоинты вычищены на Этапе 18 (`AGENTS.codex.md` нёс будущее «Stage 19»), осталось ≈3 прошлых атрибуции «Stage 13/14» в `skills/amg-retrieve/SKILL.md` — заменить смысловой формулировкой; (б) в **комментариях скриптов движка** атрибуция **прошедших** этапов и пунктов аудита (`Stage N`, `audit 1.XX`) допустима как конвенция провенанса решения — номера закрытых этапов и пунктов §1 стабильны при сворачивании (append-only), — но ссылок на **будущие** этапы («будет сделано на Этапе N») быть не должно; греп на Этапе 18 подтвердил, что таких в скриптах не осталось. Проход выполняется этим же этапом вместе с §1.33/§1.34 (тот же набор файлов).
