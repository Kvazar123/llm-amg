...
10. **Живой прогон на dogfood-стенде** исправленным движком ПОСЛЕ задач 1–8 (сессии пользователя, вне dev-сессий): связность (один крупный компонент, ~0 внутренних висячих, `gate: ok`) **и** стоимость — число субагентов/ходов по фазам и токены из панели пользователя, сравнение с базлайном первой сборки (~30 ч / ~70 Мтокенов) и второй (~2 ч / ~80 Мтокенов cache-read). Цель ×20–30 к первичному базлайну подтверждается, либо остаток раскладывается по статьям с решением по каждой. Здесь же решается судьба задачи 6 (напоминание), проверяется приём внутриагентного прогресса (задача 8) и снимаются два отложенных dogfood-пункта Этапа 17 (эмиссия паттернов живым synth; `usage.log` начинает копиться для будущего флипа Хебба).
...
14. **Write воркерам и гигиена записи (§1.62).** `agents/amg-{builder,linker,synth,consolidator}.md`: `tools: … Write`; в промпты — «части пишутся инструментом Write (не heredoc)» и «валидация без перечитывания: `python -c "import json;json.load(open(...));print('ok')"`»; Write — только в `work/` (read-only на источники остаётся правилом). `selftest_install` не трогается (рендер тот же).
15. **Полевые недоговорённости (§1.63).** Промпты линкера/билдера — явная конвенция направления item.id→to (с примерами `documents`/`exemplifies`); SKILL: прогресс волн линковки («X/Y пачек»), oversized-оговорка («чтение среза юнита без `text` законно всегда, волновые инструкции его не отменяют»), критерий полноты линковки («повторять `link_candidates` до нуля новых пачек либо `gate: ok`»), «выводы о движке AMG — пользователю, в граф проекта не писать»; двигательный гард — `partition_queue`/`link_candidates` при непустых `work/derived-*.json` печатают «apply-derived first» (+ то же поле `leftover_derived` в выводе `plan`).
