...
Обнаружено итоговым аудитом (после Этапа 20). `extract_structure.load_gitignore` намеренно пропускает правила-исключения (`!pattern`), и `_gitignored` сопоставляет без порядка правил — упрощение задокументировано в `04-ingest.md` («Игнорирование», слой 2). Следствие — **пере-игнорирование**: файл, исключённый широким правилом и возвращённый в git через `!` (например, `logs/` + `!logs/keep.md`), в граф молча не попадает, если не назван источником явно. Это узкий, но настоящий случай класса «молчаливая потеря материала» (родня §1.30/§1.50); гарантия «явный источник важнее `.gitignore`» его смягчает лишь для корня источника. Починка: упорядоченное сопоставление «последнее совпавшее правило побеждает» с поддержкой `!` в `_gitignored` + сценарии в `selftest_ignore`. **Дом:** отдельный standalone-фикс между этапами (bump `z`) либо попутно Этапу 22 (первые живые чужие репозитории — там правило `!` и встретится).
...
- **Прозаический регистр всей документации + повторная зачистка §3 (по запросу пользователя, добавлено после Этапа 20).** Тексты Этапов 8–20 добавлялись сжато и обрывисто; финальный проход приводит их к регистру живого объяснения (§3.4, CLAUDE.md §7 п.7): полные фразы со связками и интуицией, термин объяснён на месте — так, как понятное объяснение человеку в разговоре, а не конспект для другого ИИ. Одновременно — повторная зачистка §3 по всему корпусу, включая расширение таблицы §3.2 (рецидив «деривации», «батч», «ресенд», «чекпоинт», «резюмируемость», «линковка»).
- **Зеркальность требований в переводе (по запросу пользователя, добавлено после Этапа 20).** Все требования к русской документации — ясность, полнота, регистр §3.4, отсутствие калек — действуют **зеркально** для английского результата: перевод должен читаться англоязычным пользователем как родной технический текст той же связности и красоты (native-level, без калек уже с русского), а глоссарий держит соответствия терминов в обе стороны.
