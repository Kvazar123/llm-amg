...
**Состояние (живое).** Группа 1 закрыта целиком (фундамент + хвост: bench + барьер); Группы 2–4 не начаты. Порядок — по группам, коммит после группы. Развилки ниже зафиксированы с пользователем (после изучения всех доков/кода) — это расширенное, детальное понимание этапа, более широкое, чем исходные тезисы.
...
### Группа 2 — ИНДЕКС (ядро DoD; НЕ начато) — задачи 1–3
Производный SQLite-индекс под `retrieve.load_nodes` — единственный по-запросный горячий путь (сейчас `nodes_dir.rglob('*.md')` + `yaml.safe_load` каждого файла; §4.1: для десятков тысяч узлов заметно). Зафиксированные решения:
- **Где:** `cache/index.sqlite` (+ `cache/index.meta` сигнатура), рядом с `pack.md`/`embeddings.json` — **disposable, НЕ source of truth** (канон — Markdown; §4.1 «сломался → удалить и пересобрать»); индекс **вне** транзакционного журнала.
- **Новый модуль** `skills/amg-retrieve/scripts/index_store.py` (+`selftest_index.py`): строит индекс из `nodes/*.md`. На узел хранит `id`/`type`/`status`/`summary`/`source_path`/`lineno`/searchable-текст (тот же «мешок слов», что собирает `load_nodes`: хвост id + summary + topics + tags + первые 600 символов тела)/`edges`(json)/`part_of`(json). Тело узла НЕ хранить (или лениво) — оно нужно лишь немногим узлам operational-яруса при рендере пакета.
- **Жизненный цикл = эжер-обновление писателями + защитная валидация в читателе.** ВСЕ писатели графа — `reconcile.plan`/`apply_derivation`, `consolidate.apply_actions`/`fold_weights`, `notes.add_note`, `migrate_schema` — после `tx.commit()` best-effort обновляют индекс (под уже взятым локом). `retrieve.load_nodes` сверяет дешёвую сигнатуру `nodes/` (stat-обход путь→mtime+size — в 10–100× дешевле, чем read+yaml каждого файла) с сигнатурой из индекса: совпало → читать индекс (быстро); не совпало / порча / нет SQLite → скан + пересборка индекса. **Скан-fallback безусловен** — индекс никогда не отдаёт неверного (деградация до скана, не до ошибки); «удалить и пересобрать» автоматизировано.
- **Задача 3 — кэш adjacency:** хранить рядом симметризованную проводимость (`build_adjacency`); инвалидация по версии графа (та же сигнатура) + хешу `relation_priors`/`default_edge_weight` (cfg сменился → пересобрать).
- **BM25 — оставить Python-овским** поверх хранимого в индексе текста (НЕ FTS5 `bm25()`), чтобы не сдвинуть eval/ранжирование. Форма словаря, возвращаемого `load_nodes`, не меняется → BM25/`build_adjacency`/`assemble_pack` не трогаются.
- **DoD группы:** малый граф — Markdown-путь прост (индекс опционален, fallback всегда); большой — retrieve не парсит все `.md`; индекс удаляем/пересобираем. `selftest_index`: build→read→delete→rebuild даёт идентичный retrieve-выход; протухание → fallback на скан. `eval_retrieval --make-demo` сохраняет hop-recall 1.00 (индекс не меняет ранжирование). Бенч (задача 9) показывает ускорение `load_nodes`.
