...
**Реализовано на Этапе 11 (Группа D):** `extract_structure.detect_policy_conflicts` выводит id под >1 политикой в `reconcile.plan` (`policy_conflicts`) и в `--stats` (`overlapping_sources` + `overlap_hint`); дедуп по id оставляет последнюю в `resolve_sources` (наиболее сохраняющую) политику — `absorb_once` > `absorb` > `mirror`, — конфликт больше не молчит. Приоритет задокументирован в `config.yml`; синхронизация `09-config`/`INSTALL` — на закрытии этапа. Regression — `selftest_ignore.test_overlap_warns`.
...
**Реализовано (Этап 10 + Этап 11, Группа D):** `--stats` показывает `by_source` со счётчиком и `found` (Этап 10); `reconcile.plan` теперь выводит `missing_sources` — опечатка в `mirror_path` больше не маскируется под «added: 0». Regression — `selftest_ignore.test_missing_in_plan`.
...
1. Chat/session chunker — **выполнено** (Группа C): нарезатель внешних структурных чат-экспортов `_chat_units` (JSON-массив / `{messages: […]}` / NDJSON сообщений; синонимы ключей роли/текста/времени/id/треда; content-блоки → маркеры вложений) — один эпизодический `section` на сообщение, роль/timestamp/тред вложены в текст единицы, id из собственного поля сообщения. Слабое ребро `follows` к предыдущему ходу того же треда (смежность диалога, 4.2; `build_adjacency` симметризует — одно ребро достаёт обоих соседей; β 0.4, w 0.3). Детект структуры над json/ndjson; обычный массив записей уходит к data-нарезателю. Плоский маркерный дамп (`=== Human ===`), положенный обычным источником, маршрутизируется в существующий `_session_units` (`_has_role_markers`) — без дублирования.
2. Semantic drift segmenter — **ОТЛОЖЕНО** (не реализуется на Этапе 11; не блокирует DoD). Полная спецификация и обоснование переноса — в §4.6: реализовывать ПОСЛЕДНИМ, только под автоматической проверкой полноты (Этап 4) и лишь при измеренной боли от структурных нарезателей. Структурные нарезатели этого этапа (chat/log-группировка/recursive-JSON/RST) закрывают подавляющую часть входов, а точность важнее экономии токенов.
3. Recursive JSON chunker — **выполнено** (Группа B): крупный вложенный контейнер (сериализованный JSON > `json_recurse_min_chars`, содержащий вложенную структуру) дробится на под-записи по пути ключей (`a.b.c` / `a.b[0]`); мелкое/плоское значение сохраняет прежнюю форму одной записи и хеш (обычные data-файлы не меняются); крупный ПЛОСКИЙ список скаляров не взрывается на узел-на-элемент. Лимиты `json_max_depth` / `json_max_nodes`; стабильные id (head + хеш полного пути при длинном пути).
4. NDJSON parser — **выполнено** (Группа A): запись на строку JSON; стабильный qual из поля id/key/name; реальный `lineno`; общий кап `json_max_nodes`.
5. CSV parser — **выполнено** (Группа A): один структурный `sheet`-юнит на файл (заголовки + образцы строк + размеры), как XLSX; разделитель по суффиксу/сниффу.
6. Log parser — **выполнено** (Группа A): строки с ведущим timestamp подтверждают лог; ограниченные эпизоды-окна `log_group_lines` строк (один `block` на окно), timestamp/level в тексте; не-лог → абзацы.
7. RST heading parser — **выполнено** (Группа A): заголовки-подчёркивания/надчёркивания reStructuredText → разделы (как markdown), `_preamble` до первого.
8. PPTX parser через `python-pptx` — **выполнено** (Группа A): один `section` на слайд, опц. с мягким пропуском (как PDF/DOCX/XLSX); `.pptx` выведен из `BINARY_EXT`.
9. Optional true absorb freeze — **выполнено** (Группа E): новая политика `absorb_once` (ключ `absorb_once_path`): впитать один раз и заморозить — изменения источника игнорируются (нет повторного обогащения и дрейфа указателя), удаление не вычищает (как absorb). Гейт в `reconcile.plan` (счётчик `frozen`); на пересечении выигрывает наиболее сохраняющая политика (`absorb_once` > `absorb` > `mirror`).
10. Валидировать пересечение `mirror_path`/`absorb_path` (1.29) — **выполнено** (Группа D): `detect_policy_conflicts` показывает id под >1 политикой в `plan` (`policy_conflicts`) и `--stats` (`overlapping_sources` + подсказка); дедуп оставляет последнюю (наиболее сохраняющую) политику — конфликт больше не молчит.
11. Явная диагностика несуществующего пути источника (остаток 1.30) — **выполнено** (Группа D): `plan` выводит `missing_sources` (опечатка в `mirror_path` больше не маскируется под «added: 0»); `by_source` в `--stats` — с Этапа 10.

Definition of done:

- длинные тексты не превращаются в огромные случайные blocks — **выполнено** (log-группировка эпизодов; semantic-drift отложен под §4.6, не блокирует);
- structured data не теряет вложенность — **выполнено** (recursive JSON со стабильными id);
- новые форматы мягко пропускаются без dependencies — **выполнено** (PPTX через `python-pptx`, мягкий пропуск).
...
8. Оптимизировать модельные затраты (**механизм 1.14 готов**: per-role `model`+`reasoning_effort` рендерится установщиком в субагентов; здесь — измерить и настроить сами тиры под eval):
   - cheap model только там, где eval не проседает;
   - reasoning субагентов не урезать, если качество падает;
   - подобрать `reasoning_effort` по ролям под eval (низкий на лёгких ролях вроде discovery, высокий — на synthesis).
...
14. **Транзакционный `log.md` (см. 1.15).** Сейчас `consolidate.py` дописывает строку обычным append вне транзакции, а `reconcile.py` не пишет вовсе — это расходится с заявленной моделью «append как часть committed transaction с de-dup по txid». Привести к модели: читать текущий `log.md`, добавлять строку с `txid` через `tx.write`, предотвращать дубли по `txid`. (Привязано из §1 в подэтапе models/env между Этапами 10 и 11.)
