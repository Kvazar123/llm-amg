...
**Реализовано на Этапе 12 (задача 14):** транзакционный `GraphStore.append_log` (дедуп по `txid` + ротация в `archive/`), пишут и `consolidate`, и `reconcile`; детали → `03-storage` («Журнал действий — транзакционный аудит-след»), `consistency-model §11`; regression — `selftest_graph_store.case_action_log`. Полная свёртка пункта — при закрытии этапа.
...
**Состояние (живое).** Группы 1–4 закрыты (фундамент + bench + барьер; индекс под `load_nodes`, 15×; near-dup §1.27 + возобновляемая деривация + хелперы очереди; тиринг моделей). Осталось — **закрытие этапа**: doc-sync + сворачивание §1/§5 + релиз v1.3.0. Развилки ниже зафиксированы с пользователем (после изучения всех доков/кода) — это расширенное, детальное понимание этапа, более широкое, чем исходные тезисы.

**Выполнено (свёрнуто):**
- **Задача 12 — `mypy --strict` (§1.31). ✅** 12 модулей движка проходят strict единым проходом (12-й — `bench.py`, добавлен в гейт в Группе 1) (параметризованы дженерики, аннотированы вложенные хелперы, снят implicit-Optional, обёрнуты `Any`-возвраты `yaml.safe_load`, `stdout.reconfigure` → локальный ignore, динамический tree-sitter → `Any`+ignore). `mypy.ini` (`files=` фиксирует движок; гейт — `python -m mypy` без аргументов) + dev-группа (`mypy`, `types-PyYAML`) в `requirements.txt`. **Решение: селфтесты ИСКЛЮЧЕНЫ из гейта** (test-код; `selftest_embed` намеренно подсовывает несовместимые стабы — strict дал бы `# type: ignore`-шум без отлова багов движка). Поведение-сохраняюще (аннотации не исполняются при `from __future__`); 13 селфтестов + живой CLI-цикл чисты. Коммиты `c34c4bd` + mypy.ini-гейт.
- **Задача 14 — транзакционный `log.md` (§1.15). ✅** `GraphStore.append_log` (дедуп по txid + ротация в `archive/`); `consolidate`+`reconcile` пишут под гейтом `if txid`; `lifecycle._last_consolidation` фильтрует источник; доки `03`/`consistency-model §11` синхр.; `selftest_graph_store.case_action_log`. Коммит `8524939`.
- **Задача 4 — валидация embedding-кэша по node-text-хешу. ✅** реализована в `embed.node_embeddings` (хеш-гейт `_text_hash(node_text(n))` + чистка удалённых узлов), описана в `06-retrieval`; **подтверждено (Группа 1)** регрессией `selftest_embed` check 3 (cold → warm 0 → одна правка → ровно 1 пересчёт).
- **Задача 9 — performance benchmarks. ✅ (Группа 1)** Новый автономный `skills/amg-retrieve/scripts/bench.py` поверх готовых модулей: самодостаточный детерминированный генератор `--make-bench --nodes N --seed S` (пишет узлы прямо в `nodes/`, эмбеддинги off) + замер на любом `--store`; мерит `load_nodes` (главная метрика — цель индекса), `build_adjacency`, `retrieve` (writes off — не загрязняет пакет/журнал), `eval`, и bootstrap (`reconcile.plan`) по `--project`. Best-of-N. Добавлен в гейт mypy (12-й модуль) + `selftest_bench.py` (smoke: граф загружаем, ключи времён, read-only). **База до индекса:** на 3800 узлах `load_nodes` ≈ 4.3 с, `retrieve`/`eval` целиком в неё упираются — индекс Группы 2 меряется тем же bench. Решение (уточнение STATUS): самодостаточный генератор вместо масштабирования `gen_big.py` — воспроизводим офлайн/в CI, развязан с хеббовым стендом `amg-bigtest`; индекс-замеры (build/index-load/rebuild) допишутся в bench в Группе 2 (API `index_store` ещё нет — не угадываем).
- **Задача 10 — мягкий барьер. ✅ (Группа 1)** Усилен и выделен в отдельный раздел «Boundaries» в трёх блоках активации (`entrypoint/CLAUDE.md`/`AGENTS.md`/`AGENTS.codex.md`) + зеркально в `08-agents-skills.md` (петля): источники read-only; движок не править по ходу задачи (расхождение с канонической версией + порча используемого графа); служебное состояние (`nodes/`, кэши `cache/`/`work/`) пересобирается `repair`/`sync`, не правится руками. Реестр **2.16 п.6 закрыт**.

### Группа 2 — ИНДЕКС — ✅ ВЫПОЛНЕНО (задачи 1–2; задача 3 отложена по измерению)
Производный SQLite-индекс под `retrieve.load_nodes` — единственный по-запросный горячий путь (был `nodes_dir.rglob('*.md')` + `yaml.safe_load` каждого файла; §4.1). Реализовано:
- **Где:** `cache/index.sqlite`, рядом с `pack.md`/`embeddings.json` — **disposable, НЕ source of truth** (канон — Markdown; §4.1 «сломался → удалить и пересобрать»); вне транзакционного журнала. **Уточнение:** сигнатура хранится ВНУТРИ sqlite (мета-таблица), а не отдельным `cache/index.meta` — данные и их freshness-тег пишутся атомарно одним файлом, нет рассинхрона.
- **Новый модуль** `skills/amg-retrieve/scripts/index_store.py` (+`selftest_index.py`, 5 проверок). На узел хранит `id`/`type`/`status`/`summary`/`source_path`/`lineno`/searchable-`text` (тот же «мешок слов» `load_nodes`)/`edges`(json)/`part_of`(json)/`body`. **Уточнение:** тело узла ХРАНИТСЯ (в реальном графе тела почти всегда пустые — непустые лишь у синтез-хабов; так форма `load_nodes` идентична и `assemble_pack` не тронут). Скан и индекс делят общий `retrieve._node_from_meta` — выход совпадает побайтно (доказано `selftest_index` identity).
- **Жизненный цикл = эжер-обновление писателями + защитная валидация в читателе.** ВСЕ 6 писателей (`reconcile.plan`/`apply_derivation`, `consolidate.fold_weights`/`apply_actions`, `notes.add_note`, `migrate_schema`) после `tx.commit()` под локом делают **инкрементальный** `index_store.refresh_after_commit` (upsert изменённых строк + новая сигнатура одной sqlite-транзакцией) через новый `graph_store.Transaction.node_paths()` — дёшево для одиночной заметки, не полная пересборка; индекса нет → no-op (читатель построит лениво). `retrieve.load_nodes` сверяет дешёвую сигнатуру `nodes/` (stat-обход путь→mtime+size) с хранимой: совпало → читать индекс; иначе/порча/нет SQLite → **безусловный** скан + best-effort пересборка. **Уточнение (гонка читателя):** сигнатура берётся ДО скана, поэтому индекс, построенный из гонявшего с записью скана, не «доверяется устаревшим» — следующая проверка свежести его пересоберёт.
- **BM25 — Python-овский** поверх хранимого `text` (НЕ FTS5), eval/ранжирование не сдвинуты; форма `load_nodes` не изменилась.
- **Задача 3 (adjacency-кэш) — ОТЛОЖЕНА по измерению (bench, задача 9):** `build_adjacency` = 23–49 мс против `load_nodes` 4–8 с (<1%); сериализация adjacency-блоба (сотни тыс. рёбер) на большом графе рискует стоить дороже самой пересборки → кэш net-negative. Индекс уже убрал доминирующую стоимость, а `build_adjacency` работает над быстро загруженными из индекса узлами. Вернуть, если измерение на десятках тысяч узлов опровергнет (тогда — бинарный формат, не JSON). Решение обосновано числом, появившимся только сейчас (bench — задача 9 этой группы-предшественницы).
- **DoD группы выполнен:** малый граф — путь прост (индекс опционален, fallback всегда); большой — retrieve не парсит все `.md` (**измерено: 7600 узлов load scan 7.9 с → index 0.52 с, 15×**; build/rebuild 0.15 с); индекс удаляем/пересобираем. `selftest_index` (build→read→delete→corrupt→upsert) зелёный; `eval_retrieval --make-demo` hop-recall 1.00 не изменился; 16 селфтестов зелёные; mypy — 13 модулей (`index_store` добавлен).

### Группа 3 — ✅ ВЫПОЛНЕНО (план / возобновляемость / хелперы; задачи 11, 13, 5–6, 7)
- **Задача 11 — near-dup в `consolidate.make_plan` (§1.27). ✅** Жаккар теперь по эпизодическим не-derived узлам (`type in episodic_types and source_kind != derived_from_file`) — тот же фильтр, что у `episodic_candidates` ниже: O(k²) по эпизодическим вместо O(n²) по всему графу, и не предлагаются футильные слияния зеркал (reconcile их пересоздаёт). MinHash/LSH-предфильтр — только если измерение покажет большой k. Регресс — `selftest_consolidate.test_near_dup_scope` (две code-зеркала с одинаковой сводкой НЕ кандидаты; две заметки — да).
- **Задача 13 — возобновляемая деривация. ✅** (а) `agents/amg-builder.md` эхоит `content_sha` в каждый derived-элемент; (б) `reconcile.apply_derivation` сверяет `content_sha` элемента с ТЕКУЩИМ `source_hash` узла — расходится → skip (`skipped_stale`), узел остаётся stale и пересверка его вернёт (никакой слепой деривации против изменившегося источника); элемент без `content_sha` применяется по-прежнему (back-compat / синтез-хабы); (в) SKILL `amg-bootstrap` — заметка «применить уцелевшие `derived-*.json` ПЕРВЫМИ (freshness-safe), затем bootstrap, билдеры — на остаток». Регресс — `selftest_reconcile.case_resume_freshness`.
- **Задачи 5–6 — хелперы очереди. ✅** Новые `skills/amg-bootstrap/scripts/partition_queue.py` (делит `work/queue.json` на `work/queue-<part>.json` по поддеревьям, `--depth`) и `inspect_queue.py` (сводка: счётчики по category/kind/поддереву + сколько единиц несут `text`); общий `subtree_key`. SKILL шаг 3 указывает на них вместо ad-hoc нарезки. Регресс — `selftest_queue.py`; CLI-смоук на реальном bootstrap зелёный. В гейте mypy (15 модулей).
- **Задача 7 — lazy/on-demand: блоки на месте, полная лень отложена. ✅** Скрытый переключатель `derivation` НЕ введён (нельзя обоснованно включить без большого графа + стабильного eval; §6.3). Строительные блоки готовы: структурный-граф-первым by construction; scoping — `partition_queue` (5/6); безопасный токен-выигрыш — возобновляемость (13). Полная on-demand/фоновая лень с eval-проверенным переключением дефолта — **§4.10** (фазы A–D, предохранители, DoD), дом реализации — **Этап 17, задача 7**.

### Группа 4 — тиринг моделей — ✅ ВЫПОЛНЕНО (задача 8)
Механизм 1.14 готов (per-role `model`+`reasoning_effort` рендерится установщиком в субагентов: discovery→{classifier,retriever}, module_summary→{builder}, synthesis→{synth,consolidator}). Сделано: шаблон `config.yml` блок `models` переведён в структурную форму с градиентом усилия — `discovery: {haiku, low}` (простые read-only), `synthesis: {opus, high}` (фундаментальный синтез), а `module_summary: sonnet` оставлен **плоским** (усилие по умолчанию среды) намеренно: его посводочные сводки питают извлечение (BM25 + эмбеддинги), усилие билдера не срезается без замера («reasoning не урезать, где качество падает»). Протокол замера тиров задокументирован в `09-config` «Модели субагентов» (база → понизить тир → пересобрать сводки → eval off/on → оставить дешёвый там, где `recall`/`hop_recall` держатся). **Живой (платный) замер отложен** (как Хебб / Этап 19) — дефолты + метод. `selftest_install.test_models_render`/`test_codex_env` зелёные (module_summary плоский → существующие проверки не сломаны).

### Закрытие этапа (после всех групп)
doc-sync `03`/`06`/`09`/`10` + THEORY/GUIDE/READMEs под индекс и задачи; **починить дрейф `episodic_types`** в `07-consolidation` §«План»/таблица (`[section, note]` → `[section, note, open_question, plan]`); снять остаточные «best-effort log»-упоминания в `02-data-model` (диаграмма+таблица) и `THEORY §9`; дописать строку Этапа 13 в `BASE-PROMPT.md`; свернуть §1.15/1.27/1.31 и тело §5; выпуск **v1.3.0** (VERSION + CHANGELOG + аннотированный тег, пуш с тегами).
