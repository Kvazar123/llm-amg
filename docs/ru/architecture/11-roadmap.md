...
1. **Текст юнита в `queue.json` (§1.47, крупнейший выигрыш).** `reconcile._queue_item` кладёт уже нарезанный `extract_structure` текст юнита (`slice_src` и аналоги) для кода/markdown, а не только для заранее извлечённых форматов (сейчас `text` несут бинарные PDF/DOCX/XLSX/PPTX плюс CSV/log/chat — сверить формулировку в `05-reconcile`, она упоминает только бинарные). Билдер делает ~1 чтение (очередь) вместо десятков `Read`/`Grep` → обвал cache-read/ресенда. Контент тот же — **качество не меняется**; попутно снимается половина причины висячих id (билдеру меньше поводов угадывать). Заметки по реализации: добавить в элемент очереди и `line_end` (билдер видит границы среза без повторного парса); для аномально крупных юнитов предусмотреть порог (текст свыше порога — указатель как раньше, чтобы `queue.json` не разбухал на файлах-гигантах). — **выполнено** (сессия 1: текст прикрепляют **все** нарезатели единообразно — python/treesitter/markdown/rst/paragraphs/json/ndjson/session, включая модульные юниты целым файлом; порог `queue_text_max_chars` дефолт 20000, 0 = очередь-указателями; `line_end` в элементе; дублирование модуль↔функции в батче ×2–3 принято осознанно — инлайн всегда дешевле `Read`+ресенда; regression `selftest_build`/`selftest_chunkers`)
2. **Агрегация/пропуск тривиальных юнитов (§1.47).** Дандеры и однострочные геттеры не суммировать каждый отдельной LLM-задачей — агрегировать (напр. сводкой класса) или пропускать под guard'ом (значимые дандеры — `__call__`/`__enter__` — сохранять). Конфиг-ключ порога. **Ловушка (из аудита): пропущенный юнит нельзя просто не деривировать** — правило requeue в `plan()` (`derived_from_hash != source_hash` или `status: stale`) вернёт его в очередь навечно; тривиальный юнит должен получать **детерминированную авто-сводку** (шаблон из сигнатуры, без LLM) с выставлением `derived_from_hash = source_hash` — тогда он «выведен» и очередь чиста, а указатель и структурные рёбра сохраняются. — **выполнено** (сессия 1: критерий — `kind: function` + диапазон ≤ `trivial_unit_max_lines`, guard-набор протокольных дандеров `_SIGNIFICANT_DUNDERS` (`__call__`/`__enter__`/`__getitem__`/…); авто-сводка = код юнита одной строкой ≤240 симв. — язык-нейтральна (§1.33 цел), даёт BM25-токены; **кэш побеждает шаблон**: юнит с попаданием в кэше дериваций идёт в очередь на restore заработанной сводки; дефолт в коде 0 (выключено), в шаблоне 3 — конвенция «код ≠ шаблон», рукописные фикстуры не задеты; `confidence` не ставится (детерминизм — не суждение); счётчик `auto_summarized` в plan/логе; ветки added/changed/requeue; regression `selftest_build`)
3. **Тонкая оркестрация (§1.47).** Инструкция SKILL: оркестратор держит контекст тонким (агрегаты, не сырые листинги узлов/дерева файлов, которые ресендятся каждый ход), делегирует чтение субагентам. — **выполнено** (сессия 1: раздел «Thin orchestration (token hygiene)» в SKILL — субагентам только пути файлов, осмотр только агрегатами `inspect_queue`/счётчиками, по-батчевые результаты одной строкой; шаги 3–4 переписаны под это)
4. **Чекпоинты, авто-чанкинг, авто-resume билдеров (§1.48).** Инкрементальные чекпоинты — почти-законченный батч не теряется при обрыве/лимите; авто-чанкинг батча по лимиту вывода (не упираться в 32K): `partition_queue` получает кап по числу юнитов/оценке объёма вывода, потому что разбиение только по поддереву даёт и сверхкрупные батчи (плотный каталог документации целиком); авто-resume павших батчей. **Чанкинг и честный сигнал распространяются и на synth/глобальный линковщик** — их вывод на большом графе упирается в тот же потолок. Опирается на возобновляемость Этапа 12 (`content_sha`) и derivation-cache Этапа 19 (готовые юниты не передеривируются). — **выполнено** (сессия 1: капы `builder.batch_units` 40 / `builder.batch_max_chars` 120000 — по сумме `text`, юнит без текста номиналом 2000; части `queue-<part>-NN.json`, повторный запуск заменяет набор батчей, `queue-priority/deferred` не трогает; **чекпоинты без нового кода движка** — билдер пишет вывод частями `derived-<батч>-pNN.json`, `apply` и так пофайловый, freshness по `content_sha` делает повторы безопасными; resume в SKILL: уцелевшие части применяются первыми; synth чекпоинтит `derived-synth-pNN.json`; линковщик уже ограничен `linker.batch_nodes`; regression `selftest_queue.test_caps`)
5. **Честный сигнал завершения и прогресс (§1.48).** Упавший в середине агент **не пишет «Done»/«Finish»** — явный маркер «успех/обрыв» (иначе не отличить); индикатор **прогресса в процентах** (сколько юнитов очереди деривировано/применено), чтобы оркестратор и человек видели пройденную часть пути. — **выполнено** (сессия 1: протокол финальной строки `BATCH COMPLETE: N/M` / `BATCH PARTIAL: N/M` у билдера и линковщика, `SYNTH COMPLETE|PARTIAL` у synth; SKILL «доверяй, но проверяй» — сверка `applied + skipped_*` с размером батча детерминированно, недостача = оборванный батч → добор; блок `progress` в `inspect_queue` (derived/stale/percent по графу), отчёт процентом после каждого раунда apply и в финале)
6. **Реальные номера строк у абзацного нарезателя (§1.51).** `extract_structure._text_units` проставляет фактические `lineno`/`line_end` блоков (сейчас у всех блоков `lineno: 1`) — указатели `путь:строка` и срезы для очереди становятся точными и у обычного текста. — **выполнено** (сессия 1: нарезка пробегом по строкам с теми же границами, что у прежнего regex-разбиения → **хеши блоков не изменились** (тест паритета в `selftest_chunkers`), старые графы сходятся дешёвым дрейфом указателя без передеривации)
...
Закрытие (доки): `08-agents-skills.md` (текст-в-очереди, чекпоинты/чанкинг/resume, тонкая оркестрация, сигнал/прогресс), `04-ingest.md`/`05-reconcile.md` (текст юнита в очереди — снять формулировку «только бинарные»; авто-сводка тривиальных; реальные lineno у paragraphs), `09-config.md` (ключи `queue_text_max_chars`/`trivial_unit_max_lines`/блок `builder`; конвенция «код ≠ шаблон» у trivial: код 0, шаблон 3), `GUIDE.md` (ожидания по стоимости и устойчивости сборки), READMEs. Новой теории этап обычно не вводит (инженерная экономия); принцип «summary — это сжатие, линковке не нужны исходники» уже ляжет в THEORY на Этапе 19.

**Состояние после сессии 1 (2026-07-04):** задачи 1–6 реализованы и покрыты тестами (22 селфтеста движка + `selftest_install` + mypy(18) + eval-демо pack-recall 1.00 — зелёные); коммиты `d116aef`/`b88642e`/`8218fca`/`0f13ace`. Живой замер ×20–30 и связности — совместная приёмка Этапов 19–20 на dogfood-стенде исправленным движком (решение пользователя, вне dev-сессий). Впереди закрывающая сессия этапа: синхронизация доков по списку выше → DoD/селфтесты → сворачивание (§1.47/§1.48/§1.51 и тело этапа) → выпуск v1.11.0.
