...
**Решения проектирования и ход (стартовая сессия Этапа 16):**

- **Два канала обмена.** (1) Общая папка — одна память, single-writer lock, низкая конкурентность; (2) **опциональный граф-в-git** — хранилище можно трекать в git (ветки несут память, git-merge `nodes/*.md`, отчёт о конфликтах), с рекомендованным `.gitignore` (коммитим `nodes/`+`config.yml`+`digest.md`; игнорируем `journal/`/`LOCK`/`cache/`/`work/`/`archive/` и лог-файл). Это **существующий git проекта**, read-only по умолчанию (AMG не делает авто-`commit`/`merge`); своей VCS нет.
- **Командный режим — минимально-аддитивный** (выбор пользователя; чтит задачу 5): ветка/commit в `status`, read-only сравнение состояний, «граф по веткам» — документированный паттерн (git-изоляция или отдельный `agent_dir`), без машинерии переключения. Функциональные границы (нет одновременной многозаписи; merge текстовый, не семантический) приняты как есть — самоизлечиваются `bootstrap`/пере-деривацией.
- **Ветка — на уровне store, не схемы узла:** `_provenance(kind, commit)` не трогаем (иначе раздувание frontmatter + churn + работа `migrate_schema`); `_git_branch` — best-effort рядом с `_git_commit`.
- **Переносимость по средам (важно).** Все правки — в **универсальных скриптах** (`graph_store`/`lifecycle`/`reconcile`), НЕ в хуках. Память работает и в **бесскиловых средах без сессионных хуков** (generic AGENTS.md), и в **Codex** (механизм иной, чем Claude Code): там петлю ведёт модель прямыми вызовами скриптов. Поэтому деградация контеншена реализована и в `lifecycle` (хук-путь), и в самом `graph_store.py` CLI (`recover`/`verify --repair`, которые петля/оператор зовут напрямую без хука).
- **Находки изучения, повышающие надёжность (входят в этап).** (а) `_lock_is_stale` крал чужой lock по **локальной** пробе pid — на общей папке между машинами это ложная кража живого чужого lock; фикс — проба pid только при совпадении host, иначе по возрасту (одномашинный случай неизменен). (б) Один узел с git-конфликт-маркерами в frontmatter ронял `load_nodes` целиком (три парсера `reconcile.parse_node`/`retrieve._parse`/`consolidate` зовут `yaml.safe_load` без защиты; `export_graph` уже ловит `yaml.YAMLError` — прецедент); фикс — защитить три парсера (битый узел пропускается) + детектор конфликт-маркеров в отчёт. «Проверка графа после merge» = существующие `recover`+`verify --repair`+`bootstrap`.
- **Не трогаем:** `migrate_schema`, `index_store`, `extract_structure`, логику `consolidate`, `embed`, `_provenance`.
- **Группы работ:** **(1) — выполнено:** устойчивость хранилища под общий доступ — host-aware `_lock_is_stale` (+ docstring `lock`); мягкая `StoreLockError`-деградация в `lifecycle` (`_heal`/`session_end`/`repair`) и в `graph_store` CLI; кейсы `selftest_graph_store.case_lock_cross_host` и `selftest_lifecycle.case_shared_folder_contention`. **(2) — выполнено:** устойчивость к конфликтам — три парсера узлов (`reconcile.parse_node`/`retrieve._parse`/`consolidate._parse`) защищены от `yaml.YAMLError`+non-dict (битый узел пропускается, не валит `load_nodes`); `reconcile.find_conflict_markers` (регэксп `<<<<<<<`/`>>>>>>>`/`|||||||`; `=======` исключён ради setext-заголовков); вывод в `reconcile.plan` summary, `lifecycle.status` (поле `conflicts`+строка), `repair`/`session_start` (нота). Тесты: `selftest_reconcile.case_merge_conflict_resilience`, `selftest_consolidate.test_conflict_skip`, `selftest_lifecycle.case_merge_conflict_surfaced`. **(3) — выполнено:** git-aware — `reconcile._git_branch` + `_git_changed_since` (best-effort, `--relative` для совпадения путей с `source_path`); ветка/commit в `lifecycle.status` (+ строка); `verify_claims.verify_by_commit` + флаг `--by-commit` (свежесть источника по коммиту: один `git diff --name-only` на distinct ingest-коммит, без пере-нарезки, **дополняет** хеш-сверку, не заменяет; нет git → `unresolved`/`head=None`). `_provenance(kind, commit)` не тронут (ветка на уровне store, не схемы узла). Тесты: `selftest_verify.case_freshness_by_commit`, расширен `selftest_lifecycle.case_status`. 20 селфтестов + mypy(17) зелёные. **(4) — выполнено:** документация по слоям. GUIDE — раздел «Командная работа» (общая папка, граф в git с рецептом `.gitignore`, слияние/конфликты, `--by-commit`, ветка/commit, сравнение веток, границы, оговорка про среды без хуков); READMEs (RU+EN) — раздел в теле + перенос из планов в реализованное; `consistency-model §9` (host-aware блокировка) + §12 (git-merge/конфликты); `03-storage` (устаревание + «Командный режим»); `02-data-model` (что трекать в git); `06-retrieval` (`--by-commit`; термин — «актуальность источника»). Сравнение веток — документированный git-паттерн (`git diff` / `export_graph --json`), без нового кода. Строка Этапа 17 в `BASE-PROMPT.md` расширена. Новой теории нет.
