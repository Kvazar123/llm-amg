...
24. **Offline-first загрузка эмбеддинг-модели (§1.70).** `embed.py`: `_SentenceTransformers` пробует `local_files_only=True`, при любом исключении — обычная загрузка (первое скачивание живо; старые версии ST без параметра — тоже); для model2vec — тот же паттерн, если сигнатура установленной версии позволяет (проверить, не гадать). Доки (зеркально): READMEs «Опциональные зависимости» + GUIDE — латентность transformer-бэкенда на каждый процесс, `HF_HUB_OFFLINE=1`, рекомендация model2vec при чувствительности к латентности.
25. **Двухуровневое обращение к памяти (§1.71).** SKILL amg-retrieve: workflow «по умолчанию — прямой вызов `retrieve.py` (напечатанный пакет = рабочий контекст; `--intent` ставишь сам); субагент amg-retriever — когда пакет в основном окне не нужен (вопрос-выжимка к памяти; перегруженный контекст)»; блоки CLAUDE.md/Codex — то же в пункте петли; `amg-retriever.md` — роль сужена до изоляции. GUIDE/READMEs зеркально.
26. **Триггер судейской консолидации и её документация (§1.72).** Блоки ×3: «пользователь сигналит о завершении работы → судейская консолидация при `automation: true` (или предложить)»; реакция на старт-предупреждение 21/C. READMEs «Режимы и управление»: три фазы, «гигиена, не сохранение», ритм, как позвать словами; честная строка судейской в таблице «кто что запускает». GUIDE — глубже.
27. **Правило отсутствия и точный путь verify_claims (§1.73).** `amg-retriever.md`: правило отсутствия («прямого узла нет → сказать явно; соседи — не ответ»); полный путь `.claude/skills/amg-retrieve/scripts/verify_claims.py` в промпте ретривера и в блоке CLAUDE.md.
28. **`verify_claims --all --write` в закрытии sync (§1.74).** Новый флаг `--all` в `verify_claims.py` (обход всех файловых узлов); SKILL amg-bootstrap — закрывающий шаг после гейта связности; GUIDE/09-config — семантика unverified после конвейерного прогона.
