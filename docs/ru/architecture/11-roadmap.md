...
**Состояние (живое).** Группа 1 «фундамент» почти закрыта; Группы 2–4 не начаты. Порядок — по группам, коммит после группы. Развилки ниже зафиксированы с пользователем (после изучения всех доков/кода) — это расширенное, детальное понимание этапа, более широкое, чем исходные тезисы.

**Выполнено (свёрнуто):**
- **Задача 12 — `mypy --strict` (§1.31). ✅** 11 модулей движка проходят strict единым проходом (параметризованы дженерики, аннотированы вложенные хелперы, снят implicit-Optional, обёрнуты `Any`-возвраты `yaml.safe_load`, `stdout.reconfigure` → локальный ignore, динамический tree-sitter → `Any`+ignore). `mypy.ini` (`files=` фиксирует движок; гейт — `python -m mypy` без аргументов) + dev-группа (`mypy`, `types-PyYAML`) в `requirements.txt`. **Решение: селфтесты ИСКЛЮЧЕНЫ из гейта** (test-код; `selftest_embed` намеренно подсовывает несовместимые стабы — strict дал бы `# type: ignore`-шум без отлова багов движка). Поведение-сохраняюще (аннотации не исполняются при `from __future__`); 13 селфтестов + живой CLI-цикл чисты. Коммиты `c34c4bd` + mypy.ini-гейт.
...
- **Задача 4 — валидация embedding-кэша по node-text-хешу. ✅-фактически** уже реализована в `embed.node_embeddings` (хеш-гейт `_text_hash(node_text(n))` + чистка удалённых узлов), описана в `06-retrieval`. Осталось подтвердить при закрытии.

### Группа 1 — хвост (фундамент)
- **Задача 9 — performance benchmarks.** Новый автономный `bench.py` поверх готовых модулей: число узлов, время retrieval / bootstrap / eval / index-rebuild. Стенд — масштабировать `../amg-bigtest/gen_big.py` до 10–50k узлов (там же замер тиров off/on, обходя медленную загрузку эмбеддингов в подпроцессе). Назначение — регрессия скорости до/после индекса (Группа 2).
- **Задача 10 — мягкий барьер «не править инфраструктуру AMG во время использования».** Уже действует: `entrypoint/CLAUDE.md` (раздел «Where things are» — `.claude/skills/amg-*`/`agents/amg-*` инфраструктура, не править как часть задачи; источники read-only), `skills/amg-bootstrap/SKILL.md`, `08-agents-skills.md` (петля). Осталось: выверить/усилить формулировку; закрыть реестр **2.16 п.6**.
