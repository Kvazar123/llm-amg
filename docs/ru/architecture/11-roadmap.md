...
Обнаружено пользователем на живой dogfood-установке (после v1.11.1). Два дефекта одного корня — установка ведётся промптом, а промпт был недостаточно императивен: (1) модель задала **4 вопроса из 12** (язык, зеркала, локально/глобально, активация) — впитывание, исключения, эмбеддинги и прочее ушли в молчаливые умолчания: INSTALL.md перечислял вопросы, но не требовал задать каждый и не велел осмотреть проект и предложить классификацию папок; (2) в распакованном каталоге AMG лежит dev-`CLAUDE.md` репозитория — при распаковке каталога внутрь проекта Claude Code подхватывает его как под-каталожную память, и правила разработки движка попадают в установочную сессию.

**Исправлено на Этапе 21 (сессия 2), сразу:** INSTALL.md ужесточён — обязательный **осмотр проекта** с предложением классификации папок (зеркала / впитывание / исключения — подаётся вместе с вопросами 3–5), правило «**задавать каждый вопрос по одному**, пустой ответ берёт умолчание, но вопрос звучит», вопросы 6–7 помечены **KEY** (рабочий язык — сводки и кэш дериваций ключуются языком, поздняя смена ничего не пересводит, чистый переход = полное повторное обогащение; эмбеддинги — по их векторам линковка при сборке номинирует кросс-доменных кандидатов, включение после сборки требует повторного `/amg sync`), новый раздел «Cheap to change later vs decide before building» (тиринг `models` вступает в силу через переустановку; пути/исключения/автоматика/бюджеты — следующим прогоном). Попутно INSTALL.md **переведён на английский** как управляющий слой (модель его исполняет), русское зеркало — корневой `INSTALL_RU.md` по образцу `README_RU.md`, ссылки русской документации перенаправлены; кратко «что спрашивается и что ключевое» отражено в READMEs и GUIDE. Против (2): в шапке dev-`CLAUDE.md` — **scope-guard** («при установке этот файл игнорировать — действует только INSTALL.md»), та же оговорка в обоих INSTALL. Живая проверка ужесточённого потока — при следующей dogfood-установке.
...
- **Нейтрализация `.claude/*` во всех документах.** Вошло шагом 1 про-файлового протокола перевода (см. «Оставшийся объём этапа» ниже).
- **Применить §7-чеклист, особенно пункт 7 (регистр прозы), ко ВСЕЙ документации.** Вошло шагом 1 про-файлового протокола перевода (см. ниже).
- **Аудит язык-универсальности кода движка (§1.33).** **Выполнено (сессия 1)** — см. свёрнутый §1.33: других языковых допущений в коде нет, пример в `inspect_graph.py` нейтрализован.
- **Очистка не-английских примеров в промптах и блоках точки входа (§1.34).** **Выполнено (сессия 1)** — см. свёрнутый §1.34: примеры `entrypoint/CLAUDE.md` переведены, остальной управляющий слой подтверждён чистым.
- **Привязка словесных команд к ключу «AMG»/«память» (§1.35).** **Выполнено (сессия 1)** — см. свёрнутый §1.35: правило и привязанные примеры во всех блоках активации, GUIDE, READMEs, INSTALL и `SKILL.md`.
- **Сверка ссылок на номера этапов (перенумерация при вставке Этапов 18–20).** **Выполнено (сессия 1):** остаток вычищен — `CLAUDE.md` (3 ссылки 18→21) и `CHANGELOG.md` (6 перенаправлены: 18→21 перевод, 19→22 среды; записи о настоящих Этапах 18–19 не тронуты); остальные файлы списка (`README*`, `GUIDE`, `INSTALL`, `12-install`, `08`, `09`, `entrypoint/AGENTS.codex.md`) уже были чистыми после досрочной зачистки Этапа 18, `STATUS.md` переписывается по ходу.
- **§1.36 и §1.52 выполнены досрочно** standalone-фиксом v1.11.1 (журнал действий — плоский `actions.log`; абзац «Применение» в `07-consolidation.md` синхронизирован) — из этого прохода исключены.
- **Полнота GUIDE как эксплуатационного справочника (по запросу пользователя, добавлено после Этапа 20).** **Выполнено (сессия 1):** все перечисленные пробелы закрыты по коду — `--intent`, `partition_queue` (все флаги + `--depth`), полный набор флагов `notes.py` (`--kind`/`--confidence`/`--id`/`--status`/`--part-of`/`--edges`), `migrate_schema.py` (новая строка), `--pattern-demo`, `--compare-embeddings`, `verify_claims --by-commit` в таблице извлечения, `lifecycle.py session-start`/`session-end [--transcript]`. Попутно сверены на полноту INSTALL.md (покрывает весь реальный набор флагов `install.py`) и `09-config.md` (по запросу пользователя: покрытие сверено с шаблоном `config.yml` И с ключами, читаемыми кодом, — все ключи описаны, включая code-only `convergence_tol`/`seed_floor`/`relation_prior_default`; дописана легаси-форма `sources:`). Финальная вычитка «от общего к частному» — вместе с прозаическим проходом.
- **Прозаический регистр всей документации + повторная зачистка §3 (по запросу пользователя, добавлено после Этапа 20).** Таблица §3.2 расширена в v1.11.1; сами проходы по текстам вошли шагом 1 про-файлового протокола перевода (см. ниже).
- **Зеркальность требований в переводе (по запросу пользователя, добавлено после Этапа 20).** Вошло шагом 2 про-файлового протокола перевода (см. ниже).
- **Ссылки на дорожную карту в управляющем слое (зафиксировано на Этапе 18, по вопросу пользователя).** Установленному проекту roadmap не поставляется (копируются только skills/agents/блок), а номера **будущих** этапов уже один раз сдвигались (вставка Этапов 18–20: перевод 18→21, среды 19→22), поэтому правило двухуровневое: (а) в **исполняемых промптах** (блоки точки входа, `SKILL.md`, `agents/*.md`) ссылок на этапы не должно быть вовсе — формулировка обязана быть самодостаточной; энтрипоинты вычищены на Этапе 18 (`AGENTS.codex.md` нёс будущее «Stage 19»), осталось ≈3 прошлых атрибуции «Stage 13/14» в `skills/amg-retrieve/SKILL.md` — заменить смысловой формулировкой; (б) в **комментариях скриптов движка** атрибуция **прошедших** этапов и пунктов аудита (`Stage N`, `audit 1.XX`) допустима как конвенция провенанса решения — номера закрытых этапов и пунктов §1 стабильны при сворачивании (append-only), — но ссылок на **будущие** этапы («будет сделано на Этапе N») быть не должно; греп на Этапе 18 подтвердил, что таких в скриптах не осталось. Проход выполняется этим же этапом вместе с §1.33/§1.34 (тот же набор файлов). **Дополнение (Этап 19, по замечанию пользователя):** (в) **шаблон `config.yml` — поставляемый артефакт** и приравнен к исполняемым промптам: ссылок на этапы в его комментариях быть не должно (внесённые на Этапе 19 вычищены сразу, механизмы описаны по смыслу); (г) конвенцию (б) для комментариев скриптов при этом проходе **пересмотреть в сторону ужесточения** — roadmap живой документ, и слои приложения не должны на него опираться: прошлую stage/audit-атрибуцию либо удалить, либо заменить смысловой формулировкой («почему так»), решив единообразно на месте. **Выполнено (сессия 1) целиком:** (а) `skills/amg-retrieve/SKILL.md` вычищен (попутно починен разорванный список Reference), (в) шаблон `config.yml` вычищен (6 легаси-ссылок), (г) конвенция ужесточена и применена единообразно — все ~245 stage/audit/roadmap-атрибуций по скриптам движка, селфтестам, вьюеру, `consistency-model.md` и `install.py` удалены либо заменены смысловой формулировкой на месте (одно внутреннее имя теста переименовано); греп `Stage \d|stage \d|audit 1\.|roadmap` по `skills/ agents/ entrypoint/ config.yml install.py` чист; 22 селфтеста и mypy(18) зелёные.
- **Реструктуризация READMEs и первые правки прохода (по запросу пользователя, выполнено в сессии 1):** «Карта документации» перенесена в самый конец обоих README (после «Лицензии»); раздел «Что реализовано и что в планах» переписан структурированным списком возможностей и очищен от «планов»; в список «Чем AMG выделяется» добавлены три преимущества (отбор в память по ценности информации — THEORY §9; перенос опыта по аналогии узлами-паттернами — §13; диалоги как память: заметки + авто-выгрузка + дайджест); в карте документации GUIDE появилась настоящая ссылка на INSTALL.md.
- **INSTALL переведён досрочно и ужесточён (сессия 2, по итогам dogfood-установки — §1.54):** корневой `INSTALL.md` теперь английский (управляющий слой — модель исполняет его при «установи AMG по …»), русское зеркало — корневой `INSTALL_RU.md` по образцу `README_RU.md`; поток вопросов императивен (осмотр проекта + классификация папок, каждый вопрос по одному, KEY-пометки у языка и эмбеддингов), новый раздел «что менять потом дёшево, а что решить до сборки» в обеих версиях; ссылки русских доков (GUIDE, 09, 12, README_RU) перенаправлены на зеркало. Из корпуса перевода INSTALL исключён (готов); при переводе GUIDE/09/12 на английский их ссылки ведут на корневой `INSTALL.md`.

**Оставшийся объём этапа (после сессий 1–3 подготовка закрыта целиком) — ПЕРЕВОД по про-файловому протоколу.** Все технические пункты выше выполнены (глоссарий, §1.33–§1.35, свип roadmap-ссылок, перенумерация, полнота GUIDE/INSTALL/09-config, READMEs, INSTALL en+ru, §1.53/§1.54); дальше каждый файл `docs/ru/*` проходит два шага подряд — так файл трогается один раз:

1. **Вычитка русского оригинала перед переводом** (расхождения чинятся в русском ДО перевода, чтобы английский не зафиксировал устаревшее): чеклист CLAUDE.md §7 целиком, особенно п. 7 — регистр живого объяснения (§3.4: полные фразы со связками и интуицией, не конспект; тексты Этапов 8–20 писались сжато); повторная зачистка §3.1/§3.2 (рецидивы: «деривация» → «семантическое обогащение», батч, ресенд, чекпоинт, резюмируемость, линковка — имена артефактов кода остаются английскими, при первом упоминании оригинал в скобках); сверка чисел, имён, дефолтов и краевых случаев с фактическим кодом; нейтрализация `.claude/*` гибридом — в копипастящихся командах конкретный `.claude/...`, в прозе и заголовках «каталог агента» / `<agent_dir>`, ведущая оговорка в каждом файле; привязка оставшихся опережающих описаний к их этапам.
2. **Перевод на английский уровня носителя** (требования пользователя, обязательные к каждому файлу): строго по глоссарию `docs/en/glossary.md` (новые термины дописываются в него на месте; ссылок на глоссарий из документов нет); **не потерять ни одной детали** — смысл один в один, включая примеры, числа и краевые случаи; **никаких калек и дословных русских фраз** — идиоматичный технический английский, каким его написал бы носитель (исключение — идиомы и конструкции, присущие только целевому языку); регистр §3.4 действует зеркально в английском; Mermaid-диаграммы переводятся; **ссылки в en-файлах ведут только на en-зеркала**: дерево `docs/en/*` строго зеркально `docs/ru/*`, поэтому относительные ссылки сохраняют ту же форму (`./01-overview.md`, `../THEORY.md` и т. п. работают без правки пути), а корневые цели меняются на английские пары — README.md и INSTALL.md (в ru-файлах соответственно README_RU.md и INSTALL_RU.md); результат кладётся в `docs/en/*` строго зеркально `docs/ru/*`.

Корпус и порядок (рекомендация): `architecture/README` → `01`–`10`, `12` → `THEORY` → `GUIDE` → в конце до-синхронизация корневого `README.md` с переведённым корпусом и сверка с `README_RU.md`. `INSTALL` уже переведён (en в корне + `INSTALL_RU.md`); `consistency-model.md` живёт в `skills/` и уже английский. По `11-roadmap.md` решить в переводной сессии: живой рабочий документ — переводить целиком или зафиксировать английским снимком на момент закрытия этапа (ссылка из README.md на `docs/en/architecture/11-roadmap.md` уже проставлена).

**Выполнено (сессия 4): переведён весь набор `docs/en/architecture/*` — README, 01–10, 12** (коммиты `ff71f55`…`a3dd06b`), по про-файловому протоколу. Шаг 1 к каждому файлу: сверка чисел/дефолтов/CLI с кодом, регистр §3.4, зачистка из реализованного остаточных ссылок «Этап N / аудит 1.xx» (ссылки на разделы §4 roadmap как на решения — сохранены; та же конвенция продолжается в THEORY/GUIDE); найденные и исправленные в ru дефекты: фантомное `chat` в `provenance.kind` (02 + THEORY §15.2 — код пишет только code/doc/data/user/model_inference), пропуски в 01 (`lifecycle.py`, `migrate_schema.py`, глагол `digest`, полный список самотестов и layout), глагол `digest` в CLI 07 (без блокировки), устаревшие «usage.log копится до Этапа 14 / консолидация его не читает» в 03/08 (правило реализовано, читается при `apply_hebbian`), неверный дефолт `amg_root` в 04 (резолв цепочкой, не жёсткий `.claude/amg`), неполный состав `/amg status` и пропуск глагола `view` в 08, устаревший маркер «(заглушка)» в README архитектуры, дефолт `--store` в 10 (`_default_store`, не «относительно скрипта»), заголовок «Три команды» в 05 (команд пять); глоссарий пополнен парами «правило сохранности → preservation rule», «заработанный след → earned trace». **Решение сессии по `11-roadmap.md`: живой документ остаётся русским, при закрытии этапа фиксируется английский снимок** `docs/en/architecture/11-roadmap.md` с шапкой-оговоркой (перевод целиком дублировал бы каждую сессионную правку). Остаток перевода: `THEORY` → `GUIDE` → до-синхронизация корневого `README.md`; en-ссылки architecture-зеркала на `../THEORY.md`/`../GUIDE.md` заработают после их перевода (прецедент README.md с Этапа 7). Селфтесты (21 + install) и mypy(18) зелёные.
