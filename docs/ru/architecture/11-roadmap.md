...
7. **Lazy/on-demand derivation — строительные блоки сейчас, полная лень отложена (спецификация → §4.10, дом реализации → Этап 17, задача 7).** В Этапе 12 НЕ вводится скрытый переключатель `derivation`: его нельзя обоснованно включить без индекса и eval на большом графе (§6.3 — против спекулятивной настройки). Реальные строительные блоки закладываются здесь и полезны сами по себе: структурный-граф-первым уже существует by construction (`bootstrap` создаёт скелет + очередь, смысловая деривация — отдельный шаг); scoping деривации подмножеством даёт `partition_queue.py` (задачи 5/6); главный безопасный токен-выигрыш — возобновляемость без перерасхода (задача 13), без потери качества. Полная on-demand/фоновая лень с eval-проверенным переключением дефолта расписана в §4.10 (честный разбор «лень стоит качества до первого касания», фазы A–D, предохранители, DoD) — зафиксирована, пока рассуждение свежо.
...
14. **Транзакционный `log.md` (см. 1.15).** — **выполнено.** `GraphStore.append_log(source, msg, txid)` дописывает строку через транзакцию (весь лог — один блоб, повтор идемпотентен), дедуп по `txid` (повтор зафиксированной транзакции не задваивает), ротация (перелив старых строк в `archive/log-<время>.md`, хвост — в живом `log.md`, чтобы долгоживущий граф не переписывал неограниченный файл). `consolidate._log` перенаправлен на него; `reconcile` пишет итог `bootstrap`/`apply` под гейтом `if txid` — неизменный прогон не логирует (идемпотентность сохранена). Источник в строке (`consolidate`/`reconcile`) различим: `lifecycle._last_consolidation` фильтрует по нему, `/amg status` не сломан. Доки `03-storage` («Журнал действий») и `consistency-model §11` синхронизированы; regression — `selftest_graph_store.case_action_log`. (Привязано из §1 в подэтапе models/env между Этапами 10 и 11.)
