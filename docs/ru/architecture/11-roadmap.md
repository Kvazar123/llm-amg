...
### Этап 17 — Группа D + закрытие: план следующей сессии (handoff)

Группы A–C выполнены и закоммичены (`a5767d5` реформа карты; `6947063` Группа B — pattern-узлы + метрики; `cd49eb0` Группа C — ленивая деривация). **Реформа Этапа 17 учтена: scopes — внутрипроектные слои (атрибуция уже в `provenance.kind`, Этап 13), НЕ межрепозиторная память (§4.7 + раздел 8) — не повторять эскалацию «общей памяти для нескольких проектов».** 21/21 селфтест + mypy(17) зелёные, НЕ запушено. Осталось: Группа D (замеры-и-решения + заметка) и закрытие.

**Группа D:**

1. **Хебб (задача 8).** Перемерить улучшенное правило (`consolidate.fold_weights`) на `../amg-bigtest` (`_measure_big.py`, три засева BM25/model2vec/ST) — подтвердить выигрыш Этапа 14 (regression). **Реального `usage.log` нет → дефолт `apply_hebbian: false` остаётся** (флип — только на живом проекте с реальным провенансом использования). Обновить «когда включать» в GUIDE/READMEs числами + рецептом включения.

2. **Замер ленивой деривации off/on на `../amg-bigtest` (НОВЫЙ код этапа — обещан).** Доказать численно, что предохранитель «первое касание синхронно» держит качество, а «голая лень» — нет. **Дизайн — три условия на размеченных кейсах bigtest (три засева):**
   - **eager** — полный граф (у всех узлов сводки + смысловые рёбра) = эталон recall/hop/pack;
   - **naive-lazy** — у отложенного подмножества (листовые kinds `function/section/record`, т.е. НЕ `partition_queue.PRIORITY_KINDS`) убрать сводку + смысловые рёбра, оставив структурный скелет (структурные рёбра/`part_of`, пустая сводка, `status: stale`) → recall/hop должны **просесть**;
   - **lazy+first-touch** — то же, но для каждого запроса перед скорингом **восстановить** сводку/рёбра у *активированных* отложенных узлов (имитация фазы B; какие узлы — из `retrieve.stale_in_pack`) → recall/hop **держатся ≈ eager**.
   Вывод: lazy+first-touch ≈ eager (качество держится), naive-lazy < eager (падает), доля отложенного = экономия токенов. Расширить `../amg-bigtest/_measure_big.py` (+ при нужде `gen_big.py`); опора — `partition_queue.priority_split` (что отложено) и `retrieve.stale_in_pack` (что доактивировать). Результат — в THEORY §4.10 и GUIDE как подтверждение предохранителей; дефолт `derivation` остаётся `eager`.

3. **Semantic-drift (задача 6).** Измерить боль: прогнать длинную неструктурированную прозу (расшифровка созвона / plain-text без заголовков) через нарезатели Этапа 11 (`extract_structure`: chat/log/recursive-JSON/RST), проверить по eval размер единиц (раздутые / просадка полноты). **Вероятный исход: структурные нарезатели достаточны → сегментатор НЕ нужен** (обосновать, §4.6). Если боль подтверждена — реализовать под eval-гейтом (спецификация §4.6).

4. **Hypervector (задача 4).** Исследовательская заметка + рекомендация в THEORY (новый раздел) и карту: что такое HV/symbolic-connectionist слой, как он лёг бы **производным индексом поверх** графа (НЕ замена), оценка пользы/риска/масштаба, рекомендация (скорее «не сейчас»). Кода нет.

**Закрытие Этапа 17 (после Группы D; протокол CLAUDE.md §4.6, строго по порядку):**
- **Документация:** THEORY (НОВОЕ: pattern-узлы как внутрипроектный перенос опыта/аналогии; заметка hypervector; §8.2 Хебб — перемеренные числа; §4.7 reframe scopes; §4.10 подтверждён); `02-data-model` (типы pattern-узлов в классе synthesized); `06-retrieval` (strategic-ярус паттернов, `stale_in_pack`); `07-consolidation` (числа Хебба); `09-config` (ключ `derivation`); `10-eval-tools` (метрики паттернов + `--pattern-demo` + off/on-протоколы); `04-ingest` (решение по semantic-drift); `08-agents-skills` (эмиссия паттернов в `amg-synth`, ленивая оркестрация в SKILL bootstrap/retrieve); GUIDE (pattern-узлы; **`derivation: lazy` в полной мере** — управляется одним ключом `derivation: eager|lazy` (дефолт `eager`, явный opt-in, **НЕ авто-по-размеру**); вызывается на построении графа (деривируется «карта», листья откладываются) + при запросе (первое касание синхронно) + в фоне по usage; предохранители; осмыслен **только на больших разрежённо-опрашиваемых графах**, на малом — оставлять `eager`; + когда включать Хебб реальными числами); READMEs (RU+EN; кратко упомянуть **возможности**: проектные pattern-узлы как перенос аналогий внутри проекта; опциональная ленивая/on-demand деривация для очень больших графов, off по умолчанию). §7-чеклист + §3.4 к каждому файлу.
- DoD Этапа 17 (все пункты) + все селфтесты + mypy зелёные.
- Сворачивание: тело Этапа 17 в §5 → «Выполнено: …»; раздел 8 и §1.37/§1.38 остаются.
- Релиз: VERSION → **v1.8.0** (MINOR, аддитивно); CHANGELOG; `chore(release): v1.8.0 — этап 17 закрыт`; аннотированный тег `v1.8.0`; пуш `--follow-tags`.

**Вне Этапа 17 (отдельная config/install-сессия между 17 и 18):** §1.37 (мерж глобального→локального конфига, вариант A, разные корневые папки сред через `resolve_amg_root`) + §1.38 (gitignore-движок при локальной установке).
