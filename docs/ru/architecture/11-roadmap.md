...
22 (D). **Зарядить eval-харнесс на стенде и перемерить Хебб на реальном usage.log.** Разметить ~10 реальных запросов по стенду (gold_ids из `inspect_graph.py`), `cases.json` вне каталога движка, `eval_gate.cases` → на него; `usage.log` уже копится (3 записи после досборки) — по мере накопления прогнать `fold_weights` off/on на КОПИИ store и снять recall/hop-recall; решение о флипе `apply_hebbian` — только при устойчивом выигрыше (иначе зафиксировать «остаётся false» с числами).
...
- селфтесты зелёные; закрыты §1.55–§1.60; правка каждого документа выполнена зеркально в `docs/ru` и `docs/en` (§3.5).
