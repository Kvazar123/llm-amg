...
- живой прогон: `gate: ok` и измеренное снижение стоимости против третьей установки по двум метрикам (некэшевые ≤ ~3 М; cache-read ≤ ~15 М чистого прогона; время — десятки минут); вердикт по «×20–30» с разъяснением его области действия (повторная работа, не ресенд среды) зафиксирован в этапе и GUIDE;
