...
29. **Компактный профиль пакета `--compact` (§1.75).** Дизайн и приёмка — в пункте аудита (флаг у retrieve.py: скромные бюджеты + указатели без тел; SKILL/блоки: «точечный запрос → `--compact`»); worst-rank-данные для калибровки уже сняты (отметка задачи 22). Доки зеркально: 06/09, GUIDE («Что происходит автоматически» + «Тонкая настройка»), READMEs — одной фразой в петле.
30. **Gated-напоминание о памяти по ходу сессии (§1.76).** `lifecycle.py prompt-hint` + хук UserPromptSubmit (только claude-code; условия срабатывания и приёмка — в пункте аудита); фиксация в 08 («Слой жизненного цикла») и GUIDE; в блоках Codex/generic — честная оговорка об отсутствии механизма.
