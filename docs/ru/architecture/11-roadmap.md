...
**Привязка и решения (внесено 2026-06-16; проверкой кода/git подтверждено — фича НИКОГДА не реализовывалась, отката не было; блок `models` всегда был декларацией).** Привязать к **Этапу 12** (там задача 8 «оптимизировать модельные затраты» — 1.14 её прямой пререквизит); по желанию пользователя — раньше. Согласованные детали:

- **thinking-уровни:** `off | low | medium | high | xhigh (≡ extra) | max` (свести к фактическим уровням Claude Code на момент реализации).
- **Версия модели:** не вводить кастомный микро-формат (`opus_4_8`/`opus_48`/`opus_4.8`); принимать либо **семейный алиас** (`opus`/`sonnet`/`haiku` = последняя), либо **точный id** в стиле реальных id Claude (`claude-opus-4-8`, через дефис) — снимает спор разделителей и сразу мульти-провайдерно (финально подтвердить с пользователем).
- **Мульти-провайдер:** `model:` принимает любую модель, поддерживаемую Claude Code, включая модели **других провайдеров**; нужен веб-ресёрч точного синтаксиса (Claude Code это уже умеет).
- **Открытый веб-ресёрч до реализации (в свежей сессии, не переполняя контекст):** актуальные возможности Codex (скиллы / субагенты / выбор модели под задачу?). Если они есть — env-адаптацию Этапа 10, возможно, переработать (отдельный режим `--env codex` вместо сваливания в `generic` «без скиллов»), и тогда `models`-тиринг применим и к Codex. Подробности и контекст — память проекта `models-1.14-and-env-research`.
- В средах **без скиллов** субагентов нет — все роли исполняет один экземпляр модели, поэтому per-role `models`-тиринг там инертен (одна модель среды).

> **Ревизия §1 (по запросу пользователя):** в разделе 1 несколько открытых пунктов не привязаны к этапам — пройтись и каждому открытому дать «→ Этап N» (кандидаты: 1.14 ← сделано здесь; 1.15 транзакционный `log.md`; 1.28 `--no-pack`↔журнал ко-активаций; 1.29 пересечение `mirror`/`absorb`; 1.30 диагностика несуществующего пути — частично закрыт `by_source` на Этапе 10). Задача зафиксирована в памяти проекта.
