...
- **Полнота GUIDE как эксплуатационного справочника (по запросу пользователя, добавлено после Этапа 20).** В руководстве обязана быть **вся** информация о пользовании системой, и каждая операция — обоими путями: глаголом `/amg` И консольным вызовом скрипта. Известные пробелы на момент записи: `retrieve.py --intent history|conflict` описан только словесно (флага нет в таблицах команд); `partition_queue.py --priority/--usage/--max-units/--max-chars`; `notes.py --kind/--confidence/--id`; `migrate_schema.py` не упомянут в GUIDE вовсе; `eval_retrieval.py --pattern-demo`; сверить `--compare-embeddings` и `lifecycle.py session-end --transcript`. Пройти по чеклисту недостаточности (CLAUDE.md §7 п.1) каждый скрипт и каждый флаг; изложение строго от общего к частному.
