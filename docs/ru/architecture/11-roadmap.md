...
Пункты 1.55–1.60 добавлены по итогам **второй установки на реальный проект** (~1 МБ; тот же приёмочный стенд) уже исправленным движком v1.12.0. Итог поля: сборка стала быстрее (~2 ч против ~8) и прошла без ручной доводки store, но **стоимость осталась порядка прежней** (~80 Мтокенов чтения кэша одной модели; цель ×20–30 не подтверждена — совместная живая приёмка Этапов 19–20 так и не выполнялась), а **использование памяти в реальной работе** выявило пробел управляющего слоя (модель после активации решала сложный промпт сплошным поиском по файлам, минуя граф). Аудит текущего кода подтвердил: механизмы Этапов 18–20 реализованы полностью (текст юнита в очереди, авто-сводка тривиальных, кэш обогащения, резолвер/канонизация, валидация apply, гейт связности, чекпоинты билдера и synth, вопросы установщика, слои конфига); остаточные дефекты — не в объёме данных, а в **числе ХОДОВ** оркестратора и synth (каждый ход агента переотправляет весь его накопленный контекст — ресенд-экономика THEORY §4.4 не доведена до этих двух ролей), в отсутствии чекпоинтов у линковщика и в петле использования. Дом всех шести — Этап 22.

---

### 1.55. `amg-synth` сам сканирует `nodes/` — компактный вход для синтеза не подготовлен

Обнаружено разбором остаточной стоимости второй установки. Промпт `amg-synth` велит читать «the populated node set under `nodes/` (frontmatter)»: на сотнях узлов это сотни `Read`/`Grep`-ходов изолированного агента, каждый — с переотправкой всего накопленного контекста; квадратичный сток токенов, крупнейший остаточный после Этапа 20 (замысел «summary-слой уже материализован — скан не нужен» реализован для линковщика подготовленными пачками `link-batch-*.json`, а для synth входа никто не готовит). Решение: детерминированная выжимка слоя сводок одним файлом `work/synth-input.json` (id/type/qualname/summary/part_of, сгруппировано по поддеревьям, с капом объёма; готовит `link_candidates.py`), промпт synth читает **только** её + `hub-candidates.json` — скан `nodes/` и чтение исходников запрещаются. **Та же болезнь — у `amg-consolidator`:** его вход (`consolidation-plan.json`) несёт только id и числа, а промпт велит читать «the relevant node bodies» — на большом плане судья ходит по узлам поштучно; лечится тем же способом — `make_plan` вкладывает в план сводки (и короткие выдержки) всех кандидатов под капом, точечное чтение остаётся только для финалистов действий. **Дом — Этап 22 (задача 2).**

---

### 1.56. Оркестратор применяет derived-части по одной команде на файл — O(частей) ходов с полным ресендом

Обнаружено там же; **полем подтверждено как главный сток**: пользователь сессию перезапускал, то есть сборка шла ЧЕРЕЗ SKILL — и оркестратор честно исполнял его тогдашнюю букву. SKILL (шаги 4–6) велел применять каждый `derived-*-pNN.json` отдельным вызовом `reconcile.py apply <файл>`: на реальном графе это ~сотня Bash-ходов оркестратора, и каждый ход переотправляет весь его контекст — «тонкая оркестрация» Этапа 20 (агрегаты вместо содержимого) обесценивается самой множественностью ходов. Решение: **пакетный apply** — `reconcile.py apply` принимает несколько путей и глоб-шаблоны (расширять внутри: оболочка Windows глоб не раскрывает), плюс команда `apply-derived` (применить все `work/derived-*.json` одним вызовом: детерминированный порядок, агрегированный итог `{applied, created, skipped_*, invalid}`, перенос применённых файлов в `work/applied/` — повторный вызов и есть resume); SKILL: один вызов на раунд. **Дом — Этап 22.**

---

### 1.57. Линковщик не пишет чекпоинт-части — обрыв теряет всю пачку

Наблюдение пользователя на второй установке: упавший линковщик начинает свою пачку заново. `amg-builder` и `amg-synth` фиксируют вывод частями `-pNN` (Этап 20), а `amg-linker` пишет один файл на пачку: внутрипачечная работа при обрыве пропадает (пасс-уровневый resume есть — пары упавшей пачки не «уже-связаны», и повторный `link_candidates.py` их переноминирует, — но подтверждённые до обрыва связи выбрасываются). Решение: тот же протокол контрольных частей `derived-links-<n>-pNN.json` (~10 узлов на часть) + правило resume в SKILL «уцелевшие части применяются первыми» (безопасность повторов уже гарантирована идемпотентным apply). **Дом — Этап 22.**

---

### 1.58. Петля активации не выдерживает сложный промпт: нет декомпозиции и правила «граф раньше сплошного поиска»

Наблюдение пользователя: на большом многотемном промпте модель отрапортовала активацию памяти, а затем искала контекст обычными grep/чтением подряд (задев и лишние файлы) — граф в работе не участвовал. Блок активации несёт одно предложение «Retrieve before you work»; в нём нет: декомпозиции сложного промпта на подзапросы (отдельный retrieve на каждую самостоятельную тему), повторного обращения при смене фокуса (README/GUIDE это обещают — «before each task and on a focus change», — в исполняемом блоке этого нет), правила «сначала пакет и его указатели `path:line`, сплошной grep/glob — только при промахе пакета, и сказать об этом», и видимости использования («собрал пакет из памяти по X»). Решение: переписать раздел петли во всех трёх блоках активации в императивный протокол; опционально (решить измерением на стенде) — `UserPromptSubmit`-хук с однострочным напоминанием. **Дом — Этап 22.**

---

### 1.59. Поток установки молчит о перезапуске сессии — команда и скиллы регистрируются при старте

Наблюдение пользователя: после установки модель сообщает «команды `/amg` доступны», но текущая сессия Claude Code свежесозданные скиллы и слэш-команду не подхватывает (реестр читается при старте сессии) — пользователь получает «unknown command». Хуже того: сборка, запущенная в той же установочной сессии, шла бы **без** скилла `amg-bootstrap` — модель импровизировала бы конвейер по INSTALL.md и блоку, теряя тонкую оркестрацию и чекпоинт-дисциплину (риск худшего случая; в наблюдавшейся установке пользователь сессию перезапускал, так что стоимость дал не этот путь, а §1.55–§1.57). Решение: INSTALL.md — финальное сообщение установщика требует **перезапустить сессию** перед активацией петли и первой сборкой (сборку в установочной сессии не начинать); шаг «перезапуск» — в quick start README/README_RU и GUIDE. **Дом — Этап 22.**

---

### 1.60. Отчётность сборки не доезжает до пользователя; «Done» среды поверх упавшего агента

Средовая метка «Done» на Task-вызове — механика Claude Code (метка завершения инструмента), изнутри AMG не меняется; защита AMG — финальная строка агента `BATCH COMPLETE|PARTIAL`, сверка счётчиков и отчёт прогресса оркестратором (Этап 20). На второй установке пользователь не видел ни процентов, ни явного «агент оборвался»: требование SKILL «отчитаться процентом после каждого раунда apply» усилить до обязательного элемента **каждого** промежуточного сообщения оркестратора и финала (вместе с явным «пачка X оборвалась (PARTIAL n/m) — дособираю»), а объяснение «Done среды ≠ успех агента; смотрите строку BATCH …» дать в GUIDE. Смежно (вопрос пользователя о самодельных скриптах): штатные CLI покрывают все шаги конвейера — в SKILL добавить запрет ad-hoc скриптов правки графа («не хватает шага — сообщить, не костылить»; сами вызовы `python …/reconcile.py …` — это поставляемые скрипты, а не сгенерированный код). **Дом — Этап 22.**

---

Пункты 1.61–1.65 добавлены по итогам **третьей установки** (тот же стенд, движок с задачами 1–9 Этапа 22) — двух живых стенограмм: сборки (модель-установщик) и работы с графом (модель-тестировщик). Итог поля: механика Этапа 22 работает (apply-derived применялся одним вызовом, synth работал от `synth-input.json` 532 КБ без скана, деривация 100 % за 5 волн без ручной доводки, `0 skipped_invalid`), связность 97.1 % / stale 0; суммарно ~76 Мтокенов cache-read (sonnet 55М + opus 21.3М; главная сессия-оркестратор ≈ 66 % расхода — субагенты лишь 34 %: линкеры 15 %, билдеры 5 %). Проверка суждений установщика по коду: его механизм «дедуп линковки исключает НЕприменённые пары» — **ошибочен** (`link_candidates._linked_pairs` строится по рёбрам УЖЕ в графе, краш до apply переноминирует пары; расхождение 900/1000 против 940/3000 узлов/рёбер объясняется прежней сборкой ДО чекпоинтов линкера и пакетного apply — осиротевшие части и частичный синтез, плюс §1.61); остальные его находки подтверждены и легли ниже.

---

### 1.61. Канонизатор не чинит цель-«голое имя» (`code:<qualname>` без пути)

Подтверждено полем и воспроизведено кодом: `_build_suffix_index` ключует `(категория, qual-после-::)`, а у цели `code:_resolve_extends_spec` всё имя попадает в path-часть (`qual=""`) — ключ не совпадает; `_repair_target` дополнительно требует суффикс пути с `/`. Итог на стенде: 169 повисших внутренних, из них **106 уникально-резолвимы** (ровно один узел оканчивается на `::<имя>`), 47 внешние, 16 неоднозначные; гейт уходит в `attention` при фактически связном графе (97.1 %). Формулировка «heals on one bootstrap» в докстринге честна лишь для path-суффиксов — свип работал, этот класс целей не покрыт. Фикс: второй индекс `qualname → [ids]`; чинить голое имя ТОЛЬКО при единственном кандидате (неоднозначное/внешнее — оставить висячим: неверное ребро хуже висячего); докстринг уточнить; regression — случай в `selftest_build`/`selftest_reconcile`. **Дом — Этап 22 (задача 13).**

---

### 1.62. У воркер-агентов нет инструмента Write — JSON пишется bash-heredoc'ом и срывается на спецсимволах

Подтверждено полем: `tools: Read, Grep, Glob, Bash` у builder/linker/synth/consolidator; запись derived-частей через heredoc дважды срывалась на кавычках/апострофах в doc-текстах (наблюдение: одна пачка — 24 tool-вызова и 101 тыс. токенов вместо ~7 вызовов; лечилось устной подсказкой «пиши через Write» — значит право на Write агенту среда даёт, а frontmatter его не объявляет). Смежная утечка — валидация перечитыванием: Read-back готовой части заново вливает весь JSON в контекст. Фикс: добавить `Write` в tools четырёх воркеров; в промпты — «части писать инструментом Write, не heredoc» и «валидировать без перечитывания: `python -c "import json;json.load(open('<f>'));print('ok')"`»; read-only на источники остаётся правилом промпта (Write — только в `work/`). **Дом — Этап 22 (задача 14).**
...
Пять мелких, но дорогих на практике неясностей: (1) **направление ребра в item** — линкеры выводили конвенцию, подглядывая в чужие derived-файлы: прописать явно «item.id = узел-источник отношения, to = цель; `documents`: id=doc-узел → to=код; `exemplifies`: id=конкретика → to=обобщение» (линкеру и билдеру); (2) **прогресс фазы линковки** — оркестратор докладывал проценты только по билдерам: SKILL шаг 6 — «после каждой волны — X/Y пачек»; (3) **oversized-юниты** — волновая инструкция оркестратора «не читай исходники» перекрыла промпт-правило «юнит без text читай по указателю», и такие юниты получили выдуманные сводки (confidence 0.4–0.5): в SKILL явную оговорку «чтение среза для юнита без `text` — единственное законное чтение исходников, волновые инструкции его не отменяют»; (4) **критерий полноты линковки** — по построению есть (повторный `link_candidates.py` номинирует только несвязанные пары): SKILL шаг 6 — «повторяй проход до нуля новых пачек либо `gate: ok`» — закрывает и «ledger покрытия» установщика без нового кода; двигательный гард от осиротевших частей: `partition_queue`/`link_candidates` при непустых `work/derived-*.json` печатают предупреждение «сначала apply-derived» (enforce resume-правила инструментом, не только текстом); (5) **заметки о самом движке** — установщик записал open_question/decision про внутренности AMG в граф хост-проекта (и они попали в дайджест, засоряя его): в SKILL/блок — «выводы о работе движка AMG сообщай пользователю; в память проекта их не пиши». **Дом — Этап 22 (задача 15).**

---

### 1.64. Пакет пустеет на реальном графе: абсолютный `activation_threshold` не масштабируется с размером графа

**Критический** (ломает главную функцию памяти) — подтверждено полем: на графе ~937 узлов ранжирование отличное (нужные файлы/доки/функции в топе), а `cache/pack.md` — «только заголовок»: масса PPR (сумма = 1) размазывается по ~10³ узлов, топ-активации падают ниже абсолютного порога `activation_threshold: 0.02`, и `assemble_pack` отбрасывает всё (ranked-список печатается до порога, потому расхождение и видно). Память вырождается в индекс указателей — теряются ярусы, тела, пометки доверия. Фикс: порог сделать **относительным** — нормировать активации к максимуму (max = 1) перед сборкой пакета либо трактовать порог как долю от максимальной активации (абсолютный — как необязательный floor); ранжирование не меняется, малые графы сохраняют прежнее поведение (там топ ≫ порога). Обязательно: eval до/после (recall/hop-recall демо не проседают), дополнение `selftest_retrieve` случаем «большой разреженный граф → пакет непуст», сверка потребителей (`stale_in_pack`, ко-активационный лог — оба строятся от tiers). Смежный шум: варнинг HF-токена от ST-бэкенда при извлечении — задокументировать как безвредный. **Дом — Этап 22 (задача 12, ПЕРВЫЙ приоритет).**

---

### 1.65. Установщик задаёт не все вопросы: инструмент опроса среды ограничен четырьмя вопросами за вызов

Подтверждено полем: модель-установщик задала 4 вопроса (scope, зеркала, язык, эмбеддинги), сама решила «absorb не нужен» и не спросила исключения; после напоминания задала ещё серию (активация+скелет, политика сессий, автоматика). Причина — механика среды: инструмент опроса (AskUserQuestion-класс) принимает не более 4 вопросов за вызов, и модель сжала поток до одного вызова. Фикс INSTALL.md/INSTALL_RU: предписать **серии** вопросов с фиксированным составом (например: I — scope+среда; II — источники ПОСЛЕ осмотра: зеркала И absorb/absorb_once И исключения; III — KEY: язык+эмбеддинги; IV — автоматика, сессии, бюджеты, зависимости, активация+`--build`) и явно: «вопрос про absorb обязателен, даже если предложение осмотра пустое; вопросы, чьи значения в существующем конфиге уже не по умолчанию, не задавать — показать в `in force:`». **Дом — Этап 22 (задача 17).**

---

Пункты 1.66–1.69 добавлены по итогам **четвёртой досборки** (тот же стенд, движок со ВСЕЙ группой E): досборка 186 юнитов (168 — дампы сессий, 18 — правки src/docs/tests) обошлась в ~38 М cache-read / 1.67 М output (~$66) — билдеры дёшевы (панель: ~1 %), дорога линковка (18 %) и главная сессия. Три корня стоимости: линковка не сходится (§1.66 — главный), в конфиге стенда остались капы 40/40 третьей установки (переустановка конфиг бережёт по дизайну — новые умолчания 60/80 живут в шаблоне и не доезжают до старых конфигов; 1082 узла-кандидата / 40 = 28 пачек), и само-жор дампов (168 из 186 юнитов — стенограммы сессий об AMG, их сводка и линковка — заметная статья). Механика группы E при этом отработала: 11 билдеров BATCH COMPLETE, один apply-derived на раунд, synth от листа, волны, честный отчёт установщика с обоснованным отказом от четвёртой волны. `usage.log` ЗАРАБОТАЛ (3 записи с непустым `used` после правок пользователя) — цепочка §1.64 подтверждена и с положительной стороны. Дом всех четырёх — Этап 22, группа F (решение пользователя: четвёртая досборка и есть полевой прогон, всё связанное закрывается в 22; Этап 23 остаётся средовым).

---

### 1.66. Линковка не сходится: номинатор не помнит отклонённые пары — «repeat until zero batches» недостижим

Подтверждено полем и прямой проверкой установщика: после применения 887 link-элементов повторная номинация выдала СНОВА 28 пачек (1082 узла с кандидатами против 1083), переноминировав в том числе пары, которые линкер явно отклонил (`BaseContainer.__repr__` — «debug-only»), — `_linked_pairs` исключает только ПОДТВЕРЖДЁННЫЕ пары (рёбра в графе), память ОТКЛОНЕНИЙ отсутствует, и номинатор просто сползает по ранжиру похожести к всё более слабым кандидатам. Критерий полноты SKILL («повторять до нуля новых пачек») недостижим — хвост фактически бесконечен и слабеет; каждая лишняя волна = ~28 субагентов на пере-отклонение того же. Фикс: **персистентная память рассуженных пар** — варианты дизайна (решить в сессии реализации): (а) ledger `work/judged-pairs.jsonl`, который пишет apply при потреблении `derived-links-*` (беда: apply видит подтверждённое, а не отклонённое); (б) линкер в конце пачки эмитит служебный элемент со списком ВСЕХ рассуженных пар своей пачки (расширение схемы части; apply складывает в ledger, не трогая граф); (в) `link_candidates` при новом прогоне читает состав СТАРЫХ `link-batch-*.json` и считает их пары рассуженными, перемещая файлы в `work/judged/` (осторожно: пары упавшей пачки перестанут переноминироваться — нужна связка с честной строкой `BATCH COMPLETE`/PARTIAL, например перемещать пачку в judged/ только после успешного apply её частей). Вариант (в) без изменения схемы выглядит предпочтительным, но требует аккуратного resume-поведения. Плюс SKILL: критерий полноты переформулировать («один-два прохода линковки; дальше — по gate и здравому смыслу», пока не реализована память). **Дом — Этап 22, группа F (задача 19/A).**

---

### 1.67. `_toklen` (4 символа/токен) занижает стоимость кириллицы — бюджеты пакета фактически превышаются в ~1.5 раза

Наблюдение тестировщика, подтверждено арифметикой: `pack.md` на стенде — 166 КБ (623 строки) при сумме бюджетов 38k токенов; эвристика `len//4` калибрована под английский, у кириллицы BPE даёт ~2–2.5 симв./токен → реальный пак ~55–66k токенов, и «читай пак» из SKILL на русскоязычном графе дороже заявленного в полтора раза. Фикс движка (не компенсация в конфигах проектов — «сломанную линейку чинят»): смешанная оценка без новых зависимостей — считать ASCII и не-ASCII раздельно (`ascii/4 + non_ascii/2.2` или близкая), опционально — замер фактического коэффициента по выборке узлов при сборке в кэш. Периферию (`periphery_links` — счёт ссылок, не токенов) пересмотреть заодно. Проверить потребителей `_toklen`; eval-демо (англ.) не сдвинется. **Дом — Этап 22, группа F (задача 20/B).**

---

### 1.68. Сплошной поиск из петли влетает в собственное хранилище памяти

Наблюдение тестировщика: Grep по корню проекта вернул 34 КБ мусора из `.claude/amg/` (кэш дериваций, link-пачки, журналы) — пришлось перезапускать поиск с ограничением пути. Петля предписывает «сплошной grep — крайний случай», но не говорит исключать каталог агента. Фикс: в трёх блоках активации к правилу сплошного поиска добавить «исключай каталог агента (`.claude/` и настроенный) из сплошного поиска — хранилище памяти не является исходниками»; GUIDE — упомянуть в петле. **Дом — Этап 22, группа F (задача 21/C).**

---

### 1.69. Авто-sync петлёй де-факто не запускается; нужен дешёвый детерминированный дифф + вопрос пользователю

Наблюдение пользователя: множество перезапусков сессий с правками файлов — ни одного самозапуска sync (блок: «если источники могли измениться — запусти», модель оценить «могли ли» не может и на осторожности пропускает; вопрос стоимости усиливает уклонение). Фикс (поведенческий, блоки ×3 + GUIDE/README): в начале сессии при `automation: true` модель запускает **дешёвый детерминированный дифф** — `reconcile.py plan` (секунды, ноль модельных токенов; печатает added/changed/deleted и заодно `leftover_derived`) — и при ненулевом диффе **спрашивает пользователя**: «в источниках N изменений — синхронизировать сейчас (оценка: ~K юнитов) или отложить?»; при отказе — одна строка-напоминание «граф отстаёт; `/amg sync` когда будете готовы», и вопрос честно повторяется в следующей сессии (дифф пересчитывается заново); при нулевом диффе — молчание и никакого sync. Это же закрывает судьбу задачи 6 Этапа 22 (prompt-hint отклонён — вместо напоминания на каждый промпт один дешёвый вопрос по факту диффа). **Дом — Этап 22, группа F (задача 21/C).**

---

Пункты 1.70–1.74 добавлены по итогам **пятнадцати рабочих сессий на полевом стенде** (движок группы E; четыре стенограммы модели-тестировщика + вопросы пользователя). Общий итог поля: детерминированная половина петли работает без участия человека (15/15 свёрток весов хуком, usage.log вырос до 11 записей), а слабые места — в цене и триггерах модельной половины: обращение к памяти дорого и медленно (§1.70–§1.71), судейская консолидация не запускается никогда (§1.72), ретривер уверенно говорит об отсутствующем (§1.73), флаг доверия не различает (§1.74). Проверка утверждений тестировщика по коду: «коактивации копятся 14 сессий — обнулите журнал перед флипом Хебба» — **ошибочно** (`fold_weights` потребляет и ротирует `coactivation.log` при каждой свёртке — на стенде 12 ротированных архивов; разовый «залп» при флипе даёт только накопленный `usage.log`, и это честный сценарий «включили сегодня»); «186 юнитов — остаточный файл, не backlog» — **ошибочно** (очередь жива, 16 дампов сессий не впитаны — §1.69 вживую); остальные наблюдения подтверждены и легли ниже.

---

### 1.70. Холодный старт эмбеддинг-извлечения ходит в HuggingFace Hub при локально лежащей модели

Полевой замер: retrieve с мультиязычным sentence-transformers-бэкендом — десятки секунд на запрос, из них ~29 с из 49 — проверка модели на HuggingFace Hub, хотя модель уже в локальном кэше (отсюда же stderr-варнинг про HF_TOKEN). Причина в коде: `embed._SentenceTransformers` создаёт `SentenceTransformer(model)` без `local_files_only`, и huggingface_hub делает сетевую сверку на каждый холодный процесс — а каждый вызов `retrieve.py` и есть новый процесс. Фикс движка: offline-first загрузка — сначала `local_files_only=True`, при любом сбое (модели нет в кэше; старая версия ST без параметра) повтор обычным путём: первое скачивание не ломается, повторные запуски в сеть не ходят. Документировать: READMEs («Опциональные зависимости») и GUIDE — ожидание латентности (transformer-бэкенд грузит модель в каждый процесс: секунды–десятки секунд), `HF_HUB_OFFLINE=1` как пользовательский рычаг для всех бэкендов, и практическая рекомендация «латентность важна → `pip install model2vec`» (статический бэкенд стоит первым в порядке выбора, грузится мгновенно, мультиязычная модель есть; качество мерить `--compare-embeddings`). **Дом — Этап 22, группа G (задача 24).**

---

### 1.71. Петля зовёт субагента-ретривера на каждое обращение — указательный запрос переплачивает ~50 тыс. токенов и минуты

Полевой замер тестировщика: одна ретривка через субагента ≈ 52 тыс. токенов и ~3 минуты, чтобы вернуть «четыре пути», — при том что `retrieve.py` печатает пакет в stdout, и прямой Bash-вызов из главной сессии кладёт тот же пакет в контекст одним ходом: без фиксированного оверхеда субагента (системный промпт + схемы инструментов на каждом его шаге), без двойного чтения пакета и без ожидания спавна. Буква SKILL amg-retrieve безальтернативна («Spawn amg-retriever»); бесскилловый блок уже зовёт CLI напрямую — расхождение блоков. Эта же асимметрия цены — главный механизм «затухания петли» (§1.58, подтверждено самоанализом тестировщика: «когда правило требует суждения, а одна из веток дешевле, дрейф идёт в сторону дешёвой»). Фикс: двухуровневый протокол в SKILL и блоках CLAUDE.md/Codex — по умолчанию прямой CLI-вызов (напечатанный пакет и есть рабочий контекст; `--intent history|conflict` модель ставит сама), субагент amg-retriever — осознанная изоляция (вопрос к памяти, чей ответ — короткая выжимка, а пакет в основное окно не нужен; уже перегруженный контекст). Отклонено: PreToolUse-гард на Grep/Glob (идея тестировщика) — это возврат отклонённой задачи 6 в другой одежде (механический налог, блокирующий deny даёт ложные срабатывания на законном grep-после-промаха); вместо надзора удешевляется правильное действие (прямой вызов + §1.70 + задача 23). **Дом — Этап 22, группа G (задача 25).**

---

### 1.72. У судейской консолидации нет исполняемого триггера; документация обещает «петля в конце сессии»

Поле (15 сессий): детерминированная половина отработала хуком все 15 раз, судейская — ни разу; README-строка «запускается петлёй в конце сессии (перед /clear)» неисполнима — момент «сессия кончается» модели не виден (хук субагента позвать не может, /clear предвидеть нельзя), и `last consolidation` в status не отличает судейскую от хуковой свёртки весов (обе пишут `consolidate |`-строки). Фикс тремя слоями: (а) блоки ×3 — явный наблюдаемый триггер: сигнал пользователя о завершении работы («заканчиваем», «на сегодня всё») = запустить судейскую при `automation: true` (или предложить); (б) движок — предупреждение об отставании и различение половин в status (реализация — в задаче 21/C); (в) READMEs/GUIDE — честная таблица «кто что запускает» (судейская: по сигналу завершения / вручную / по старт-предупреждению) и разъяснение «консолидация — гигиена, не сохранение»: три фазы (weights — детерминированная свёртка сигналов без модели; plan — read-only план; apply — судейские действия с архивом и кодовой защитой `_is_protected`), разумный ритм — насыщенная сессия или зашумевшая выдача, а не каждый заход, и как позвать словами (фраза называет память — §1.35). От идей тестировщика: «только сообщать, не запускать» — принято (совпадает с 21/C); гистерезис — принято в форме «одна строка на старт сессии, гаснет после консолидации»; прокси давления через make_plan — не нужно (арифметика по actions.log дешевле и достаточна; точные числа давления модель берёт `consolidate.py plan` по факту предупреждения). **Дом — Этап 22, группа G (задача 26; движковая часть — 21/C).**

---

### 1.73. Ретривер подаёт отсутствие как факт; в промпте — команда verify_claims без пути

Поле: на вопрос «где гайд по X» ретривер уверенно ответил «интегрирован в core docs» — отсутствующий узел неотличим от ненайденного, ближайшие соседи поданы как подтверждение существования; правду дал обычный `ls`. Смежное: ретривер посоветовал `.../skills/amg-bootstrap/scripts/verify_claims.py` — скрипт живёт в `amg-retrieve/scripts/`; источник ошибки — голое имя `verify_claims.py <id>` в промпте ретривера и в блоке активации (модель достраивает путь по памяти и промахивается). Фикс: правило отсутствия в `amg-retriever.md` («запрос ищет конкретный артефакт/ответ, прямого узла нет → сказать „в памяти этого нет“ явно; соседей подавать как соседей, не как ответ») и полный отрендеренный путь verify_claims в промпте и блоке. **Дом — Этап 22, группа G (задача 27).**

---

### 1.74. ⟨unverified⟩ горит на всём — флаг не различает; verify_claims --write не входит в конвейер

Поле: все code-узлы пакета помечены ⟨unverified⟩ — сигнал, который горит всегда, сигналом не является, и тестировщик честно открыл исходники целиком. Причина: reconcile ставит `verification: unverified` на added/changed (корректно — сводку никто не сверял), а штатного прогона `verify_claims --write` нет ни в одном конвейере — только ручной совет в SKILL. При этом сразу после apply сводка выведена ровно из текущего содержимого (`derived_from_hash == source_hash`), и детерминированная сверка хешей превращает unverified → verified за секунды без модели. Фикс: свип в CLI уже есть (`verify_claims.py --all|--code` без id обходит файловые узлы — движок готов, не хватает только вызова в конвейере) — добавить закрывающий шаг sync в SKILL amg-bootstrap «после гейта связности прогнать `verify_claims --all --write`» (read-only по источникам, пишет только verification-блоки; на ~1000 узлов — секунды, один re-chunk на файл); GUIDE/09-config — семантика флага после этого: unverified — метка «изменилось с последней сверки», а не фон. **Дом — Этап 22, группа G (задача 28).**

---

### 1.75. Точечный запрос получает пакет-потолок: на связном графе бюджеты всегда заполняются — нужен компактный профиль

Переоткрыто вопросом пользователя после сессии 7 (первая попытка — задача 23, масс-стоп — отклонена измерением честно, но БОЛЬ осталась): на связном графе ~600 узлов проходят относительный порог, ярусные бюджеты заполняются до потолка, и «лёгкий указательный запрос» получает те же ~130 КБ (~45k токенов), что и вход в незнакомую подсистему, — а при прямом вызове (§1.71) весь напечатанный пакет ложится в основное окно. Полевые жалобы тестировщиков («50k на любой запрос», «пак читаем целиком») — об этом. Данные для дизайна (сессия 7, ST-засев): worst-rank gold-узлов по 11 кейсам = [32…222] — глубокое gold реально, универсальное срезание пакета режет полноту (потому статистические правила и были отклонены). Решение — не статистика, а ЯВНЫЙ выбор вызывающего: **флаг `--compact` у retrieve.py** (профиль указателей: встроенные скромные бюджеты 1200/2500/6000/периферия 40 — те же, что код-дефолты; тела в operational НЕ разворачиваются — только `путь:строка — имя — сводка`, у doc-узлов сводка без тела; decision/adr сохраняют тело — их ценность в мотивировке); полный профиль остаётся для входа в незнакомое. SKILL amg-retrieve + блоки ×3: «точечный вопрос/указатели → `--compact`; вход в новую тему/подсистему → полный». Приёмка: замерить compact-пакет на 11 кейсах стенда (размер и какой pack-recall он держит С УЧЁТОМ периферии; честно зафиксировать размен глубины на размер), eval-демо 1.00 держится. **Дом — Этап 22, группа G (задача 29).** **Почему всё семейство «умных» скалярных отсечек несостоятельно структурно (не пересуживать):** распределение PPR-активаций двурежимно по построению — засеянная голова получает (1−d)·p плюс приток, незасеянные узлы ТОЛЬКО приток (на порядок меньше), — поэтому «излом» (knee/elbow/Kneedle), наибольший разрыв, Otsu и двухкомпонентная смесь все находят одну и ту же границу «засев/приток» в первых десятках рангов и ампутируют именно многошаговый хвост, ради которого граф существует (полевые gold — до ранга 222); а внутри притокового режима релевантный сосед и фоновый узел имеют перекрывающиеся значения — в скаляре активации нет информации «откуда пришла масса». Что научно живо, но исследовательски дорого (записано, отложено до полевой боли ПОСЛЕ compact): (а) атрибуционный отбор — оставлять узлы, чья масса пришла концентрированно от засеянных путей (машина `_explain_inflow` уже есть; нужен харнесс существенно больше 11 кейсов, риск переобучения); (б) субмодулярное покрытие с маргинальной полезностью по эмбеддингам (стоп, когда прирост покрытия темы < ε; требует всегда включённых эмбеддингов и заметного компьюта). Оба — только под расширенный харнесс и только если compact-профиля окажется мало в поле.
...
Задача 6 (prompt-hint на каждый промпт) была отклонена в сессии 6 на тогдашних данных («слабина — незапуск стартового sync, а не забывание по ходу»). Пятнадцать полевых сессий дали НОВОЕ свидетельство: затухание по ходу сессии реально (самоанализ тестировщика: «удерживающая часть держится на моей самодисциплине, а она затухает…; чинится тем, чтобы момент „пора в граф“ и „пора записать“ приходил извне — как дайджест»), и удешевление правильного действия (§1.70–§1.71) снимает причину дрейфа, но не возвращает внимание само по себе. Переоткрыть с ИЗМЕНЁННЫМ дизайном — не пер-промптный налог, а **gated-напоминание**: `lifecycle.py prompt-hint` под хук `UserPromptSubmit` — печатает ОДНУ короткую строку («AMG: последний пакет N мин назад / память в этой сессии ещё не опрашивалась — новая тема начинается с retrieve») ТОЛЬКО когда (а) `active`+`automation`, (б) `work/pack-log.jsonl` отсутствует ИЛИ старше ~15 мин, (в) промпт похож на задачу (длина > ~200 знаков), (г) с прошлого напоминания прошло ≥ ~10 мин (штамп `work/hint-stamp`); во всех прочих промптах — молчание, ноль токенов. Установщик пишет хук в settings.json (только claude-code); для Codex/generic механизма нет — честная оговорка в доках, дисциплина петли остаётся (как с автодампом стенограммы). Приёмка: selftest_lifecycle (gate-условия), selftest_install (рендер хука), «тихий» промпт не получает строку. **Дом — Этап 22, группа G (задача 30).**
...
**Происхождение.** Этап добавлен по итогам **второй установки на реальный проект** (~1 МБ; тот же приёмочный стенд, что у Этапов 18–20) движком v1.12.0. Поле подтвердило часть результатов Этапов 18–20 (store резолвится верно, установка не захламляет проект, поток задаёт вопросы, сборка идёт без ручной доводки и быстрее — ~2 ч против ~8) и выявило два системных остатка: **стоимость сборки осталась порядка прежней** (~80 Мтокенов чтения кэша одной модели на проект в 1 МБ; заявленная цель ×20–30 так и не была измерена совместной приёмкой — пользователь отложил её, и поле её не подтвердило) и **память не участвует в реальной работе на сложных промптах** (модель после активации ищет контекст сплошным поиском по файлам, минуя граф). Разбор по коду — §1.55–§1.60: механика Этапов 18–20 реализована полностью; остаточная стоимость сидит в **числе ходов** оркестратора и synth (ресенд-экономика THEORY §4.4 не доведена до этих двух ролей), устойчивость — в отсутствии чекпоинтов линковщика, а петля использования — в неисполняемой мягкости блока активации.

**Мандат.** После этапа: (а) стоимость сборки определяется объёмом содержимого, а не числом ходов — оркестратор тратит O(раундов), не O(файлов), synth получает вход одним файлом; (б) обрыв любого субагента теряет не больше последней контрольной части; (в) сложный многотемный промпт обслуживается графом по протоколу (декомпозиция → пакеты → точечные чтения по указателям), и пользователь это видит; (г) цель ×20–30 к первичному dogfood-базлайну наконец **измерена** на живом стенде, либо остаточная стоимость честно разложена по статьям.

Задачи:

**Группа A — остаточная стоимость сборки (§1.55/§1.56/§1.57).**
1. **Пакетный apply (§1.56).** `reconcile.py apply` принимает несколько путей и глоб-шаблоны (расширение внутри скрипта — оболочка Windows глоб не раскрывает); новая команда `apply-derived`: применить все `work/derived-*.json` (включая `-pNN`-части) одним вызовом — детерминированный порядок, один агрегированный итог `{applied, created, skipped_missing, skipped_stale, skipped_invalid, files}`, перенос применённых файлов в `work/applied/` (повторный вызов = resume, ничего не переприменяется впустую). SKILL шаги 4–6: **один** вызов apply на раунд. Regression: `selftest_build` (apply-derived на частях, идемпотентность повтора, перенос в applied/), `selftest_queue`. — **выполнено (сессия 1):** ядро выделено в `_apply_items` (кэш/валидация/канонизация не тронуты), `apply_derivation` — обёртка; рваный JSON-чекпоинт уходит в `work/invalid/` (`malformed_files`), не роняя остальное; одна транзакция на весь вызов; `selftest_build` проверка 7.
2. **Подготовленный вход синтеза — и консолидатора (§1.55).** `link_candidates.py --synth-input` → `work/synth-input.json`: компактная выжимка слоя сводок (id/type/qualname/summary/part_of, сгруппировано по поддеревьям, счётчики; кап объёма — на графе-гиганте выжимка режется по бюджету с пометкой `truncated`) через `retrieve.load_nodes` (читает индекс, не скан). Промпт `amg-synth`: вход — **только** `synth-input.json` + `hub-candidates.json`; скан `nodes/` и чтение исходников запрещены; чекпоинт-части и финальная строка — как раньше. SKILL шаг 5 обновить (сначала `--hubs`, затем `--synth-input`, затем один synth). **Симметрично для консолидатора:** `consolidate.make_plan` вкладывает в `consolidation-plan.json` сводки кандидатов — эпизодических, обеих сторон дублей и противоречий, предпросмотр членов переполненных веток (первые ~60, id/type/summary) — под капом; промпт `amg-consolidator`: работать от плана, тела узлов читать точечно и только для финалистов действий (скан `nodes/` запрещён). — **выполнено (сессия 1):** `--synth-input` пишет `{nodes_total, stale_total, truncated, gaps, groups}` через `rt.load_nodes` (индекс, не скан); кап двухступенчатый (800 тыс. знаков: сначала обрезка сводок до 160, затем счётные сэмплы групп с точными `count`); блок `gaps` — `undocumented_code`+`_total`, `drifted_doc_refs`, `contradiction_pairs` — детерминированное сырьё gap-report (stale не считаются недокументированными); промпты synth/консолидатора переписаны («рабочий слой — входной файл/план»); план консолидации несёт `a/b_summary` у дублей и противоречий, `type`+`summary` у эпизодических, `summary` у source_contradicted, `members_preview` у веток; `selftest_build` проверка 8.
3. **Чекпоинты линковщика (§1.57).** Промпт `amg-linker`: вывод частями `derived-links-<n>-pNN.json` (~10 узлов на часть; не переписывать написанную часть), финальная строка прежняя; SKILL resume: уцелевшие части применяются первыми (идемпотентность apply уже гарантирует безопасность повторов). — **выполнено (сессия 1):** протокол частей добавлен в промпт (финальная строка уточнена — «written parts cover the first N»), SKILL шаг 6 и resume-абзац обновлены (части потребляет тот же `apply-derived`).
4. **Бюджет ходов как протокол (§1.56/§1.60).** В промпты и SKILL — явные бюджеты: билдер = 0 чтений исходников при наличии `text`, ≤1 чтение батч-файла, части по ~10 юнитов; линковщик = 0 чтений вне пачки (исходник — только при действительно двусмысленной сводке); оркестратор = спавн параллельных агентов одним сообщением, один apply-вызов на раунд, осмотр только агрегатами, **запрет ad-hoc скриптов правки графа** («не хватает шага — сообщить пользователю, не костылить»). Regression: конвейер-фикстура `selftest_build` дополняется проверкой «число вызовов CLI на раунд = O(1)» (пакетный apply вместо O(файлов)); дисциплина промптов проверяется живым прогоном (Группа D). — **выполнено (сессия 1):** SKILL «Thin orchestration» дополнен правилом «число ходов — та же валюта, что объём» (один `apply-derived` на раунд; спавн раунда одним сообщением) и запретом ad-hoc скриптов; бюджеты исполнителей уже стояли в промптах (билдер/линковщик) и дополнены задачей 2 (synth/консолидатор); O(1)-механика подтверждена `selftest_build` (все части одной командой; повтор — no-op); живая дисциплина — Группа D.

**Группа B — петля использования памяти (§1.58).**
5. **Императивный протокол петли** во всех трёх блоках активации (`entrypoint/CLAUDE.md`/`AGENTS.md`/`AGENTS.codex.md`), взамен одного предложения «Retrieve before you work». Концепция (формулировка пользователя, она и есть норма): **всё, что можно получить из графа, модель сначала получает из графа — и только потом решает, что ещё искать в файлах.** Отсюда пункты протокола: (а) перед любой нетривиальной задачей — пакет из графа; (б) **сложный многотемный промпт декомпозируется** — отдельный retrieve на каждую самостоятельную тему/часть промпта (по мере перехода к ней, не все сразу), собранные ветви графа вместе и образуют контекст задачи; (в) **смена фокуса в диалоге = новый retrieve** (обещание README/GUIDE становится исполняемым правилом блока); (г) **граф — первичный источник контекста**: сперва пакет и его указатели `path:line` → точечные чтения источников от указателей; файловый поиск — только для того, чего пакет не дал; сплошной grep/glob по проекту — крайний случай, с явным словом об этом (и с мыслью «источники менялись? — сначала `/amg sync`»); (д) **видимость**: одной строкой сказать пользователю, что контекст собран из памяти («пакет по X: N узлов»). Зеркально — GUIDE («Что происходит автоматически» → как выглядит правильная петля и как проверить, что память используется: `last_pack` в `/amg status`, `cache/pack.md`) и READMEs (см. задачу 9, «How it works»). — **выполнено (сессия 1, блоки):** все три блока активации несут протокол (заголовок пункта — «The graph is the primary context source»); в бесскилловом блоке заодно шаг 1 переведён на `apply-derived`; GUIDE/READMEs — задачи 7–9.
6. **`UserPromptSubmit`-напоминание — решить измерением.** Кандидат: `lifecycle.py prompt-hint` (гейт `active`+`automation`) печатает одну строку-напоминание о петле на каждый промпт; установщик пишет хук. Принять только если живой прогон (Группа D) покажет, что протокол задачи 5 сам по себе недостаточно устойчив; отклонить с фиксацией причины, если устойчив (лишний токен-налог на каждый промпт). — **решено (сессия 6): отклонён.** Полевая слабина петли оказалась не в «забыл retrieve по ходу», а в незапуске стартового sync (§1.69) — per-prompt налог её не лечит; вместо него — дешёвый детерминированный дифф-вопрос в начале сессии (группа F, задача 21/C).

**Группа C — поток установки и объяснимость (§1.59/§1.60 и вопросы пользователя).**
7. **Перезапуск сессии после установки (§1.59).** INSTALL.md/INSTALL_RU: финальное сообщение установщика — «перезапустите сессию: скиллы и `/amg` регистрируются при старте; первую сборку запускать в новой сессии» (сборка в установочной сессии идёт без SKILL и теряет всю оркестрационную дисциплину — прямо запретить); шаг «перезапуск» в quick start README/README_RU («First run») и GUIDE («Активация и режимы»); там же усилить акцент **«`sync` — это и есть первичная сборка графа»** (после установки → перезапуск → `/amg sync` или первая задача при `automation: true`) и «`consolidate` — регулярное обслуживание, не сборка». — **выполнено (сессия 1):** INSTALL/INSTALL_RU — абзац «Close with the restart instruction» в потоке модели + переработан «После установки» (вариант «в текущей сессии» удалён как неверный); READMEs — шаг «Перезапустите сессию» и новый шаг «Сборка» («sync = полная первая сборка», consolidate = обслуживание) в «Первом запуске», п.4 установки поправлен; GUIDE — «Активация ≠ построение графа» несёт перезапуск с обоснованием; `12-install` (ru+en) — новый подраздел «Пост-установочный шаг: перезапуск сессии».
8. **Отчётность сборки (§1.60).** SKILL: прогресс-процент (`inspect_queue → progress.derived_percent`) — обязательный элемент каждого промежуточного отчёта оркестратора и финала; оборванные пачки называются явно («пачка X: PARTIAL n/m — дособираю»). GUIDE: короткое объяснение «метка Done среды ≠ успех агента; успех = строка `BATCH COMPLETE` и сошедшиеся счётчики». — **выполнено, кроме живой проверки (сессия 1):** SKILL-половина (шаг 4: «Every between-rounds report to the user carries the progress percentage» + явное имя оборванной пачки) и GUIDE-объяснение («метка Done среды ≠ успех агента; честный сигнал — финальная строка `BATCH …` и сошедшиеся счётчики»; ru+en, «Качество и стоимость сборки») сделаны. **Живой ответ (третья установка):** интерфейс среды больше не показывает список агентов со статусами — прогресс доносит сам оркестратор словами (по билдерам работало, по линковке — задача 15), явные «упал/закончил» тоже; внутриагентные `progress: N/M`-строки в блоке агента не видны → приём отклонён, вопрос закрыт. **Проверить живьём приём внутриагентного прогресса:** если интерфейс Claude Code показывает последние строки работающего субагента, то периодическая печать `progress: N/M` после каждой чекпоинт-части даёт видимый прогресс прямо в блоке агента почти бесплатно (одна строка на часть) — подтвердится: добавить в промпты билдера/линковщика/synth и описать в GUIDE; нет — зафиксировать отказ с причиной (иного канала «проценты рядом с агентом» среда не даёт: метка и спиннер — её механика).
9. **Документация понятий, вьюера и петли — детальный план для READMEs/GUIDE (вопросы пользователя 3.4–3.7 и 4–5; всё зеркально ru+en).**
   - **READMEs, «How it works»:** абзац про петлю использования (суть задачи 5, кратко и ясно): модель декомпозирует сложный запрос, по каждой его части подтягивает ветви графа и лишь затем решает, что ещё смотреть в файлах; граф — первичный источник контекста.
   - **READMEs, «What sets AMG apart» / «What's implemented»:** сверить после правок — пункт про экономную сборку дополнить устойчивостью («все воркеры пишут результат контрольными частями; обрыв теряет минуты, а не партию»), пункт про петлю добавить, если его нет; тон README — лёгкий рекламный шлейф сохраняется (это витрина технологии).
   - **READMEs, раздел 3D-вьюера — переписать с инструментной подачей:** это не «красивая фича», а диагностический инструмент всегда под рукой: увидеть основные узлы проекта и их связи, найти острова и несвязанные узлы, doc-узлы без связи с кодом (несвязанность документации и кода), конфликты; краткая легенда — цвет = категория **содержимого** (код / документы / данные / заметки / хабы), подсветка конфликтов, размер = связность; вызов в одну команду / «open the memory graph».
   - **GUIDE:** «Build quality and cost» — подраздел про устойчивость к сбоям (пачки + контрольные части у билдера/synth/линковщика, что именно теряется при обрыве — хвост после последней части, как выглядит resume: `apply-derived` + повторный bootstrap); раздел вьюера — те же диагностические сценарии + легенда полнее; «Активация и режимы» — перезапуск после установки.
   - **Понятия (вопрос 3.6):** GUIDE (вьюер + «Сессии»/«Активация»), кратко READMEs: бакеты легенды = категории **содержимого**, не папки проекта и не политика источника (тесты на Python = code; впитанный markdown = doc, а не data); `data` = структурированные данные (JSON/YAML/CSV/XLSX…), `notes` = авторские захваты и выводы (`notes.py`, решения/вопросы/планы), дампы сессий = doc-секции; повторные обсуждения не плодят дубли (контентные id заметок; near-dup-слияние и свёртка эпизодов на консолидации — с честной оговоркой, что перефразированные повторы сливаются по порогу похожести, а не мгновенно); почему `consolidate` заметно тратит токены (судейский шаг синтез-класса — цена отбора в долгую память; после задачи 2 его вход компактен, но судья остаётся моделью синтез-класса).
   - **INSTALL.md/INSTALL_RU + 12-install.md:** перезапуск сессии как штатный пост-установочный шаг (в архитектуре установщика — почему: реестр скиллов/команд читается при старте сессии; формулировка средо-нейтральная — перезапуск безопасен в любой среде).

   — **дополнено по ревью пользователя (сессия 2, READMEs зеркально):** научная формулировка ограничения LLM и «граф взвешенных типизированных связей с извлечением активацией» в «Что это»; `derivation: eager` в «Консервативных умолчаниях»; раздел «Первый запуск **и сборка**» с капс-требованием «ОБЯЗАТЕЛЬНО ПЕРЕЗАПУСТИТЕ СЕССИЮ», запретом сборки до перезапуска и конкретикой (`sync` инкрементально сам доводит изменившееся; детерминированная половина consolidate — хуками, судейская — петлёй/командой); контраст `absorb` ↔ `absorb_once` («следит за изменениями» ↔ «заморожен»); строка `/amg view` в таблице команд + словесные триггеры; раздел вьюера переструктурирован списком возможностей (подсветка `disputed`/`rejected`/`superseded`/`stale` с пользой каждого; оборванные рёбра не рисуются — видны островами, счёт в `connectivity`) и таблицей легенды.
   — **выполнено (сессия 1, все подпункты, зеркально ru+en):** READMEs — «How it works» дополнен декомпозицией и «граф — первичный источник», пункт «воспроизводимая и экономная сборка» — контрольными частями всех исполнителей, «Build economy and resilience» в «What's implemented» — однофайловыми входами глобальных проходов и пакетным применением, раздел 3D-вьюера переписан инструментной подачей (диагностика островов/несвязанной документации, легенда «цвет = категория содержимого», tests=code / absorb-md=doc / notes=захваты / сессии=doc), пиар-тон сохранён; GUIDE — петля с проверяемостью (`last pack`, `cache/pack.md`) в «Что происходит автоматически», устойчивость с `apply-derived` и Done-объяснением в «Качестве и стоимости сборки», вьюер-диагностика + легенда, новый абзац «Куда попадает материал диалогов и почему повторы не копятся» (контентные id, near-dup-порог, свёртка эпизодов, цена consolidate) в «Сессиях»; INSTALL/12-install — задача 7. Ссылки провалидированы (кросс-языковых нет, битых нет).

**Группа D — совместная живая приёмка Этапов 19–20 (перенесённая) и закрытие мандата стоимости.**
10. **Живой прогон на dogfood-стенде** исправленным движком ПОСЛЕ задач 1–8 **и группы E (задачи 12–18)** (сессии пользователя, вне dev-сессий): связность (один крупный компонент, ~0 внутренних висячих, `gate: ok`) **и** стоимость — число субагентов/ходов по фазам и токены из панели пользователя, сравнение с базлайном первой сборки (~30 ч / ~70 Мтокенов) и второй (~2 ч / ~80 Мтокенов cache-read). Ограничение диагностики (поле): среда не персистит транскрипты субагентов (файлы 0 байт) — ретроспективный разбор cache-read невозможен, надёжный per-agent сигнал только `<usage>subagent_tokens` из уведомления о завершении (вести по ходу). **Пересчёт цели (третий проход стенограмм, честная арифметика пола):** буквальное ×20–30 по cache-read недостижимо механикой агентной среды — каждый шаг каждого субагента переотправляет системный промпт и схемы инструментов (≈40–70 тыс. токенов, вне контроля AMG) плюс резидентный вход, и даже минимальный конвейер (~30–40 агентов × 5–7 шагов + оркестратор) упирается в пол ≈10–12 Мтокенов cache-read на проект ~1 МБ; по **некэшевым** токенам (input + output + cache-write — реальная новизна) пол ≈2–2.5 М (только вывод сводок и рёбер ~1 М неустраним). Приёмка меряет ОБЕ метрики; цель: некэшевые ≤ ~3 М (≈×10 к первичному базлайну ~30 М некэшевых), cache-read ≤ ~15 М при чистом прогоне без диалога (≈×5 к 76 М третьей установки), время — часы → десятки минут; остаток сверх целей раскладывается по статьям с решением по каждой. Обещание «×20–30» Этапа 20 относилось к устранённому классу ПОВТОРНОЙ работы (перечитывание источников, ресенд содержимого, переделки — устранено Этапами 19–20 и группами A/E) и не распространяется на неустранимый пошаговый ресенд самой агентной среды — это разъяснение донести в GUIDE «Качество и стоимость сборки» (задача 18). Здесь же решается судьба задачи 6 (напоминание), проверяется приём внутриагентного прогресса (задача 8) и снимаются два отложенных dogfood-пункта Этапа 17 (эмиссия паттернов живым synth; `usage.log` начинает копиться для будущего флипа Хебба). — **Решение пользователя (2026-07-11): группа переформатирована.** Стоимостной вердикт закрыт **данными четвёртой досборки** (решение пользователя 2026-07-12: досборка и есть полевой прогон, пятый ради формальных порогов не делается): повторная работа устранена (билдеры ~1 % расхода — цель Этапа 20 по своей области достигнута), остаток стоимости назван и заведён — несходимость линковки §1.66 (главный, чинит задача 19/A), старые капы 40/40 в конфиге стенда, само-жор дампов; формальные пороги задачи 10 (некэшевые ≤ ~3 М, cache-read ≤ ~15 М полного чистого прогона) остаются ориентиром для ближайшей реальной ПОЛНОЙ сборки (например, установка Этапа 23) — сверить попутно, НЕ блокер; в Этапе 22 её замещают честное разложение стоимости по статьям (пересчёт выше + GUIDE) и **качественная половина на графе третьей установки** (dev-проверка): `gate`/связность после фильтра сессий, §1.64 вживую (непустой пакет на ~937 узлах новым движком), §1.61 вживую (свип лечит ~106 из 169 висячих одним bootstrap), эмиссия паттернов. Судьба задачи 6 и Хебб-флип остаются за рабочими Claude-Code-сессиями пользователя на стенде **после переустановки движка**: на третьей установке `usage.log` не мог начаться по причинной цепочке §1.64 — пустые tiers → `_log_pack` не писал `work/pack-log.jsonl` → `_record_usage` завершался «no pack log this session»; отсутствие файла — прямое следствие бага порога, а не отдельный дефект. — **Качественная половина выполнена (сессия 5, dev, на копии графа третьей установки, 942 узла):** §1.64 вживую — пакет непуст новым движком (полные ярусы: overview + 13 хабов + 3 узла-паттерна в strategic, тематическая окрестность, топ ранжирования 1.0); §1.61 вживую — 169→62 одним bootstrap (107 вылечено, детали в пометке задачи 13); session-фильтр метрики — `doc_without_documents` 191→169 (ровно −22 хода стенограмм); **эмиссия узлов-паттернов живым synth подтверждена** (`pattern:two-style-one-per-file`, `two-phase-component-loading`, `backend-adapter-isolation` — первый из двух отложенных dogfood-пунктов Этапа 17 снят); диагноз каналов подтверждён эмпирически — в `work/` нет ни `usage.log`, ни `pack-log.jsonl`, ни `coactivation.log` (пустой пакет глушил все три); HF-варнинг ST-бэкенда воспроизведён и безвреден, как задокументировано. **Четвёртая досборка пользователя (движок группы E, `/amg sync` на 186 юнитах):** механика зелёная (11 билдеров COMPLETE, один apply на раунд, synth от листа, `usage.log` пошёл — 3 записи), стоимость ~38 М cache-read / ~$66 — разложена по корням: линковка без памяти отклонений (§1.66, главный), старые капы 40/40 в конфиге стенда (переустановка конфиг бережёт), само-жор сессионных дампов (168/186 юнитов); граф: 1118 узлов, 5034 ребра (4823 разрешено), связность 94.3 %, stale 0, gate attention (62 изолированных — ходы стенограмм, 90 висячих — канонизация: голые имена методов + свежие цели линкеров). Полные тексты стенограмм установщика и тестировщика — у пользователя; выжимки — §1.66–§1.69 и группа 0 Этапа 23.
11. **Измерительные резервы стоимости — только под eval, дефолты без замера не менять.** *(Выполняется после задачи 22/D группы F — зарядка харнесса на стенде — пререквизит любого из замеров ниже.)* Дальнейшее сжатие сверх задачи 18 возможно лишь ценой, которую обязана одобрить метрика: (а) `models.module_summary: haiku` — тиринг уже позволяет, нужен только замер recall/hop-recall off/on на стенде (сводки питают BM25/эмбеддинги — понижение модели без замера запрещено конвенцией 09-config); (б) `linker.top_k`/`min_sim` (меньше кандидатов = дешевле подтверждение, риск недосвязности — смотреть на гейт; **полевые данные третьей установки:** линкеры отбрасывают 60–70 % номинаций как случайные, пачка ≈130–150 тыс. токенов — самый дорогой субагент, 15 % всего расхода; резать `top_k 5→3` / `min_sim 0.35→0.5` только с eval-контролем — реальное doc↔code ребро бывает с умеренным косинусом); (в) кап `synth-input` 800→400 тыс. знаков (полевой sheet — 532 КБ ≈ 133 тыс. токенов, резидентен весь прогон synth; ступень «сводки до 160 знаков» включилась бы раньше — под eval качества хабов). Итоги замеров — в протокол 09-config «Модели субагентов»; отвергнутое фиксировать с числами. — **выполнено ретроспективами (сессия 7), (а) — резерв следующей полной сборки.** (б) капы линковщика перемерены детерминированной ретроспективой на копии стенда (номинация по кэшу эмбеддингов против 2879 ПОДТВЕРЖДЁННЫХ межфайловых смысловых пар): штатные top_k 5 / min_sim 0.35 покрывают 67 % подтверждённых пар, ужатие до 3/0.5 — лишь 47 % (−20 п.п. реальных связей) — **ужатие отклонено с числами** (заодно снята одноимённая рекомендация из дайджеста стенда); (в) кап synth-input 800k валиден: реальный лист 1130 узлов = 448 тыс. знаков (запас ×1.8, без усечения), кап 400k уже резал бы сводки (truncated: summaries) — **не снижать**; (а) `module_summary: haiku` требует живой пересборки — задокументированный резерв ближайшей полной сборки (прецедент задачи 10).

**Группа E — полевые дефекты третьей установки (§1.61–§1.65; выполнять в новой сессии, задача 12 — первой).**
12. **Относительный порог сборки пакета (§1.64, КРИТИЧЕСКИЙ).** `retrieve.py`: нормировать активации к максимуму (max = 1) перед `assemble_pack` либо трактовать `activation_threshold` как долю от максимума (абсолютное значение — необязательный floor); ранжирование и PPR не меняются; сверить потребителей tiers (`stale_in_pack`, `_log_coactivation`, `_log_pack`). Regression: `selftest_retrieve` — «размазанный большой граф → пакет непуст»; eval-демо recall/hop-recall/pack-recall 1.00 держатся; `09-config`/GUIDE — семантика порога (зеркально ru+en). Смежно: задокументировать безвредный HF-варнинг ST-бэкенда. — **выполнено (сессия 4):** `_rescale_to_max` в `retrieve()` сразу после `_apply_status_prior` (выбрана нормировка, не floor-ключ — новых ключей нет; `ranked` несёт нормированные значения, топ всегда 1.0, сопоставимо между графами); потребители tiers сверены — все от состава пакета, он только оживает; regression `test_pack_survives_large_graph` (302 узла, размазанный засев: пакет непуст, multi-hop сосед в пакете, топ = 1.0), eval-демо 1.00 держится; семантика порога — 06/09/GUIDE + THEORY §6.3 (относительность как инвариант масштаба), HF-варнинг — GUIDE «Опциональные зависимости»; всё зеркально ru+en.
13. **Канонизация голых имён (§1.61).** `reconcile.py`: второй индекс `qualname → [ids]` в `_build_suffix_index`, ветка в `_repair_target` — голое имя (без `/` и `::`) чинится ТОЛЬКО при единственном кандидате; докстринг «heals on one bootstrap» уточнить; regression — случаи в `selftest_build` (page-объём: 106/169 полевых повисших должны лечиться свипом) и `selftest_reconcile`. — **выполнено (сессия 4):** `_TargetIndex` (NamedTuple: суффикс + `(категория, qualname) → [ids]`); голое имя пробуется ПОСЛЕ промаха path-suffix-ветки, точное совпадение qualname, единственный кандидат, и два гарда сверх буквы задачи — никогда для рёбер `imports` (`code:json` — легитимно висячий stdlib, а одноимённая функция проекта реальна) и никогда при имени, читающемся и как файл (узел с путём `…/<имя>` существует — два прочтения, нет починки); докстринги модуля/`_repair_target` уточнены; regression — `case_prefix_repair_sweep` (apply + свип: уникальное чинится, неоднозначное `f` и `imports`-`code:json` не трогаются, свип идемпотентен) и `selftest_build` (голое имя в стабе билдера). **Полевая валидация (сессия 5, копия графа третьей установки, 942 узла):** один bootstrap вылечил свипом **107 из 169** внутренних висячих (169→62; полевой прогноз — 106), включая оба примера §1.61 (`_resolve_extends_spec`, `_owns_function`). Разбор остатка (62): 34 голых имени — в основном имена МЕТОДОВ (`add_api_route` при реальном `RouterInterface.add_api_route`) и несуществующие символы, 28 — path-цели несуществующих символов (`hello.py::ping` при реальном `get_ping`); расширение «уникальный последний сегмент qualname» добрало бы лишь +12 (2 неоднозначны, 48 из 62 подлинно мертвы) — записано идеей на случай будущей боли, сознательно не реализовано (неверное ребро хуже висячего, выигрыш мал).
14. **Write воркерам и гигиена записи (§1.62).** `agents/amg-{builder,linker,synth,consolidator}.md`: `tools: … Write`; в промпты — «части пишутся инструментом Write (не heredoc)» и «валидация без перечитывания: `python -c "import json;json.load(open(...));print('ok')"`»; Write — только в `work/` (read-only на источники остаётся правилом). `selftest_install` не трогается (рендер тот же). **Решение по консолидатору (вопрос пользователя, принято):** он уже оптимизирован теми же принципами — вход одним файлом дала задача 2 (сводки кандидатов в плане, тела только финалистам), Write/анти-Read-back даёт эта задача; **чекпоинт-части для `actions.json` сознательно НЕ вводятся** — план персистентен и перезапуск судьи дешев (в отличие от билдера, чей батч-контекст дорог), консолидация выполняется раз в сессию десятками действий, а дробление усложнило бы `apply_actions` (и вернуло бы O(частей) ходов применения) ради страховки редкого и дешёвого обрыва; итоговая цена консолидации = цена судейства опус-класса над компактным планом, дальше без потери качества не сжимается. **Отклонено и «писать одним файлом вместо чекпоинт-частей» у билдера/линкера** (рекомендация установщика из поля): результат инструмента Write — короткое подтверждение, а не эхо содержимого; само содержимое части в любом случае живёт в контексте агента как ввод его же вызова — выигрыш от слияния частей исчисляется единицами тысяч токенов, а страховка обрыва теряется целиком. — **выполнено (сессия 4):** `tools: Read, Grep, Glob, Bash, Write` у builder/linker/synth/consolidator; в промптах — «части пишутся инструментом Write, не heredoc» (heredoc рвётся на кавычках/апострофах сводок), валидация без перечитывания `python -c "import json;json.load(open(...))"`, Write — только собственные артефакты (части в `work/`, gap-report, `actions.json`), остальное read-only; Codex-TOML-рендер не затронут (tools в TOML не переносятся), `selftest_install` зелёный.
15. **Полевые недоговорённости (§1.63).** Промпты линкера/билдера — явная конвенция направления item.id→to (с примерами `documents`/`exemplifies`); SKILL: прогресс волн линковки («X/Y пачек»), oversized-оговорка («чтение среза юнита без `text` законно всегда, волновые инструкции его не отменяют»), критерий полноты линковки («повторять `link_candidates` до нуля новых пачек либо `gate: ok`»), «выводы о движке AMG — пользователю, в граф проекта не писать»; двигательный гард — `partition_queue`/`link_candidates` при непустых `work/derived-*.json` печатают «apply-derived first» (+ то же поле `leftover_derived` в выводе `plan`). **Ещё два полевых среза (найдены повторным проходом стенограммы):** (а) каждый билдер читал `config.yml` (6.4 тыс. токенов + лишний ход) ради одного `working_language` — SKILL: оркестратор передаёт `working_language` прямо в задании спавна, промпт билдера/линкера: «язык передан в задании, `config.yml` не читать»; (б) метрика `doc_without_documents` шумит на дампах сессий (полевые 191 — в основном 22 ходовые секции стенограммы, у которых предмета законно нет): `graph_metrics` не считает doc-узлы, чей `source_path` лежит под каталогом сессий (`<store>/sessions/`), — гейт остаётся честным для настоящих документов. Смежно (проверено кодом, фикс не нужен): resume синтеза уже покрыт — чекпоинт-части + якорение к `existing_hubs` делают повторный synth идемпотентной доводкой. — **выполнено (сессия 4):** конвенция направления item.id→to с примерами — в промптах билдера и линкера (линкеру дополнительно: отношение в обратную сторону эмитится под id кандидата); SKILL — прогресс волн линковки («X/Y пачек» после каждой волны), oversized-оговорка, критерий полноты («повторять `link_candidates` до нуля новых пачек либо `gate: ok`»), «выводы о движке — пользователю, не в граф» (и в трёх блоках активации), `working_language` в задании спавна (bootstrap- и consolidate-SKILL; промпты воркеров: «config.yml не читать»); (а) закрыт; (б) `graph_metrics(…, session_prefix)` + хелпер `session_source_prefix` от общего `session_dir` — оба вызывающих (CLI `metrics`, `lifecycle.status`) передают префикс, doc-узлы сессионных дампов исключены из `doc_without_documents`; двигательный гард — `warn_leftover_derived` (partition_queue, импорт в link_candidates: stderr-предупреждение + поле в JSON-выводе CLI) и поле `leftover_derived` в `plan`; regression в `selftest_build`/`selftest_queue`.
16. **Безопасное укрупнение партий — крупнейший оставшийся рычаг БЕЗ качественного риска (нумерация: выполнять с группой E).** Ключевое наблюдение третьего прохода: фикс-оверхед агентной среды (системный промпт + схемы инструментов, ≈40–70 тыс. токенов, переотправляется на КАЖДОМ шаге КАЖДОГО агента) масштабируется числом агентов, а не объёмом работы — и качество сводок/подтверждений от размера партии НЕ зависит (те же юниты, те же кандидаты, те же решения; страховку обрыва держат чекпоинт-части по ~10, они остаются). Поднять дефолты: `builder.batch_units` 40→60 (23→~15 агентов), `linker.batch_nodes` 40→80 (24→12 агентов; вход-ресенд на агента ×2, но агентов вдвое меньше — вход суммарно тот же, фикс-оверхед и cache-write падают вдвое); один прогон на стенде подтверждает лишь, что вывод агента не упирается в потолок ответа (подтверждений на пачку ×2 ≈ 8–10 тыс. токенов — запас велик). SKILL: волна = 5–10 агентов одним сообщением (меньше волн — меньше ходов оркестратора). Ожидание: −30–40 % субагентной стоимости и cache-write. Это НЕ задача 11 (там качественные размены под eval) — здесь инвариант качества по построению. — **выполнено (сессия 4):** дефолты подняты в коде (`BUILDER_DEFAULTS`/`LINKER_DEFAULTS`) и шаблоне `config.yml` синхронно (случай «код ≠ шаблон» не заводился — оба были равны и остались равны), обоснование размера — комментариями в коде/шаблоне и в 09-config (ru+en); SKILL и 08 — «волна = 5–10 агентов одним сообщением»; стендовое подтверждение запаса потолка вывода — группа D.
17. **Серии вопросов установщика (§1.65).** INSTALL.md/INSTALL_RU: вопросы задаются фиксированными сериями по ≤4 (лимит инструмента опроса среды): I — scope+среда; II — источники после осмотра (зеркала И absorb/absorb_once И исключения; absorb обязателен даже при пустом предложении); III — KEY: язык+эмбеддинги; IV — автоматика/сессии/бюджеты/зависимости/активация+`--build`; уже настроенные не-дефолтные значения существующего конфига не переспрашивать (показать `in force:`). — **выполнено (сессия 4):** серии I (1–2), II (3–5, absorb обязателен), III (6–7 KEY), IV (8–11) + завершающий вопрос 12 отдельно — активация с `--build` внутри вариантов ответа (серия из пяти нарушила бы лимит ≤4); правило «не-дефолтное из конфига не переспрашивать, показать `in force:`» вписано; зеркально INSTALL.md/INSTALL_RU. Дополнено по ревью пользователя: формулировка серий средо-честная — «≤4 за вызов» назван фактом инструмента опроса Claude Code, а в прочих средах те же серии сохраняются намеренно (тематические партии против сворачивания потока в одно сообщение), не из-за лимита.
18. **Доки жизненного цикла и стоимости (вопросы пользователя 2–4 третьей установки; зеркально ru+en).** READMEs: в «Режимы и управление» — компактная таблица «кто что запускает и когда» (sync = сборка/сверка: первый раз полная, дальше инкрементальная, запускается петлёй в начале сессии при изменённых источниках или командой — хук её НЕ запускает; консолидация: детерминированная половина — хук SessionEnd, судейская — петлёй в конце работы или `/amg consolidate`; в средах без хуков — всё петлёй/командами при `automation: true`); в «Сохранении сессий» — порядок «дамп пишется хуком SessionEnd → узлами становится при СЛЕДУЮЩЕМ sync → консолидация потом отбирает» и честно про жёсткий килл (дампа не будет — страховка заметки; сессия, закрытая штатно перед лимитом, дамп получает); в «Первый запуск и сборка» — абзац «Стоимость первой сборки»: дорого и упирается в лимиты тарифа — рычаги: (а) вести сессию сборки на более дешёвой модели среды — оркестратор только координирует, 66 % расхода третьей установки пришлись на главную сессию; (б) тиринг `models.*` (какие ключи); (в) `builder.batch_units` 40→60 / `batch_max_chars` — что это, и почему дальше растить нет смысла (оверхед уже съеден, а потеря пачки при обрыве дорожает); (г) `derivation: lazy` как альтернатива — быстрее приступить, но достройка деталей размазывается по сессиям. GUIDE — то же глубже («Активация и режимы»/«Что происходит автоматически»/«Качество и стоимость сборки»). Блоки активации ×3 — исключение к протоколу петли: «если промпт сам даёт точные входные точки (файл/строка/готовый фикс), ритуальный пакет не нужен — граф для входа в незнакомое» (полевое наблюдение тестировщика: на разжёванной задаче петля толкает к пустому ритуалу, на поведенчески-заданной граф дал настоящий рычаг). — **выполнено (сессия 4, зеркально ru+en):** READMEs — таблица «кто что запускает и когда» в «Режимах и управлении» (sync — петлёй/командой, не хуком; консолидация двумя половинами; бесхуковые среды — всё петлёй), трёхшаговый путь диалога в память + честный абзац о жёстком килле в «Сохранении сессий», «Стоимость первой сборки» с четырьмя рычагами в «Первом запуске»; GUIDE — «Во сколько обходится полная сборка — и где проходит её пол» (устранённая повторная работа против пошагового ресенда среды — область действия многократного удешевления; две метрики панели — некэшевые и cache-read; рычаги детально; закрывает и разъяснение из задачи 10), «сборку запускает петля, не хук» — явно; блоки ×3 — исключение внесено с оговоркой «протокол возобновляется, как только задача выходит за данное в промпте». Дополнено по ревью пользователя (та же сессия, зеркально ru+en): READMEs, установка п.3 — все три роли `models` в примере, «`model` — любая строка, которую примет среда (закреплённый id / модель другого провайдера при шлюзе/Bedrock/Vertex; AMG маршрутизацию не берёт)», совет «поток спрашивает только важное — полный контроль = заранее отредактировать шаблон `<amg-dir>/config.yml` (write_config пишет проектный конфиг из шаблона; сверено по install.py) либо править конфиг после установки»; таблица «кто что запускает» — судейская консолидация привязана к концу СЕССИИ (смена фокуса её не запускает, там только новый retrieve) + абзац о лимитах (хук — скрипт без модели, лимит его не останавливает: дамп/веса/дайджест пишутся и перед самым лимитом; судейская при исчерпанном лимите не выполняется и неразрушительно откладывается до `/amg consolidate`); новый контраст-абзац **`sync` ↔ `consolidate`** («граф ↔ файлы» против «граф ↔ собственный опыт»; правка содержимого файла — целиком забота sync, включая пересводку и связи; друг друга не вызывают) — READMEs «Режимы и управление» и GUIDE «Активация и режимы», лимиты — и в GUIDE «Что происходит автоматически». Второй заход ревью (та же сессия, зеркально ru+en): READMEs — абзац «Обновление движка — без потери графа» в конце «Установки» (распаковка вне проекта как при первой установке → одна команда или просьба модели → движок обновлён, граф и конфиг нетронуты, `in force:` → перезапуск → `/amg sync` дёшево доводит) и лёгкий пиар-абзац самообслуживания памяти в «Что это» (сверка узлов/связей, самовосстановление, сохранение сессий, консолидация — участие пользователя не требуется); GUIDE — подраздел переименован в «Переустановка, обновление движка и удаление» и расширен точной механикой (`write_config`: существующий конфиг возвращается нетронутым безусловно, `--set` к нему не применяется, «начисто» = удалить конфиг; `models` ре-рендерится из конфига пользователя).

**Группа F — полевые фиксы четвёртой досборки (§1.66–§1.69; решение пользователя 2026-07-12: досборка на стенде и есть полевой прогон, всё закрывается в Этапе 22; выполнять следующей dev-сессией, затем закрывающая):**

19 (A). **Память рассуженных пар линковки (§1.66).** Дизайн-развилка описана в пункте аудита (варианты а/б/в; предпочтителен (в) — «старые `link-batch-*.json` считаются рассужеными и уезжают в `work/judged/`», но с аккуратным resume: перемещение только после успешного apply частей пачки); SKILL — переформулировать критерий полноты; regression в `selftest_build` (повторная номинация после apply не переноминирует рассуженное; упавшая пачка — переноминирует). — **выполнено (сессия 7): гибрид (б)+(в)** — каждая чекпоинт-часть линкера заканчивается служебным элементом `{"judged": [id, …]}` (все рассуженные узлы части, включая «ничего не подтвердил»); `apply-derived` собирает записи по пачкам и переносит ПОЛНОСТЬЮ покрытую `link-batch-<n>.json` в `work/judged/` (`_pop_judged`/`_retire_judged_batches`; покрытие доказывается, не предполагается — недокрытая/рваная пачка остаётся и честно переноминируется); свод покрытия из `work/applied/` прошлых вызовов сознательно НЕ делается — метки пачек позиционные, архивная часть могла бы ложно закрыть перенумерованную пачку новой волны (молчаливая потеря номинаций хуже пере-суждения хвоста); `link_candidates._judged_pairs` читает `work/judged/` и объединяет с `_linked_pairs` — «повторять до нуля пачек» теперь сходится; переоткрытие суждений = удалить файлы из `work/judged/`; служебный элемент, дошедший до одиночного `apply`, отбрасывается молча (не брак); regression в `selftest_build` (полное покрытие уводит пачку, частичное — переноминирует).
20 (B). **Честный `_toklen` (§1.67).** Смешанная оценка ASCII/не-ASCII без новых зависимостей; сверить потребителей; `selftest_retrieve` — случай «кириллический пак не превышает бюджет в токенах» (оценкой той же формулы); README/GUIDE/09-config — упоминание, что бюджеты считаются в токенах честно для любого языка. — **выполнено (сессия 7):** трёхдиапазонная оценка — ASCII/4, прочие алфавиты/2.2, CJK/1.5 (диапазоны U+2E80–9FFF, кана, хангыль, совместимость; чистый ASCII сохраняет быстрый путь `len//4`); калибровка по реальному полевому паку (167 тыс. знаков, 45 % кириллицы): старая оценка 41.7k токенов, новая 57.2k — совпала с полевой арифметикой «55–66k»; оба потребителя (`retrieve._toklen` — сборка пакета, `consolidate._toklen` — бюджеты веток) зеркально; измеренный эффект на стенде: средний пак на 11 кейсах ужался с ~244 КБ (полевой) до ~130 КБ при тех же бюджетах — токенные бюджеты держатся честно; regression `test_toklen_scripts` в `selftest_retrieve`.
21 (C). **Петля: дифф-вопрос вместо слепого авто-sync (§1.69) + исключение хранилища из сплошного поиска (§1.68).** Блоки ×3: старт сессии = `reconcile.py plan` (дёшево, ноль модели) → при ненулевом диффе вопрос пользователю (синхронизировать/отложить + напоминание), при нулевом — молчание; та же стартовая рутина смотрит на возраст судейской консолидации (`/amg status` → `last consolidation`; остаточные `work/consolidation-plan.json`/`actions.json` без следа apply в `actions.log`) и при отставании ПРЕДУПРЕЖДАЕТ одной строкой: «судейская консолидация с прошлых сессий не выполнялась — сделаю в конце этой, либо запустите `/amg consolidate`» (автозапуск в начале сессии сознательно НЕ делать — дорого; детерминированную половину хук и так закрыл); правило сплошного поиска дополняется «исключай каталог агента»; GUIDE/README — зеркально («Что происходит автоматически», таблица «кто что запускает»). Сюда же мелкая промпт-оптимизация ходов воркеров: валидация записанных частей НЕ по одной (отдельный Bash-ход на часть), а одним вызовом в конце на все части — apply и так карантинит рваное, финальная сверка счётчиков остаётся. И в конвенцию целей (промпты билдера/линкера): doc-цель всегда именуется с конкретной секцией `::slug` — файлового doc-узла не существует, цель на весь файл заведомо висяча (полевые «file-level doc: targets» установщика). **Уточнения реализации (сессия 7, по коду):** `reconcile.py plan` — не read-only дифф, а полная детерминированная сверка с записью скелета (CLI `plan` ≡ `bootstrap`) — для стартовой рутины это принято и намеренно: бесплатная детерминированная половина sync выполняется автоматически (как хуки), вопрос пользователю касается только модельной половины, оценка объёма = `queued_for_semantic`; повтор вопроса в следующей сессии держится на ненулевом `queued_for_semantic`, а не на файловом диффе (скелет уже записан). Стартовая рутина — ОДИН Bash-вызов (plan и status цепочкой), не два хода. Движковая половина предупреждения: `lifecycle.status` различает судейскую консолидацию (`consolidation applied`) и свёртку весов (`weights folded`) отдельными полями и сообщает остаточные `plan`/`actions`; `lifecycle.session-start` печатает одну строку-предупреждение, когда после последней судейской накопилось ≥3 свёрток весов (гистерезис: одна строка на старт сессии, гаснет после консолидации; в бесхуковых средах ту же строку даёт status в стартовой рутине). — **выполнено (сессия 7):** движок — `lifecycle._consolidation_state` (арифметика по actions.log: `weights folded` против `consolidation applied` + остаточные plan/actions новее последней судейской), `_consolidation_note` (порог `_JUDGED_LAG_WARN = 3` свёртки; единая формулировка для всех сред), поля в `status` (`last_judged_consolidation`, `weight_folds_since_judged`, `consolidation_leftover`, `consolidation_note`) и строка в `session-start`; блоки ×3 — стартовая рутина (CLAUDE.md: hook-строка + `plan`; AGENTS/Codex: recover+verify+plan+status минимальным числом вызовов, без `&&` — оболочко-нейтрально) с вопросом пользователю при ненулевом `queued_for_semantic` и молчанием при нуле; исключение каталога агента из сплошного поиска — во всех трёх; валидация частей одним вызовом в конце — в промптах билдера/линкера/synth; конвенция «doc-цель всегда с `::slug`» — в промптах билдера и линкера; regression `case_consolidation_nudge` в `selftest_lifecycle`.
22 (D). **Зарядить eval-харнесс на стенде и перемерить Хебб на реальном usage.log.** Разметить ~10 реальных запросов по стенду (gold_ids из `inspect_graph.py`), `cases.json` вне каталога движка, `eval_gate.cases` → на него; `usage.log` уже копится (3 записи после досборки) — по мере накопления прогнать `fold_weights` off/on на КОПИИ store и снять recall/hop-recall; решение о флипе `apply_hebbian` — только при устойчивом выигрыше (иначе зафиксировать «остаётся false» с числами). **Дополнение (сессия 7):** после 15 рабочих сессий usage.log вырос до 11 записей (1–60 узлов на запись) — субстрат достаточен, вердикт снимается без ожидания; сценарий замера = «флип сегодня» (fold потребляет накопленный usage.log разом; coactivation.log пуст — он ротируется каждой свёрткой, залпа по нему не будет; реплей архивных коактиваций — только как проверка чувствительности decay-половины). Смежный дефект по месту: дефолт и шаблонное значение `eval_gate.cases` указывали в каталог движка (`skills/amg-retrieve/evals/`) — переустановка затирает разметку пользователя; перенесено под store (`<store>/evals/cases.json`), путь в старых конфигах безопасен (нерезолвящиеся кейсы → гейт скипает). На стенде при закладке cases поднять капы 40/40 → 60/80 (шаблонные умолчания до старых конфигов не доезжают). — **выполнено (сессия 7): харнесс заряжен, Хебб перемерен на реальном usage.log — дефолт остаётся `false`, с числами.** Разметка: 11 кейсов / 60 gold-id по сводкам графа (все id валидированы), `cases.json` доставлен на стенд (`<store>/evals/`), капы подняты 60/80, `eval_gate.cases` переключён. Baseline (копия стенда, 1130 узлов, эмбеддинги off): pack-recall **1.00**, hop-recall **0.22 против 0.00 у лексического top-K** (multi-hop-преимущество подтверждено на живом графе), recall@K 0.30 (лексический 0.37 — на месте gold сидят хабы). Хебб «флип сегодня» (fold всего накопленного usage.log: 11 записей, 4748 co-used пар, 541 ребро усилено, 986 узлов переписано): все дельты recall/hop/pack-recall = **0.0000**; чувствительный вариант с реплеем архивных коактиваций (3352 ребра погашено decay) — тоже 0.0000. Диагноз совпадает с THEORY §8.2 (режим плотного графа: исход решает засев, веса инертны): включение на этом графе безвредно, но и бесполезно — дефолт `false` остаётся, флип задокументирован как выбор для разреженных multi-hop-графов. Попутная поправка суждения тестировщика: coactivation.log потребляется и ротируется КАЖДОЙ свёрткой (12 архивов на стенде) — «обнулить перед флипом» не нужно; разовый залп при флипе даёт только usage.log. **Дозамер (сессия 7, по вопросу пользователя): Хебб перемерен и с ТРАНСФОРМЕРНЫМ засевом** (sentence-transformers multilingual — реальный режим стенда, blend 0.5): дельты снова ровно 0.0000 по всем метрикам (pack-recall 1.0) — вердикт полный, данных поля достаточно (541 ребро реально усилено, эффект нулевой в обоих режимах засева). Попутные наблюдения для будущих замеров: recall@K при ST-засеве на кириллических кейсах НИЖЕ лексического (0.12 против 0.30 при pack-recall 1.0 у обоих — мультиязычная MiniLM размывает засев) — кандидат на замер blend/модели при следующей полной сборке; глубина gold в ранжировании (worst-rank по 11 кейсам): [32, 69, 70, 96, 111, 113, 146, 166, 172, 180, 222] из ~600 ранжированных — вход для компактного профиля (задача 29).
23 (E). **Адаптивная сборка пакета — правило остановки поверх бюджета-предохранителя (только под заряженный харнесс задачи D).** Кандидат: добор в ярусы прекращается по накопленной/маргинальной массе активаций (безразмерно; фокусный запрос останавливается сам), плюс MMR-дедуп почти-дублей по жаккаровой близости сводок (переиспользовать `near_duplicate_sim`-машину) — бюджеты остаются жёстким потолком, `activation_threshold` — нижним floor. Вводить ТОЛЬКО при измеренном удержании recall/hop-recall и заметном сокращении пака; иначе зафиксировать отказ с числами. (Идеи knee-детекции отклонить: хрупка на ступенчатых кривых и дублирует mass-критерий.) — **решено (сессия 7): ОТКЛОНЕНО с числами.** Прототип обоих гардов реализован и измерен на 11 кейсах стенда: масс-стоп 0.9 ужал пак в **×1.01** (масса PPR фундаментально размазана — сумма 1 по ВСЕМУ графу, хвост из сотен узлов держит большую долю, префикс ранжирования не концентрируется), Jaccard-дедуп 0.82 не сдвинул ничего измеримого (курирование почти-дублей — дело консолидации); порог как рычаг: 0.05/0.10 дают −1 %/−15 % объёма при recall 1.00, 0.15 уже теряет gold (pack-recall 0.982) — дефолт 0.02 остаётся. Реальные измеренные рычаги размера: честный `_toklen` (задача 20 — −47 % на неанглийском паке при тех же бюджетах) и сами ярусные бюджеты (связывающее ограничение; пользовательский рычаг, документирован). Машинерия из кода убрана, отказ зафиксирован в докстринге `assemble_pack`; вернуться только при живой боли с новыми числами.

**Группа G — полевые вопросы пятнадцати рабочих сессий (§1.70–§1.74; решение пользователя 2026-07-13: закрываются в Этапе 22 вместе с группой F, та же dev-сессия):**

24. **Offline-first загрузка эмбеддинг-модели (§1.70).** `embed.py`: `_SentenceTransformers` пробует `local_files_only=True`, при любом исключении — обычная загрузка (первое скачивание живо; старые версии ST без параметра — тоже); для model2vec — тот же паттерн, если сигнатура установленной версии позволяет (проверить, не гадать). Доки (зеркально): READMEs «Опциональные зависимости» + GUIDE — латентность transformer-бэкенда на каждый процесс, `HF_HUB_OFFLINE=1`, рекомендация model2vec при чувствительности к латентности. — **выполнено (сессия 7, код):** `_SentenceTransformers` — `local_files_only=True` с откатом на обычную загрузку при любом исключении (первое скачивание живо; старые версии ST без параметра — тоже); `_Model2Vec` — `force_download=False` с откатом по TypeError (сигнатура установленной версии сверена: параметр есть, умолчание True); доки внесены зеркально (READMEs «Опциональные зависимости», GUIDE «Скорость: transformer-бэкенд…»: латентность на процесс, `HF_HUB_OFFLINE=1`, рекомендация model2vec).
25. **Двухуровневое обращение к памяти (§1.71).** SKILL amg-retrieve: workflow «по умолчанию — прямой вызов `retrieve.py` (напечатанный пакет = рабочий контекст; `--intent` ставишь сам); субагент amg-retriever — когда пакет в основном окне не нужен (вопрос-выжимка к памяти; перегруженный контекст)»; блоки CLAUDE.md/Codex — то же в пункте петли; `amg-retriever.md` — роль сужена до изоляции. GUIDE/READMEs зеркально. — **выполнено (сессия 7, код+промпты):** SKILL amg-retrieve — workflow переписан (шаг 2: прямой вызов = дефолт, напечатанный пакет = рабочий контекст, `--intent` ставит сама модель; спавн ретривера — явное исключение изоляции с назваными критериями); блоки CLAUDE.md и Codex несут команду прямого вызова в пункте петли; `amg-retriever.md` — описание роли сужено до изолированной выжимки; GUIDE («Перед каждой задачей…», раздел скилла в 08) и блоки — зеркально.
26. **Триггер судейской консолидации и её документация (§1.72).** Блоки ×3: «пользователь сигналит о завершении работы → судейская консолидация при `automation: true` (или предложить)»; реакция на старт-предупреждение 21/C. READMEs «Режимы и управление»: три фазы, «гигиена, не сохранение», ритм, как позвать словами; честная строка судейской в таблице «кто что запускает». GUIDE — глубже. — **выполнено (сессия 7, блоки):** все три блока активации — «конец сессии невидим, пока пользователь не подал сигнал»: сигнал завершения работы (на любом языке, по смыслу) = запуск судейской консолидации (в бесхуковых средах — обеих половин), плюс предложение при насыщенной сессии и при старт-предупреждении; READMEs/GUIDE внесены зеркально (таблица «кто что запускает», абзац «Консолидация — гигиена, а не сохранение»: три фазы, ритм, словесный вызов). Ревью пользователя (та же сессия): абзац о фазах переписан его формулировками (weights: «усиливает те, у которых оба конца реально пригодились… гасит всплывшие, но не использованные, прореживает совсем слабые», с оговоркой про apply_hebbian; plan: «считает переполнение бюджетов веток, кандидатов в почти-дубликаты, значимость заметок и отдаёт план субагенту»), убраны «покупает остроту»/«пухнет»/«Разумный ритм»/«закругляемся»/«сигналите» и лишняя скобка в таблице — зеркально в README_RU/GUIDE.ru, по смыслу в README/GUIDE.en.
27. **Правило отсутствия и точный путь verify_claims (§1.73).** `amg-retriever.md`: правило отсутствия («прямого узла нет → сказать явно; соседи — не ответ»); полный путь `.claude/skills/amg-retrieve/scripts/verify_claims.py` в промпте ретривера и в блоке CLAUDE.md. — **выполнено (сессия 7):** правило отсутствия в `amg-retriever.md` («память не держит ничего о X само по себе» — первым; соседи подаются как соседи); полный отрендеренный путь verify_claims — в промпте ретривера и блоке CLAUDE.md.
28. **`verify_claims --all --write` в закрытии sync (§1.74).** Свип-режим в CLI уже есть (`--all|--code` без id) — добавить его вызов закрывающим шагом SKILL amg-bootstrap после гейта связности; GUIDE/09-config — семантика unverified после конвейерного прогона. — **выполнено (сессия 7, SKILL):** шаг 7 переименован «Acceptance gate + verification stamp»: после гейта — `verify_claims.py --all --write --store` одним детерминированным свипом (свежевыведенная сводка совпадает с источником по построению → verified; флаг снова различает «изменилось с последней сверки»); GUIDE («После сборки верификация штампуется одним свипом») и 08 (шаг 7) внесены зеркально.
29. **Компактный профиль пакета `--compact` (§1.75).** Дизайн и приёмка — в пункте аудита (флаг у retrieve.py: скромные бюджеты + указатели без тел; SKILL/блоки: «точечный запрос → `--compact`»); worst-rank-данные для калибровки уже сняты (отметка задачи 22). Доки зеркально: 06/09, GUIDE («Что происходит автоматически» + «Тонкая настройка»), READMEs — одной фразой в петле. — **выполнено (сессия 8):** `_COMPACT_BUDGET = dict(DEFAULTS["token_budget"])` — буквально код-дефолты 1200/2500/6000/40, ключ `token_budget` конфига не читается; параметр `compact` у `retrieve()`/`assemble_pack()`/`_render()`, CLI-флаг, заголовок пакета `(compact)`; в operational файловый узел любого типа рендерится указательной строкой `путь:строка — имя — сводка`, узел без источника — строкой id, `decision`/`adr` сохраняют тело в любом ярусе. Замер на копии стенда (11 кейсов): compact **×3.1 меньше** (128 → 42 КБ; 45.4k → 14.8k оценочных токенов), pack-recall с периферией **0.918** при лексическом засеве (55/60 gold, худший кейс 0.67) и **0.806** при ST (48/60, худший 0.50) — теряется ровно глубокое многошаговое gold за периферией-40, потому вход в незнакомое остаётся полному профилю; eval-демо 1.00 держится, полный профиль не изменён; regression `test_compact_profile` (указатели вместо тел, тело решений, независимость от конфиг-бюджетов). Доки зеркально: 06 (подраздел «Компактный профиль» + CLI + сноска у token_budget), 09, GUIDE, READMEs, THEORY §7 (профиль — объявленное намерение вызывающего: скаляр активации не несёт различия «указатель/вход в незнакомое»); SKILL и блоки ×3 несут правило выбора.
30. **Gated-напоминание о памяти по ходу сессии (§1.76).** `lifecycle.py prompt-hint` + хук UserPromptSubmit (только claude-code; условия срабатывания и приёмка — в пункте аудита); фиксация в 08 («Слой жизненного цикла») и GUIDE; в блоках Codex/generic — честная оговорка об отсутствии механизма. — **выполнено (сессия 8):** `lifecycle.prompt_hint()` — четыре гейта буквой §1.76 (active+automation; промпт ≥200 знаков; кулдаун ≥10 мин через `work/hint-stamp`; `work/pack-log.jsonl` отсутствует или старше 15 мин — журнал сессионный, потребляется session-end, потому его наличие и возраст означают «опрашивались в этой сессии и когда»); CLI `prompt-hint` от stdin-пейлоада, одна строка в stdout или молчание с нулевой ценой; хук `UserPromptSubmit` (timeout 30) в шаблоне settings.json — merge/uninstall установщика уже событие-агностичны, правок install.py не потребовалось; regression `case_prompt_hint` (все гейты; тихий промпт молчит) + рендер хука в `selftest_install`; CLI-тракт проверен живьём (строка → кулдаун-молчание → короткий промпт молчит). Блок CLAUDE.md атрибутирует строку самой петле («не текст пользователя»), Codex/generic несут оговорку; доки зеркально: 08 (третий хук с гейтами), 12-install (merge по всем событиям), GUIDE, THEORY §13 (затухание середины сессии; гейт отличает сигнал от пер-промптного шума).

Definition of done:

- конвейер-фикстура: один apply-вызов на раунд применяет все части с агрегированным итогом; повторный `apply-derived` — no-op; у synth один входной файл, план консолидатора несёт сводки кандидатов; обрыв линковщика теряет ≤ последней части (regression зелёные);
- полевые дефекты закрыты: пакет непуст на большом размазанном графе (§1.64 — relative-порог, eval-демо держится), голые `code:<qualname>`-цели канонизируются при единственном кандидате (§1.61 — свип лечит полевые 106), воркеры пишут части инструментом Write без heredoc-срывов (§1.62), промпты несут конвенцию направления/oversized/критерий полноты линковки (§1.63), установщик спрашивает absorb обязательной серией (§1.65);
- блоки активации несут императивный протокол (декомпозиция, повторный retrieve, «граф раньше сплошного поиска», видимость); INSTALL/READMEs/GUIDE — шаг перезапуска и акцент «sync = сборка»;
- живой прогон: `gate: ok` и измеренное снижение стоимости против третьей установки по двум метрикам (некэшевые ≤ ~3 М; cache-read ≤ ~15 М чистого прогона; время — десятки минут); вердикт по «×20–30» с разъяснением его области действия (повторная работа, не ресенд среды) зафиксирован в этапе и GUIDE; — **закрыто решением пользователя (2026-07-12)**: качественная половина — сессией 5 на копии графа, стоимостной вердикт — данными четвёртой досборки (разложение по корням в задаче 10; пороги — ориентир ближайшей полной сборки, не блокер); остаточные полевые дефекты досборки — группа F;
- селфтесты зелёные; закрыты §1.55–§1.60 (группами A–C), §1.61–§1.65 (группой E), §1.66–§1.69 (группой F), §1.70–§1.74 (группой G) и §1.75–§1.76 (задачи 29–30 — сессия после ревью); правка каждого документа выполнена зеркально в `docs/ru` и `docs/en` (§3.5).

Закрытие (доки, всё зеркально ru+en): `08-agents-skills.md` (основной — бюджет ходов, synth-input, чекпоинты линковщика, отчётность), `05-reconcile.md` (`apply-derived`, `work/applied/`), `06-retrieval.md` + GUIDE (петля использования и её проверяемость), `09-config.md` (новые ключи, если появятся), INSTALL.md/INSTALL_RU + READMEs (перезапуск, sync-акцент), GUIDE («Качество и стоимость сборки» — статьи стоимости; вьюер/сессии/консолидация — задача 9), THEORY §4.4 (дополнение «почему»: ресенд-экономика распространяется на оркестратора и глобальные пассы — число ходов есть та же валюта, что объём данных; вход глобального пасса готовится детерминированно). — **Синхронизация выполнена (сессия 4, до живой приёмки; зеркально ru+en):** 05 (пакетный `apply`+`apply-derived`/`applied`/`invalid`, двухступенчатая канонизация с голыми именами, `leftover_derived`, session-фильтр метрики), 06 (относительный порог — механика и таблица ключей), 08 (бюджет ходов и волны во вводке и шагах 3–6, synth-input в шаге 5 и § synth, чекпоинты линкера, Write-дисциплина воркеров абзацем под таблицей, план-как-рабочий-слой консолидатора, конвенция направления), 09 (порог, капы 60/80 с рационале), THEORY §4.4 (новый абзац: арифметика переотправки правит ходами оркестратора и глобальными пассами; охват — подготовкой, не накоплением) + §6.3 (порог как доля — инвариант масштаба). Новых конфиг-ключей этап не завёл. Остаток закрытия — после группы F: свёртка этапа (доки группы F — при её реализации), релиз. — **Доки групп F и G синхронизированы в сессии 7 (зеркально ru+en):** 05 (память рассуженных пар в «Пакетной двери»), 06 (пооценочный `_toklen` + отказ от масс-стопа с обоснованием), 08 (сходимость линковки + judged-записи, двухуровневое извлечение, стартовая проверка, напоминание в хуке и status, штамп верификации, правило отсутствия у ретривера), 09 (`eval_gate.cases` под store, честные токенные бюджеты), GUIDE/READMEs (стартовый вопрос, триггер завершения, гигиена консолидации, латентность эмбеддингов, двухрежимный Хебб с полевыми числами), THEORY §4.2/§7/§8.2; валидатор ссылок чист. Осталось: только свёртка (§5-тело; §1.55–§1.74 → однострочники) + релиз v1.13.0 + en-снимок roadmap.

---

## Этап 23 — тестирование в не-Claude-Code-средах
...
1. Установить AMG в реальном Codex-проекте штатным Codex-режимом (`--env codex`; каталоги `.agents`/`.codex` по умолчанию среды) и пройти полный цикл: активация → bootstrap со скиллами и TOML-субагентами → apply → retrieve → notes → consolidate; петля без хуков — дисциплиной модели по блоку `AGENTS.md`. Сверить с накопленными фактами о среде (баг #44385-класса, кламп `model_reasoning_effort`). Попутно сверить стоимость полной сборки с ориентиром задачи 10 Этапа 22 (некэшевые ≤ ~3 М, cache-read ≤ ~15 М чистого прогона на проект ~1 МБ) — сверка попутная, не блокер.
   1a. **Сверить средовую механику отображения и регистрации (перенос выводов Этапа 22 на Codex).** Как Codex показывает работу субагентов и их завершение (есть ли аналог безусловной метки «Done» поверх упавшего агента — как в Claude Code, где метка завершения инструмента не отражает успех); виден ли пользователю промежуточный вывод агента (если да — перенести приём `progress: N/M`-строк из задачи 8 Этапа 22); подхватывает ли Codex свежесозданные скиллы/TOML-субагентов на лету или, как Claude Code, только при старте сессии (совет INSTALL о перезапуске уже средо-нейтрален — здесь лишь подтвердить/уточнить его для Codex). Протоколы Этапа 22 (честная финальная строка, сверка счётчиков, отчётность оркестратора, императивная петля) уже лежат в общих промптах/SKILL и скилл-осведомлённом блоке — здесь проверить их живьём; различия зафиксировать в `08-agents-skills.md` («Переносимость управляющего слоя») и GUIDE.
...
24. **Этап 23 — тестирование в не-Claude-Code-средах (Codex и др.).**
