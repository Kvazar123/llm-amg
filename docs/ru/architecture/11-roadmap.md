...
9. Optional true absorb freeze:
   - `absorb_once`;
   - или `absorb_freeze`.
10. Валидировать пересечение `mirror_path`/`absorb_path` (см. 1.29): файл под обоими корнями обходится дважды, дедуп по id в `plan()` молча оставляет политику последнего источника — предупреждать в `plan`/`--stats` и документировать приоритет политик. Затрагивает `extract_structure.py`/`reconcile.py` (соседи нарезателей).
11. Явная диагностика несуществующего пути источника (остаток 1.30): `by_source` в `--stats` уже показывает счётчики по источникам (Этап 10); осталось вывести «путь не найден» и в `plan`, а не только в `--stats`.
