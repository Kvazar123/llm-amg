...
1. Установить AMG в реальном Codex-проекте штатным Codex-режимом (`--env codex`; каталоги `.agents`/`.codex` по умолчанию среды) и пройти полный цикл: активация → bootstrap со скиллами и TOML-субагентами → apply → retrieve → notes → consolidate; петля без хуков — дисциплиной модели по блоку `AGENTS.md`. Сверить с накопленными фактами о среде (баг #44385-класса, кламп `model_reasoning_effort`).
   ...
   1b. **Снять перенесённую стоимостную приёмку Этапа 22 на первой живой сборке этапа** (решение пользователя 2026-07-11): некэшевые токены ≤ ~3 М и cache-read ≤ ~15 М чистого прогона на проект ~1 МБ, время — десятки минут; подтвердить запас потолка вывода при партиях 60/80 (builder/linker). Если первая сборка идёт в Claude Code — сравнение с полами задачи 10 Этапа 22 прямое; в иной среде — зафиксировать её числа как собственный базлайн этой среды (пол пошагового ресенда у каждой среды свой); механика экономии от среды не зависит.
