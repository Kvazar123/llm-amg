...
Обнаружено аудитом; три конкретных дрейфа. Пункты 2–3 **исправлены на Этапе 18**: `09-config.md` показывает `schema_version: 3` (как шаблон, поднятый Этапом 13); INSTALL.md описывает `--env codex` (в флагах и «Других средах»). Остаётся один дрейф:
