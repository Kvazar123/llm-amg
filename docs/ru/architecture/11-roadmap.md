...
**Прогресс (Группа 1 — задача 8 выполнена).** Правило в `consolidate.fold_weights` переписано: **outcome-gated reward** (ребро усиливается, только если оба конца co-used в принятой сессии — из `work/usage.log`) **различающим headroom** `Δ = hebbian_rate·(1−w)` (уже-сильное растёт еле-еле) + **exposure-gated decay** (ребро, показанное в пакете через `coactivation.log`, но не использованное, гаснет на `decay_rate` — демотирует «магистрали»); поддержан и негативный исход (`reverted` → `−rate·w`, авто-детекция отката отложена). `coactivation.log` больше НЕ усиливает (только копит `coact` для значимости + даёт сигнал экспозиции); `usage.log` потребляется (ротация в `archive/`) только при `apply_hebbian:on`, иначе копится. `lifecycle.session_end` переставлен `dump → record_usage → fold → digest`. **Измерение (`../amg-bigtest/_measure_big.py`, синтетический исход gold-as-used):** новое правило МОНОТОННО **поднимает** recall/hop по всем трём засевам и всем фолдам 1→2→4→8 (BM25 rec 0.60→0.83, hop 0.25→0.68; m2v 0.60→0.85, 0.31→0.72; ST 0.58→0.85, 0.22→0.72) — инверсия слепого правила (−0.10/−0.14). **Dogfood `../amg-testbed` (33-узловой плотный):** не вредит — BM25 инертно, ST +0.03/+0.04, pack 1.0. **Решение по дефолту: `apply_hebbian` остаётся `false`.** Обоснование: оба замера — на СИНТЕТИЧЕСКОМ сигнале исхода (gold-as-used), что доказывает корректность и пользу НА ГРАФЕ, но не среднюю пользу при РЕАЛЬНОМ `usage.log` (база-промпт: «синтетика проверяет лишь корректность»); свёртка весов вне eval-предохранителя. Правило безопасно к включению (no-op без `usage.log`; outcome-gated, не слепое) — флип ждёт реального usage от dogfood-эксплуатации. Селфтесты `test_weights`/`test_hebbian_demo` переписаны под новую семантику; 18 движка + mypy зелёные. Теория (§8.1/§8.2 — числа и толкование, без маркеров этапов) и архитектура (`07-consolidation` правило/формулы, `06-retrieval` роли журналов) синхронизированы с новым правилом в этой же сессии. **На закрытии этапа:** добавить в GUIDE и READMEs понятное последовательное руководство «когда включать Хебб (большой разрежённый граф + накопленный реальный usage) и когда оставлять `false`» (правило безопасно: no-op без usage). **Перевод дефолта в `true` отложен на Этап 17, задача 8** — замер на РЕАЛЬНОМ `usage.log` (синтетика доказала лишь корректность и пользу на графе).
