...
**Состояние (живое).** Группы 1–3 закрыты (фундамент + bench + барьер; производный индекс под `load_nodes`, 15× на 7600 узлах; near-dup §1.27 + возобновляемая деривация + хелперы очереди); Группа 4 (тиринг моделей) не начата. Порядок — по группам, коммит после группы. Развилки ниже зафиксированы с пользователем (после изучения всех доков/кода) — это расширенное, детальное понимание этапа, более широкое, чем исходные тезисы.
