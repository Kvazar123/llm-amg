...
**Файлы:** все `skills/*/scripts/*.py`.

CLAUDE.md §6.9 ставит целью `mypy --strict`, но сейчас ему не удовлетворяет ни один модуль. Прогон (mypy установлен на Этапе 5) выявил три класса ошибок, **общих для всей кодовой базы**, а не специфичных для отдельного файла:

1. **`yaml` без стабов** — `types-PyYAML` не установлен (`import-untyped`); `yaml.safe_load(...) -> Any` тянет за собой «`Returning Any`» в `load_config` / `_working_language` и подобных;
2. **голые дженерики** — `dict` / `list` / `List[dict]` / `Dict[str, dict]` без параметров (`type-arg`) — это принятый в репозитории стиль (`reconcile._structural_edges -> List[dict]`, сигнатуры `graph_store`);
3. **`sys.stdout.reconfigure`** под `try/except` — `union-attr` на идиоме форсирования UTF-8 (повторяется в `retrieve.py` / `extract_structure.py` / `notes.py`).

Ни один класс не является дефектом логики (рантайм корректен) — это разрыв «заявленный стандарт ↔ реальный стиль». Приводить к `--strict` нужно **единым проходом** по всем скриптам (установить `types-PyYAML`; параметризовать дженерики; снять идиому `reconfigure` через `cast`/локальный `# type: ignore[union-attr]`; убрать `Any`-возвраты после типизации yaml), а не точечно в одном модуле — иначе типизация рассинхронизируется между соседями. Задача внесена в Этап 12 (качество/масштабирование).

**Результат:** код удовлетворяет собственной планке типизации; регрессии типов ловятся локально и в CI.
...
1. Закрыты (Этапы 2, 5 и 7): API заметок `notes.py add`, выбор моделей эмбеддингов; на Этапе 7 — ссылка на `README_RU.md`, согласование `absorb`, язык §3, **отчёт о пробелах `gap-report.md`** (2.12.6), привязка опережающих разделов пометками «Планируется». На **Этапе 8** закрыты разделы «Активация и режимы» и «Что происходит автоматически» (снята пометка «Планируется (Этап 8)»): единая команда `/amg` с синонимами и таблицей операций, `automation: true`, хуки `SessionStart`/`SessionEnd`, always-on дайджест, ручные аналоги, оговорка про среды кроме Claude Code. Сессии **закрыты на Этапе 9** (раздел «Сессии»: выгрузка на `SessionEnd`, срез thinking/счёт вложений, политика записи `session_policy`, переносимость через `notes`). Осталось опережающим: 3D (Этап 15), structured `models` (1.14).
...
> На Этапе 7 уточнены только семантика `absorb` (1.16) и язык (§3); остальное (структура установки) — Этап 10.

Нужно исправить (Этап 10):
...
6. Закрыт на Этапе 9: install snippet отражает авто-выгрузку диалога на `SessionEnd` (где есть хук) и переносимую страховку — заметки `notes` по ходу там, где хука нет; вопрос про `session_policy` уже есть в авто-установке.
...
9. Слой жизненного цикла (Этап 8): описать, что установщик раскладывает хуки `settings.json` (`SessionStart`/`SessionEnd`) и слэш-команду `commands/amg.md`, добавляет импорт дайджеста в блок и засевает пустой `digest.md`; пути рендерятся из `agent_dir`. **Оговорить переносимость механизмов:** хуки, слэш-команды и `@`-импорт — функции именно Claude Code; в средах без них автоматику и команды заменяет переносимый путь — модель-ведомая петля активации, словесные триггеры и прямой вызов `lifecycle.py`/скиллов (см. 4.9). То есть какие удобства доступны — зависит от среды, а базовая работоспособность — нет.
...
**Управляющие шаблоны Этапа 8 — в том же проходе:** `entrypoint/settings.json`, `entrypoint/commands/amg.md` и строка импорта `@.claude/amg/digest.md` в блоке несут литерал `.claude` как умолчание; установщик подставляет `agent_dir`/`entrypoint` (задача 8). Сверх замены путей оговаривается, что **хуки, слэш-команды и `@`-импорт — механизмы именно Claude Code**, а переносимый путь в иных средах — модель-ведомая петля активации, словесные триггеры и прямой вызов скриптов/скиллов (см. 4.9). Код уже портативен (`resolve_amg_root`); параметризации требуют только эти шаблоны и `DEFAULT_IGNORE_DIRS`.
...
Текущее состояние: блок реализует петлю захвата через `notes.py add` (Этап 5), команды `/amg`, `automation` и always-on дайджест (Этап 8), выгрузку стенограммы на `SessionEnd` (Этап 9); описание журнала/архива/`log.md`/`sessions/` соответствует коду. Опережающим в управляющем слое остаётся рендеринг шаблонов точки входа / `settings.json` / `commands` установщиком по `agent_dir`/`entrypoint` (Этап 10).
...
1. Реализовать installer. **[Группа 2 — выполнено:** `install.py` (преемник `sync_testbed.py`) — копирует движок (**только `amg-*`**, чужие скиллы в общем `~/.claude` сохраняются), рендерит `entrypoint/CLAUDE.md`/`settings.json`/`commands/amg.md` по `agent_dir`/`entrypoint` (global → абсолютный путь к движку, `@digest`-импорт заменяется заметкой, граф локален), вливает блок между маркерами (reinstall меняет только блок), **сливает** `settings.json` (чужие хуки целы), засевает `digest.md`, пишет `config.yml` из шаблона с ответами (`_set_scalar`/`_set_list` правят реальные ключи, не прозу в комментариях; существующий конфиг не затирает), ставит deps по группам, финал — `verify --repair` **без авто-build**. По фидбеку пользователя: `--build` (структурный `bootstrap` сразу, чтобы не ждать новой сессии), `--project-only` (добавить проект к глобальной установке — только локальный `config.yml`+digest, движок не трогается), `--uninstall [--scope global] [--purge-graph]` (снимает блок, удаляет только `amg-*`, чистит AMG-хуки, граф бережёт без `--purge-graph`). `requirements.txt` (задача 6) + группы base/embeddings/text/treesitter. `selftest_install.py` (8 кейсов, headless). Остаётся: INSTALL.md и доки (Группа 3).**]
...
3. Installer спрашивает:
   ...
   - `automation`;
   - `working_language`;
   - embeddings backend. При `working_language != en` рекомендовать многоязычную модель. Измерено (Этап 6): лёгкий дефолт `potion-multilingual-128M` (model2vec) валиден для русского — на кросс-язычном кейсе `build_embeddings_demo` recall 1.0, наравне с тяжёлой ST `paraphrase-multilingual-MiniLM-L12-v2`, тогда как чистый BM25 даёт 0.0 (обманут ложным другом). Поэтому: **порядок загрузки в `embed.py` менять НЕ нужно** (лёгкий-первым безопасен — без torch, не падает; и достаточен для русского). Тяжёлый ST — **опциональный** апгрейд по измерению `--compare-embeddings`, а не молчаливый дефолт (тянет torch, тяжелее, на сломанном torch segfault на encode не ловится). Установщик показывает выбор и при не-en подсказывает «лёгкая многоязычная — проверенный дефолт, ST — апгрейд под измерение»; опц. рантайм-подсказка в `embed.py`/петле при не-en + auto→model2vec. (Детали и числа — THEORY §8.1, GUIDE раздел про зависимости, 06-retrieval «Многоязычность».)
   - session policy;
   - бюджеты ярусов; 
   - опциональные зависимости;
   - игнорируемые объекты (глоб-шаблоны → `exclude`; per-intent ключи — задача 10);
   - **активировать ли память после установки** (`active`; по запросу пользователя): установщик НЕ активирует молча и НЕ строит граф сам — он спрашивает, при согласии пишет `active: true`. Мгновенной индексации при установке нет в любом случае: `on`/`active` лишь поднимает флаг, а структурный `bootstrap` запускает **петля активации** (следующая сессия при `automation`) либо `/amg sync`; хуки граф не строят (субагента не запускают). Финальный шаг установщика — `verify --repair` (целостность хранилища), **без авто-`bootstrap`**.
4. Правила:
...
8. Параметризация среды (см. 4.9): разрешение корня и устранение литералов `.claude` выполнены ещё на Этапе 0 — здесь довести до конца: `DEFAULT_IGNORE_DIRS` получает настроенный каталог агента; installer задаёт вопрос про `agent_dir`/`entrypoint` и записывает ответы ключами в config.yml (глобальный и/или локальный); блок активации берётся из шаблона `entrypoint/CLAUDE.md` и пишется в файл точки входа, указанный в конфиге, между теми же маркерами. **Управляющие артефакты Этапа 8 — тоже под рендеринг установщиком:** `entrypoint/settings.json` → `<agent_dir>/settings.json` (пути команд хуков из `agent_dir`; **слить** с существующим `settings.json`, не затереть); `entrypoint/commands/amg.md` → `<agent_dir>/commands/amg.md` (путь `lifecycle.py` из `agent_dir`); строка импорта дайджеста в блоке → `@<agent_dir>/amg/digest.md`; **засеять пустой `digest.md`**, чтобы импорт резолвился до первой консолидации (сейчас это делает `sync_testbed.py`). **Глубже простой замены путей:** хуки/слэш-команды/`@`-импорт — механизмы Claude Code (в Codex и подобных их может не быть); универсальный субстрат — петля активации + словесные триггеры + прямой вызов `lifecycle.py`/скиллов. Поэтому установщик не только подставляет пути, но и **разворачивает механизм под среду**: где хуков/команд нет — опирается на петлю и словесные триггеры (см. 4.9). В `INSTALL.md` описать, какие удобства доступны в какой среде.
...
10. **Управление игнором по намерениям (новая задача, по запросу пользователя).** Сейчас игнор — один глобальный список `exclude` поверх `DEFAULT_IGNORE_DIRS` и `.gitignore`, одинаковый для зеркал и впитывания; этого мало, чтобы точно контролировать, что попадает в граф (что критично: молчаливый промах игнора = важные материалы не входят). Ввести:
    - ключи `mirror_exclude` / `absorb_exclude` (глоб-шаблоны, тот же `_gitignored`/fnmatch) — применяются **дополнительно** к глобальному `exclude` для источников своего намерения; пустые по умолчанию (поведение не меняется). Форма — **параллельные ключи** (не вложенный словарь), симметрично `mirror_path`/`absorb_path`, обратная совместимость;
    - ключ `respect_gitignore` (умолчание `true`); при `false` `.gitignore` не читается вовсе, игнор полностью управляется конфигом — универсально и без зависимости от git (он и так читается лишь как файл, бинарь git и каталог `.git` не нужны; нет файла → пусто, без ошибки);
    - **явный источник важнее `.gitignore`:** если **корень** `mirror_path`/`absorb_path` сам подпадает под правило `.gitignore`, источник всё равно индексируется (намеренный opt-in), а `.gitignore` продолжает отсекать мусор *внутри* широких источников (`mirror_path: .`). Чинит молчаливую потерю: `absorb_path: logs`, где `logs/` в `.gitignore`, иначе не попал бы в граф вовсе;
    - `DEFAULT_IGNORE_DIRS` остаётся **жёстким** слоем гигиены и **неприкосновенным для каталога агента** (память не индексирует себя; параметризация литерала — задача 8) — он НЕ отдаётся под «явный источник важнее», чтобы не открыть путь к самоиндексации графа;
    - вопрос установщика про игнорируемые объекты (пишет `exclude`; per-intent ключи документируются для тонкой настройки);
    - диагностика: `--stats` показывает счётчик файлов **по каждому источнику** (включая «0 — всё отфильтровано / путь не найден»), чтобы молчаливый промах игнора был виден (перекликается с 1.29/1.30).
    DoD задачи: absorb gitignored-папки попадает в граф; per-intent excludes отсекают только свой источник; `respect_gitignore:false` отключает `.gitignore`; селфтест на временных фикстурах.
    **[Группа 1 — код выполнен:** `extract_structure` — `_excludes_for_policy` (global+per-intent, аддитивно, `_norm_globs`), `respect_gitignore`, `_gitignore_for_source` (явный корень-источник обходит `.gitignore`), `_effective_ignore_dirs` (+`.agents` в базу, + настроенный agent_dir от `amg_root.parent.name`), `iter_source_files(…, ignore_dirs)`, per-source `by_source` в `--stats`; `selftest_ignore.py` (5 кейсов). Остаётся: вопрос установщика + доки.**]
11. **Резолв корня для CLI, вызываемых блоком напрямую (найденный дефект).** `graph_store.main` (и для полноты `retrieve.main` / `inspect_graph`) берут корень относительно движка (`parents[3]/amg`), а не через `resolve_amg_root`. При глобальной установке блок зовёт `~/<agent_dir>/skills/.../graph_store.py recover|verify`, и корень разрешится в `~/<agent_dir>/amg`, а не в локальный граф проекта → DoD «global engine works / graph remains local» ломается. Перевести на `resolve_amg_root(start=cwd)` (с override `--root` / `AMG_ROOT`): глобальный движок находит локальный граф восходящим поиском конфига. Для глобальной установки блок несёт абсолютный путь к движку, а корень графа резолвится локально (хуки и `lifecycle` уже корректны — идут от cwd проекта).
    **[Группа 1 — выполнено:** `graph_store.main` через `resolve_amg_root` (+`--root`, `AMG_ROOT` как override; мёртвый `_default_root` удалён), `retrieve._default_store`/`inspect_graph` — восходящий поиск от cwd; CLI-смоук пройден (init/verify/recover от cwd проекта резолвят локальный граф).**]
...
12. Привести все скрипты к `mypy --strict` (см. 1.31): установить `types-PyYAML`, параметризовать дженерики (`dict`/`list`/`List[dict]`), снять `union-attr` на `stdout.reconfigure` и `Any`-возвраты от `yaml.safe_load` — единым проходом по всем модулям, чтобы типизация не рассинхронизировалась между соседями.
13. **Возобновляемая семантическая деривация без перерасхода токенов.** Корректность при прерывании уже надёжна: скелетные узлы — одна крэш-safe транзакция, билдеры пишут `work/derived-<партия>.json` (а не граф), `apply` транзакционен на файл-партию (всё-или-ничего), а повторный прогон **пересобирает `queue.json` из состояния графа** (`status`/`derived_from_hash`), поэтому в очередь попадают только НЕдоведённые узлы — без дублей, без потерь, без повторной обработки всей партии. Пробел — в СТОИМОСТИ: если билдер дописал JSON, но `apply` не успел, его узлы деривируются заново (зря потраченные токены LLM). Возобновление должно **переиспользовать готовый `work/derived-*.json`** до повторного запуска билдеров — но только после проверки свежести (`content_sha` элемента деривации против ТЕКУЩЕГО `source_hash` узла), чтобы деривация против с тех пор изменившегося источника отбрасывалась, а не применялась вслепую (это и есть подводный камень). Опционально — помечать партии «в работе» в очереди. Итог: прерванный bootstrap возобновляется, не переплачивая токенами за уже выведенные единицы. (Связано с задачей 7 — lazy/on-demand деривация и фоновая очередь.)
...
8. **Улучшенное правило хеббова обновления весов (outcome-gated + различающее).** Закрывает отложенное с Этапа 6: измерение задачи 7 показало, что **слепое** усиление от ко-активаций бесполезно при пиковом засеве (BM25/model2vec: off≡on) и даёт лишь **малый** выигрыш при размытом семантическом засеве (ST-мультиязычная: recall +0.03, hop +0.04 на 33-узловом графе) — потому что сигнал циркулярен и насыщает подграф равномерно (§8.1). На входе — провенанс использования и исхода из Этапа 13 (задача 9, `usage.log`). Заменить правило в `consolidate.fold_weights`:
   - **outcome-gated (разрыв циркулярности):** усиливать ребро, только если оно лежало на пути к узлу, **реально использованному** в сессии с **принятым** исходом (а не просто «попал в пакет»); отрицательный исход — ослаблять. Сигнал приходит извне ранжирования → не само-подтверждается.
   - **различающее усиление (против равномерного насыщения):** не тянуть все ко-активированные рёбра к 1.0. Кандидаты (выбрать по измерению): (а) нормировка прироста **по узлу** — исходящие рёбра конкурируют за бюджет (softmax/симплекс), усиление одного ослабляет прочие; (б) «удивление» — прирост на величину ко-активации **сверх** предсказанной текущим весом `Δ ∝ (co − E[co∣w])`, чтобы уже-сильные не росли даром; (в) усиление относительно медианы ко-активаций соседей. Цель — чтобы обновление несло различающую информацию.
   - **валидация на большом разрежённом графе** (где веса имеют рычаг; малый плотный граф Этапа 6 мог скрыть пользу даже корректного правила): протокол eval off/on из задачи 7 Этапа 6, **с ST-засевом** (там, где эффект виден), плюс случаи под outcome-сигнал. Дефолт `apply_hebbian: true` ставить **только** при устойчивом измеренном выигрыше нового правила без просадки.
   - **взаимодействие с засевом:** зафиксировать в THEORY §8.1, что польза Хебба зависит от пиковости засева (сильный засев → seed доминирует, Хебб инертен; слабый/размытый засев или разрежённый граф → spread решает, веса получают рычаг); это объясняет, почему дефолт весов и дефолт эмбеддингов связаны.

   **Причина вреда слепого правила (ИЗМЕРЕНО на Этапе 6, задача 7).** Слепая ко-активация копит сигнал на уже-центральных/ранних рёбрах; равномерное усиление их к потолку весов делает из них «магистрали» проводимости, и PPR-масса утекает с multi-hop-периферии, где и живёт gold → hop_recall падает. На большом разрежённом графе это **монотонный вред** (recall −0.10, hop −0.14 за 8 фолдов по всем засевам; на малом плотном графе веса не влияли — эффект был скрыт). Оба улучшения выше бьют ровно по этой причине: **различающее** усиление не даёт уже-сильному расти даром, **outcome-gating** усиливает только реально полезное (сигнал извне петли).

   **Готовый стенд проверки — собран на Этапе 6, НЕ удалять:** `../amg-bigtest` — 190-узловой разрежённый граф цепочек `depends_on` (генератор `gen_big.py`: домены SaaS, программная деривация сводок из докстрингов + нейтральные веса 0.5; 12 многошаговых cases `cases.json` с gold за рёбрами; харнесс `_measure_big.py` меряет off/on по трём засевам BM25/model2vec/ST в подпроцессах — обходит segfault torch). Именно на нём слепое правило показало вред. **Полная инструкция — в `../amg-bigtest/README.md`** (что где лежит, как пересобрать, как запустить замер); ключевые команды продублированы здесь, чтобы стенд воспроизводился даже без него.

   *Пересборка стенда* (если каталога нет или граф испорчен; команды — из корня `amg-bigtest`):
   ```bash
   python gen_big.py                                          # src/ + derivation.json + cases.json
   python <repo>/sync_testbed.py <абс. путь к amg-bigtest>     # синхронизировать движок
   python .claude/skills/amg-bootstrap/scripts/reconcile.py bootstrap .
   python .claude/skills/amg-bootstrap/scripts/reconcile.py apply derivation.json .
   cp cases.json .claude/amg/cases.json
   ```

   *Замер и сравнительные метрики:* `python _measure_big.py` печатает по каждому засеву (BM25 / model2vec / ST) строку `recall off→on`, `hop off→on`, `pack off→on` — это и есть метрики сравнения (ST-eval — в подпроцессе, фолды — на BM25). Эталон слепого правила: recall −0.10, hop −0.14. Для нового правила — реализовать его в `consolidate.fold_weights`, затем запустить **тот же** `_measure_big.py` и сравнить дельты.

   *Протокол валидации улучшенного правила* (с usage-сигналом Этапа 13):
   1. off-эталон по трём засевам — `_measure_big.py` при `apply_hebbian:false`;
   2. накопить `usage.log` (Этап 13, задача 9): прогнать cases, пометить использованные узлы + исход (для синтетики — gold как «использованные» при успехе);
   3. свернуть **новым** правилом (outcome-gated + различающим), трактория 1→2→4→8 фолдов;
   4. on-замер тем же `_measure_big.py`;
   5. **критерий успеха: hop_recall и recall НЕ падают** (в идеале растут) монотонно по фолдам и по всем засевам — в противоположность слепому правилу, монотонно их роняющему;
   6. дефолт `apply_hebbian:true` — только при удержании/росте на `amg-bigtest` И повторении на ≥1 реальном dogfood-графе.
...
- хеббово правило обновлено на outcome-gated + различающее, перемерено на большом графе `amg-bigtest` по протоколу выше (hop/recall не падают), дефолт `apply_hebbian` выставлен по измерению нового правила.
