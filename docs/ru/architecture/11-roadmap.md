...
В документации и `consistency-model.md` заявлена логика:

> если `derived_from_hash != source_hash`, узел должен попасть на повторную деривацию.

В коде сейчас `plan()` проверяет только `source_hash`:
...
Если узел уже создан со статусом `stale`, но деривация не была применена, повторный `bootstrap` видит совпавший `source_hash` и считает узел `unchanged`. В результате узел может навсегда остаться без актуальной сводки.

Нужно учитывать не только изменение источника, но и отставание смысловой деривации:

```py
needs_rederive = (
    node.get("source_hash") != unit["content_sha"]
    or node.get("derived_from_hash") != unit["content_sha"]
    or node.get("status") == "stale"
)
```

Если `source_hash` совпадает, но `derived_from_hash` не равен текущему хешу, узел не нужно переписывать как изменённый источник, но нужно снова положить его в `work/queue.json`.

Дополнительное уточнение: `plan()` каждый раз перезаписывает `work/queue.json` целиком. Поэтому сбой между транзакцией узлов и записью очереди при повторном `bootstrap` не просто «не переочередит» stale-узлы — он сотрёт прежнюю очередь (новая будет пустой, раз `source_hash` совпали). Сценарий «повторный `plan()` перезаписал очередь пустым» включить в regression-тесты Этапа 0 явно.

**Результат:** после сбоя между записью структурного узла и записью очереди следующий `bootstrap` сам восстановит очередь.
...
`extract_structure.py` извлекает `lineno` и `qualname`, но `reconcile.py` не записывает их во frontmatter. Retrieval при этом обещает указатели вида `path:line`. Сейчас code-узлы могут отображаться как `src/file.py:None`.

Нужно:

- при создании узла писать:
...
- при изменении узла обновлять эти поля;
- в `_queue_item()` добавить `qualname`, `lineno`, `lang`, чтобы `amg-builder` мог фокусироваться на нужном срезе источника.

**Результат:** operational tier выдаёт точные указатели на код, а builder получает все поля, которые описаны в его prompt.
...
Документация говорит, что при `changed` обновляются структурные поля и сохраняется заработанная семантика. Но код делает только:
...
- `qualname`;
- `lang`;
- структурные `imports` / `calls`;
- первичное `part_of`, если путь/политика поменялись.

Главное: если функция начала вызывать новую функцию, структурное ребро `calls` **не появится** при изменении существующего узла.

**Что исправить:**

Нужно разделить рёбра по происхождению:

```yaml
edges:
  - rel: calls
    to: code:src/app.py::handler
    w: 0.7
    coact: 0
    origin: structural
```
...
1. удалить старые structural-рёбра (`origin: structural` или `rel in imports/calls/defines`);
2. добавить новые из `_structural_edges(unit)`;
3. сохранить смысловые рёбра, созданные субагентами.

Тогда при `changed` можно заменить только `origin: structural`, сохранив semantic edges от LLM.
Если поле `origin` не вводится сразу, минимальный вариант — заменить старые `imports`/`calls` на новые, не трогая остальные типы рёбер.
...
**Файлы:** `reconcile.py`, `consolidate.py`, `02-data-model.md`, `consistency-model.md`

Сейчас используются разные значения:
...
В `02-data-model.md` перечислены:

```txt
derived_from_file / synthesized / authored
```

В `consistency-model.md` есть старое:

```txt
derived
```

Это не ломает deletion rule, потому что удаляются только `derived_from_file + mirror`, но ломает документацию и будущую логику.

Нужно использовать единый набор значений:

| `source_kind` | Смысл |
|---|---|
| `derived_from_file` | узел получен из source unit файла |
| `synthesized` | узел создан как обобщение графа: hub, overview, summary |
| `authored` | авторская заметка, решение, вывод, вручную/моделью внесённая память |

Значение `derived` исключается из новой схемы. Узлы, создаваемые `summarize_episodes` и `introduce_subhub`, должны получать `source_kind: synthesized`.

**Результат:** документация, код и lifecycle-правила deletion/update используют одну схему.
...
Документация говорит, что несколько элементов деривации на один узел накапливаются. Для `edges` это правда:

```py
node["edges"] = _merge_edges(...)
```
...
То есть второй элемент может затереть членство, добавленное первым.

Нужно реализовать merge по `topic`:

```py
def _merge_part_of(existing, incoming):
    ...
```

Правило:

- одинаковый `topic` — берётся больший вес или новое значение по явному режиму;
- разные `topic` — добавляются;
- после merge применяется нормализация суммы весов, если включён `part_of_renormalize`.

**Результат:** несколько элементов деривации на один узел действительно накапливаются, как описано в документации.

Решение (Этап 0, задача 6): для совпавшей темы выбран режим «новое значение побеждает», без отдельного ключа конфигурации: элемент деривации — актуальное суждение слоя о теме, а «больший вес» делал бы веса монотонно растущими и мешал бы synth перебалансировать членство; после слияния — нормализация к симплексу при `weights.part_of_renormalize` (то же правило, что в консолидации).
...
Любое обновление существующего узла делает:
...
Даже если item содержал только `edges` или только `part_of`, без новой `summary`.

В текущем workflow это частично прикрыто порядком: сначала builder, потом synth. Но код сам по себе допускает ошибочный переход stale → active без полноценной деривации.

Нужно переводить source-derived узел в `active` только если применена полноценная смысловая деривация:
...
Для synthesized/authored-узлов правило может отличаться: они создаются сразу `active`.

**Результат:** stale-узлы не скрываются как актуальные до появления новой сводки.
...
`_structural_edges()` создаёт ребро `imports` с целью из *точечного имени модуля*:

```py
edges.append({"rel": "imports", "to": f"code:{mod}", "w": 0.6, "coact": 0})
```

то есть `code:billing` или `code:pkg.sub`. Но id узлов-модулей строятся из *пути файла* (`code:src/billing.py`). Эти формы не совпадают никогда, а `build_adjacency()` отбрасывает рёбра, чья цель не существует. Итог: 100 % imports-рёбер висячие, включая внутрипроектные импорты, — при том что 02-data-model и THEORY (§16) подают их как работающую связность.

Нужно построить карту «имя модуля → путь файла» по файлам проекта (для Python тривиально: `src/billing.py` ↔ `billing`) и резолвить цель в id существующего узла. До реализации резолвера imports честно пометить в документации как нерезолвящиеся.

**Результат:** imports-связность реально существует в графе, а не только во frontmatter.
...
- `extract_structure.py` (`_markdown_units`)
- `docs/ru/architecture/04-ingest.md`

`_HEADING` матчит строку вида `# установить зависимости` внутри код-ограждения: раздел разрывается посреди примера кода, возникают ложные секции с нестабильными слагами. Для документации с bash-примерами (включая документацию самого AMG) это системно.

Нужно добавить в `_markdown_units` отслеживание состояния код-ограждения и игнорировать «заголовки» внутри него; покрыть тестом.

**Результат:** секции совпадают с реальными заголовками документа.
...
9. (добавлено на Этапе 0) Раздел «Рёбра»: imports-рёбра внутрипроектных модулей теперь резолвятся в id узла модуля (`code:{путь}`); внешние остаются точечным именем и отбрасываются при извлечении как висячие.
...
5. (добавлено на Этапе 0) Упомянуть `resolve_amg_root` — теперь в `graph_store.py` живёт и разрешение корня хранилища по цепочке 4.9 (используется reconcile/consolidate; CLI самого graph_store по-прежнему берёт `AMG_ROOT`/расположение скрипта).
...
6. Добавить planned:
   ...
   - CSV/log parser;
   - PPTX slide parser.
7. (добавлено на Этапе 0) Описать в разделе markdown-нарезателя отслеживание код-ограждений: «заголовок» внутри ``` / `~~~`-блока (отступ до 3 пробелов; закрытие — тот же символ длиной не меньше открывшего) не создаёт секцию.
...
5. Описать merge-логику `part_of`, если она реализована. — Реализована на Этапе 0 (задача 6): `_merge_part_of` — темы накапливаются, совпавшая тема берёт новый вес, сумма > 1 нормализуется к симплексу при `weights.part_of_renormalize`.
...
7. Уточнить условия перевода в `active`. — Реализовано на Этапе 0 (задача 6): `derived_from_hash`/`active` выставляются только если item несёт `summary` либо узел не `derived_from_file`; элемент только с рёбрами/`part_of` оставляет узел `stale`, и requeue продолжает возвращать его в очередь.
8. (добавлено на Этапе 0) Описать тихое обновление указателя: правка выше по файлу сдвигает единицу без смены её контентного хеша — `plan()` обновляет у узла только `lineno`/`qualname` (и `policy` при смене политики источника), без передеривации и смены статуса (счётчик `pointer_refreshed` в сводке).
9. (добавлено на Этапе 0) Описать случай `moved`: пары deleted+added с одинаковым `content_sha` мигрируют earned-поля (summary, `lang`, смысловые рёбра с их `coact`, `derived_from_hash`, мультичленство с переписыванием первичной темы на новый каталог); внутрифайловые цели рёбер переводятся на новый путь, цели в одновременно перенесённых файлах — по карте переносов; входящие рёбра и `part_of` остальных узлов перенаправляются; чистый перенос не тратит вызовов модели, перенос с правкой = delete+add; absorb-узлы в обнаружение не входят (их дифф не удаляет: сирота + новый узел, дубли сливает консолидация); счётчик `moved` в сводке.
10. (добавлено на Этапе 0) Описать резолвер imports: карта «точечное имя → путь» строится по Python-module-юнитам проекта (полный точечный путь и все суффиксы; неоднозначный суффикс не резолвится); нерезолвённые (stdlib/сторонние) остаются `code:{имя}` и отбрасываются при извлечении как висячие.
11. (добавлено на Этапе 0) Описать разрешение корня графа и флаг `--root <agent_dir>` у всех команд: цепочка `--root` → `AMG_AGENT_DIR` → поиск `amg/config.yml` (и `.claude/amg/config.yml`) вверх от `project_root` → каталог движка (если его `amg/` существует) → умолчание `.claude` (`graph_store.resolve_amg_root`); `project_root` остаётся корнем источников и отделён от корня графа.
...
8. (добавлено на Этапе 0) CLI consolidate принимает `--root <agent_dir>`; корень графа резолвится цепочкой 4.9 (см. 2.6, п. 11), литерал `.claude` устранён.
...
Цель: сделать `bootstrap/reconcile/apply` корректными и самовосстанавливающимися.
...
5. Нормализовать `source_kind` — выполнено: `summarize_episodes` и `introduce_subhub` создают узлы с `source_kind: synthesized`; значение `derived` из кода исключено (legacy-узлы — миграция Этапа 1, задача 7).
6. Исправить `apply_derivation()` — выполнено (`_merge_part_of`; перевод в `active` только при новой `summary` либо для не-`derived_from_file` узлов; см. пометку к 1.6):
   - merge `part_of`;
   - не переводить узел в `active` без новой summary;
   - корректно обрабатывать несколько items на один узел.
7. Обнаружение переименований и переносов: пары deleted+added с одинаковым `content_sha` трактовать как перенос — мигрировать заработанные поля (summary, смысловые рёбра, `coact`, `derived_from_hash`) на новый id вместо purge+create. Сейчас любой рефакторинг зеркала (перенос файла, переименование заголовка) стирает earned-знание — это крупнейшая дыра в его долговечности. — Выполнено: `_detect_moves`/`_migrate_node` + перенаправление входящих рёбер и `part_of`; чистый перенос приезжает `active` без вызова модели (счётчик `moved`); детали — пометка в 2.6, п. 9.
8. Резолвер `imports`: цель пишется как точечное имя модуля (`code:billing`), а id модулей строятся из пути (`code:src/billing.py`) — формы не совпадают никогда, и 100 % imports-рёбер висячие (см. 1.19). Построить карту «имя модуля → путь файла» проекта; до резолвера честно пометить imports нерабочими в документации. — Выполнено: `_module_map` (полный точечный путь и его суффиксы; `pkg/__init__.py` → `pkg`; неоднозначный суффикс не резолвится — неверное ребро хуже висячего); внутрипроектные imports резолвятся в id узла модуля, внешние остаются висячим точечным именем; «пометить нерабочими в документации» не требуется — резолвер реализован сразу.
9. Исправить markdown-нарезатель: отслеживать состояние код-ограждений и игнорировать «заголовки» внутри них (см. 1.24). — Выполнено: `_markdown_units` отслеживает ограждения (``` и `~~~`, 3+ символов, отступ до 3 пробелов; закрывает только тот же символ длиной не меньше открывшего; чужой маркер внутри блока не закрывает).
10. Добавить regression-тесты — выполнено (`selftest_reconcile.py`, 16 сценариев; покрыты все восемь пунктов):
   - crash после создания stale-узла до записи queue;
   - повторный bootstrap переочередит stale — в том числе когда повторный `plan()` уже перезаписал `queue.json` пустым;
   - изменение calls обновляет structural edges;
   - `lineno` появляется в pack;
   - multi-item derivation не теряет `part_of`;
   - перенос файла сохраняет сводку и смысловые рёбра;
   - внутрипроектный `import` даёт резолвящееся ребро;
   - заголовок внутри код-блока не создаёт секцию.
11. Разрешение корня графа (цепочка из 4.9): явный `--root` → env `AMG_AGENT_DIR` (если задана) → поиск `amg/config.yml` вверх от текущего каталога (расположение конфига и есть каталог агента) → расположение самих скриптов → умолчание `.claude`; устранить литералы `.claude` в `reconcile.py` и `consolidate.py`. Делается уже здесь: дев-раскладка живёт вне `.claude` с первого этапа, а поиск по конфигу заодно чинит глобальную установку (скрипты в `~/.claude`, граф — в проекте). Installer, ключи `agent_dir`/`entrypoint` в config.yml, `DEFAULT_IGNORE_DIRS` от настроенного каталога и документация (2.15) — Этап 10. — Выполнено: `graph_store.resolve_amg_root`; литералы устранены, CLI reconcile/consolidate принимают `--root <agent_dir>`. Два уточнения цепочки: (а) при подъёме на каждом уровне пробуется и `<предок>/.claude/amg/config.yml` — иначе глобальная установка не находится буквально (конфиг проекта лежит в дочернем `.claude`, а не в самом предке); (б) шаг «расположение скриптов» срабатывает, только если его `amg/` уже существует — иначе глобальный движок в `~/.claude` перехватывал бы корень у свежего проекта вместо умолчания `<проект>/.claude`.

Definition of done:

- `selftest_graph_store.py`, `selftest_stage2.py`, новые reconcile-тесты зелёные;
- повторный `bootstrap` после искусственного сбоя восстанавливает очередь;
- pack не содержит `:None` для code pointers;
- перенос/переименование файла не теряет earned-поля;
- рёбра `imports` резолвятся во внутрипроектные модули;
- скрипты ядра работают из дев-раскладки и из глобальной установки (корень — по цепочке 4.9: конфиг от CWD или расположение скрипта), литералов `.claude` в `reconcile.py`/`consolidate.py` нет.
