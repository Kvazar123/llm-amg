...
1. Закрыты (Этапы 0, 1, 5, 6 и 7): типы хабов, поля очереди, петля классификатора, безопасный API заметок `notes.py add`; на Этапе 7 уточнено, что `amg-synth` не редактирует `nodes/`, но пишет `gap-report.md`.
2. **Хуки, команды `/amg`, sessions, `automation`** — опережающее: сверяется на Этапах 8–9 (раздел «Планируется (дорожная карта)» в документе).
...
1. Закрыты (Этапы 2, 5 и 7): API заметок `notes.py add`, выбор моделей эмбеддингов; на Этапе 7 — ссылка на `README_RU.md`, согласование `absorb`, язык §3, **отчёт о пробелах `gap-report.md`** (2.12.6), привязка опережающих разделов пометками «Планируется» (`/amg`/`automation` → Этап 8, sessions → Этап 9, 3D → Этап 15, structured `models` → 1.14).
...
Текущее состояние: блок честен — сессии, команды `/amg` и `automation` в нём не упомянуты, описание журнала/архива/`log.md` соответствует коду; на Этапе 5 петля захвата в блоке переведена на безопасный API `notes.py add` (п. 1 выше).
