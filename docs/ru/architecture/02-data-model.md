...
Поле `type` описывает **форму** узла и не несёт происхождения: происхождение задаёт `source_kind` (см. выше). Историческое значение `type: derived` у хабов упразднено — новые узлы его не получают, а в старых графах его заменит миграционный скрипт (Этап 1 дорожной карты, задача 7). Канонический набор по классам узлов:
...
| `authored` | `note` — заметка по ходу работы; `decision` — зафиксированное решение; `adr` — architecture decision record | пользователь или модель; безопасный API заметок — Этап 5 дорожной карты |

**Допустимое расширение:** у единиц, нарезанных tree-sitter, `type` равен типу узла грамматики (`function_definition`, `class_declaration`, …). Такие значения легальны, но пока выпадают из ярусов пакета и формата указателей `путь:строка`; их канонизация к `function` / `class` — задача 8 Этапа 1 (дорожная карта, 1.25).
