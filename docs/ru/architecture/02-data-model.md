...
flowchart TD
    ...
    R --> CA["cache/ — pack.md, embeddings.json, index.sqlite"]
...
| `cache/` | производные кэши, **одноразовые и пересобираемые**: пакет `pack.md`, кэш эмбеддингов `embeddings.json`, read-индекс `index.sqlite` (ускоряет `load_nodes` на больших графах; см. [Извлечение из графа](./06-retrieval.md)) | по необходимости (извлечение / запись) |
