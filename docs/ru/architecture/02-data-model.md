...
Корень хранилища — `.claude/amg/`. При инициализации (`graph_store.init`) создаются только `journal/` и `nodes/<бакет>/` (пять бакетов: `code/ doc/ data/ notes/ _hubs/`). Всё остальное — `work/`, `archive/`, `cache/` и файлы `log.md`, `digest.md` — рождается по необходимости, при первой записи.
