...
| Сверка | `reconcile.py` | диф графа с источником, очередь на обогащение, применение; кэш обогащения, метрики связности и аудит инвариантов хранилища | `bootstrap` · `plan` · `apply` · `apply-cached` · `metrics` · `audit` |
...
| `amg-retriever-fork` | наследует вызывающего (агент `context: fork`, только Claude Code) | наследуются | контекстно-осведомлённая консультация с памятью: читает пакет в своём окне, возвращает короткую выжимку, взвешенную против сессии |
