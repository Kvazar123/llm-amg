...
Граница между слоями экономит главный ресурс — токены модели. Дорогая семантическая работа выполняется только над *изменившимися* единицами: каждая единица хешируется по содержимому, и при повторном прогоне неизменное пропускается (хеш-гейт; см. [Сверка и деривация](./05-reconcile.md)). Скелет можно перестроить в любой момент бесплатно, а семантику — только там, где она устарела.
...
Рядом с каждым модулем лежит самотест (`selftest_*.py`), проверяющий его инварианты; самотесты не входят в рабочий поток и запускаются вручную при изменениях.
...
agents/                          субагенты — по файлу на роль
  amg-builder.md                 деривация сводок и смысловых рёбер
...
Контентная плоскость (граф проекта) создаётся в `.claude/amg/` и описана в разделах [Модель данных](./02-data-model.md) и [Хранилище и транзакции](./03-storage.md): каталоги `nodes/`, `journal/`, `work/`, `cache/`, `archive/`, файл `LOCK` и локальный `config.yml`.
...
**Ингест — построение и обновление графа** (запускается на старте и при изменении источников). Источники читаются строго на чтение; граф меняет только последовательный `apply` под локом.
...
flowchart LR
    ...
    SUB --> APPLY["reconcile apply<br/>под локом"]
    APPLY --> G["граф (nodes/)"]
```

Детерминированная часть (`extract_structure` → `reconcile plan`) создаёт скелет узлов и кладёт единицы в очередь `queue.json`; семантическая часть (субагенты) читает очередь и пишет результат в отдельные файлы `derived-*.json`; затем `reconcile apply` вносит их в граф. Повторный прогон сверяет, а не дублирует (хеш-гейт). Подробности — [Извлечение структуры](./04-ingest.md) и [Сверка и деривация](./05-reconcile.md).
...
- [05 — Сверка и деривация](./05-reconcile.md) — очередь, `mirror`/`absorb`, идемпотентность.
