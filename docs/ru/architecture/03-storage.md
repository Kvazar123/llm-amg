...
Корень хранилища для CLI самого `graph_store.py` определяется переменной окружения `AMG_ROOT`, а по умолчанию (`_default_root`) вычисляется относительно расположения скрипта (`.claude/amg/`), поэтому CLI работает из любого каталога.

Здесь же живёт `resolve_amg_root` — разрешение корня хранилища для сверки и консолидации без зашитого имени `.claude`: явный `--root <agent_dir>` → переменная `AMG_AGENT_DIR` → поиск `amg/config.yml` вверх от каталога проекта (на каждом уровне проверяется и `<каталог>/.claude/amg/config.yml` — так глобально установленный движок находит граф проекта) → каталог самого движка, если его `amg/` уже существует → умолчание `.claude`. Подробности применения — [Сверка и деривация](./05-reconcile.md).
