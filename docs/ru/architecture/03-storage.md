...
- блокировка **мёртвого процесса на той же машине** устарела: живость проверяется `_pid_alive` через `os.kill(pid, 0)` (зонд POSIX) **только при совпадении `host`** — проба pid осмысленна лишь на той же машине; `ProcessLookupError` означает «мёртв», `PermissionError` — «жив, но чужой», а на платформах без такой проверки (например, Windows) процесс консервативно считается живым;
- блокировка **с другого хоста** (общая папка) забирается **только по возрасту**: локальная проба чужого pid между машинами бессмысленна и ложно отняла бы живую блокировку коллеги, поэтому чужой хост ждёт порог `stale_seconds`;
...
**Командный режим.** Та же модель «один писатель под блокировкой» распространяется на простую командную работу по очереди, без распределённой блокировки. На **общей папке** блокировка различает машины (host-aware, см. правила устаревания выше) и не отнимает живую блокировку коллеги с другого хоста; при занятой блокировке автоматическое обслуживание мягко пропускает цикл (идемпотентно повторится на следующем запуске), а явные команды сообщают «заблокировано другим писателем». Альтернативный канал обмена — **git поверх markdown-канона**: ветки несут свою память, слияние идёт по файлам `nodes/*.md`, а узел, оставшийся с git-конфликт-маркерами, безопасно пропускается чтением (`load_nodes`) и чинится разрешением маркеров плюс `bootstrap`; детектор — `reconcile.find_conflict_markers`, поверхности — `/amg status`/`repair`/`bootstrap` (формальная модель — `consistency-model.md` §9 и §12). Настоящая одновременная многозапись с нескольких машин намеренно не строится. Актуальность источника по коммиту проверяет `verify_claims --by-commit` (см. [Извлечение из графа](./06-retrieval.md)).

## Журнал действий `actions.log` — транзакционный аудит-след

Отдельно от низкоуровневого журнала `journal/` хранилище ведёт человекочитаемый `actions.log` — аудит-след выполненных действий. Это **плоский лог**: записи одинаковы по форме, поэтому файл ведётся без разметки (исторически он назывался `log.md`, и каждую строку предварял заголовок `## `, ничего не добавлявший; при первой записи новый писатель перенимает строки прежнего `log.md`, срезая префиксы, и дальше его не трогает). Метод `GraphStore.append_log(source, msg, txid)` дописывает одну строку вида `[<время>] <txid> <источник> | <сообщение>` **через транзакцию** (а не обычным дописыванием): весь лог стадируется одним контентно-адресуемым блобом, поэтому повторное проигрывание при восстановлении идемпотентно. Три свойства:
...
- **ограниченность ротацией:** как только лог превысил предел строк (по умолчанию 500), старые строки уходят в `archive/actions-<время>.log`, а в живом `actions.log` остаётся хвост (100 строк) — долгоживущий граф не переписывает неограниченно растущий файл при каждой записи;
...
Пишут лог оба слоя записи: `consolidate.py` (свёртка весов, применённые действия, вердикт eval-предохранителя) и `reconcile.py` (итог `bootstrap`/`apply`, но **только когда дифф реально изменил граф**: при пустом диффе транзакции нет, и строка не добавляется — неизменный прогон остаётся идемпотентным). Источник в строке (`consolidate` / `reconcile`) позволяет `/amg status` выбрать последнюю именно консолидацию (`lifecycle._last_consolidation` фильтрует по нему). Запись идёт под той же блокировкой, что держит вызывающий, и приводит лог к модели «append как часть committed transaction с дедупом по `txid`». `verify_claims.py --write` тоже дописывает строку (источник `verify_claims`) под своей блокировкой.

**Поля слоя доверия и журналы-сигналы.** Поля слоя доверия (`confidence`, `provenance`, `verification`, `line_end`) живут во frontmatter узла и проходят через транзакции прозрачно: хранилище **доменно-слепо** — оно адресует файлы по содержимому и не знает, что внутри, поэтому новые поля переживают `recover()` так же, как любые другие, без правок в `graph_store`. Отдельно от графа извлечение и конец сессии ведут **журналы-сигналы** в `work/` — `coactivation.log` (сигнал экспозиции), `pack-log.jsonl` (состав пакета) и `usage.log` (провенанс использования): это best-effort рабочие данные **вне** транзакций и вне `journal/` (ошибка их записи, как и у строки `actions.log`, безвредна), не влияющие на целостность графа; `pack-log` посессионный (потребляется на session-end), `usage.log` копится как сигнал исхода для правила весов (его читает свёртка при включённом `apply_hebbian`; см. [Консолидация](./07-consolidation.md)). Подробности — [Извлечение из графа](./06-retrieval.md) и [Субагенты и скиллы](./08-agents-skills.md).
...
`resolve_amg_root` (живёт в `graph_store.py`) разрешает корень без зашитого имени `.claude`. Цепочка, побеждает первое совпадение:

1. явный `--root <agent_dir>`;
2. переменная `AMG_AGENT_DIR` (тот же смысл);
3. поиск вверх от текущего каталога. На каждом уровне **сперва** проверяются пресеты каталога агента — `<каталог>/.claude/amg/config.yml` и `<каталог>/.agents/amg/config.yml` (однозначная раскладка установки; так глобально установленный движок находит граф проекта в любой из этих сред) — и лишь затем «голый» `<каталог>/amg/config.yml`, который признаётся хранилищем **только инициализированным** (внутри есть `nodes/` и `journal/`; случай — запуск изнутри самого каталога агента). Уровень **домашнего каталога пропускается**: `~/<каталог агента>/amg/` несёт глобальный конфиг личных умолчаний (слой конфигурации, см. [12-install](./12-install.md)), а не хранилище проекта;
4. расположение самого движка (`<agent_dir>/skills/…` → `<agent_dir>/amg`) — тоже только если это инициализированное хранилище: покрывает дев-раскладку и локальную установку, но не даёт глобальному движку перехватить умолчание свежего проекта;
5. умолчание `.claude/amg`.

**Вето по сигнатуре движка.** Кандидат-каталог `amg/`, внутри которого лежат `skills/`, `agents/` или `install.py`, — это распакованный чекаут исходников AMG (его корневой `config.yml` — шаблон с примерами источников), а не хранилище; резолвер отвергает его на любом шаге поиска. Проверка «инициализированности» сама по себе недостаточна: один-единственный промахнувшийся прогон создаёт `nodes/`+`journal/` прямо в чекауте (`plan()` безусловно вызывает `init()`), после чего наивная проверка проходила бы вечно — поэтому вето действует независимо от наличия этих каталогов. Так чекаут движка рядом с проектом (или внутри него) никогда не перехватывает операции памяти, а промах разрешения виден сразу: извлечение структуры без `config.yml` по разрешённому корню завершается диагностикой «AMG is not installed…» с именем этого корня, а не молчаливым пустым прогоном.

Зеркальную копию той же логики несёт `retrieve._default_store` (у `retrieve.py`/`inspect_graph.py`/`export_graph.py` для дефолтного `--store`; без импорта `graph_store` — модуль извлечения самодостаточен). Подробности применения — [Сверка и семантическое обогащение](./05-reconcile.md).
