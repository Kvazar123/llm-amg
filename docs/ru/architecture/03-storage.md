...
## Журнал действий `log.md` — вне транзакции

Отдельно от транзакционного журнала `journal/` хранилище ведёт человекочитаемый `log.md` — аудит-след действий консолидации. Он пишется **best-effort**: обычным дописыванием в конец, вне транзакции, строкой с `txid` (ошибка записи молча проглатывается). Это намеренно: `log.md` не несёт нагрузки — целостность графа гарантирует `journal/`, а не лог, поэтому оборванная при сбое строка безвредна. Пишет его только `consolidate.py`; `reconcile.py` в `log.md` не пишет. Транзакционный лог с де-дупликацией по `txid` остаётся возможным улучшением (см. [Дорожная карта](./11-roadmap.md), п. 1.15).
