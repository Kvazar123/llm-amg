...
- блокировка **с другого хоста** (общая папка) забирается **только по возрасту**: локальная проба чужого pid между машинами бессмысленна и ложно отняла бы живую блокировку коллеги, поэтому чужой хост ждёт порог `stale_seconds` (стабилизация командного режима, Этап 16);
...
- **ограниченность ротацией:** как только лог превысил предел строк, старые строки уходят в `archive/actions-<время>.log`, а в живом `actions.log` остаётся хвост — долгоживущий граф не переписывает неограниченно растущий файл при каждой записи;
...
**Поля слоя доверия и журналы-сигналы.** Поля Этапа 13 (`confidence`, `provenance`, `verification`, `line_end`) живут во frontmatter узла и проходят через транзакции прозрачно: хранилище **доменно-слепо** — оно адресует файлы по содержимому и не знает, что внутри, поэтому новые поля переживают `recover()` так же, как любые другие, без правок в `graph_store`. Отдельно от графа извлечение и конец сессии ведут **журналы-сигналы** в `work/` — `coactivation.log` (хебб), `pack-log.jsonl` (состав пакета) и `usage.log` (провенанс использования): это best-effort рабочие данные **вне** транзакций и вне `journal/` (ошибка их записи, как и у строки `actions.log`, безвредна), не влияющие на целостность графа; `pack-log` посессионный (потребляется на session-end), `usage.log` накапливается до Этапа 14. Подробности — [Извлечение из графа](./06-retrieval.md) и [Субагенты и скиллы](./08-agents-skills.md).
