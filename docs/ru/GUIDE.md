...
**Консолидация — гигиена, а не сохранение.** Пока вы её пропускаете, ничего не теряется: заметки фиксируются транзакционно в момент захвата, стенограмму выгружает хук в любом случае. Консолидация покупает остроту — без неё граф постепенно пухнет, а выдача становится шумнее. Три её фазы: **weights** — детерминированная свёртка сигналов без модели (копятся счётчики ко-активаций; само хеббово обновление весов действует только при `weights.apply_hebbian`); **plan** — анализ только чтением (переполненные ветки, кандидаты-дубли, значимость заметок); **apply** — судейский проход (повысить / слить / свернуть эпизоды / ввести под-хаб / ужать / отправить в архив) с обратимым архивом оригиналов и защитой решений и ADR в коде, не только в промпте. Разумный ритм: сессия, насыщенная решениями, или заметно зашумевшая выдача — не каждый заход. Детерминированная фаза выполняется сама при каждом завершении сессии; судейская ждёт наблюдаемого триггера — ваших слов о завершении работы («подведём итоги и сохраним в память», `/amg consolidate`), а когда она пропущена несколько сессий подряд, начало следующей скажет об этом одной строкой и предложит её.
