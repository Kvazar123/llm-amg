...
- **`codex`** — OpenAI Codex со скиллами и субагентами: скиллы кладутся в `.agents/skills`, субагенты — **TOML в `.codex/agents`** (с `model`/`model_reasoning_effort` из блока `models`), вливается skill-aware блок `AGENTS.md`. Хуки не пишутся — хуковая поверхность Codex не подтверждена и проверяется живьём; сессионные половины выполняет стартовая проверка блока.
- **`opencode`** — OpenCode находит скиллы в `.agents/skills` **сам**; субагенты рендерятся в `.opencode/agent/*.md` (`mode: subagent`; настоящий идентификатор модели из `models` проходит, Claude-алиас опускается — идентификаторы OpenCode имеют вид `провайдер/модель`), вливается переносимый skill-aware блок. Хуков нет (аналог у OpenCode — плагинный JS-API; адаптация на будущее, для базовой работы не нужна).
- **`qwen`** — Qwen Code, пресет `.qwen` / `QWEN.md`: скиллы кладутся в `.qwen/skills`, промпты исполнителей — в `.qwen/agents/*.md`, **родные markdown-субагенты Qwen**, с очисткой Claude-специфичного фронтматтера (`tools`/`effort` убираются; Claude-алиас модели убирается, настоящий идентификатор вроде `qwen3-coder-plus` проходит), а сессионные хуки сливаются в `.qwen/settings.json` — Qwen читает их в той же форме, что Claude Code.
...
Базовая работоспособность от среды не зависит, набор удобств — зависит. **Режимы кроме Claude Code пока не протестированы живьём, стабильность не гарантирована** — проверка сред и есть идущий сейчас этап дорожной карты.
