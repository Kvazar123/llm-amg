...
Формат скиллов SKILL.md — межинструментный стандарт (Agent Skills), его одинаково читают Codex, OpenCode и Qwen Code; различается остальная управляющая поверхность — хуки, слэш-команды, `@`-импорт дайджеста, формат субагентов. Поэтому установщик разворачивает под среду **разный механизм**, а не только подставляет пути:

- **`claude-code`** (умолчание) — блок со скиллами + хуки `SessionStart`/`SessionEnd`/`UserPromptSubmit` + команда `/amg`; модели рендерятся во frontmatter `agents/amg-*.md`.
- **`codex`** — OpenAI Codex со скиллами, субагентами и хуковым движком ядра: скиллы кладутся в `.agents/skills`, субагенты — **TOML в `.codex/agents`** (с `model`/`model_reasoning_effort` из блока `models`), вливается skill-aware блок `AGENTS.md`, а в `.codex/hooks.json` сливаются два хука — стартовая проверка сессии и гейтованное напоминание середины сессии. Один шаг за вами: недоверенные хуки Codex **не запускает — откройте `/hooks` и доверьте AMG-хуки** (переустановка, меняющая хук, снова попросит одобрения); до этого сессионные половины выполняет стартовая проверка блока. События конца сессии у Codex нет — завершение остаётся за вашим сигналом; поверхности своих команд тоже нет — глаголы пишутся словами, а скиллы доступны из попапа `$`.
- **`opencode`** — OpenCode находит скиллы в `.agents/skills` **сам**; субагенты рендерятся в `.opencode/agent/*.md` (`mode: subagent`; настоящий идентификатор модели из `models` проходит, Claude-алиас опускается — идентификаторы OpenCode имеют вид `провайдер/модель`), вливается переносимый skill-aware блок, `/amg` дополняется из `.opencode/command/amg.md`, а роль хуков берёт **событийный AMG-плагин** (`.opencode/plugin/amg.js`): при открытии сессии он лечит хранилище и задаёт вопрос о синхронизации, один раз напоминает, когда память заметно не спрашивалась, и ведёт **инкрементальный дамп стенограммы** — диалог ложится в каталог сессий прямо по ходу работы и переписывается в тихие моменты, так что даже убитый терминал теряет максимум последние пару минут.
- **`qwen`** — Qwen Code, пресет `.qwen` / `QWEN.md`: скиллы кладутся в `.qwen/skills`, промпты исполнителей — в `.qwen/agents/*.md`, **родные markdown-субагенты Qwen**, с очисткой Claude-специфичного фронтматтера (`tools`/`effort` убираются; Claude-алиас модели убирается, настоящий идентификатор вроде `qwen3-coder-plus` проходит), `/amg` дополняется из `.qwen/commands/amg.md`, а сессионные хуки сливаются в `.qwen/settings.json` — Qwen читает их в той же форме, что Claude Code.
- **`generic`** — **неизвестная** AGENTS.md-среда: переносимый блок без скиллов, та же петля прямыми вызовами скриптов (промпты `agents/*.md` модель читает как инструкцию). Скиллы при этом всё равно ставятся в `.agents/skills` — межинструментное место, — и блок велит предпочесть их, если среда, как выяснится, их обнаруживает (ровно так вёл себя OpenCode, пока не получил собственный режим).

Базовая работоспособность от среды не зависит, набор удобств — зависит. **Полностью протестированная среда — Claude Code.** Полный цикл памяти в OpenCode — установка, сборка, извлечение, заметки, консолидация — воспроизведён живыми полевыми прогонами (свежедобавленным плагину и команде своя проверка ещё предстоит); режимы Codex и Qwen Code собраны по их проверенным поверхностям, но **живьём пока не подтверждены**. Проверка сред и есть идущий сейчас этап дорожной карты.
...
### Переустановка, обновление движка и удаление

**Переустановка** — это просто повторный запуск `install.py`, идемпотентный: он заменяет только блок между маркерами и движок `amg-*`, а ваши инструкции в `CLAUDE.md`, существующий `config.yml`, граф и чужие скиллы/хуки **бережёт**. **Обновление движка на новую версию — та же переустановка.** Распакуйте свежий AMG в любую папку вне проекта (как при первой установке; исходный каталог после прогона снова не нужен) и запустите `python install.py --target /путь/к/проекту` — или попросите модель: «переустанови/обнови AMG по `<путь>/INSTALL.md`». Специального режима «обновление» нет и не нужно: режим привязан к **состоянию** проекта, а не к формулировке — проект, где уже есть `config.yml`, трактуется как переустановка при любых словах (включая «установи»): модель показывает действующие значения и спрашивает только об изменениях, а не полный опросник первой установки. Установщик смотрит на факты на диске — существующий `config.yml` целиком никогда не переписывается (в выводе — `kept existing`, действующие значения строкой `in force:`), но ключи, **явно переданные в запуске** — `--set`, `--mirror`, `--absorb`, `--absorb-once`, `--exclude`, — применяются к нему точечными правками и перечисляются строкой `updated keys:`, так что собранный установочным потоком ответ ложится в конфиг, а не пропадает молча; всё не переданное остаётся как было (или меняется правкой файла). Блок `models` при этом заново рендерится во frontmatter агентов уже из **вашего** конфига, так что тиринг переезжает в обновлённые агенты сам. После обновления перезапустите сессию (скиллы регистрируются при старте) и выполните `/amg sync`: неизменные узлы не стоят ни токена, детерминированные улучшения (починка связей и т. п.) применяются сами, доплатить придётся только за реально изменившиеся с прошлой сверки файлы. Одно следствие бережного отношения к конфигу: **новые умолчания шаблона свежей версии в ваш конфиг не доезжают** — если релиз поменял, например, размеры партий (`builder.batch_units`, `linker.batch_nodes`), в вашем конфиге останутся прежние значения; после обновления стоит свериться с шаблоном новой версии и поднять нужные ключи вручную. **Удаление** — `python install.py --target . --uninstall [--scope global] [--purge-graph]`: срезает блок активации (ваш контент остаётся), убирает скиллы/агентов/команду `amg-*` и AMG-хуки; граф **сохраняется**, если не передан `--purge-graph`.

### Активация ≠ построение графа

`/amg on` (или согласие на активацию при установке) лишь поднимает флаг `active`; граф строит петля активации в **новой сессии** — перед первой задачей (при `automation: true`) либо по `/amg sync` (словами: «построй / синхронизируй граф памяти»). **После установки перезапустите сессию**: среда регистрирует скиллы и команду `/amg` при старте сессии, поэтому установочная сессия их ещё не видит — и первую сборку в ней запускать нельзя (без скилла `amg-bootstrap` модель импровизирует конвейер и теряет дисциплину оркестрации: партии, контрольные части, пакетное применение). Чтобы структурный скелет был готов уже в момент установки — флаг `--build` (смысловой слой всё равно добавит петля новой сессии).
...
| `/amg repair` | «почини / проверь граф памяти» | проиграть незавершённые записи, снять устаревшую блокировку (`recover` + `verify --repair`), затем отчётный аудит инвариантов (полная строка — «Ручные команды») |
...
| `/amg consult <запрос>` | «сверься с памятью с учётом нашей сессии» | консультация с памятью: копия помощника, унаследовавшая весь разговор, читает пакет у себя и возвращает заключение — что память добавляет / подтверждает / чему противоречит — не раздувая окно (субагент `amg-retriever-fork`; только Claude Code) |
...
| `/amg relink` | «досвяжи изолированные узлы памяти» | пересудить именно бродяг: номинация только для узлов без единой связи (`link_candidates.py --isolated`), их прежние отклонения переоткрываются — ответ на «во вьюере летают узлы, а `sync` говорит „нового нет“» (те пары уже рассужены; удаление `work/judged/` остаётся полным переоткрытием, крайняя мера) |
| `/amg version` | «какая версия AMG установлена» | версия установленного движка (печатается и в шапке status) |
...
| `/amg help` | — | полный список глаголов с назначением |

`/amg` — это «парадная дверь»: управляющие глаголы (`status`/`version`/`on`/`off`/`repair`/`help`) выполняет служебный скрипт `lifecycle.py`, а рабочие (`sync`/`retrieve`/`consolidate`/`relink`) делегируют профильным скиллам (те доступны и напрямую — `/amg-bootstrap` и т. д.). Важно: `on` лишь поднимает флаг — **граф строит `sync`**, а не `on`.

**Разделение `sync` и `consolidate` — по оси, а не по «структура/семантика», и друг друга они не вызывают.** `sync` отвечает за равенство графа **источникам** целиком: и за скелет, и за семантику изменившегося — правка содержимого файла при том же пути ловится контентным хешем, узел обновляется, сводка пересоставляется, связи достраиваются; отдельная консолидация после правки файлов не нужна. `consolidate` отвечает за **саму память**: веса из накопленных сигналов использования, отбор выводов диалогов в долгую память, слияние дублей, сжатие переполненных веток, арбитраж противоречий — узлы и рёбра он тоже меняет, но его вход — накопленный опыт (журналы, заметки, разросшиеся ветки), а не файлы проекта. Формула: `sync` — «граф ↔ файлы», `consolidate` — «граф ↔ его собственный опыт».

**Консолидация — гигиена, а не сохранение.** Пока вы её пропускаете, ничего не теряется: заметки фиксируются транзакционно в момент захвата, стенограмму выгружает хук в любом случае. Нужна она, чтобы память не теряла точность: без неё граф понемногу разрастается, а выдача из памяти становится шумнее. Три её фазы: **weights** — детерминированная, без модели: правит рёбра — усиливает те, у которых оба конца реально пригодились в принятой сессии, гасит те, что всплыли в выдаче, но не были использованы, прореживает совсем слабые; логика — «что помогло задаче — срастается, что показали и не пригодилось — ослабевает» (само обновление весов действует при включённом `weights.apply_hebbian`; по умолчанию копятся только счётчики ко-активаций); **plan** — только чтение: считает переполнение бюджетов веток, кандидатов в почти-дубликаты, значимость заметок и отдаёт план субагенту; **apply** — судейский проход (повысить / слить / свернуть эпизоды / ввести под-хаб / ужать / отправить в архив) с обратимым архивом оригиналов и защитой решений и ADR в коде, не только в промпте. Запускать её стоит после насыщенной решениями сессии или когда выдача из памяти заметно поплыла — не после каждой мелочи. Детерминированная фаза выполняется сама при каждом завершении сессии; судейская ждёт наблюдаемого триггера — ваших слов о завершении работы («заканчиваем работу», «подведём итоги и сохраним в память», `/amg consolidate`), а когда она пропущена несколько сессий подряд, начало следующей скажет об этом одной строкой и предложит её.
...
Когда AMG включён и `automation: true`, память ведёт себя сама — двумя дополняющими механизмами. **Сессионные хуки** (`SessionStart` / `SessionEnd` / `UserPromptSubmit` — механизм Claude Code, прописываются установщиком в `settings.json` каталога агента) выполняют детерминированную рутину без участия модели. **Блок активации в `CLAUDE.md`** ведёт ту часть петли, что требует суждения модели, — хук субагента запустить не может. При `automation: false` само не срабатывает ни то, ни другое (только команды и явные запросы). Шаги петли:
...
- **В начале сессии (`SessionStart`).** Хук исцеляет хранилище — проигрывает незавершённые записи прошлого прогона и снимает устаревшую блокировку (самовосстановление), — обновляет дайджест и, когда судейская консолидация пропущена несколько сессий подряд, вбрасывает однострочное напоминание. Затем модель выполняет свою **детерминированную стартовую проверку** — один бесплатный вызов `reconcile.py plan .`: он сводит структурный скелет с источниками (точно, без модели) и печатает, сколько осталось модельной половине. Если что-то изменилось, модель **спрашивает вас** — «в источниках N новых / M изменённых; синхронизировать смысловой слой сейчас (~K единиц) или отложить?» — и никогда не запускает платную половину молча; при отказе говорит одну строку и честно спросит снова в следующей сессии. Нулевой дифф — тишина, сразу к работе. Согласие запускает полную сверку (скилл `amg-bootstrap`; заметьте: сборку запускает **модель по петле**, а не хук — хук делает только детерминированные шаги, субагента он запустить не может): построение с нуля и пересборка — одна и та же операция, устойчивая к сбоям и идемпотентная. Сборка идёт слоями: детерминированный скелет со структурными рёбрами (импорты, вызовы, принадлежность, наследование — их извлекает код, точно и бесплатно), затем сводки изменившихся единиц параллельными сборщиками, затем хабы и **глобальная смысловая линковка** — отдельный проход, который связывает документы, примеры и код между собой по смыслу (без него они остались бы островами). Уже деривированное восстанавливается из кэша бесплатно, поэтому пересборка над неизменными файлами не тратит модель вовсе. Субагент `amg-synth` заодно выдаёт **отчёт о пробелах** `gap-report.md` — один из главных полезных выходов: недокументированный код (узлы кода без входящего ребра `documents`), рассинхронизированные с кодом документы и противоречия. Его стоит просмотреть после первой сборки и после крупных изменений; итоговое качество сборки видно строкой `connectivity` в `/amg status` (подробнее — «Качество и стоимость сборки» ниже).
- **Перед каждой задачей и при смене фокуса.** Прежде чем трогать код, модель собирает из графа пакет контекста по задаче (скилл `amg-retrieve`): засев → распространение активации → пакет под бюджет. Умолчание — **прямой вызов**: одна дешёвая команда `retrieve.py`, чей напечатанный пакет и становится рабочим контекстом; изолированный субагент `amg-retriever` — осознанное исключение для случаев, когда пакет не должен попадать в основное окно (вопрос-выжимка к памяти, уже перегруженный контекст), — субагент платит собственный фиксированный оверхед на каждом шаге, и если гонять через него каждое обращение, память ощущается дорогой и медленной. Выборка идёт **не только в начале**: когда по ходу диалога фокус смещается (новое требование, другой вопрос, другая подсистема), пакет пересобирается под текущий запрос. Сложный многотемный запрос **декомпозируется** — каждая самостоятельная часть получает своё обращение к графу по мере того, как модель к ней переходит, и собранные ветви вместе образуют контекст задачи. Граф — **первичный источник контекста**: исходники открываются точечно по указателям `путь:строка` из пакета, а прямой поиск по файлам — запасной ход для того, чего пакет не дал, и модель говорит об этом одной строкой; сплошной поиск по проекту исключает каталог агента (хранилище памяти — не исходники; его кэши и рабочие файлы только затопили бы результаты). Она же называет использованную память («пакет по X: N узлов»); что выборка действительно была, видно в `/amg status` (строка `last pack`), а сам пакет лежит в `cache/pack.md`. Весь проект в окно не загружается. **Профиль пакета подбирается под запрос**: точечный указательный вопрос («где лежит X», «в каком файле Y») выполняется с `--compact` — указательные строки вместо развёрнутых тел, примерно треть от полного пакета, — а вход в незнакомую тему остаётся за полным профилем: его глубина ровно то, зачем такой запрос и делается. А когда дисциплина петли ослабевает по ходу сессии, память замечает это **извне**: хук `UserPromptSubmit` вбрасывает одну короткую строку `AMG: …`, когда похожий на задачу промпт приходит при неопрошенной памяти (пакета в этой сессии не было либо последний старше ~15 минут; не чаще одного напоминания в ~10 минут), — на всех остальных промптах он молчит и ничего не стоит. Хук — механизм Claude Code; в Codex и других средах та же дисциплина держится на одном блоке активации.
...
- **В конце сессии (`SessionEnd`).** Хук детерминированно сворачивает журнал ко-активаций в веса (накопление `coact`; хеббовское обновление по умолчанию выключено — см. «Веса по умолчанию не обновляются» ниже) и обновляет дайджест. Хук также **выгружает стенограмму диалога** в каталог сессий (см. «Сессии» ниже) и записывает **провенанс использования** `work/usage.log` — какие узлы памяти были не просто извлечены, а реально использованы (их источник правился в сессии); это задел под честное обучение весов, консолидация его пока не трогает. Полную консолидацию с суждением — отбор ценных заметок в долговременную память и при необходимости компрессию раздутых веток (скилл `amg-consolidate`, субагент `amg-consolidator`) — ведёт модель: хук этого сделать не может, а момент конца сессии модели не виден, пока вы его не обозначили. Поэтому её триггер — **ваши слова о завершении работы** («подведём итоги», «на сегодня всё» — модель читает смысл на любом языке и при `automation: true` запускает проход); если проход всё же пропущен, начало следующей сессии напомнит о нём одной строкой, а `/amg consolidate` запускает его в любой момент. `SessionEnd` срабатывает на `/clear` и на выходе из среды (`exit`/logout — одинаково); при жёстком завершении (закрыли терминал, `kill`) — нет, и тогда это обслуживание просто догоняется на следующем старте (см. «Восстановление при сбоях»), а о залеченном сбое следующий старт дружелюбно сообщит. Тарифные лимиты хуку не помеха: это обычный скрипт без модели, токенов он не тратит — при штатном закрытии сессии перед самым лимитом дамп, веса и дайджест всё равно записываются. Не выполнится в такой ситуации только судейская половина (она модельная): это безопасно и неразрушительно — заметки уже зафиксированы, а отбор просто ждёт `/amg consolidate` в любой следующей сессии.
...
Диалог с моделью — это тоже память: решения и выводы, прозвучавшие в разговоре, пропали бы на `/clear`. Поэтому в конце сессии хук `SessionEnd` **выгружает стенограмму** в каталог сессий — **`<проект>/.claude/amg/sessions/ГГГГ-ММ-ДД-ЧЧММ.md`** — а дальше она попадает в граф как обычный источник.
...
**Куда попадает материал диалогов и почему повторы не копятся.** Выгрузка сессии нарезается на документ-секции по ходам — во вьюере они лежат в категории **документов**; категория **заметок** держит авторские захваты (`notes.py` — решения, выводы, открытые вопросы, планы). Обсуждение одного и того же в разных сессиях узлы не размножает: идентичный захват заметки контентно-адресуем (тот же id — дубля нет), почти-дубли эпизодических узлов сливает консолидация (по порогу похожести, так что перефразированные повторы сходятся за одну-две консолидации, а не мгновенно), а накопившиеся эпизоды чата сворачиваются в одну сводку, когда их ветвь перерастает бюджет. Туда же уходит и заметный расход токенов `/amg consolidate`: судейский шаг — проход сильной модели, которая взвешивает кандидатов плана (их сводки лежат прямо в плане) и читает тела только у финалистов, — это цена отбора в долговременную память.

> **Переносимость.** Выгрузка следует за тем, что среда даёт памяти увидеть от сессии. Claude Code выгружает один раз, при закрытии (хук `SessionEnd` и его формат стенограммы); в Qwen Code тот же хук подключён, формат стенограммы подтвердится живой проверкой; у OpenCode события конца сессии нет, поэтому AMG-плагин выгружает **инкрементально** — стенограмма переписывается в тихие моменты по ходу работы под одним стабильным для сессии именем; у Codex стенограмма есть, но сигналятся только концы ходов — выгрузка там пока не подключена. Везде «диалог не теряется» в конечном счёте обеспечивает фиксация заметок по ходу (`notes.py`) — заметки переживают даже жёсткое завершение. Где путь стенограммы известен, выгрузку можно вызвать и вручную: `lifecycle.py session-end --transcript <путь>`.
...
| `python .claude/skills/amg-bootstrap/scripts/partition_queue.py --priority [--usage] .` | режим ленивого обогащения: отделить приоритетную партию (структурную карту — модули, классы, файлы) от отложенного остатка; `--usage` дополнительно поднимает в приоритет узлы, реально использованные по `work/usage.log` |
...
| `... retrieve.py "<запрос>" --compact` | профиль указателей для точечного запроса («где лежит X»): встроенные скромные бюджеты, указательные строки вместо развёрнутых тел (`decision`/`adr` сохраняют мотивировку) — примерно треть полного пакета; вход в незнакомую тему остаётся за полным профилем |
...
**Пользуйтесь им как диагностическим инструментом, а не украшением.** Он даёт проект одним взглядом — хабы и ключевые узлы с их связями, тематические кластеры, различимые цветом, — и делает проблемы сборки буквально видимыми: **остров**, плавающий отдельно от основного графа, **изолированный узел** без связей, кластер документации, повисший в стороне от кода, который должен описывать. Каждый такой случай — пробел, который закрывает проход связывания (повторите `/amg sync`); строка `connectivity` в `/amg status` выносит тот же вердикт числом (см. «Качество и стоимость сборки»). Спорное знание тоже заметно на глаз: вердикты арбитража подсвечены цветом.

**Читайте легенду по категории содержимого, а не по папкам.** Цвета категорий хранения отражают, чем узел *является*, а не где лежал его файл и не политику источника: файл тестов — такой же **код**, как остальной код; впитанный markdown — **документ**, а не «данные»; **данные** — структурированные записи (JSON/YAML, листы CSV/XLSX); **заметки** — авторские захваты: решения, выводы, открытые вопросы (диалоги сессий нарезаются на документ-секции и лежат среди **документов**); **хабы** — синтезированный стратегический слой.
...
- **цвет узла — по категории хранения** (код / документы / данные / заметки / хабы); вердикты арбитража выделяются поверх: `disputed` — янтарный, `rejected` — красный, `superseded` — серый, `stale` — приглушённый; размер узла — по числу связей (хабы крупнее); толщина ребра — по весу `w` (той самой силе связи, что настраивает Хебб), конфликтные рёбра `contradicts`/`supersedes` — красные;
- **фильтры** по типу / статусу / категории, **поиск** по id и сводке, **раскраска по кластерам** (узлы одной темы-хаба получают общий цвет — так видно сообщества; кластер = самая весомая тема `part_of` узла) и переключатель светлой/тёмной темы;
...
и установить всё разом: `pip install -r requirements.txt`. Проверить кросс-язычность модели можно командой `python .claude/skills/amg-retrieve/scripts/embed.py`, а измерить, что даёт семантический засев на вашем графе, — `eval_retrieval.py --compare-embeddings` (выдача с эмбеддингами против чисто-лексической). Примечание: при бэкенде `sentence-transformers` загрузка модели может печатать предупреждение Hugging Face об отсутствии токена (`HF_TOKEN`) — оно безвредно и на работу не влияет: используемые модели публичны, токен для них не нужен.

**Скорость: transformer-бэкенд загружает модель в каждый вызов.** Каждый запуск `retrieve.py` — новый процесс, и `sentence-transformers` загружает в него torch и модель: рассчитывайте на секунды–десятки секунд на одно извлечение. Движок сначала пробует локальный кэш моделей (после первой загрузки сетевого похода нет); переменная окружения `HF_HUB_OFFLINE=1` закрепляет то же для любых бэкендов и версий библиотек. Когда скорость извлечения важна, предпочтите бэкенд **model2vec**: статические векторы грузятся почти мгновенно, мультиязычное умолчание есть, и в автоматическом порядке бэкендов он стоит первым — само его наличие делает извлечение быстрым, а `--compare-embeddings` покажет, что́ по качеству добавила бы тяжёлая модель именно на вашем графе.
...
Качество извлечения измеряется, а не подбирается на глаз. Разметьте горсть реальных задач (`cases.json` со списком `{id, query, gold_ids}`; идентификаторы узлов берутся из `inspect_graph.py`; держите файл **под хранилищем** — `.claude/amg/evals/cases.json`, умолчание `eval_gate.cases`: обновление движка целиком заменяет дерево скиллов и стёрло бы разметку, лежащую там, а хранилище не трогается никогда) и прогоняйте `eval_retrieval.py` — он сравнивает выдачу с лексической базой по полноте, точности и **hop-recall** (полнота на узлах, достижимых только по связям, — то, что добавляет распространение активации над плоским поиском). По числам крутите ручки в `config.yml → retrieval`: `damping` (дальность охвата), `activation_threshold` (плотность пакета: **доля от максимальной активации**, ниже которой узел в пакет не берётся, — перед сборкой пакета активации нормируются к максимуму, поэтому порог не зависит от размера графа и пакет не пустеет на большом), `token_budget` по ярусам (он задаёт размер **полного** профиля; профиль указателей `--compact` для точечного запроса намеренно держит собственные встроенные скромные бюджеты, так что эти ручки его не раздувают), `relation_priors` (как сильно проводит каждый тип ребра). Безопасность компрессии проверяется **автоматически**: при консолидации полнота меряется тем же измерительным стендом на *копии* графа до и после сжатия, и компрессия вносится только при её удержании (автоматическая проверка полноты, блок `eval_gate` в `config.yml`; `on_fail` задаёт реакцию на просадку — `reject`/`warn`). По умолчанию она безопасно неактивна, пока вы не разметите свои случаи и не укажете на них `eval_gate.cases`.

**Обучение весов связей (`weights.apply_hebbian`).** По умолчанию **выключено**, и для большинства проектов так и стоит оставить. Включать (`apply_hebbian: true`) имеет смысл, когда совпали условия: граф **большой и разрежённый** (ответы часто собираются многошагово — нужный узел в нескольких связях от запроса, а не находится словами напрямую); запросы бывают **по смыслу или кросс-язычными** (включены эмбеддинги); и в проекте **накопился реальный `usage.log`** — то есть вы действительно работаете через память сессиями, правя файлы. Тогда правило усиливает связи, которые **реально приводили к полезной работе**, и гасит те, что лишь мелькали в выдаче. На малом или простом проекте, при простых словарных запросах или в сомнении — оставьте `false`: выигрыша почти не будет. Включение **безопасно и модельных токенов не тратит** (свёртка весов — скриптовая работа в конце сессии, без модели): без накопленного `usage.log` правило вообще ничего не делает, а когда действует — усиливает только по реальному исходу и не разгоняет связи без меры. Подтвердить пользу на своём графе — тем же `eval_retrieval.py` (полнота при `apply_hebbian` off против on). Для ориентира — два замера, задающие ожидание. На стенде большого **разрежённого** графа улучшенное правило за несколько свёрток подняло полноту примерно с `0.60` до `0.85`, а hop-recall — с `~0.25` до `~0.70` (по всем засевам), но с синтетическим сигналом исхода (он доказывает корректность правила, а не среднюю пользу). А на реальном **плотно связанном** графе проекта (~1100 узлов, ~5000 рёбер) с настоящим `usage.log`, накопленным живыми сессиями, флип флага и свёртка всего накопленного не сдвинули ни полноту, ни hop-recall, ни полноту пакета — ни в одну сторону: тот самый плотный режим, который предсказывает теория (исход решает засев, веса инертны; [§8.2](./THEORY.md)). Вместе оба замера дают правилу окончательную практическую форму: флип безвреден везде, полезен только там, где ответы действительно идут многошагово по разрежённому графу, и бесполезен на хорошо связанном — потому дефолт и остаётся `false`.
...
**Связность видна числом.** После сборки строка `connectivity` в `/amg status` (или `reconcile.py metrics`) показывает, получился ли один связный граф: число компонент, долю крупнейшей, висячие цели рёбер и doc-файлы без ребра `documents`. Висячие цели разведены по ответственности: **структурные** (собственные рёбра движка — на свежей сборке их ноль по построению) входят в вердикт, а **модельные** (слой суждения написал цель с несуществующим слагом) только репортятся с образцами — такие рёбра инертны, извлечение их отбрасывает, и автоматического лекарства у них нет, поэтому держать из-за них вечный `attention` было бы ложной тревогой. Вердикт `ok` — всё в порядке; `attention` — обычно достаточно повторить глобальную линковку (`/amg sync` доведёт), а образцы проблемных рёбер команда печатает сразу. Висячие ссылки на stdlib и сторонние библиотеки — норма, они не считаются: память честно фиксирует факт импорта, не притворяясь, что знает чужой код.

**Пересборка бесплатна.** Однажды написанные сводки и рёбра сохраняются в кэше обогащения (`cache/derivations/`) под хешом содержимого: пересборка графа с нуля — после `git clone`, чистки, эксперимента — восстанавливает их дословно, не обращаясь к модели, и платит только за действительно новый контент. Заодно это даёт воспроизводимость: тот же вход — тот же граф. Смена `working_language` кэш честно обесценивает (сводки на другом языке не подставятся); принудительно передеривировать всё — ключ `derivation_cache: false` или удаление каталога (например, после перехода на более сильную модель).

**Связи между доменами достраивает линковка — и она сходится.** Документы, примеры и код сводятся вместе отдельным проходом: код предлагает каждому узлу похожих кандидатов из других доменов (по тем же векторам, что и семантический засев; без бэкенда — по совпадению слов), а модель лишь подтверждает реальные связи. Ложная связь хуже пропущенной, поэтому сомнительные кандидаты отклоняются — и отклонения **запоминаются**: полностью рассуженная пачка кандидатов уезжает в `work/judged/`, и её пары больше не предлагаются (без этой памяти повтор прохода сползал бы по ранжиру похожести, заново отклоняя те же пары до бесконечности). Повтор прохода поэтому достигает «нуля новых пачек», а не бесконечного хвоста; переноминируется только действительно оборванная пачка. Сознательно переоткрыть старые суждения — удалить файлы из `work/judged/`: это учёт, как и всё под `work/`.

**После сборки верификация штампуется одним свипом.** Свежевыведенная сводка совпадает со своим источником по построению, поэтому закрывающий проход `verify_claims.py --all --write` (секунды, без модели — сверка хешей по файлам) переводит свежие узлы в `verified`. С этого момента пометка `unverified`/`stale` в пакете означает «изменилось с последней сверки» — сигнал, который действительно различает, а не горит на каждом узле.

**Сборка экономна по построению.** Токены при сборке уходят в основном не на сами сводки, а на лишние перечитывания и пересылку контекста — и ровно это устранено. Текст каждой единицы (функции, раздела, записи) кладётся прямо в очередь работ, поэтому сборщики пишут сводки из очереди и не открывают исходники файл за файлом; тривиальные функции — дандеры и однострочные геттеры — сводятся кодом вообще без модели (их сводкой становится их собственная строка кода; порог задаёт ключ `trivial_unit_max_lines`, `0` — выключить); всё однажды выведенное восстанавливается из кэша обогащения бесплатно. Качество при этом не срезается: содержимое сводок то же, экономится только повторная работа.

**Сборка устойчива к обрывам и честно рапортует.** Очередь дробится на ограниченные партии (блок `builder` в `config.yml`), и каждый исполнитель — сборщики, глобальный связыватель, синтез — записывает результат частями, контрольными записями по мере готовности, — поэтому лимит, разрыв связи или закрытый терминал теряют максимум несколько последних единиц, а не часы работы. Следующий запуск сначала применяет уцелевшие части — один вызов `reconcile.py apply-derived .` потребляет все файлы результатов разом и убирает применённые в сторону, — потом доводит остаток (недоведённая единица сама возвращается в очередь). Агент обязан отличать успех от обрыва — вместо ложного «готово» он отчитывается «выведено N из M», и система сверяет эти числа с фактически применённым. Полезно знать привычку среды: метка «Done» у агента в интерфейсе означает лишь, что его запуск **завершился**, а не что он преуспел, — честный сигнал это финальная строка агента (`BATCH COMPLETE: N/M` или `BATCH PARTIAL: N/M`) и сошедшиеся счётчики; оборванную партию оркестратор называет явно и заново выполняет только её остаток. Ход сборки виден в процентах: `inspect_queue.py .` показывает, какая доля узлов уже выведена, и оркестратор сообщает её после каждого раунда применения и после каждой волны линковки.

**Во сколько обходится полная сборка — и где проходит её пол.** Экономия выше устраняет класс **повторной** работы — перечитывание источников, переотправку содержимого, переделку потерянного, — и именно этот класс сжимается в разы и десятки раз. Но у стоимости есть и неустранимый пол, который задаёт сама агентная среда: каждый шаг каждого субагента заново отправляет модели системный промпт, схемы инструментов и весь накопленный вход, поэтому на полной сборке реального проекта (порядка мегабайта исходников) счёт **чтения кэша** идёт на миллионы токенов при любом устройстве конвейера — и тарифные лимиты считают его наравне с остальным. В панели расхода поэтому стоит смотреть на две цифры порознь: **некэшевые** токены (ввод, вывод, запись кэша — реальная новизна; у экономной сборки их на порядок меньше, чем у наивной) и **чтение кэша** (пошаговая переотправка среды — пол, одинаковый для любого конвейера, исполняемого субагентами). Рычаги, которые реально снижают счёт:

- **недорогая модель самой сессии сборки.** Главная сессия только координирует — готовит партии, запускает волны исполнителей, применяет результаты, — но каждый её ход переотправляет весь её накопленный контекст, и в полевом замере на неё пришлось около двух третей всего расхода. Понижение модели именно этой сессии не трогает качество сводок: их пишут субагенты со своими моделями из блока `models`;
- **тиринг моделей** (`config.yml → models`): `discovery` — дешёвый, `synthesis` — сильный; `module_summary` без замера не понижать — его сводки питают лексический и семантический засев;
- **размер партий** (`builder.batch_units` 60, `linker.batch_nodes` 80) уже выбран около оптимума: накладные расходы среды съедаются укрупнением партий, но дальше растить их почти бессмысленно — экономия сходит на нет, а цена потери партии при обрыве растёт (страховку держат контрольные части по ~10 единиц);
- **`derivation: lazy`** — для очень большого графа, который опрашивается лишь частично (см. «Ленивое обогащение»).
...
## Файлы каталога памяти

Всё, что память хранит, лежит в одном каталоге (`.claude/amg/`), и ничего непрозрачного там нет. Правило простое: **канон** — узлы, конфигурация и ваши собственные размеченные данные — удалять нельзя; всё **производное** (каталоги `work/` и `cache/`, пустующий журнал) восстановимо, и удаление стоит максимум пересчёта. Полная опись:

| Путь | Что хранит и показывает | Можно ли удалять? |
|---|---|---|
| `config.yml` | конфигурация и главный выключатель памяти (`active`) | нет — канон |
| `nodes/<категория>/*.md` | узлы графа — сама память, по файлу на узел | нет — канон |
| `digest.md` | always-on дайджест: самые значимые решения и открытые вопросы, грузится в каждую сессию | перегенерируется следующей консолидацией |
| `gap-report.md` | отчёт о пробелах после сборки: недокументированный код, разъехавшиеся ссылки документации, противоречия | да — информационный; перезапишет следующий синтез |
| `arbitration.md` | аудит арбитража: каждый вердикт по противоречию с основанием и сравнёнными источниками | удаление теряет историю аудита |
| `actions.log` | плоский журнал операций (дедуп по txid; ротируется в `archive/`) | ротация автоматическая; удаление теряет локальную историю |
| `journal/` | транзакции упреждающей записи; в покое пуст | нет — им управляет восстановление |
| `LOCK` | единственная блокировка на запись | управляется сам (`/amg repair` снимает зависшую) |
| `sessions/` | авто-выгруженные стенограммы сессий, впитываются как источник | при `absorb` дистиллят переживает удаление; при `mirror` удаление вычищает узлы |
| `evals/cases.json` | ваши размеченные eval-случаи — ими взводится предохранитель полноты | нет — ваши данные (переустановка движка хранилище не трогает) |
| `work/queue.json` | семантическая очередь: единицы, ждущие обогащения | перестроит следующий `plan`/`bootstrap` |
| `work/queue-*.json` | партии сборщиков, нарезанные из очереди | перегенерирует `partition_queue` |
| `work/derived-*.json` | контрольные части исполнителей, ждущие применения | потребляет `apply-derived`; удаление теряет неприменённые суждения |
| `work/applied/` · `work/invalid/` | применённые части; карантин рваных файлов | да — учёт |
| `work/judged/` | полностью рассуженные пачки линковки — **память отклонений** связующего прохода | удаление сознательно переоткрывает суждения (пере-суд за модельную цену) |
| `work/link-batch-*.json` | пачки номинаций линковки | перегенерирует `link_candidates` |
| `work/hub-candidates.json` · `work/synth-input*.json` | подготовленные входы глобальных проходов (якоря хабов; лист синтеза, при превышении капа — части `-pNN`) | перегенерируются |
| `work/consolidation-plan.json` · `work/actions.json` | план консолидации; действия судьи, ждущие применения | план перегенерируется; неприменённые действия — оплаченный вывод судьи |
| `work/eval-gate-report.json` | последний вердикт предохранителя полноты (дельты, регрессии, атрибуция) | да — информационный |
| `work/pack-log.jsonl` | пакеты этой сессии (атрибуция использования; потребляется в конце сессии) | управляется сам |
| `work/coactivation.log` | сигнал показов до следующей свёртки весов | потребляет свёртка |
| `work/usage.log` | сигнал исхода (узлы, со-использованные в принятых сессиях) — честный субстрат Хебба | потребляется при включённом `apply_hebbian`; иначе копится |
| `work/hint-stamp` · `work/sync-defer.json` | кулдаун напоминания; зафиксированное откладывание sync | управляются сами |
| `cache/pack.md` | последний собранный пакет контекста — ровно то, что получила модель | одноразовый |
| `cache/embeddings.json` | кэш векторов узлов для семантического засева | одноразовый — перекодируется по требованию |
| `cache/index.sqlite` | производный read-индекс под `load_nodes` | одноразовый — перестроится при следующем чтении |
| `cache/graph.html` / `graph.json` | экспорт 3D-просмотра / сырой JSON | одноразовые |
| `cache/derivations/` | кэш обогащения: применённые сводки/рёбра по контентному хешу — пересборка восстанавливает их дословно | удалять можно ценой повторного обогащения |
| `archive/` | оригиналы компрессии (обратимое забывание) + ротированные журналы `coactivation-*` / `usage-*` | удаление теряет возможность восстановления и реплея |
