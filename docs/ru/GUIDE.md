...
*Силами модели (проще).* В сессии внутри проекта скажите «**установи AMG по `<путь-к-AMG>/INSTALL.md`**» (путь — к распакованному вне проекта каталогу AMG) — модель прочитает инструкцию, проведёт диалог (локально/глобально; каталог агента и точка входа; зеркала и впитывание, включая замороженные снимки; что игнорировать; рабочий язык; бэкенд эмбеддингов; политика сессий; бюджеты ярусов; автоматика; активировать ли память), покажет действующие значения, если конфиг уже существует, и сама вызовет установщик с ответами.
...
- [INSTALL.md](../../INSTALL.md) (в корне репозитория) — установка силами модели или командой, переустановка, удаление.
