...
- **В конце сессии (`SessionEnd`).** Запускается консолидация (скилл `amg-consolidate`): хеббовское укрепление и затухание весов, отбор ценных заметок в долговременную память, при необходимости — компрессия раздутых веток. Здесь же текущий диалог выгружается в каталог сессий (см. ниже).
...
| Команда | Действие |
|---|---|
| `python .claude/skills/amg-consolidate/scripts/consolidate.py weights .` | свернуть журнал ко-активаций в веса (Хебб + затухание + обрезка) |
