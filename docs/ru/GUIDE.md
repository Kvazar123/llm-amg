...
- **`derivation: lazy`** — для очень большого графа, который опрашивается лишь частично (см. «Ленивая деривация»).
