...
Диалог — это тоже память: решения и выводы, прозвучавшие в разговоре, пропали бы на `/clear`. Поэтому в конце сессии хук `SessionEnd` **выгружает стенограмму** в каталог сессий — **`<проект>/.claude/amg/sessions/ГГГГ-ММ-ДД-ЧЧММ.md`** — а дальше она ингестируется как обычный источник.
...
- вызовы инструментов, их результаты и вложения не воспроизводятся, а считаются (маркер `== Attachments N ==`); служебные обёртки (слэш-команды, `!`-bash) отфильтровываются. В дампе остаётся суть разговора, а не его механика.
