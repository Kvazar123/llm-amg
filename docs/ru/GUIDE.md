...
Устанавливать можно **локально** (в один проект) или **глобально** (движок один на все проекты). В обоих случаях **граф всегда локальный** — он лежит в `<проект>/.claude/amg/`, потому что память относится к конкретному проекту. Ставит всё установщик `install.py`: его ведёт сама модель по фразе «установи AMG по INSTALL.md» либо запускают командой. Он размещает движок, дописывает блок AMG в корневой `CLAUDE.md` между маркерами `<!-- AMG:BEGIN -->` и `<!-- AMG:END -->` (ваши инструкции остаются первыми; переустановка заменяет только блок и не трогает граф), пишет `config.yml` и проверяет хранилище. Полный порядок, переустановка и удаление — в корневом [INSTALL.md](../../INSTALL.md).

**Активация не строит граф.** `/amg on` (или согласие на активацию при установке) лишь поднимает флаг `active`; граф строит петля активации — в новой сессии перед первой задачей (при `automation: true`) или сразу по `/amg sync` (словами: «построй / синхронизируй / проиндексируй граф»). При установке можно выбрать немедленную сборку, чтобы не ждать новой сессии.
...
> **Среды кроме Claude Code.** Переносимость шире замены имени папки. Слэш-команды `/amg`, хуки и `@`-импорт дайджеста — механизмы именно Claude Code, и в других средах (Codex, Qwen Coder и прочие, читающие `AGENTS.md`) их может не быть. Для них установщик ставит память флагом `--env generic`: вместо скиллов/хуков вливается **переносимый блок без скиллов** `AGENTS.md` — та же петля прямыми вызовами скриптов (промпты `agents/*.md` модель читает как инструкцию, дайджест — сама, исцеление и консолидацию запускает сама). Базовая работоспособность от среды не зависит, набор удобств — зависит. **Но этот режим в Codex / Qwen Coder и любой иной не-Claude-Code-среде пока не протестирован и стабильность не гарантирована** — все тесты в Claude Code (проверка — Этап 19 дорожной карты). Имена `.claude` / `CLAUDE.md` здесь и далее — умолчания Claude Code; в иной среде подставляются её имена.
