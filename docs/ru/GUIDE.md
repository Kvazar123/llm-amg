...
- **`absorb_path` — впитывание один раз.** Источник вбирается в независимые узлы; удаление источника позже узел не вычищает. Для разового материала (логи чатов, дампы данных).
