...
> **Планируется (Этап 8).** Команды `/amg` и флаг `automation` — часть слоя жизненного цикла, который ещё реализуется ([дорожная карта](./architecture/11-roadmap.md)). Сегодня память включается наличием `config.yml` с `active: true`, а петлю ведёт блок активации в `CLAUDE.md`.

## Что происходит автоматически

Когда AMG включён, модель ведёт память по петле — без ручных команд. Сегодня петлю исполняет блок активации в `CLAUDE.md` (модель сама выполняет шаги); автоматический запуск по хукам `SessionStart` / `SessionEnd` — на [дорожной карте](./architecture/11-roadmap.md) (Этап 8). Шаги петли:
