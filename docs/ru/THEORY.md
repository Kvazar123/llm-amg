...
Сила связи `w_s ∈ (0, 1]` живёт на рёбрах (не на папках) и меняется по правилу **Хебба** — «что срабатывает вместе, то связывается» (neurons that fire together wire together). В переводе на наш случай: если два узла часто активируются в одних и тех же запросах, связь между ними усиливается; если связь долго не используется — слабеет.

Непрерывная форма — «дырявый интегратор» (leaky integrator):
...
Рёбра, упавшие ниже порога (`prune_below: 0.05`), **обрезаются** — это аналог синаптического прунинга (отмирания неиспользуемых синапсов). Новые рёбра стартуют с `default_edge_weight: 0.5`. У членства `part_of` есть дополнительное ограничение-симплекс (сумма весов по родителям ≤ 1, ключ `part_of_renormalize: true`), чтобы «степень принадлежности» узла к темам оставалась согласованной.

**Две скорости.** Чтобы извлечение оставалось быстрым и без блокировок, совместная активация (ко-активация) при поиске **не пишется** в граф немедленно — она лишь дописывается в отдельный журнал (только добавление). Обновление весов по Хеббу, пассивное затухание и обрезка применяются **позже**, на консолидации (§13; реализация — [Консолидация](./architecture/07-consolidation.md)). Так граф адаптируется к тому, *как проект реально запрашивают*, без ручного вмешательства: часто используемые вместе узлы со временем стягиваются (связь крепнет), а узлы, которые перестают спрашивать, отдаляются (связь слабеет и может быть обрезана).
...
flowchart LR
    ...
    S3 --> S4["лоссово<br/>укоротить"]
    S4 --> STOP(["стоп — под бюджетом"])
    CHK -->|нет| STOP
```

1. `summarize_episodes` — свернуть устаревшие эпизоды (старые заметки с низкой активностью) в один узел-сводку; сырьё — в архив. Это *семантизация*: суть остаётся, эпизодические детали уходят (схемы Бартлетта, 1932). Решения и обязательства сюда не попадают.
...
- лоссовый шаг всегда сначала **архивирует оригинал** (`archive_dir: archive`): деталь уходит в архив, в графе остаётся суть со ссылкой; восстановление возможно;
- каждое сжатие логируется; его эффект можно проверить eval-харнессом — измерением полноты извлечения до и после (автоматический гейт, ослабляющий порог при просадке полноты, предусмотрен [дорожной картой](./architecture/11-roadmap.md)).
...
**Захват (capture)** и **консолидация (consolidation)** — разные операции, и их нельзя путать. *По ходу сессии* выполняется только дешёвый захват в заметки, без тяжёлой правки графа. *Шаг консолидации* (конец сессии, по запросу или асинхронно) выполняет всю «хирургию»:

```mermaid
flowchart LR
    CAP["захват по ходу<br/>дёшево, в notes"] --> END([конец сессии])
    END --> HEB["веса по Хеббу<br/>+ затухание + обрезка"]
...
- **Проверяемо и проверено:** слой согласованности и крэш-безопасности (атомарные записи, журнал упреждающей записи с декларативным повтором, единый лок на запись, чтения без блокировок). Эта часть допускает доказательство и прогнана тестами (см. [Хранилище и транзакции](./architecture/03-storage.md) и формальную модель `consistency-model.md`).

Существенно, что всё перечисленное **измеримо**. Eval-харнесс (набор размеченных запросов с эталонными узлами) задаёт multi-hop вопросы и проверяет, попали ли в собранный пакет нужные узлы (инструменты и метрики — [Измерение и инструменты](./architecture/10-eval-tools.md)). Метрики:
...
- *объяснимость* — путь активации можно инспектировать, векторную близость — нет;
...
- слой согласованности и крэш-безопасности, которого у вики нет;
