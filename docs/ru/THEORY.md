...
Важная оговорка о статусе вывода. В обоих замерах сигнал исхода был *синтетическим*: «золотые» узлы каждого случая считались «использованными» при успехе. Это честно проверяет, что правило **корректно** и **полезно, когда сигнал использования точен**, но не доказывает средней пользы при *настоящем* `usage.log`, который шумнее. Поэтому — а также потому, что свёртка весов, в отличие от компрессии, не прикрыта автоматической проверкой полноты, — дефолт `apply_hebbian` остаётся **`false`**: правило поставляется выключенным, безопасно к включению (без журнала использования оно вовсе бездействует, а действуя — gated по исходу и ограничено потолком) и рекомендуется к включению на проекте, где накопился реальный провенанс использования. Перевод дефолта в `true` ждёт измерения на реальном usage.
