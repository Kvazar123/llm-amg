...
- [4. Узел и ребро: модель данных](#4-узел-и-ребро-модель-данных)
  ...
  - [4.3. Воспроизводимость сборки: кэш обогащения](#43-воспроизводимость-сборки-кэш-обогащения)
...
### 4.3. Воспроизводимость сборки: кэш обогащения

Суждение модели недетерминировано: одна и та же единица, поданная дважды, получает разные сводки, и «граф каждый раз другой» — даже при неизменных исходниках. Снижать разброс настройками генерации можно, но слабее и дороже, чем просто **не спрашивать дважды**: однажды принятое обогащение (сводка, рёбра, уверенность) сохраняется под ключом из хеша содержимого единицы, и пересборка с нуля восстанавливает её дословно. Тот же вход даёт тот же граф — воспроизводимость становится практическим свойством, а повторная сборка почти ничего не стоит: платится только за действительно новый контент. Ключ включает язык сводок и версию контракта обогащения — смена любого из них честно обесценивает кэш, вместо того чтобы молча вернуть сводки не на том языке или не той формы.
...
Существенно, что вся эта экономия берётся из устранения **повторной** работы — перечитывания, переотправки, переделки потерянного, — а не из урезания смысла: содержимое сводок и связей не меняется, как и у кэша обогащения (§4.3), удешевляющего повторную сборку тем же способом — не спрашивая дважды.
...
Что сборка всё же предлагает — это второй **профиль**, и выбирает его вызывающий, а не статистика. Точечный указательный вопрос («где лежит X») и вход в незнакомую подсистему — разные запросы с разной потребностью в объёме, но изнутри вектора активаций эта разница не видна: засеянная голова и питающийся притоком хвост перекрываются по значениям, скаляр активации не хранит, *откуда пришла его масса*, а действительно релевантные многошаговые узлы ранжируются сколь угодно глубоко (размеченное полевое gold наблюдалось и за двухсотым рангом). Любая автоматическая отсечка, достаточно резкая, чтобы ужать указательный запрос, ампутировала бы тот самый многошаговый хвост, ради которого граф и существует. Единственное место, где различие существует, — голова вызывающего: он знает, какой вопрос задаёт. Отсюда **компактный профиль** — фиксированные скромные бюджеты и указательные строки вместо развёрнутых тел (только записанные решения сохраняют мотивировку), — выбираемый явным объявлением намерения. Это то же разделение труда, что и всюду в AMG: смысл (что за запрос) — за моделью, механика (как собирается каждый профиль) — за кодом.
...
Начало сессии — не единственное место, где утекает внимание. Полевое наблюдение показывает, что дисциплина извлечения *затухает по ходу сессии*: модель начинает с графа и постепенно сползает обратно к сплошному поиску по файлам, и никакая инструкция не чинит это изнутри — собственный дрейф не виден тому, кто дрейфует. Поэтому принцип внешнего триггера, стоящий за дайджестом, продолжается и в середину сессии — **напоминанием со строгим условием срабатывания**: одна строка, приходящая вместе с промптом пользователя, и только когда память доказуемо не опрашивалась, а похожие на задачу промпты продолжают идти. Именно условие срабатывания и отличает его от назойливого напоминания на каждый промпт: безусловное напоминание становится шумом, который модель научается игнорировать, тогда как строка, появляющаяся ровно в момент провала петли, остаётся сигналом (условия срабатывания и механика — слой жизненного цикла, там же).
...
Корень различия в том, что **похожесть — ещё не связь**. Всё решение RAG об извлечении опирается на косинусную близость, а близость — слабый заместитель связи: по замеру на живом графе проекта пары-кандидаты с косинусом выше 0,8 подтверждались суждением модели как действительно связанные лишь примерно в четверти случаев — верхнюю полосу заполняют двойники (почти совпадающие ходы разных диалогов, одноимённые методы разных классов), — а среди всех номинированных похожестью кандидатов суждение отклоняет 60–70 %. Поэтому AMG отдаёт похожести ровно один голос и никогда — вердикт. На сборке она *номинирует*: структурные рёбра извлекаются из синтаксиса детерминированно (Python — штатным `ast`, прочие языки — через tree-sitter, документы PDF/DOCX/XLSX — своими экстракторами), а семантических кандидатов, предложенных близостью, суждение подтверждает, типизирует и взвешивает — или отклоняет (§4.2). На запросе она лишь *засевает*: косинус задаёт стартовую релевантность, дальше активация идёт по явным рёбрам — и в окно попадает релевантная окрестность, включая узлы, не разделяющие с запросом ни одного слова и ни одного вектора (§6.4). Это и есть многошаговость, которой у RAG нет в принципе: у него нет фильтра суждения над «связанностью», нет детерминированных структурных рёбер, по которым можно идти, и нет типизированных взвешенных связей с проводимостью по типу — у ручной wiki явные ссылки есть, но нетипизированные, невзвешенные и проходимые только вручную.
