...
- **`codex`** — OpenAI Codex with skills, subagents, and a core hooks engine: the skills go to `.agents/skills`, the subagents as **TOML in `.codex/agents`** (with `model`/`model_reasoning_effort` from the `models` block), the skill-aware `AGENTS.md` block is injected, and two hooks merge into `.codex/hooks.json` — the session start check and the gated mid-session reminder. One step is yours: Codex runs unmanaged hooks **only after you trust them via `/hooks`** (a reinstall that changes a hook asks for the review again); until then the block's start check covers the session halves. Codex has no session-end event — the wrap-up stays on your signal — and no custom-command surface: type the verbs in words, or reach the skills from the `$` popup.
- **`opencode`** — OpenCode discovers the skills in `.agents/skills` **natively**; the subagents render to `.opencode/agent/*.md` (`mode: subagent`; a real model id from `models` passes through, a Claude alias is omitted — OpenCode model ids are `provider/model` strings), the portable skill-aware block is injected, `/amg` autocompletes from `.opencode/command/amg.md`, and the **AMG event plugin** (`.opencode/plugin/amg.js`) stands in for hooks: it heals the store and asks the sync question as a session opens, nudges once when the memory has demonstrably gone unconsulted, and keeps an **incremental transcript dump** — the dialogue lands in the sessions folder as you work and is re-written on quiet moments, so even a killed terminal loses at most the last couple of minutes.
- **`qwen`** — Qwen Code, preset `.qwen` / `QWEN.md`: the skills go to `.qwen/skills`, the worker prompts land in `.qwen/agents/*.md` — Qwen's **native markdown subagents** — with the Claude-only frontmatter sanitized (`tools`/`effort` dropped; a Claude-alias model dropped, a real id such as `qwen3-coder-plus` passes through), `/amg` autocompletes from `.qwen/commands/amg.md`, and the session hooks merge into `.qwen/settings.json`, which Qwen reads in the same shape Claude Code does.
...
Baseline functionality does not depend on the environment; the set of conveniences does. **Claude Code is the fully tested environment.** OpenCode's full memory cycle — install, build, retrieval, notes, consolidation — has been reproduced on live field runs (the freshly added plugin and command await theirs); the Codex and Qwen Code modes are wired from their verified surfaces but **not yet confirmed live**. Verifying these environments is the roadmap stage now in progress.
...
> **Portability.** The dump follows what each environment lets the memory see of a session. Claude Code dumps once, at close (the `SessionEnd` hook and its transcript format); Qwen Code gets the same hook wired, with its transcript format to be confirmed live; OpenCode has no end-of-session event, so the AMG plugin dumps **incrementally** — the transcript is re-written at quiet moments as you work, under one session-stable filename; Codex keeps a transcript but signals only turn ends — a dump is not wired there yet. Everywhere, "the dialogue is not lost" is ultimately ensured by note capture along the way (`notes.py`) — notes survive even a hard kill. Where a transcript path is known, the dump can also be invoked by hand: `lifecycle.py session-end --transcript <path>`.
...
**Connectivity is a number.** After a build, the `connectivity` line in `/amg status` (or `reconcile.py metrics`) shows whether one connected graph came out: the component count, the largest one's share, the dangling edge targets, and the doc files without a `documents` edge. Dangling targets are split by responsibility: the **structural** ones (the engine's own edges — zero on a fresh build by construction) enter the verdict, while the **model-written** ones (the judgment layer wrote a target with a nonexistent slug) are only reported with samples — such edges are inert, retrieval drops them, and no automatic remedy exists, so holding a permanent `attention` over them would be a false alarm. The verdict `ok` — all is well; `attention` — usually rerunning the global linking is enough (`/amg sync` finishes the job), and the command prints samples of the problem edges right away. Dangling references to the stdlib and third-party libraries are normal and not counted: the memory faithfully records the fact of an import without pretending to know foreign code.
