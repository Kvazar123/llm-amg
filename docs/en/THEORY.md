...
For the pass to **terminate**, one more memory is required — the memory of rejections. The graph itself remembers only the confirmed half of every judgment: a rejected candidate leaves no trace, so a re-run would nominate it again — and, with the confirmed pairs excluded, similarity would slide down its own ranking to ever-weaker candidates, wave after wave, re-rejecting the same pairs at full price. Remembering what was *ruled on* (not only what was accepted) turns "repeat until nothing new" from an unreachable ideal into the pass's actual stopping condition. This is the same economy as the derivation cache (§4.3) — never ask twice — extended from generation to judgment: a verdict once paid for is kept, and only genuinely new pairs reach the judge. The memory is bookkeeping, not knowledge: discarding it re-opens the judgments at the cost of re-judging, never at the cost of correctness.
...
The same re-send arithmetic governs the **orchestrator's own turns** and the **global passes**, not just the per-unit workers. Every command the orchestrator runs is a turn that re-sends its whole accumulated conversation — so the number of turns is the same currency as the volume of data. Results produced as many durable portions are therefore folded in by **one batched call**, never one command per portion, and a round of workers is launched together, not one by one: with the coordinator, saving turns *is* saving tokens. And a global pass — the synthesis that names the topic layer, the judgment that consolidates memory — receives its whole input as **one deterministically prepared file**: the summary layer, distilled and grouped by code, arrives in the assignment instead of the agent walking the store file by file in its own context, where each next read re-sends everything read before it. Global reach is delivered by preparation, not by accumulation inside the agent.
...
**Convergence** is guaranteed: for `d < 1` the iteration is a contraction mapping (the Perron–Frobenius theorem), so a few passes converge to a unique `π` regardless of the starting point. In the config, the number of passes is bounded from above (`max_hops: 30`), and nodes below the activation threshold (`activation_threshold: 0.02`) are dropped — the threshold is a **share of the top activation**, not an absolute value: `π` sums to 1 over the whole graph, so absolute activations shrink as the graph grows while the ranking stays meaningful, and only a relative cutoff is scale-free. The implementation of seeding, the walk, and pack assembly — [Retrieval](./architecture/06-retrieval.md).
...
The budgets are also deliberately the **only** size lever. A tempting refinement — stop assembling once the ranked prefix holds "most" of the activation mass — fails against a measured property of the stationary distribution itself: on a connected graph PPR mass is *spread by construction* (it sums to 1 across all nodes, and conductance keeps the tail warm), so the prefix never concentrates enough for any mass cutoff to separate signal from tail. Focus lives in the *ranking*, not in the mass profile; volume is governed honestly by the budgets and the relative threshold, and tightening the threshold trims the pack only at the point where it starts costing recall.

What assembly does offer is a second **profile**, and its chooser is the caller, not a statistic. A targeted pointer lookup ("where does X live") and an entry into an unfamiliar subsystem are different queries with different size needs — but the difference is invisible from inside the activation vector: the seeded head and the inflow-fed tail overlap in value, a scalar activation carries no record of *where its mass came from*, and genuinely relevant multi-hop nodes rank arbitrarily deep (field-labeled gold has been observed past rank two hundred). Any automatic cutoff sharp enough to shrink the pointer query would therefore amputate the very multi-hop tail the graph exists for. The one place the distinction does exist is the caller's head: it knows which kind of question it is asking. Hence the **compact profile** — fixed modest budgets and pointer lines instead of unfolded bodies (only recorded decisions keep their rationale) — chosen by an explicit declaration of intent. It is the same division of labor as everywhere in AMG: meaning (what kind of query this is) belongs to the model, mechanics (how each profile assembles) to the code.
...
The link strength `w_s ∈ (0, 1]` lives on the edges (not on folders). The intent is to change it by **Hebb's rule** ("neurons that fire together wire together"): if two nodes often activate in the same queries, the link strengthens; if a link goes long unused, it weakens. New edges start at `default_edge_weight` (`0.5`).
...
The result on the large sparse graph is the mirror image of the old harm: with every fold recall does not fall but **rises monotonically**, under all three seedings. Over eight folds recall climbed from about `0.60` to `0.85`, and hop-recall from `~0.25` to `~0.70` (BM25, the light static model2vec embeddings, and the multilingual transformer gave a close picture); as a reminder, the blind rule on the same rig dropped these same figures monotonically (recall ≈ −0.10, hop-recall ≈ −0.14). On the small dense graph (the dogfood sandbox) the new rule, as expected of a dense graph, barely moves the output — but, unlike the blind one, it does **no harm** either.

An important caveat on the conclusion's status. In both measurements the outcome signal was *synthetic*: each case's gold nodes counted as "used" on success. This honestly verifies that the rule is **correct** and **useful when the usage signal is accurate**, but it does not prove average benefit under a *real* `usage.log`, which is noisier.

The real-usage measurement has since been taken — on a live, densely linked project graph (about a thousand nodes and five thousand edges) with a usage log genuinely accumulated over working sessions. Folding it all in one flip reinforced hundreds of co-used edges — and moved recall, hop-recall, and pack recall by exactly nothing, in either direction; replaying the accumulated exposure signal on top (thousands of shown-but-unused edges faded) changed nothing either. This is the dense regime the sharpened principle above predicts, now confirmed in the field: where the graph is well connected and the seed strong, retrieval's outcome is decided by the seed and the structure, and weight nuances are inert — the flip is harmless and useless at once. The two regimes together give the default its final shape: `apply_hebbian` stays **`false`** because the rule pays off only where answers genuinely travel multi-hop over a sparse graph — there the sparse-rig numbers apply — while on a well-connected graph it spends effort on nothing. The rule ships disabled, is safe to enable (with no usage log it simply does nothing, and when acting it is outcome-gated and ceiling-bounded), and is worth enabling on sparse, multi-hop-heavy graphs with accumulated real usage.
...
The session's start is not the only place attention leaks. Field observation shows the retrieval discipline *decays as a session runs*: the model begins with the graph and drifts back toward raw file search, and no instruction fixes that from inside — the drift is invisible to the one drifting. So the external-trigger principle behind the digest extends into mid-session as a **gated reminder**: one line, arriving with the user's prompt, only when the memory has demonstrably gone unconsulted while task-shaped prompts keep coming. The gate is what separates it from a per-prompt nag — an unconditional reminder becomes noise the model learns to ignore, whereas a line that appears exactly when the loop has lapsed stays a signal (the firing conditions and mechanics — the lifecycle layer, ibid.).
...
The root of the difference is that **similarity is not relation**. RAG's entire retrieval decision rests on cosine closeness, and closeness is a weak proxy for connection: measured on a live project graph, candidate pairs whose vectors stood above cosine 0.8 were confirmed as genuinely related by the model's judgment in barely a quarter of cases — the high band is dominated by look-alikes (near-duplicate turns of different conversations, same-named methods of different classes) — and across all similarity-nominated candidates the judgment rejects 60–70%. AMG therefore grants similarity exactly one vote and never the verdict. At build time it *nominates*: structural edges are extracted from syntax deterministically (Python through the standard `ast`, other languages through tree-sitter, PDF/DOCX/XLSX documents through their extractors), while the semantic candidates closeness proposes are confirmed, typed, and weighed — or rejected — by judgment (§4.2). At query time it only *seeds*: cosine sets the starting relevance, and from there activation travels along the explicit edges, so the window receives the relevant neighborhood — including nodes that share not a single word or vector with the query (§6.4). That is the multi-hop reach RAG lacks in principle: it has no judgment filter over what counts as related, no deterministic structural edges to travel, and no typed, weighted links with per-type conductance — links a hand-maintained wiki has only as untyped, unweighted hyperlinks walked by hand.
