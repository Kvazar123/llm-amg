...
| `linker.synth_input_max_chars` | 800000 | the ceiling (chars) on ONE synthesis-input sheet; above it the sheet splits into whole-group `synth-input-pNN.json` parts processed one synth spawn at a time — lower it for a smaller-context model (e.g. 300000 for a 256k-token window over non-English summaries) instead of losing rows to overflow; trimming summaries is the last resort within a single oversized group |
...
| `retrieval.compact_ratio` | 0.25 | the `--compact` pointer profile's share of the effective `token_budget` tiers; `periphery_links` is deliberately not scaled (the periphery's one-liners carry the compact profile's measured recall) |
...
| — | — | these budgets govern the **full** profile only: the pointer profile (`retrieve.py --compact` — a targeted lookup) deliberately uses the built-in modest budgets (1200/2500/6000/40) and unfolds no bodies, so widening the config for deep-context work never inflates a targeted query (see [Retrieval](./06-retrieval.md), "The compact profile") |
| `retrieval.relation_priors` | a dictionary | the conductance prior `β` by edge type: `documents`/`implements`/`specifies` 0.9, `calls`/`depends_on`/`inherits` 0.8, `defines`/`part_of` 0.7, `imports`/`refines`/`exemplifies` 0.6, `relates_to` 0.5, `follows` 0.4 (chat-turn adjacency — weaker than semantic links), `supersedes`/`contradicts` 0.3. The numbers are an **ordinal scale of semantic strength** (a doc that describes code conducts harder than a mere topical kinship; a retired-claim link barely conducts), tuned constants rather than derived quantities (THEORY §14.1) — validated by the eval harness and changed only against its numbers, never by feel |
...
| `eval_gate.min_hop_recall_delta` | −0.02 | the allowed `pack_hop_recall` drop (the pack-based hop metric; hop-gold frozen from the baseline) |
