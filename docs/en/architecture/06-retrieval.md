...
The `retrieve.py` and `embed.py` files live in `skills/amg-retrieve/scripts/`. `retrieve.py` optionally imports `embed` (semantic seeding) and accesses the graph **read-only**; it writes only `cache/pack.md` and `work/coactivation.log`. It is run by the `amg-retrieve` skill through the `amg-retriever` subagent, which works in an isolated context and read-only. Next to it in the same directory is `eval_retrieval.py` (the quality metrics, see [Evaluation and tools](./10-eval-tools.md)).
