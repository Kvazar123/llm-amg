...
| `python reconcile.py apply <derivation.json\|glob> … [<project_root>] [--root <agent_dir>]` | fold in semantic output: several paths and glob patterns in one call (globs are expanded inside the script — a Windows shell passes them through verbatim), one transaction, one aggregated result |
...
On top of the resolver runs **target canonicalization** (`_normalize_edges`): the target of any edge that does not name an existing node is repaired, in order of specificity and always only on a **unique** candidate — on any ambiguity the target is left alone (a wrong edge is worse than a dangling one):

- **by path suffix** — a target missing its leading directories (`code:core/foo.py::Bar` → `code:src/pkg/core/foo.py::Bar`) binds to the one node whose path ends with the written path on a `/` boundary;
- **by bare qualname** — a target written as a bare symbol name with no path at all (`code:helper`: the judgment layer saw the symbol in a summary without knowing its file) binds to the one node whose id ends with `::helper` (exact qualname, same category). Two guards keep this from ever mis-binding: it **never applies to `imports` edges** — their dangling targets are legitimate external modules (`code:json`), and a same-named in-project symbol must not capture them — and never when the name could equally be a file (some node path ends with `/<name>`): two readings mean no repair.

Canonicalization applies at `apply` (the judgment layer's edges), in the added/changed/moved branches, and **as a whole-graph sweep on every `plan()`** — including nodes without units (hubs, notes, absorbed orphans). The sweep writes a node only on an actual change, and the edge comparison is order-insensitive (edge order carries no meaning) — so a graph built before either repair existed is healed by a single `bootstrap` (the `edges_refreshed` counter), while an already-canonical one stays a strict no-op. Along the same path, unchanged units get their structural edges rebuilt too: an extraction improvement (new edge types, removed noise) reaches old graphs without touching the sources.
...
The file also stays honest **between** plans: every apply immediately rewrites it down to the units whose node still awaits derivation (`_refresh_queue` — a unit stays only while its node is `stale`; a unit whose node is gone drops too, the next plan re-queues it if the source still exists). Without this, the raw file lied to every consumer from the apply until the next plan — `status` printed hundreds of "awaiting" units on a fully derived graph, `sync-defer` recorded the stale figure, and `partition_queue` would re-batch work already done.

## Applying the semantics (`apply_derivation`)

The builder subagent reads the queue and writes its result to a `derived-*.json` file — a list of items; the `apply` command folds them into the graph.

**The batch door: `apply-derived`.** Workers checkpoint their output in many small part files, and applying them one command each would cost the orchestrator one turn per file — with its whole conversation re-sent every turn (turn count is the same currency as data volume). So the working entrance is the `apply-derived` command: it reads **every** `work/derived-*.json` (checkpoint `-pNN` parts included) in deterministic sorted order, applies them as one item stream — one transaction, one aggregated result `{applied, created, skipped_missing, skipped_stale, skipped_invalid, files, queue_remaining}` — and moves the consumed files to `work/applied/`. A file that is not valid JSON (a checkpoint torn by a hard kill) goes to `work/invalid/` and is reported in `malformed_files`, never aborting the rest. Re-running `apply-derived` is therefore a cheap no-op over what already landed — and it is also the resume path after an interruption. The plain `apply` command remains for targeted use and likewise takes several paths and glob patterns in one call (expanded inside the script — a Windows shell does not expand globs; a trailing existing directory names the project root). Moving a file back out of `work/applied/` and re-applying it by hand stays safe: apply is idempotent and freshness-checked.

**The judged-pair memory (linker batches).** A linker part may end with a service record `{"judged": [<node id>, …]}` naming every node ruled on in that part — including nodes where nothing was confirmed (they produce no graph items, so without the record their coverage would be unprovable). The record never touches the graph: `apply-derived` pops it before applying (`_pop_judged`; one reaching a plain `apply` is dropped silently, not counted as invalid), collects the ids per batch by the part's filename label (`derived-links-<batch>-pNN.json` ↔ `work/link-batch-<batch>.json`), and when the collected ids cover **every** node the batch carries, moves the batch file into `work/judged/` (`_retire_judged_batches`; the result lists them as `judged_batches`). The nomination pass (`link_candidates.py`) reads `work/judged/` and treats each retired batch's (node, candidate) pairs as already ruled on — the memory of **rejections** that lets "repeat until zero batches" actually terminate: graph edges remember only the confirmed half, and without the judged memory similarity would re-nominate the freshly rejected pairs every wave, sliding to ever-weaker candidates. Coverage is proven within one `apply-derived` call, deliberately not folded in from the `work/applied/` archive: batch labels are positional (a re-nomination reuses `001`, `002`, …), so an archived part could falsely cover a renumbered batch — and silently losing nominations is worse than re-judging a crashed batch's tail. Re-opening old judgments is as simple as deleting files from `work/judged/`.

**Two item forms** are supported:
...
`reconcile.py metrics` (read-only; the same numbers appear as the `connectivity` block in `/amg status`) measures the quality of the built graph: the number of connected components and the largest one's share (over the same symmetric conductance retrieval uses: edges to existing nodes + `part_of` memberships naming a node), isolated nodes, edge resolution, doc files without a `documents` edge, and the `stale` count.

Unresolved edge targets are split **by responsibility**, because the remedies differ. External `imports` (stdlib/third-party) are legitimately dead — a record of the import fact, never a defect. A dangling **structural** edge (`dangling_structural`; `origin: structural`, plus legacy unmarked `imports`/`calls`) is the engine's own correctness: the resolver never guesses, so on a fresh build this class is zero by construction — and it is the class the `max_dangling_internal` threshold gates. A dangling **model-written** edge (`dangling_semantic`; a judgment-layer target that names no node — an invented slug the canonicalizer cannot bind) is inert at retrieval (dropped) and has no automatic remedy, so it is reported with a count and samples but **never flips the verdict** — a gate that cannot return to `ok` stops carrying signal.

The doc metric is aggregated **per file** (`doc_files_without_documents`): a doc file counts as unanchored only when *none* of its sections carries an outgoing `documents` edge. Per-section counting drowned the signal in structure — an ADR's context/consequences subsections document nothing by themselves; their file anchors through its subject section. A file whose every section is still `stale` is not counted (it has not had its chance yet), and **session-dump files** — `source_path` under the sessions directory (both callers pass the prefix via the shared `session_dir` resolution) — are excluded: a dialogue turn has no subject by nature; a file of an external chat export legitimately lacks the edge too — the metric is indicative.
...
Crash safety rests on three things: `recover()` at the start of every command replays unfinished transactions; all changes go as one store transaction (the commit is atomic); the queue is written atomically. So a crash at any point is safe: died during derivation — the structural nodes stay `stale` with their previous summaries, the queue is intact, just restart the builder and `apply`; died during `apply` — the transaction is recovered on the next run. All workers, moreover, write their results **in parts** — numbered checkpoint files (`derived-<batch>-p01.json`, `derived-links-<n>-p01.json`, `derived-synth-p01.json`, …) as they go — so even work torn off mid-batch leaves everything already done on disk: the last few units are lost, not hours of work (see [Subagents and skills](./08-agents-skills.md)). The `derived-*.json` files that survived a crash are applied **first** on the next run, by one `apply-derived` call (the `amg-bootstrap` skill says exactly that; `plan` nudges with the `leftover_derived` field): the `content_sha` check skips items whose source has changed, so only the remainder is re-derived, not the whole queue — resumability without token waste. The `work/applied/` move exists for turn economy, not correctness: apply is idempotent and freshness-checked, so re-applying a part is harmless — moving consumed files aside just keeps the next call from re-reading them.
