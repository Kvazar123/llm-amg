...
| `codex` | OpenAI Codex (has skills and subagents) | the skills into `.agents/skills`, the subagents as **TOML in `.codex/agents`** (`model`/`model_reasoning_effort`), the skill-aware `entrypoint/AGENTS.codex.md` block; no hooks are written (Codex's hook surface is unconfirmed — being verified live) and no `/amg` |
| `opencode` | OpenCode (has skills; discovers `.agents/skills` natively) | the skills into `.agents/skills`, the subagents rendered to **`.opencode/agent/*.md`** (`mode: subagent`; a real model id passes, a Claude alias is omitted), the portable skill-aware `entrypoint/AGENTS.skills.md` block; no hooks (OpenCode's analogue is a JS plugin API) or command |
| `qwen` | Qwen Code (has skills, markdown subagents, **and hooks**) | the skills into `.qwen/skills`, the `agents/amg-*.md` prompts land in `.qwen/agents` (Qwen's native subagents) with Claude-only frontmatter sanitized (`tools`/`effort` dropped; an alias model dropped, a real id passes), the same `AGENTS.skills.md` block into `QWEN.md`, and the session hooks merged into `.qwen/settings.json` (Qwen reads a Claude-shaped `hooks` block) |
...
The agent directory and the entry point are substituted per environment when not set explicitly: Claude Code → `.claude`/`CLAUDE.md`, Qwen Code → `.qwen`/`QWEN.md`, otherwise → `.agents`/`AGENTS.md`. **Modes other than Claude Code are not yet tested live** (environment verification is the roadmap stage in progress).
...
For `--env claude-code` **and `--env qwen`** the installer merges the `SessionStart`/`SessionEnd`/`UserPromptSubmit` hooks from the `entrypoint/settings.json` template with the existing `settings.json` (`merge_settings`): foreign hooks and keys are preserved, previous AMG hooks are replaced (matched by the `lifecycle.py` signature in the command, whatever the event) — Qwen Code reads a hooks block of the same shape in `.qwen/settings.json`, so one template serves both. The `entrypoint/commands/amg.md` slash command is rendered into `<agent_dir>/commands/` for Claude Code only. For `codex`/`opencode`/`generic` neither the hooks nor the command are written — the activation loop's start check and wrap-up signal take their role.
