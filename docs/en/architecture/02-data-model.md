...
| `gap-report.md` | the synthesis pass's gap report: undocumented code, drifted doc references, contradictions (see [Subagents and skills](./08-agents-skills.md), `amg-synth`) | as needed (synthesis) |
| `arbitration.md` | the arbitration audit trail: every contradiction verdict with its reason and compared sources (see [Consolidation](./07-consolidation.md)) | as needed (arbitration verdicts) |
| `evals/cases.json` | the user's labeled eval cases — arms the compaction recall gate (`eval_gate.cases`); lives under the store so an engine reinstall never wipes it | by the user |
