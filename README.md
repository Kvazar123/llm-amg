...
> The `SessionStart`/`SessionEnd`/`UserPromptSubmit` hooks and the digest `@`-import are Claude Code mechanisms; hooks of the same shape are also read by Qwen Code and (behind its `/hooks` trust review) by Codex, OpenCode covers the same ground with the AMG event plugin, and the `/amg` command is rendered into every environment that has a command surface. The exact set of conveniences depends on the `--env` chosen at install — see "Installation", "Other environments."
