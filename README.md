...
**Weaker models benefit from this memory the most.** A strong model can reconstruct the project's picture empirically over and over — sweeping the structure with search and reads, with omissions and at a steep price; a weaker model gets lost on that same path and misses what matters. From the graph, any model receives ready, objective data instead — the gist of every part of the project and its place among the rest — rather than its own guesses over search fragments: for a weak model the memory restores the bearings it lacks, for a strong one it saves the cost of re-scouting.
...
- **retrieval by judgment, not only by raw context** — the memory can answer with a conclusion instead of material: a copy of the assistant that knows the whole current conversation reads the findings outside the working window and returns a short verdict — what the memory adds, what it confirms, what it contradicts; the window stays lean while the answer weighs the session and the graph at once (on Claude Code);
...
**Retrieval — per request, driven by the model itself.** Before taking on a task — and every time the focus shifts mid-conversation (a new requirement, a different subsystem) — the model assembles a compact context pack from the graph rather than loading the whole project. That is, retrieval happens not once at session start but before each task, initiated by the model as it reasons. A complex, multi-part request is **decomposed**: each distinct topic gets its own retrieval as the model turns to it, and the retrieved branches of the graph together form the task's context. 

The graph is the **primary source** — everything it can give is taken from it first (sources are opened point-wise from the pack's `path:line` pointers), and only then does the model decide what remains to be searched in the files directly. The pack's size follows the query too: a targeted pointer lookup ("where is X") runs a **compact profile** — pointer lines at a fraction of the size — while entering an unfamiliar subsystem gets the full-depth pack. The mechanism is **query-biased Personalized PageRank**: the query is seeded lexically (BM25) and, optionally, semantically (embeddings), and activation then spreads over the structural edges. Those edges are **query-independent** — and that is what preserves multi-hop reach: activation mass flows *through* a node that is itself irrelevant to the query toward a relevant neighbor ("bias, don't gate").
...
The model reads the instructions, determines the mode by the project's state (a config already present means a reinstall — it shows the settings in force and asks only what to change, never the full questionnaire), surveys the project and proposes a folder classification (what to mirror, what to absorb, what to exclude), asks the full question list one by one (local/global; environment; mirror and absorb paths; excludes; working language; embeddings — with the backend pinned to your choice; automation; session policy; tier budgets; remaining dependencies; model tiering; compaction; the synthesis-sheet ceiling; activation) — and calls the installer for you. 

Two questions are key and worth deciding before the first build: the **working language** (a later change re-summarizes nothing by itself — summaries and the cache are keyed by language) and **embeddings** (the build-time linking pass finds cross-domain links by their vectors). Everything else is safe to adjust in `config.yml` at any time — changes apply on the next run; see [INSTALL.md](INSTALL.md), "Cheap to change later vs decide before building".
...
```
The `model` key is not limited to the aliases. Any string your environment understands will do: a pinned identifier like `claude-opus-4-8`, or another provider's model like `gpt-5.5` when the environment points at a gateway, Bedrock, or Vertex. AMG simply hands the string to the environment and does no provider routing of its own. The one thing to watch is the format, and each environment has its own: OpenCode expects a `provider/model` pair (e.g. `anthropic/claude-sonnet-4-5`), Qwen Code its native names like `qwen3-coder-plus` — or, for a foreign provider, the colon form `openai:gpt-4o`. The Claude aliases (`opus`, `sonnet`, `haiku`) mean something only to Claude Code: the installer does not carry them into other environments' subagents, and the environment falls back to its own default model.

The install flow deliberately asks only what matters most. For full control over the rest — weights, compaction, batch sizes, the viewer, thresholds — the simplest way is to **edit the template `<amg-dir>/config.yml` beforehand**: the installer writes the project config from it. The other way is to edit `<project>/.claude/amg/config.yml` after the install. When an edit takes effect depends on the key, but none needs a session restart. Most keys — tier budgets, thresholds, weights, batch sizes, `automation`, `active` — act from the next operation: every run reads the config afresh. `working_language` and embeddings are decided before the first build: changing them afterwards means `/amg sync`, and a language change means a full re-derivation. Finally, `models`, `agent_dir`, and `entrypoint` are rendered by the installer, so they take effect through a reinstall. The full reference for every key — [09-config](docs/en/architecture/09-config.md).
...
- **Codex** (`--env codex`) — with skills, **TOML subagents** in `.codex/agents` (with a per-role `model` and reasoning effort from the `models` block), a skill-aware `AGENTS.md` block, **and hooks** in `.codex/hooks.json` (the start check and the gated reminder); Codex runs unmanaged hooks only after you trust them via its `/hooks` review — the install ends with that step;
- **OpenCode** (`--env opencode`) — the environment **discovers** the skills in `.agents/skills` itself, subagents are rendered to `.opencode/agent` (a real model id from `models` passes through, a Claude alias is omitted), the same skill-aware block, the `/amg` command in `.opencode/command`, and the **AMG event plugin** in `.opencode/plugin` — it stands in for hooks: the start check at session creation, the gated reminder, and an incremental transcript dump as you work;
- **Qwen Code** (`--env qwen`) — the `.qwen` directory and the `QWEN.md` entry point: skills, native markdown subagents in `.qwen/agents` (the model from `models` — as a real id), the `/amg` command in `.qwen/commands`, **and session hooks** in `.qwen/settings.json` — Qwen reads a hooks block of the same shape Claude Code does;
...
Baseline functionality does not depend on the environment; the set of conveniences does. Live status: OpenCode's cycle is field-confirmed (see the warning above); Codex, Qwen Code, and generic await theirs.
...
**Running a weak model, or one with a small window? Shrink the synthesis sheet.** The global synthesis pass (hubs, the overview, the gap report) receives the whole summary layer as one prepared file, and that file's ceiling is set by the `linker.synth_input_max_chars` key — a value in characters, 800000 by default. A model with a window around 256K tokens cannot hold such a sheet and starts losing rows, so set 300000 or lower for it: past the ceiling the engine splits the sheet into whole parts on its own and runs the synthesis part by part, losing nothing.
...
Sessions exist in every environment; what differs is whether the environment lets the memory *see* them — an observable end and a reachable transcript — and the dump follows what each one gives. Claude Code gives both, so the dialogue is dumped once, at close (the `SessionEnd` hook). Qwen Code gets the same hook wired by the installer, with its transcript format still to be confirmed live. OpenCode exposes the whole dialogue through its plugin API but signals no session end — so the AMG plugin dumps **incrementally**: the transcript is re-written into the sessions folder at quiet moments as you work, and even a killed terminal loses at most the last couple of minutes. Codex keeps a transcript too, yet signals only the ends of turns, never the session's — a dump is not wired there yet (a decision for the live run), so the dialogue is preserved the portable way. 

That portable way — the **notes captured along the way** (`notes.py`) — is the "don't lose the dialogue" guarantee in every environment: one that can dump the transcript dumps it, one that cannot keeps the conclusions as notes. That is also why, in environments without the dump, it pays to say the wrap-up out loud ("wrapping up the session") — on that signal the model first files the session's conclusions as notes, then runs consolidation.
...
Where the installer wired session automation — the hooks (Claude Code and Qwen Code natively; Codex once you trust them via `/hooks`) or the OpenCode event plugin — the two "hook" rows of the table run event-driven, and the start check's findings arrive on their own, as a one-line note with your first message. Only the generic mode (and Codex while its hooks await trust) drives them by the activation block's discipline under `automation: true` — and even there the session's start needs no words: the environment loads the block into every session, and the model runs the start check by it on its own (store healing, the source diff, reading the digest) before the first task. The **wrap-up signal stays essential in every environment**: a session's end is not something the model can foresee, so the judgment half of consolidation waits for your words everywhere; where no end-of-session event exists (Codex, generic), those same words are also the moment the conclusions get filed as notes — while in OpenCode the deterministic half has already been kept up by the plugin as you worked. 

On rate limits: hooks and the plugin are plain scripts with no model — they spend no tokens and cannot hit a plan limit, so the deterministic half runs regardless. The judgment half is model work: with the limit gone, the loop simply cannot run it — and that is safe: the notes are already committed transactionally, the dump and the weights are done, and the selection is non-destructively deferred to `/amg consolidate` in any later session.
...
| `/amg relink` | re-judge exactly the unlinked nodes: nomination only for the strays with no relation at all, their past rejections re-opened (a plain `sync` honestly reports "nothing new" there — those pairs were already ruled on) |
...
Control verbs (`status`/`version`/`on`/`off`/`repair`/`help`) are run by a helper script; work verbs (`sync`/`retrieve`/`consult`/`consolidate`/`relink`) delegate to the dedicated skills and subagents (also invocable directly); `view` is a direct run of the exporter, no model involved. Every automatic operation has such a manual counterpart. Note: `on` only enables memory — **the graph is built by `sync`**. `on`/`off` write the `active` flag straight into the config and take effect **immediately**, no session restart — handy for switching the memory off over a run of simple tasks and back on. A single task can bypass the memory with plain words too ("skip AMG for this one") — the loop obeys a direct request.

**The memory consult (`/amg consult`, Claude Code only).** Ordinary retrieval brings material into the window; the consult brings a conclusion. On `/amg consult <question>` (or in words: "check this against the memory, session-aware") a copy of the assistant is started that has inherited the whole current conversation: it assembles the pack on its own side, weighs it against everything the session already knows, and returns a short verdict — what the memory adds, what it confirms, what it contradicts — without repeating what the window already holds; the pack itself never enters the main window. This is useful at any stage of the work: early — a dense briefing into the task instead of a full pack; mid-way — a top-up on a new topic without bloating the window, and a cross-check before a significant decision; at the end — a revision of what was done against the memory. The price is cache tokens and a little time (the copy re-sends the inherited conversation); the payoff is a lean, relevant window with a fully informed answer.
...
> The `SessionStart`/`SessionEnd`/`UserPromptSubmit` hooks and the digest `@`-import are Claude Code mechanisms; hooks of the same shape are also read by Qwen Code and (behind its `/hooks` trust review) by Codex, OpenCode covers the same ground with the AMG event plugin, and the `/amg` command is rendered into every environment that has a command surface. Which of these conveniences a given environment receives is decided by the install mode — see "Installation", "Other environments."
...
A note on latency: the transformer backend (`sentence-transformers`) loads its model into every retrieval process — seconds to tens of seconds per call. The engine tries the local model cache first (no network round-trip once downloaded); `HF_HUB_OFFLINE=1` in the environment enforces the same everywhere. When latency matters, install `model2vec`: its static vectors load near-instantly, it has a multilingual default, and it sits first in the automatic backend order.
...
- **Retrieval** — BM25 seeding + optional embeddings + Personalized PageRank + a tiered pack under a token budget, with a compact pointer profile for targeted lookups and the session-aware memory consult (Claude Code).
...
- **Installation and portability** — `install.py`: local and global, reinstall and uninstall, two configuration layers, and a declarative environment registry that deploys the right mechanism per environment — five modes (Claude Code / Codex / OpenCode / Qwen Code / an unknown AGENTS.md environment) with native subagent formats, session hooks or the OpenCode event plugin, and the `/amg` command on each native command surface.
