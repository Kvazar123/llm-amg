...
Graph building is now correct and connected: a deterministic edge backbone (calls resolved across files, containment and inheritance extracted by code), a global semantic linking pass across domains, an apply step resilient to malformed items, an acceptance connectivity report, and reproducible rebuilds from the derivation cache. Planned ([roadmap](docs/en/architecture/11-roadmap.md)): economical building on large projects (a multi-fold cut in cost and time at held quality); and the English translation of the documentation.
