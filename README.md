...
> ⚠️ **FULLY TESTED ON CLAUDE CODE; OPENCODE CONFIRMED ON LIVE RUNS.** The installer sets the memory up per environment, each with its own mode: **OpenCode** (`--env opencode`, skills discovered by the environment itself + subagents in `.opencode/agent` + an event plugin + the `/amg` command) — the full memory cycle (install, build, retrieval, notes, consolidation) has been reproduced on live field runs, with the freshly added plugin and command awaiting theirs; **Codex** (`--env codex`, skills + TOML subagents + hooks the user trusts via `/hooks`), **Qwen Code** (`--env qwen`, skills + subagents + session hooks + the `/amg` command, the `.qwen` directory), and **any unknown AGENTS.md environment** (`--env generic`, a portable skill-less block) — these are wired from each environment's verified surfaces but **not yet confirmed live**. Finishing that verification is the roadmap stage now in progress.
