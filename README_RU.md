...
**Перезапустите сессию.** Скиллы и команда `/amg` регистрируются при старте сессии, поэтому установочная сессия их ещё не видит — прежде всего начните новую.
...
**Сборка.** Главный разовый шаг: **`/amg sync`** (словами: «построй / синхронизируй граф памяти») выполняет полную первую сборку — структурный скелет, сводки, хабы и проход связывания. При `automation: true` петля запустит её сама перед первой задачей; дальше `sync` лишь сверяет изменившееся.

**Работа.** Дальше **просто задавайте модели вопросы по проекту** — она сама соберёт из графа нужный контекст (стратегическое окружение плюс конкретику) и будет фиксировать выводы по ходу; регулярное обслуживание — `/amg consolidate` (или автоматически).
...
Это рабочий диагностический инструмент, а не украшение — всегда под рукой, когда хочется *увидеть* свою память. Он даёт проект одним взглядом: хабы и ключевые узлы с их связями, тематические кластеры, различимые цветом. И он делает проблемы сборки буквально видимыми: **остров**, плавающий отдельно от основного графа, **несвязанный узел**, кластер документации, повисший в стороне от кода, который должен описывать, — каждый такой случай — это пробел, который закрывает проход связывания (`/amg sync`), а строка `connectivity` в `/amg status` выносит тот же вердикт числом. Спорное знание тоже заметно на глаз — вердикты арбитража (`disputed` / `rejected` / `superseded`) подсвечены.

**Легенда красит узлы по категории содержимого** (бакету хранения) — не по папкам проекта и не по политике источника: **код** — функции, классы, модули (тесты включительно: тест — это код, в какой бы папке он ни лежал); **документы** — секции документов (впитанный markdown — документ, а не «данные»); **данные** — структурированные записи (JSON/YAML, листы CSV/XLSX); **заметки** — авторские захваты (решения, выводы, открытые вопросы; диалоги сессий попадают к документам); **хабы** — синтезированный стратегический слой. Размер узла отражает связность (хабы читаются крупно), толщина ребра — вес связи.
