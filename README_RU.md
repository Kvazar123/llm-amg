...
| Консолидация, судейская половина (отбор в долгую память, слияние дублей, сжатие веток) | петля модели в конце работы; вручную — `/amg consolidate` | конец работы / по запросу |

В средах без хуков (Codex, generic-режим) обе «хуковые» строки тоже ведёт петля модели — дисциплиной блока активации при `automation: true`.
