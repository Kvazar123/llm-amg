...
Реализован и стабилизирован базовый тракт (этапы 0–12 дорожной карты): транзакционное хранилище, извлечение структуры с классификатором и широким набором нарезателей (код, markdown/RST, текст, JSON/YAML с рекурсией, NDJSON, CSV, журналы, чат-экспорты, PDF/DOCX/XLSX/PPTX), сверка `mirror`/`absorb`/`absorb_once`, извлечение (BM25 + эмбеддинги + Personalized PageRank + ярусный пакет), консолидация (веса, значимость, компрессия под eval-предохранителем), безопасный захват заметок, слой жизненного цикла (хуки сессии, команды `/amg`, режимы `automation`, always-on дайджест), сохранение сессий (авто-выгрузка диалога с политикой записи), **упаковка с установщиком** (`install.py`: локально/глобально, переустановка и удаление, переносимость по каталогу агента) и **производительность с масштабированием** (производный read-индекс под извлечение для больших графов, тиринг моделей субагентов, бенчмарки).

В планах ([дорожная карта](docs/ru/architecture/11-roadmap.md)): слой провенанса и верификации фактов; арбитраж противоречий; 3D-просмотр графа; командный режим поверх git; продвинутый семантический слой (включая сегментацию длинной прозы по смысловому дрейфу); перевод документации на английский.
