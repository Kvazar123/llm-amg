...
- **Хеббово обучение весов выключено** (`weights.apply_hebbian: false`). Слепое правило усиления **ухудшало** полноту на большом разрежённом графе («магистрали» оттягивают активацию от многошаговых узлов); улучшенное правило — усиление по **реальному исходу задачи**, а не по совместному показу — на том разрежённом стенде полноту поднимает, а на реальном плотно связанном графе проекта полевой замер по накопленному использованию не сдвинул ничего ни в одну сторону: на хорошо связанном графе исход решает засев, веса инертны. Включение безвредно, но окупается только на разрежённых многошаговых графах — потому дефолт и остаётся off. Когда включать — в [руководстве](docs/ru/GUIDE.md); подробно — [теория, §8](docs/ru/THEORY.md).
...
| Консолидация, судейская половина (отбор в долгую память, слияние дублей, сжатие веток) | петля — по **вашему сигналу о завершении работы** («закругляемся…»); вручную — `/amg consolidate` | когда вы сигналите о конце работы / по запросу; если она пропущена несколько сессий подряд, следующая сессия начнётся с однострочного напоминания (смена фокуса темы её НЕ запускает — там только новый пакет) |
...
**Консолидация — гигиена, а не сохранение.** Пока вы её пропускаете, ничего не теряется: заметки фиксируются транзакционно по ходу работы, стенограмму выгружает хук. Консолидация покупает остроту: без неё граф постепенно пухнет, а выдача становится шумнее. Три её фазы: **weights** — детерминированная свёртка сигналов без модели; **plan** — анализ только чтением (что переросло); **apply** — судейский проход (повысить / слить / свернуть эпизоды / ужать / отправить в архив) с обратимым архивом оригиналов и кодовой защитой решений. Разумный ритм — насыщенная решениями сессия или заметно зашумевшая выдача, а не каждый заход.
