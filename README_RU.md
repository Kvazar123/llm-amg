...
> ⚠️ **ПРОВЕРЕНО ТОЛЬКО В CLAUDE CODE.** Установщик умеет ставить память и в другие агентские среды — под каждую свой режим: **Codex** (`--env codex`, скиллы + TOML-субагенты), **OpenCode** (`--env opencode`, скиллы находятся средой сами + субагенты `.opencode/agent`), **Qwen Code** (`--env qwen`, скиллы + субагенты + сессионные хуки, каталог `.qwen`), **неизвестная AGENTS.md-среда** (`--env generic`, переносимый блок без скиллов). Но **работоспособность и стабильность в любой не-Claude-Code-среде пока НЕ протестированы и НЕ гарантируются** — все тесты проводились в Claude Code. Проверка этих сред — идущий сейчас этап дорожной карты.
...
- **Codex** (`--env codex`) — со скиллами и **TOML-субагентами** в `.codex/agents` (с выбором `model` и уровня рассуждения из блока `models`), skill-aware блок `AGENTS.md`; хуки не пишутся, пока их поверхность в Codex не подтверждена живьём;
- **OpenCode** (`--env opencode`) — скиллы в `.agents/skills` среда **находит сама**, субагенты рендерятся в `.opencode/agent` (настоящий идентификатор модели из `models` проходит, Claude-алиас опускается), тот же skill-aware блок; хуков нет (их аналог у OpenCode — плагинный JS-API);
- **Qwen Code** (`--env qwen`) — каталог `.qwen`, точка входа `QWEN.md`: скиллы, родные markdown-субагенты в `.qwen/agents` (модель из `models` — настоящим идентификатором) **и сессионные хуки** в `.qwen/settings.json` — Qwen читает hooks-блок той же формы, что Claude Code;
...
Базовая работоспособность от среды не зависит, набор удобств — зависит. **Эти режимы пока не протестированы живьём** — все тесты в Claude Code (проверка сред — идущий сейчас этап дорожной карты).
...
Авто-выгрузка стенограммы работает там, где среда даёт и хук `SessionEnd`, и доступ к самой стенограмме, — штатно это Claude Code (в Qwen Code хук подключается установщиком, но формат стенограммы другой — включение выгрузки там подтвердится живой проверкой). Во всех прочих средах диалог сохраняют **заметки, фиксируемые по ходу работы** (`notes.py`), — они же главная страховка «диалог не потерян» в любой среде: кто умеет выгружать стенограмму — выгружает её, кто нет — сохраняет выводы заметками. Поэтому в средах без выгрузки завершение работы стоит проговаривать явно («завершаем сессию») — по этому сигналу модель сначала фиксирует выводы заметками, затем запускает консолидацию.
...
В средах без хуков (Codex, OpenCode, generic-режим) обе «хуковые» строки тоже ведёт петля модели — дисциплиной блока активации при `automation: true`. Начало сессии при этом слов не требует: блок активации среда загружает в каждую сессию, и стартовую проверку (восстановление хранилища, сверка источников, чтение дайджеста) модель выполняет по нему сама, до первой задачи. А вот **сигнал завершения обязателен**: конец сессии модели не виден, и слова «завершаем работу» — единственное событие, по которому она успевает записать выводы заметками и запустить консолидацию. И о лимитах: хуки — обычные скрипты без модели, токенов они не тратят и в тарифный лимит упереться не могут, поэтому при штатном закрытии сессии детерминированная половина выполняется всегда, даже с исчерпанным лимитом. Судейская половина — модельная работа: если лимит кончился, петля её просто не выполнит, и это безопасно — заметки уже зафиксированы транзакционно, дамп и веса сделал хук, а отбор неразрушительно откладывается до `/amg consolidate` в любой следующей сессии.
...
Управляющие глаголы (`status`/`version`/`on`/`off`/`repair`/`help`) выполняет служебный скрипт, рабочие (`sync`/`retrieve`/`consolidate`/`relink`) — делегируют профильным скиллам (доступным и напрямую), `view` — прямой запуск экспортёра без модели; у каждой автоматической операции есть такой ручной аналог. Важно: `on` лишь включает память — **граф строит `sync`**. `on`/`off` пишут флаг `active` прямо в конфиг и действуют **немедленно**, без перезапуска сессии — ими удобно выключать память на серию простых задач и включать обратно. А разовую задачу можно провести мимо памяти и просто словами («сейчас без AMG») — петля подчиняется прямой просьбе.
...
> Хуки `SessionStart`/`SessionEnd`/`UserPromptSubmit`, слэш-команда `/amg` и `@`-импорт дайджеста — механизмы Claude Code (хуки той же формы читает и Qwen Code); в других средах состав удобств зависит от выбранного при установке `--env` (Codex / OpenCode / Qwen / generic), подробнее — раздел «Установка», пункт «Другие среды».
