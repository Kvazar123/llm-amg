...
Сборка графа сделана корректной, связной и экономной. Корректность и связность дают детерминированный хребет рёбер (вызовы резолвятся между файлами, принадлежность и наследование извлекаются кодом), глобальная смысловая линковка доменов, устойчивое к битым элементам применение и приёмочный отчёт связности; воспроизводимость — кэш дериваций. Экономность и устойчивость встроены в сам конвейер: текст каждой единицы подаётся сборщикам прямо в очереди работ (исходники не перечитываются), тривиальные единицы сводятся кодом без модели, партии ограничены и записываются частями-чекпоинтами, прерванная сборка возобновляется с места, а агенты честно отчитываются о статусе и прогрессе — экономия берётся из устранения повторной работы, не из урезания смысла. В планах ([дорожная карта](docs/ru/architecture/11-roadmap.md)): перевод документации на английский; проверка не-Claude-Code-сред.
