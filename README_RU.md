...
В средах без хуков (Codex, OpenCode, generic-режим) обе «хуковые» строки тоже ведёт петля модели — дисциплиной блока активации при `automation: true`; в таких средах **сигнал завершения обязателен**: конец сессии модели не виден, и слова «завершаем работу» — единственное событие, по которому она успевает записать выводы заметками и запустить консолидацию. И о лимитах: хуки — обычные скрипты без модели, токенов они не тратят и в тарифный лимит упереться не могут, поэтому при штатном закрытии сессии детерминированная половина выполняется всегда, даже с исчерпанным лимитом. Судейская половина — модельная работа: если лимит кончился, петля её просто не выполнит, и это безопасно — заметки уже зафиксированы транзакционно, дамп и веса сделал хук, а отбор неразрушительно откладывается до `/amg consolidate` в любой следующей сессии.
