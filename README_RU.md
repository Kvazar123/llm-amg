...
| Сборка / сверка (`sync`) | **петля модели** или команда — хук её **не** запускает | первая сборка — один раз; дальше в начале сессии при изменившихся источниках |
...
| Консолидация, судейская половина (отбор в долгую память, слияние дублей, сжатие веток) | петля модели в конце сессии (перед `/clear`/выходом); вручную — `/amg consolidate` | конец сессии / по запросу (смена фокуса темы её НЕ запускает — там только новый пакет) |
