...
> ⚠️ **ПОЛНОСТЬЮ ПРОВЕРЕНО В CLAUDE CODE; OPENCODE ПОДТВЕРЖДЁН ЖИВЫМИ ПРОГОНАМИ.** Установщик ставит память под каждую среду своим режимом: **OpenCode** (`--env opencode`, скиллы находятся средой сами + субагенты `.opencode/agent` + событийный плагин + команда `/amg`) — полный цикл памяти (установка, сборка, извлечение, заметки, консолидация) воспроизведён живыми полевыми прогонами, свежедобавленным плагину и команде своя проверка ещё предстоит; **Codex** (`--env codex`, скиллы + TOML-субагенты + хуки, которые пользователь доверяет через `/hooks`), **Qwen Code** (`--env qwen`, скиллы + субагенты + сессионные хуки + команда `/amg`, каталог `.qwen`) и **неизвестная AGENTS.md-среда** (`--env generic`, переносимый блок без скиллов) — собраны по проверенным поверхностям этих сред, но **живьём пока не подтверждены**. Завершение этой проверки — идущий сейчас этап дорожной карты.
...
> Хуки `SessionStart`/`SessionEnd`/`UserPromptSubmit` и `@`-импорт дайджеста — механизмы Claude Code; хуки той же формы читают Qwen Code и (за ревью доверия `/hooks`) Codex, у OpenCode то же самое закрывает событийный AMG-плагин, а команда `/amg` рендерится в каждую среду с командной поверхностью. Точный состав удобств зависит от выбранного при установке `--env`, подробнее — раздел «Установка», пункт «Другие среды».
