...
- **автоматический режим** (`automation: true`) — память ведёт себя сама: хуки `SessionStart`/`SessionEnd` (механизм Claude Code) детерминированно чинят граф после сбоев и сворачивают веса, а модель по петле перед каждой задачей собирает контекст, по ходу фиксирует выводы и в конце запускает консолидацию;
